...
def solve_task(task):
    ...
    for p in additional_phrases:
        if (os.path.exists(env.relative_path(p)) or os.path.exists(env.relative_path('./codebase/' + p))) and len(p) > 0:
            doc = Documentation()
            ...
            file_paths.append(env.relative_path(f'codebase/{p}'))
            new_paths.append(env.relative_path(f'codebase/{p}'))
    ...
    snippets_copy = copy.deepcopy(snippets)    
    ...
    while not didFinish and snippets:
        ...
        if res:
            ...
            for snippet in snippets:
                if not snippet.get('file_path') in file_paths:
                    new_snippets.append(snippet)
            
            snippets = new_snippets

            new_processed_snippets = []

            for snippet in processed_snippets:
                if not snippet.get('file_path') in file_paths:
                    new_processed_snippets.append(snippet)

            processed_snippets = new_processed_snippets
