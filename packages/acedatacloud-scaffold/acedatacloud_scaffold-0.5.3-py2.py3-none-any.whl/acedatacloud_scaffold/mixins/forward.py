...
class ForwardMixin(object):
    ...
    async def get_forward_response_status(self):
        return self.forward_response.status_code if self.forward_response else None

    async def get_forward_response_headers(self):
        return self.forward_response.headers if self.forward_response else None

    async def get_forward_response_body(self):
        if not self.forward_response:
            return
