...
class TestGetRSSerialNumber(unittest.TestCase):
    def test_get_checkpoint_serial_number(self):
        # Test with a segment where the  serial number is 0x00
        hex_string = "0301030001d000590100"
        segment= bytes.fromhex(hex_string)
        self.assertEqual(get_rs_serial_number(segment), 0)
        # Test with a segment with multiple byte fields and multiply byte rs serial number
        hex_string = "010281d9dba2530001008769e406d25f"
        segment= bytes.fromhex(hex_string)
        self.assertEqual(get_rs_serial_number(segment), 10591)