...
@ue.group()
def ddc():
    """work with distributed data cache"""


@ddc.command()
def show():
    """Show current DDC settings"""
    loc = tools.location_get()
    local = tools.env_var_get("UE-LocalDataCachePath")
    if local:
        echo(f"Local data cache path is set to {local} for location {loc}")
    else:
        echo(f"No local data cache path set for location {loc}")

    ddc = tools.env_var_get("UE-SharedDataCachePath")
    if ddc:
        echo(f"Shared data cache path is set to {ddc} for location {loc}")
    else:
        echo(f"No shared data cache path set for location {loc}")
...
def test():
    ...
    local = tools.env_var_get("UE-LocalDataCachePath")
    if local:
        drive, path = os.path.splitdrive(local)
        if not tools.validate_drivename(drive, check_exists=False):
            click.echo(f"Local data cache drive {drive} for {local} does not exist.")
        else:
            click.echo(f"Local data cache drive {drive} for {local} exists.")
    else:
        click.echo("No local data cache path set")

    ddc = tools.env_var_get("UE-SharedDataCachePath")
    if ddc:
        drive, path = os.path.splitdrive(ddc)
        echo(f"checking drive name {drive} for {ddc}...")
        if not tools.validate_drivename(drive, check_exists=False):
            click.echo(f"Checking shared data cache drive {drive} for {ddc}...")
            if not os.path.isdir(drive):
                click.echo(f"Shared data cache drive {drive} does not exist")
            else:
                click.echo(f"Shared data cache path {ddc} exists")
        else:
            click.echo(f"Shared data cache drive {drive} for {ddc} appears to be local")
    else:
        click.echo("No shared data cache path set")

        if not os.path.isdir(drive):
            raise click.ClickException(f"Local data cache drive {drive} does not exist")
    if os.path.isdir(local):
        echo(f"Local data cache path {local} exists")
...
def set_ddc_vars(work_drive, location):
    ...
    path = os.path.join(work_drive, config.work_drive_ddc_folder)
    ...
    # in the same place
    path = os.path.join(work_drive, config.work_drive_git_depends)
    print(f"setting git dependencies cache path to {path}")
    value = f"--cache={path}"
    tools.env_var_set("UE_GITDEPS_ARGS", value)
