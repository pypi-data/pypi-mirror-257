...
def register_current_engine_directory(
    ...
    if not desktop.platform.verify_file_associations(
        user=True, silent=True
    ) and not desktop.platform.verify_file_associations(user=False, silent=True):
        if unattended:
            do_it = True
        else:
        ...
        if do_it:
            # okay, we update it.
            # we use the engine's own UnrealVersionSelector if present, else we use this script
            desktop.platform.update_file_associations(
                engine_root=engine_root, handler=find_run_args(gui=True)
            )
...
def update_file_associations(check=False, clear=False, user=True, engine_root=None):
    if check:
        handler = desktop.platform.verify_file_associations(user=user)
        click.echo(
...
def launch_editor(uproject: Optional[str], args: List[str] = []):
    """Start the editor for project"""
...
def find_run_args(gui: bool = False):
