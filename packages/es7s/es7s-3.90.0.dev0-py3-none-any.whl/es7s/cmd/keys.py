...
class IBindCollector(metaclass=ABCMeta):
    ...
    def __init__(self, invoker_map: dict[str, KeyCombo | None], details: bool, group: bool, sort_by_title: bool) -> None:
        super().__init__()
    ...
    def _print_binds(self, bind_collector: IBindCollector):
        ...
        for key_table in bind_collector.get_key_tables():
            ...
            stdout.echo_rendered(
                ...
                + "  "
                + Fragment(
                    " " * (termw - len(key_table.name) - len(key_table.label) - (6 if key_table.name else  3)),
...
class action(_BaseAction):
    ...
    def _run(self, subjects: list[KeysMode], details: bool, group: bool, sort_by_title: bool, **kwargs):
        collectors = [
            v(details, group, sort_by_title)
