...
class Uploader:
    ...
    def import_execution(self, res_xml, execution_summary, project_id, testplan_id=''):
        """
        :param res_xml: junit format xml file path
        ...
        self.fetch_token()
        # if safe_mode:
        #     updated_xml = res_xml.replace('py_result', 'xray_result').replace('.xml', '_xray.xml')
        #     JunitXml(res_xml).dump_xray_format_xml(updated_xml)
        # else:
        #     updated_xml = res_xml
        updated_xml = res_xml
...
def parse_arguments():
    parser = argparse.ArgumentParser(formatter_class=argparse.RawTextHelpFormatter, prog='uploader',
                                     epilog="General purpose xray uploader.\n"
                                            "Currently either specify a testNG or a junit style xml file .\n")

    parser.add_argument('-x', '--xml', required=True, help='A xml file with absolute path.')
...
def main_cli():
    ...
    uploader.import_execution(args['xml'], args['summary'], args['projectId'], args['testPlan'])
