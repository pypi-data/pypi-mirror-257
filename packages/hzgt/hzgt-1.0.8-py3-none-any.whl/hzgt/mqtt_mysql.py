...
class Mysqldbop(object):
    ...
    def __init__(self, mysql_host: str, mysql_port: int, mysql_user: str, mysql_passwd: str):
        ...
        try:
            ...
            error = restrop(f"数据库连接失败,请检查数据库配置. Error: {err}")
            print(error)
            exit(-1)
    ...
    # ===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===---===
    # 插入数据
    def insert(self, tablename: str, params: dict):
        """
            往表里插入数据，单条数据
    ...
    def update(self, tablename: str, attrs_dict: dict, cond_dict: dict):
        """更新数据
            ***未完成校验***
            args：
                tablename  ：表名字\n
