...
from chik.data_layer.data_layer_errors import NodeHashError, TreeGenerationIncrementingError
...
async def test_server_selection(data_store: DataStore, tree_id: bytes32) -> None:
    ...
    await data_store.subscribe(
        Subscription(tree_id, [ServerInfo(f"http://127.0.0.1/{port}", 0, 0) for port in range(9234, 8010)])
    )
