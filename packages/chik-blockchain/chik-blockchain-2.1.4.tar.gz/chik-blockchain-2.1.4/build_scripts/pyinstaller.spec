...
ROOT = pathlib.Path(importlib.import_module("chik").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `chik` file name (which is the executable)
    and the `chik` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `chik` directory, since the names collide.
...
if os.path.exists(f"{ROOT}/madmax/chik_plot"):
    binaries.extend([
        (
            f"{ROOT}/madmax/chik_plot",
            "madmax"
        )
    ])

if os.path.exists(f"{ROOT}/madmax/chik_plot_k34",):
    binaries.extend([
        (
            f"{ROOT}/madmax/chik_plot_k34",
            "madmax"
        )
    ])
...
if THIS_IS_WINDOWS:
    chik_mod = importlib.import_module("chik")
    ...
    binaries = [
        ...
        (
            f"{ROOT}\\madmax\\chik_plot.exe",
            "madmax"
        ),
        (
            f"{ROOT}\\madmax\\chik_plot_k34.exe",
            "madmax"
        ),
        (
...
datas.append((f"{ROOT}/chik/util/english.txt", "chik/util"))
datas.append((f"{ROOT}/chik/util/initial-config.yaml", "chik/util"))
...
add_binary("chik", f"{ROOT}/chik/cmds/chik.py", COLLECT_ARGS)
...
add_binary("start_data_layer_http", f"{ROOT}/chik/data_layer/data_layer_server.py", COLLECT_ARGS)
add_binary("start_data_layer_s3_plugin", f"{ROOT}/chik/data_layer/s3_plugin_service.py", COLLECT_ARGS)
add_binary("timelord_launcher", f"{ROOT}/chik/timelord/timelord_launcher.py", COLLECT_ARGS)
