...
# the output will be printed to your chik root path, e.g. ~/.chik/mainnet/profile/
# to analyze the profile, run:

#   python chik/utils/profiler.py ~/.chik/mainnet/profile | less -r

# this will print CPU usage of the chik full node main thread at 1 second increments.
# find a time window of interest and analyze the profile file (which are in pstats format).

# for example:

#   python chik/utils/profiler.py ~/.chik/mainnet/profile 10 20
...
if __name__ == "__main__":
    ...
    def analyze_slot_range(profile_dir: pathlib.Path, first: int, last: int) -> None:
        ...
        output_file = "chik-hotspot-%d" % first
        if first < last:
            output_file += "-%d" % last
