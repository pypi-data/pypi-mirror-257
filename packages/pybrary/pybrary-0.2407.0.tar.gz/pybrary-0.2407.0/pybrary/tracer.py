...
class Tracer:
    ...
    def trace(self, file, line, func, args):
        if isinstance(args, tuple) and args[0] in ('>>>', '!!!'):
            if args[0] == '>>>':
                val = self.format_arg_return(args[1])
                args = f' -> {val}{self.elapsed}'
            elif args[0] == '!!!':
                args = f' ! {args[1].__name__} ! {args[2]}'
        else:
            args = '('+', '.join(a for a in self.format_args(args))+')'
