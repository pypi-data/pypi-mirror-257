...
class HybridScreenNew(AutomaticElement, HybridListener):
            ...
            if ShadowCongruence.check_empty_data(self.__input_data):
                if ShadowCongruence.check_empty_beam(self.__input_data.beam):
                    ...
                    try:
                        ...
                        self.progressBarInit()
                        beam, mirr, calculation_result = element.trace_beam()
                        ...
                        if self.view_type==1: self.plot_results(beam, calculation_result)

                        if self.propagation_type == HybridPropagationType.BOTH:
                            shadow_data_ff = ShadowData(beam=beam[0], beamline=beamline)
                            shadow_data_ff.scanning_data = self.__input_data.scanning_data
                            shadow_data_ff.initial_flux  = self.__input_data.initial_flux
                            shadow_data_nf = ShadowData(beam=beam[1], beamline=beamline)
                            ...
                            shadow_data_ff = ShadowData(beam=beam, beamline=beamline)
                            shadow_data_ff.scanning_data = self.__input_data.scanning_data
                            shadow_data_ff.initial_flux  = self.__input_data.initial_flux
                            self.send("Output Data (Far Field)",  shadow_data_ff)
                        elif self.propagation_type == HybridPropagationType.NEAR_FIELD:
                            shadow_data_nf = ShadowData(beam=beam, beamline=beamline)
                            shadow_data_nf.scanning_data = self.__input_data.scanning_data
                            shadow_data_nf.initial_flux  = self.__input_data.initial_flux
                            self.send("Output Data (Near Field)", shadow_data_nf)
    ...
    def plot_results(self, beam, calculation_result : HybridCalculationResult):
