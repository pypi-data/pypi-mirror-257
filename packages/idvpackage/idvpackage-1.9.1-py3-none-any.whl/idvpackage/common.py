...
def compute_ela_cv(orig_img, quality):
    ...
    # The above code is converting the color space of an image from BGR to RGB using OpenCV in Python.
...
def detect_id_card_uae(client, image_data, id_text, part=None):
    if id_text:
        ...
        tampered_result,diff_avg = calculate_error_difference(aligned_img, country = 'UAE')
        part_text=None
        if part == 'third':
            part_img = crop_third_part(aligned_img)
            part_text = extract_text_from_image_data(client, part_img)

        return  tampered_result, part_text,diff_avg
