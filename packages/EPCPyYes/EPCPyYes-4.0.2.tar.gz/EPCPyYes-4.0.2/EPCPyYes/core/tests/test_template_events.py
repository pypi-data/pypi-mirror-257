...
from EPCPyYes.core.v1_2.helpers import (
    gtin_urn_generator,
    get_current_utc_time_and_offset,
    gln13_data_to_sgln_urn,
    gtin_to_urn,
)
from EPCPyYes.core.v1_2.events import (
    BusinessTransaction,
    Source,
    Destination,
    Action,
...
from EPCPyYes.core.v1_2.template_events import (
    ObjectEvent,
    AggregationEvent,
    EPCISDocument,
    TransactionEvent,
...
from EPCPyYes.core.v1_2.json_decoders import (
    AggregationEventDecoder,
    ObjectEventDecoder,
...
class CoreEventTests(unittest.TestCase):
    ...
    def test_transaction_event_json_decode(self):
        ...
        # render the event using it's default template
        decoded = TransactionEventDecoder(te.render_pretty_json()).get_event()
    ...
    def create_aggregation_event(self, epcs, parent_id):
        ...
        trade_item = helpers.make_trade_item_master_data_urn("305555", "0", "555551")
        child_quantity_list = [
            QuantityElement(epc_class=trade_item, quantity=100),
            QuantityElement(epc_class=trade_item, quantity=94.3, uom="LB"),
        ]
    ...
    def create_sbdh(self):
        sender_partner_id = sbdh.PartnerIdentification(
            authority="SGLN", value="urn:epc:id:sgln:039999.999999.0"
        )
        receiver_partner_id = sbdh.PartnerIdentification(
            authority="SGLN", value="urn:epc:id:sgln:039999.111111.0"
        )
        sender = sbdh.Partner(
        ...
        header = template_sbdh.StandardBusinessDocumentHeader(
            document_identification=document_identification, partners=[sender, receiver]
        )
        self.assertEqual(sender_partner_id.value, "urn:epc:id:sgln:039999.999999.0")
        self.assertEqual(sender_partner_id.authority, "SGLN")
        self.assertEqual(receiver_partner_id.authority, "SGLN")
        self.assertEqual(receiver_partner_id.value, "urn:epc:id:sgln:039999.111111.0")
    ...
    def test_object_event_template(self):
        ...
        self.assertIn(
            "<epc>urn:epc:id:sgtin:305555.1555555.1000</epc>",
            data,
            "URN for start SGTIN not present.",
        )
        self.assertIn(
            "<epc>urn:epc:id:sgtin:305555.1555555.1001</epc>",
            data,
            "URN for start SGTIN not present.",
        )
        self.assertIn("<action>ADD</action>", data, "EPCIS action not present.")
        self.assertIn(
            BusinessSteps.commissioning.value, data, "Business step not present"
        )
        self.assertIn(Disposition.encoded.value, data, "Disposition not present")
    ...
    def test_transaction_event_template(self):
        ...
        self.assertIn("<action>ADD</action>", data, "EPCIS action not present.")
        self.assertIn(BusinessSteps.shipping.value, data, "Business step not present")
        self.assertIn(Disposition.in_transit.value, data, "Disposition not present")
    ...
    def test_epcis_base_template(self):
        ...
        ag1 = self.create_aggregation_event(
            self.create_epcs(1000, 1009), gtin_to_urn("305555", "2", "555555", "235")
        )
        ag2 = self.create_aggregation_event(
            self.create_epcs(1010, 1019), gtin_to_urn("305555", "2", "555555", "216")
        )
        parent_id = gtin_to_urn("305555", "1", "555551", 1000)
        epcs = self.create_epcs(1000, 1010)
    ...
    def test_epcis_event_list_template(self):
        ...
        epcis_document = EPCISEventListDocument(
            header=header, template_events=template_events
        )
        print(epcis_document.render())
        print(epcis_document.render_json())
        print(epcis_document.render_pretty_json())
    ...
    def create_transformation_event(self):
        ...
        ilmd = [
            InstanceLotMasterDataAttribute(
                name=LotLevelAttributeName.itemExpirationDate.value, value="2015-12-31"
            ),
            InstanceLotMasterDataAttribute(
                name=ItemLevelAttributeName.lotNumber.value, value="DL232"
            ),
        ]
    ...
    def create_source_list(self):
        ...
        biz_location = gln13_data_to_sgln_urn(
            company_prefix="305555", location_reference="123456"
        )
        read_point = gln13_data_to_sgln_urn(
            company_prefix="305555", location_reference="123456", extension="12"
        )
        # create a source list
        source_list = [
            Source(SourceDestinationTypes.possessing_party.value, biz_location),
    ...
    def create_destination_list(self):
        ...
        destination_party = gln13_data_to_sgln_urn(
            company_prefix="0614141", location_reference="00001"
        )
        destination_location = gln13_data_to_sgln_urn(
            company_prefix="0614141", location_reference="00001", extension="23"
        )
        destination_list = [
            Destination(SourceDestinationTypes.owning_party.value, destination_party),
            Destination(SourceDestinationTypes.location.value, destination_location),
        ]
