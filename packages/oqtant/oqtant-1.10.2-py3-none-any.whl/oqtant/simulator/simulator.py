...
class Simulator:
    ...
    def get_column_densities(self, time_ms: float) -> tuple:
        ...
        wavefunction.psi = self.psi_history[i + 1]  # load a snapshot of psi
        ...
        return column_zy, column_zx, slice_y, slice_x, wavefunction.three_d_grid
    ...
    def show_density_cylindrical(self, times_ms: list, figsize=(15, 7)) -> None:
        ...
        for time in converted_timesteps:
            ...
            wavefunction.psi = self.psi_history[i + 1]
            density = (
            ...
            plt.title(
                f"Density (Atoms/$\\mu$m$^3$) at time {time / self.qm_potential.oqt_time_to_sim} msec"
            )
    ...
    def show_column_densities(
        ...
        for time in converted_timesteps:
            ...
            plt.title(
                f"Column densities at time {time / self.qm_potential.oqt_time_to_sim} msec"
            )
            ...
            if slices:
                ...
                ax3.set_title(
                    f"Slices at time {time / self.qm_potential.oqt_time_to_sim} msec"
                )
    ...
    def show_phase(self, times_ms: list, figsize=(10, 7)) -> None:
        ...
        for time in converted_timesteps:
            ...
            plt.title(f"Phase at time {time / self.qm_potential.oqt_time_to_sim} msec")
            plt.xlabel("$X$-Position ($\\mu$m)")
    ...
    def show_current(self, times_ms: list, figsize=(10, 7)) -> None:
        ...
        for time in converted_timesteps:
            ...
            plt.title(
                f"Current at time {time / self.qm_potential.oqt_time_to_sim} msec"
            )
            plt.xlabel("$X$-Position ($\\mu$m)")
            plt.ylabel("Atom Current (# of Atoms / msec)")
            plt.show()

    def animate_current(self, frame_interval: int = 1) -> FuncAnimation:
        """
        ...
        ax1.set_ylim(-self.number_of_atoms, self.number_of_atoms)  # TBU
        ax1.set_xlim(
            -self.wavefunction.three_d_grid.Lx / 2,
            self.wavefunction.three_d_grid.Lx / 2,
        )
    ...
    def animate_phase(
        ...
        if show_potential:
            ...
            z = np.concatenate(
                (np.flip(self.qm_potential.potential), self.qm_potential.potential)
            )  # .potential_to_cartesian_oqt_units(),
    ...
    def animate_profiles(self, frame_interval=1):
        """
        Animates the density profiles and change in potentials over time.  This is an integrated profile along
        the x-direction and is different from a single slice(!).
        ...
        ax1.set_ylim(0, self.number_of_atoms / (QMPotential.sim_length_to_oqt * 5))
        ax1.set_xlim(
            -self.wavefunction.three_d_grid.Lx / 2,
            self.wavefunction.three_d_grid.Lx / 2,
        )
