...
class QuantumMatter(BaseModel):
    ...
    def show_potential(
        ...
        Args:
            ...
            include_magnetic (bool, optional): Flag to include contributions from magnetic trap
            include_terminator (bool, optional): Flag to include the position of the terminator beam relative to the trap
        """
        ...
        for time in times:
            ...
            if self.lasers and self.lasers[0].type == job_schema.LaserType.TERMINATOR:
    ...
    def show_laser_pulse_timing(self, figsize=(6, 4)) -> None:
        ...
        if self.lasers:
            for laser in self.lasers:
                ...
                times = np.arange(0, self.lifetime, 50e-3)
                ...
                for pulse in laser.pulses:
                    interpolation = job_schema.interpolation_to_kind(
                        pulse.interpolation
                    )
                    f_pulse = interp1d(
                        pulse.times_ms,
                    ...
                    det_idx = np.argwhere(
                        (times >= pulse.times_ms[0]) & (times <= pulse.times_ms[-1])
                    )
...
class QuantumMatterFactory:
    ...
    def create_terminator(time_on: float, time_off: float) -> Laser:
        ...
        laser_data = {
            ...
            "position_um": projected.TERMINATOR_POSITION,
            "pulses": [pulse],
        }
