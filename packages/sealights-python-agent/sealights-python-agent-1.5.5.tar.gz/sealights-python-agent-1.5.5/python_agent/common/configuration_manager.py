...
class ConfigurationManager(object):
    ...
    def init_configuration(self, command_type, token, buildsessionid, labid, tokenfile=TOKEN_FILE, buildsessionidfile=BUILD_SESSION_ID_FILE,
        ...
        self.try_load_configuration(scm_args, token, buildsessionid, tokenfile, buildsessionidfile, proxy,
                                    command_type, test_selection_enable, labid)
        self.init_features()
    ...
    def try_load_configuration(self, scm_args, token, buildsessionid, tokenfile, buildsessionidfile, proxy, command_type, test_selection_enable, labid):
        ...
        if command_type != CommandType.CONFIG:
            is_resolved_build_session_id = self.resolve_build_session_id(buildsessionid, buildsessionidfile, self.config_data.buildSessionIdFile, labid, backend_proxy, command_type)
    ...
    def resolve_build_session_id(self, buildsessionid, buildsessionidfile, env_buildsessionidfile, labid, backend_proxy, command_type):
        ...
        if not build_session_id and labid:
            build_session = self.resolve_build_session_from_labid(labid, backend_proxy, command_type)
    ...
    def resolve_build_session_from_labid(self, labid, backend_proxy, command_type):
        ...
        if build_session:
            ...
            log.info(f"build session id was resolved using lab id{msg_cont}. build session id: %s. labid: %s" % (build_session.buildSessionId, labid))
        else:
