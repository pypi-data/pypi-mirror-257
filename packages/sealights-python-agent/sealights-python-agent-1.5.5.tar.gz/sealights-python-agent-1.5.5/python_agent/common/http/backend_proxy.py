...
class BackendProxy(object):
    ...
    def submit_build_mapping(self, build_mapping):
        ...
        self._log_call("submit_build_mapping", build_mapping, response)
        response.raise_for_status()
    ...
    def send_footprints(self, footprints):
        ...
        self._log_call("send_footprints", footprints, response)
        response.raise_for_status()
    ...
    def send_agent_event(self, agent_event, add_metadata=False):
        ...
        self._log_call("send_agent_event", agent_event, response)
        response.raise_for_status()

    def _log_call(self, function_name, request, response):
        ...
        try:
            ...
            log_line = json.dumps(log_entry, default=lambda m: m.__dict__, indent=4)
            with open('sl_debug_calls.json', 'a') as log_file:
                log_file.write(log_line + "\n,")
        except Exception as e:
            log.warning("Failed logging call. Error: %s" % str(e))
    ...
    def _format_response(self, response):
        ...
        try:
            ...
            return json.dumps(json_response, default=lambda m: m.__dict__ if hasattr(m, "__dict__") else str(m),
                              indent=4)
        except ValueError:
            pass
        ...
        try:
            return json.dumps(response, default=lambda m: m.__dict__ if hasattr(m, "__dict__") else str(m), indent=4)
        except TypeError:
            pass
