...
def get_all_files(workspacepath, includes=None, excludes=None):
    ...
    for subdir, dirs, files in os.walk(workspacepath):
        ...
        included_files = [os.path.abspath(filename) for filename in included_files if
                          re.match(constants.PYTHON_FILES_REG, os.path.split(filename)[1])]
...
def scan_files(workspacepath, include, exclude):
    ...
    for full_path in files:
        scanned = file_scanner.calculate_file_signature(full_path,
                                                        get_top_relative_path(full_path, workspacepath=workspacepath))
        if scanned:
