...
class Serverless(object):
    ...
    def filter_build_digest(self, build_digest: dict[str, dict[str, List[int]]], full_path) -> dict[
    ...
    def save_sl_config(self):
        sl_config = LambdaConfig(self.config_data.appName,
                                 ...
                                 self.config_data.proxy)

        for sl_config_path in self.sl_config_paths:
            sl_config.buildDigest = self.filter_build_digest(self.build_digest,
                                                             os.path.abspath(sl_config_path).replace("\\", "/"))
