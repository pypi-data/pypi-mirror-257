...
from python_agent.test_listener.executors.test_frameworks.behave_execution import BehaveAgentExecution
...
def common_options(f):
    options = _common_options if (
            f.__name__ == "config" or f.__name__ == "prconfig") else _common_options + _build_session_options
...
@click.option("--exclude", help=Opts.omit.help, default='*venv*,*sealights_layer*', type=unshell_list)
@click.pass_context
...
@common_options
@click.option("--appname", required=True, help="Application name, case-sensitive.")
@click.option("--targetbranch", required=True,
...
@click.option("--pullrequestnumber", required=True,
              help="The number assigned to the Pull Request from the source control")
@click.option("--repourl", required=True,
...
@click.pass_context
def prconfig(ctx, token, tokenfile, proxy, appname, targetbranch, latestcommit, pullrequestnumber, repourl,
             buildsessionid, workspacepath, include,
             exclude):
    ctx.command_type = CommandType.CONFIG
    config_data = get_config_data(ctx, token, tokenfile, None, None, proxy, None)
    PrConfig(config_data, appname, targetbranch, latestcommit, pullrequestnumber, repourl, buildsessionid,
             workspacepath, include, exclude).execute()
...
@cli.command(context_settings=CONTEXT_SETTINGS)
@common_options
@click.option("--collectorurl", required=False, help="Provide collector url for lambda functions.", default=None,
              type=str)
@click.option("--exportlayerpath", required=False, help="Set export Sealights layer path",
              default=None, type=click.Path())
@click.option("--slconfigpaths", required=True,
              help="Set list of paths of lambdas functions to save Sealights configuration files", default=None,
              type=unshell_list)
@click.pass_context
def configlambda(ctx, token, tokenfile, proxy, buildsessionid, buildsessionidfile, collectorurl,
                 exportlayerpath, slconfigpaths):
    try:
        config_data = get_config_data(ctx, token, tokenfile, buildsessionid, buildsessionidfile, proxy, None,
                                      None)
...
def nose(ctx, token, tokenfile, proxy, buildsessionid, buildsessionidfile, labid, teststage, cov_report, per_test,
         interval, test_selection_disable, test_selection_retry_interval, test_selection_retry_timeout, testgroupid,
         args):
    args = list(args)
    config_data = get_config_data(ctx, token, tokenfile, buildsessionid, buildsessionidfile, proxy, labid)
    config_data.testSelection.update({"enable": not test_selection_disable, "interval": test_selection_retry_interval,
...
@click.argument('args', nargs=-1, type=click.UNPROCESSED)
@click.pass_context
def behave(ctx, token, tokenfile, proxy, buildsessionid, buildsessionidfile, labid, teststage, cov_report, per_test,
           interval, test_selection_disable, test_selection_retry_interval, test_selection_retry_timeout, testgroupid,
           args):
    args = list(args)
    config_data = get_config_data(ctx, token, tokenfile, buildsessionid, buildsessionidfile, proxy, labid)
    config_data.testSelection.update({"enable": not test_selection_disable, "interval": test_selection_retry_interval,
                                      "timeout": test_selection_retry_timeout})
    BehaveAgentExecution(config_data, labid, teststage, cov_report, per_test, interval, testgroupid, args).execute()
