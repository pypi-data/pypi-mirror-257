        ...
        for (let n of nodeStack) {
            ...
            if (n.data.key) {
                ...
                if (nxpfx.endsWith('/'+n.data.prefix+':'+n.data.name)) {
                    // check if list and key have same name
                    if n.data.xpath_pfx.slice(0).endsWith(']') {
                        // nxpfx will be pointing to list node now
                        // as removePredicate would have removed the
                        // key from nxpfx.
                        nxpfx = nxpfx.slice(0)
                    } else {
