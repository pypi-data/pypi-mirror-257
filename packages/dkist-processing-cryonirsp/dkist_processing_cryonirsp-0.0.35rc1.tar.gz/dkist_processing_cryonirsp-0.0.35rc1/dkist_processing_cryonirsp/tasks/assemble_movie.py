...
class AssembleCryonirspMovieBase(AssembleMovie):
    ...
    def _compute_frame_shape(self) -> tuple[int, int]:
        ...
        If the data are polarimetric the aspect ratio will still be the same so the L1 shape is OK.
        """
        ...
        if self.constants.correct_for_polarization:
            return tuple(np.array(flipped_shape) // 2)
    ...
    def write_overlay(self, draw: ImageDraw, fits_obj: CryonirspL0FitsAccess) -> None:
        ...
        if self.constants.correct_for_polarization:
            ...
            # Subtract 1 from the UV line so it is below the IQ line
            UV_line = self._get_middle_line(draw=draw, text="U   V", font=self.font_36) - 1
