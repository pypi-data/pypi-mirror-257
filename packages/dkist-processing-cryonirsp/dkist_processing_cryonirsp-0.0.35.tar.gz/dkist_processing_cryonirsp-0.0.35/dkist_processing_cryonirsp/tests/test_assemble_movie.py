...
def assemble_movie_task_with_tagged_movie_frames(
    tmp_path, recipe_run_id, init_cryonirsp_constants_db
):
    num_map_scans = 10
    num_scan_steps = 1
    frame_shape = (100, 200)
...
def test_assemble_movie(assemble_movie_task_with_tagged_movie_frames, mocker):
    ...
    task, _ = assemble_movie_task_with_tagged_movie_frames
    task()
...
def test_compute_frame_shape(assemble_movie_task_with_tagged_movie_frames):
    ...
    """
    task, frame_shape = assemble_movie_task_with_tagged_movie_frames

    # Task starts as polarimetric from fixture constants
    assert task._compute_frame_shape() == tuple(np.array(frame_shape)[::-1] // 2)
    ...
    assert task._compute_frame_shape() == tuple(np.array(frame_shape)[::-1])
