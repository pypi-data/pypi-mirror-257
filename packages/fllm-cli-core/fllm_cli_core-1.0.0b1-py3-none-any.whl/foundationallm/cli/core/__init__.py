...
def _configure_knack():
    """Override consts defined in knack to make them FLLM CLI-specific."""
...
class AzCli(CLI):
    ...
    def __init__(self, **kwargs):
        ...
        from foundationallm.cli.core.commands import register_cache_arguments
        from foundationallm.cli.core.commands.arm import (
            register_ids_argument, register_global_subscription_argument)
        from foundationallm.cli.core.cloud import get_active_cloud
        from foundationallm.cli.core.commands.transform import register_global_transforms
        from foundationallm.cli.core._session import ACCOUNT, CONFIG, SESSION, INDEX, VERSIONS
        from foundationallm.cli.core.util import handle_version_update
    ...
    def get_progress_controller(self, det=False, spinner=None):
        from foundationallm.cli.core.commands import progress
    ...
    def show_version(self):
        from foundationallm.cli.core.util import get_az_version_string, show_updates
    ...
    def exception_handler(self, ex):  # pylint: disable=no-self-use
        from foundationallm.cli.core.util import handle_exception
    ...
    def _configure_style(self):
        from foundationallm.cli.core.util import in_cloud_console
        from foundationallm.cli.core.style import format_styled_text, get_theme_dict, Style
...
class MainCommandsLoader(CLICommandsLoader):
    ...
    def load_command_table(self, args):
        ...
        def _update_command_table_from_modules(args, command_modules=None):
            ...
            if command_modules is not None:
                ...
                command_modules = []
                try:
                    mods_ns_pkg = import_module('foundationallm.cli.command_modules')
            ...
            for mod in [m for m in command_modules if m not in BLOCKED_MODS]:
                try:
                    ...
                    # module loading.
                    from foundationallm.cli.core import telemetry
            ...
            if extensions:
                ...
                for ext in allowed_extensions:
                    try:
                        ...
                        from foundationallm.cli.core.extension.operations import check_version_compatibility
        ...
        if use_command_index:
            ...
            if index_result:
                ...
                from foundationallm.cli.core.util import roughly_parse_command
                ...
                for cmd in self.command_table:
                    if raw_cmd.startswith(cmd):
                        ...
                        # command table. For example, `fllm find vm create` won't exist in the command table, but the
                        # corresponding command should be `fllm find`.
                        # raw command  : fllm find vm create
                        # command table: fllm find
                        # remaining    :         vm create
    ...
    def load_arguments(self, command=None):
        from foundationallm.cli.core.commands.parameters import (
...
class CommandIndex:
    ...
    def __init__(self, cli_ctx=None):
        ...
        """
        from foundationallm.cli.core._session import INDEX
        self.INDEX = INDEX
        if cli_ctx:
    ...
    def get(self, args):
        ...
        # "network": ["foundationallm.cli.command_modules.natgateway", "foundationallm.cli.command_modules.network", "azext_firewall"]
        ...
        if index_modules_extensions:
            ...
            command_module_prefix = 'foundationallm.cli.command_modules.'
    ...
    def update(self, command_table):
        ...
        :param command_table: The command table built by foundationallm.cli.core.MainCommandsLoader.load_command_table
        """
        start_time = timeit.default_timer()
        ...
        for command_name, command in command_table.items():
            ...
            # Get module name, like foundationallm.cli.command_modules.vm, azext_webapp
...
class AzCommandsLoader(CLICommandsLoader):  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(self, cli_ctx=None, command_group_cls=None, argument_context_cls=None,
        ...
        from foundationallm.cli.core.commands import AzCliCommand, AzCommandGroup, AzArgumentContext
    ...
    def _apply_doc_string(self, dest, command_kwargs):
        from foundationallm.cli.core.profiles._shared import APIVersionException
    ...
    def get_api_version(self, resource_type=None, operation_group=None):
        from foundationallm.cli.core.profiles import get_api_version
    ...
    def supported_api_version(self, resource_type=None, min_api=None, max_api=None, operation_group=None):
        from foundationallm.cli.core.profiles import supported_api_version
    ...
    def supported_resource_type(self, resource_type=None):
        from foundationallm.cli.core.profiles import supported_resource_type
        return supported_resource_type(
            cli_ctx=self.cli_ctx,
            resource_type=resource_type or self._get_resource_type())

    def get_sdk(self, *attr_args, **kwargs):
        from foundationallm.cli.core.profiles import get_sdk
    ...
    def _cli_command(self, name, operation=None, handler=None, argument_loader=None, description_loader=None, **kwargs):
        ...
        def default_command_handler(command_args):
            from foundationallm.cli.core.util import get_arg_list, augment_no_wait_handler_args
            from foundationallm.cli.core.commands.client_factory import resolve_client_arg_name
    ...
    def get_op_handler(self, operation, operation_group=None):
        ...
        from foundationallm.cli.core.profiles import AZURE_API_PROFILES
        from foundationallm.cli.core.profiles._shared import get_versioned_sdk_path
...
def get_default_cli():
    from foundationallm.cli.core.azlogging import AzCliLogging
    from foundationallm.cli.core.commands import AzCliCommandInvoker
    from foundationallm.cli.core.parser import AzCliCommandParser
    from foundationallm.cli.core._config import GLOBAL_CONFIG_DIR, ENV_VAR_PREFIX
    from foundationallm.cli.core._help import AzCliHelp
    from foundationallm.cli.core._output import AzOutputProducer
