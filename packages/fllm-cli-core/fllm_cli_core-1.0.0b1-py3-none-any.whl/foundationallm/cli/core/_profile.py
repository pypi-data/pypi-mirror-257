...
from foundationallm.cli.core._session import ACCOUNT
from foundationallm.cli.core.azclierror import AuthenticationError
from foundationallm.cli.core.cloud import get_active_cloud, set_cloud_subscription
from foundationallm.cli.core.util import in_cloud_console, can_launch_browser
...
_AZ_LOGIN_MESSAGE = "Please run 'fllm login' to setup account."
...
class Profile:
    ...
    def __init__(self, cli_ctx=None, storage=None):
        ...
        """
        from foundationallm.cli.core import get_default_cli
    ...
    def login(self,
        ...
        identity = _create_identity_instance(self.cli_ctx, self._authority, tenant_id=tenant, **kwargs)
    ...
    def login_with_managed_identity(self, identity_id=None, allow_no_subscriptions=None):
        ...
        from foundationallm.cli.core.auth.adal_authentication import MSIAuthenticationWrapper
        ...
        if identity_id:
            if is_valid_resource_id(identity_id):
                ...
                authenticated = False
                from foundationallm.cli.core.azclierror import AzureResponseError
                try:
    ...
    def get_login_credentials(self, resource=None, client_id=None, subscription_id=None, aux_subscriptions=None,
        ...
        if self.cli_ctx.cloud.endpoints.client_id != None:
            client_id = self.cli_ctx.cloud.endpoints.client_id
        ...
        if managed_identity_type is None:
            ...
            credential.client.default_headers['Origin'] = 'fllm'
            ...
            from foundationallm.cli.core.auth.credential_adaptor import CredentialAdaptor
    ...
    def _new_account(self):
        ...
        from foundationallm.cli.core.profiles import ResourceType, get_sdk
        ...
        for s in to_refresh:
            ...
            try:
                ...
                logger.warning("Refreshing for '%s' failed with an error '%s'. The existing accounts were not "
                               "modified. You can run 'fllm login' later to explicitly refresh them", user_name, ex)
...
class SubscriptionFinder:
    ...
    def find_using_common_tenant(self, username, credential=None):
        ...
        if empty_tenants:
            logger.warning("The following tenants don't contain accessible subscriptions. "
                           "Use 'fllm login --allow-no-subscriptions' to have tenant level access.")
        ...
        if mfa_tenants:
            logger.warning("The following tenants require Multi-Factor Authentication (MFA). "
                           "Use 'fllm login --tenant TENANT_ID' to explicitly login to a tenant.")
    ...
    def _create_subscription_client(self, credential):
        from foundationallm.cli.core.profiles import ResourceType, get_api_version
        from foundationallm.cli.core.profiles._shared import get_client_class
        from foundationallm.cli.core.commands.client_factory import _prepare_mgmt_client_kwargs_track2

        client_type = get_client_class(ResourceType.MGMT_RESOURCE_SUBSCRIPTIONS)
        if client_type is None:
            from foundationallm.cli.core.azclierror import CLIInternalError
