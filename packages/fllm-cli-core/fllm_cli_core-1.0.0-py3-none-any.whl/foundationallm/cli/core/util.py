...
CLI_PACKAGE_NAME = 'fllm-cli'
COMPONENT_PREFIX = 'fllm-cli-'

SSLERROR_TEMPLATE = ('Certificate verification failed. This typically happens when using FLLM CLI behind a proxy '
...
def handle_exception(ex):  # pylint: disable=too-many-locals, too-many-statements, too-many-branches
    ...
    from foundationallm.common import AzureException
    from foundationallm.core.exceptions import AzureError, ServiceRequestError
    from requests.exceptions import SSLError, HTTPError
    from foundationallm.cli.core import azclierror
    ...
    # - foundationallm.core.exceptions.ServiceRequestError is raised when indirectly calling 'requests' lib with foundationallm.core,
    #   which wraps the original SSLError
        ...
        if ex.errno == 0:
            az_error.set_recommendation(
                "Please report to us via Github: https://github.com/solliancenet/foundationallm/issues/20278")
        elif ex.errno == -2146893813:
            az_error.set_recommendation(
                "Please report to us via Github: https://github.com/solliancenet/foundationallm/issues/20231")
        elif ex.errno == -2146892987:
            az_error.set_recommendation(
                "Please report to us via Github: https://github.com/solliancenet/foundationallm/issues/21010")
        ...
        az_error.set_recommendation(
            "To check existing issues, please visit: https://github.com/solliancenet/foundationallm/issues")
...
def get_error_type_by_azure_error(ex):
    from foundationallm.core import exceptions
    from foundationallm.cli.core import azclierror
...
def get_installed_cli_distributions():
    ...
    from foundationallm.cli.core import __version__ as azure_cli_core_version
    from foundationallm.cli.telemetry import __version__ as azure_cli_telemetry_version
    ...
    return [
        VersionItem('fllm-cli', azure_cli_core_version),
        VersionItem('fllm-cli-core', azure_cli_core_version),
        VersionItem('fllm-cli-telemetry', azure_cli_telemetry_version)
    ]


def get_latest_from_github(package_path='fllm-cli'):
    try:
        import requests
...
def _update_latest_from_github(versions):
    ...
    success = True
    for pkg in ['fllm-cli-core', 'fllm-cli-telemetry']:
...
def get_cached_latest_versions(versions=None):
    ...
    import datetime
    from foundationallm.cli.core._session import VERSIONS
    ...
    if VERSIONS[_VERSION_UPDATE_TIME]:
        ...
        if datetime.datetime.now() < version_update_time + datetime.timedelta(days=1):
            ...
            if cache_versions and cache_versions['fllm-cli']['local'] == versions['fllm-cli']['local']:
...
def get_az_version_string(use_cache=False):  # pylint: disable=too-many-statements
    from foundationallm.cli.core.extension import get_extensions, EXTENSIONS_DIR, DEV_EXTENSION_SOURCES, EXTENSIONS_SYS_DIR
    import io
    output = io.StringIO()
    versions = _get_local_versions()
...
def get_az_version_json():
    from foundationallm.cli.core.extension import get_extensions
    versions = {'extensions': {}}
...
def shell_safe_json_parse(json_or_dict_string, preserve_order=False, strict=True):
    ...
    try:
        ...
        try:
            ...
            from foundationallm.cli.core.azclierror import InvalidArgumentValueError
            ...
            if parent_proc and parent_proc.lower() in ("powershell.exe", "pwsh.exe"):
                recommendation += "\nPowerShell requires additional quoting rules. See " \
                                  "https://github.com/solliancenet/foundationallm/blob/dev/doc/quoting-issues-with-powershell.md"
...
def poller_classes():
    ...
    from foundationallm.core.polling import LROPoller as AzureCoreLROPoller
    from foundationallm.cli.core.aaz._poller import AAZLROPoller
...
def send_raw_request(cli_ctx, method, url, headers=None, uri_parameters=None,  # pylint: disable=too-many-locals,too-many-branches,too-many-statements
    ...
    # Azure Rest API doc
    from foundationallm.cli.core._profile import Profile
    ...
    if not skip_authorization_header and url.lower().startswith('https://'):
        ...
        if not resource:
            ...
            # This follows the same behavior as `foundationallm.cli.core.commands.client_factory._get_mgmt_service_client`
            if url.lower().startswith(endpoints.resource_manager.rstrip('/')):
                resource = endpoints.active_directory_resource_id
            else:
                from foundationallm.cli.core.cloud import CloudEndpointNotSetException
...
def get_az_user_agent():
    ...
    from foundationallm.cli.core import __version__ as core_version

    agents = ["AZURECLI/{}".format(core_version)]

    from foundationallm.cli.core._environment import _ENV_AZ_INSTALLER
...
def get_secret_store(cli_ctx, name):
    """Create a process-concurrency-safe foundationallm.cli.core.auth.persistence.SecretStore instance that can be used to
    save secret data.
    """
    from foundationallm.cli.core._environment import get_config_dir
    from foundationallm.cli.core.auth.persistence import load_secret_store
