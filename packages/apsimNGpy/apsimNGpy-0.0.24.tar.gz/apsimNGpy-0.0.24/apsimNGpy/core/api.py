...
class APSIMNG():
    ...
    def replicate_file(self, k, path=None):
        """
        ...
        if path is None:
            ...
            return [shutil.copy(self.path, f"{file_name}_{i}_.apsimx") for i in range(k)]
        else:
            b_name = os.path.basename(self.path).rsplit('.apsimx', 1)[0]
            return [shutil.copy(self.path, os.path.join(path, f"{b_name}_{i}.apsimx")) for i in range(k)]
