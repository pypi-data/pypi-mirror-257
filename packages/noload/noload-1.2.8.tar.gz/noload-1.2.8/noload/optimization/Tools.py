...
class StructList:
    ...
    """
    List = None # Liste regroupant l'ensemble des contraintes scalaires
    # et/ou vectorielles
    ...
    def __init__(self, List=[], etat='unflattened', shape=[]):
        ...
        if self.etat == 'unflattened': # si la liste n'est pas "aplatie"
            if len(self.List) == 1 and isinstance(self.List[0], (int, float)):
                ...
                for i in range(len(self.List)): # on parcourt les éléments de
                    ...
                    if isinstance(self.List[i], (list, np.ndarray)): # si la
                        ...
                        sh.append(0) # on ajoute un 0 (signe que la contrainte
                        # est scalaire)
                shape = sh
        self.shape = shape

    def flatten(self): # fonction pour "aplatir" une listes de contraintes
        # complexes
        """
        ...
        if len(self.List) == 1 and isinstance(self.List[0], (int, float)):
            ...
            return self.List
        res = []
        for i in range(len(self.List)): # on parcourt chaque type de contrainte
            # dans la liste
            if isinstance(self.List[i], (list, np.ndarray)): # si la contrainte
                # est vectorielle
                for j in range(len(self.List[i])): # pour chaque composante de
                    # cette contrainte vectorielle
    ...
    def unflatten(self): # fonction pour "reformer" une liste de contraintes
        # complexes "aplatie"
        """
        "Reforms" a "flattened" list of mixed constraints (from 'flattened'
        to 'unflattened')
