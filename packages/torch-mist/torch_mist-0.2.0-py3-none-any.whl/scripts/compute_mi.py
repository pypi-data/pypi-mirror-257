...
def default_logged_methods(estimator: MIEstimator) -> Tuple[
    List[Tuple[str, Callable]], List[Tuple[str, Callable]]
]:
    eval_logged_methods = [
...
def prepare_data(conf: DictConfig) -> Tuple[
    TensorDictLike, Optional[TensorDictLike], Optional[TensorDictLike], Optional[float]
]:
    # Train (mandatory)
...
def parse(conf: DictConfig):
    ...
    train_logged_methods, eval_logged_methods = default_logged_methods(estimator)
