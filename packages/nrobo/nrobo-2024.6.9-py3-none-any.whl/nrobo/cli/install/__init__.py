...
def transfer_files_to_host_project():
    ...
    if (Path(os.environ[EnvKeys.EXEC_DIR]) / NP.CONFTEST_PY).exists():
        ...
        patch_2024_6_9 = Path(os.environ[EnvKeys.NROBO_DIR]) / "patch_2024_6_9"
        if patch_2024_6_9.exists():
            return

        # Apply patches
        from nrobo.cli.upgrade import get_host_version
        if Version(get_host_version()) == Version("2024.6.9"):
            Common.write_text_to_file(Path(os.environ[EnvKeys.NROBO_DIR]) / "patch_2024_6_9", "")
