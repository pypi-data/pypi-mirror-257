...
def install_nrobo(requirements_file: Optional[str] = None) -> None:
    ...
    if os.environ[EnvKeys.ENVIRONMENT] == Environment.PRODUCTION:
        ...
        from nrobo.cli.upgrade import confirm_update, get_host_version

        if get_host_version() in ["2024.6.3", "2024.6.2", "2024.6.1"]:
            confirm_update(forced=True)
            try:
                copy_file(Path(os.environ[EnvKeys.NROBO_DIR]) / NROBO_PATHS.CONFTEST_PY,
                      Path(os.environ[EnvKeys.EXEC_DIR]) / NROBO_PATHS.CONFTEST_PY)
            except FileNotFoundError as e:
                # Meaning program is running on development machine during building packages
                # thus, just ignore it.
                pass
        ...
        if nrobo_installed:
            ...
            transfer_framework_files_to_host_dir()
