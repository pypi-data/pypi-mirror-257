...
class PackageUtils:
    ...
    def get_days_before_lock(self):
        if (isinstance(self.json_license.get(self.LicenseJson.config_constant), dict) and
                isinstance(self.json_license.get(self.LicenseJson.config_constant).get(
                    self.LicenseJson.config_expire), dict) and isinstance(
                    self.json_license.get(self.LicenseJson.config_constant).get(
                        self.LicenseJson.config_expire).get("days_before_lock"), int)):
            return self.json_license.get(self.LicenseJson.config_constant).get(self.LicenseJson.config_expire).get(
                "days_before_lock")
        return None
    ...
    def calculate_limit_number(self, number_mtp, package_code):
        ...
        if (isinstance(self.json_license.get(self.LicenseJson.config_constant), dict) and
                isinstance(self.json_license.get(self.LicenseJson.config_constant).get(
                    self.LicenseJson.coefficient_message), dict) and isinstance(
                    self.json_license.get(self.LicenseJson.config_constant).get(
                        self.LicenseJson.coefficient_message).get(package_code), int)):
            coefficient_message = self.json_license.get(self.LicenseJson.config_constant).get(
    ...
    def get_max_event_free(self):
        if (isinstance(self.json_license.get(self.LicenseJson.config_constant), dict) and
                isinstance(self.json_license.get(self.LicenseJson.config_constant).get(
                    "max_event_free"), int)):
            return self.json_license.get(self.LicenseJson.config_constant).get("max_event_free")
        return 1_000_000
