...
from bluecat_libraries.http_client import GeneralError, ErrorResponse
from bluecat_libraries.address_manager.api.rest.auto import _wadl_parser
from bluecat_libraries.address_manager.api.serialization import deserialize_joined_key_value_pairs
...
def _get_rest_service_base_url(url: str, use_https: bool = False) -> str:
    ...
    """
    parts = parse_url(url)
    scheme = "https" if use_https else (parts.scheme or "https")
    if parts.port:
...
def _mask_password_stack_trace(exc_type, exc_value, exc_traceback, password, exc_message):
    """
    Returns the modified stack trace and exception message with masked password

    :param exc_type: Exception type
    :param exc_value: Exception value
    :param exc_traceback: Exception traceback
    :param password: Password to check for in the message and traceback
    :param exc_message: Exception message
    :return: Modified stack trace and exception
    :rtype: tuple(str, str)
    """
    masked_message = exc_message.replace(f"password={password}", "password=********")
    formatted_stack_trace = traceback.format_exception(exc_type, exc_value, exc_traceback)
    for i, value in enumerate(formatted_stack_trace):
        decoded_value = unquote_plus(value)
        if password in decoded_value:
            formatted_stack_trace[i] = decoded_value.replace(
                f"password={password}", "password=********"
            )

    modified_traceback_str = "".join(formatted_stack_trace)
    return modified_traceback_str, masked_message
...
class Client:
    ...
    def login(self, username, password):
        ...
        try:
            ...
            if password is not None and password in unquote_plus(str(exc)):
                exc_type, exc_value, exc_traceback = sys.exc_info()
                # This unquoting is to handle unicode characters being sent in the URL request
                exception = unquote_plus(str(exc))
                modified_traceback_str, masked_message = _mask_password_stack_trace(
                    exc_type, exc_value, exc_traceback, password, exception
                )
                raise GeneralError(f"Connection Error: {masked_message}") from Exception(
                    f"Traceback was modified to mask passwords: \n{modified_traceback_str}"
                )
            raise
    ...
    def loginWithOptions(self, username, password, options):
        ...
        try:
            ...
            if password is not None and password in unquote_plus(str(e)):
                exc_type, exc_value, exc_traceback = sys.exc_info()
                # This unquoting is to handle unicode characters being sent in the URL request
                exception = unquote_plus(str(e))
                modified_traceback_str, masked_message = _mask_password_stack_trace(
                    exc_type, exc_value, exc_traceback, password, exception
                )
                raise GeneralError(f"Connection Error: {masked_message}") from Exception(
    ...
    def logout(self) -> str:
        """
        Wrapper for the REST logout API/endpoint.

        :return: The message from BAM for the logout request.
        :rtype: str
        """
        content = self._service.logout(self.session)
        self.clear_token()
        return content
    ...
    def _set_version(self) -> None:
        ...
        try:
            ...
            if "API method cannot be invoked from a read only session" in e.message:
                _LOGGER.warning(
