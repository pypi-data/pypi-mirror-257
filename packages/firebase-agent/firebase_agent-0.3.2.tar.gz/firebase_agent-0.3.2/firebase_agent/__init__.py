...
def run(config_entities: list[ConfigEntity]):
    ...
    for config_entity in config_entities:
        if config_entity.type == ConfigEntityType.HTTP:
            headers = {
        ...
        if config_entity.create_aggregate:
            ...
            for doc_query in docs_query.get():
                ...
                current_aggregate_hourly = dict_entity_aggregate.get(ts_key_hourly)
                current_aggregate_hourly.add_value(entity.value)
            for entity_aggregate in dict_entity_aggregate.values():
                doc_entity_aggregate_exist = (
                    coll_entity_aggregate_ref.where(filter=FieldFilter("start_ts", "==", entity_aggregate.start_ts))
                    .where(filter=FieldFilter("end_ts", "==", entity_aggregate.end_ts))
                    .limit(1)
                    .get()
                )
                ...
                if len(doc_entity_aggregate_exist) == 1 and entity_aggregate.count != doc_entity_aggregate_exist[0].get(
                    "count"
                ):
                    coll_entity_aggregate_ref.document(doc_entity_aggregate_exist[0].id).update(
                        entity_aggregate.to_dict()
                    )
                    print(
                        f"update to firestore collection: {coll_entity_aggregate_ref.id}, document: {doc_entity_aggregate_exist[0].id}"
                    )
