...
def main_movie(movie_id):
    ...
    for quality in qualities:
        if quality < height:
            ...
            m3u8_line = f"#EXT-X-STREAM-INF:PROGRAM-ID=1,BANDWIDTH={new_width*new_height},CODECS=\"avc1.4d4033,mp4a.40.2\",AUDIO=\"audio\",SUBTITLES=\"SUBS\",RESOLUTION={new_height}x{new_width}\n/video_movie/{quality}/{movie_id}.m3u8\n"
            file.append(m3u8_line)
    last_line = f"#EXT-X-STREAM-INF:PROGRAM-ID=1,BANDWIDTH={width*height},CODECS=\"avc1.4d4033,mp4a.40.2\",AUDIO=\"audio\",SUBTITLES=\"SUBS\",RESOLUTION={width}x{height}\n/video_movie/{movie_id}.m3u8\n\n\n"
...
def main_serie(episode_id):
    ...
    for quality in qualities:
        if quality < height:
            ...
            if (new_height % 2) != 0:
                new_height += 1
            m3u8_line = f"#EXT-X-STREAM-INF:BANDWIDTH={new_width*new_width},SUBTITLES=\"SUBS\",RESOLUTION={new_height}x{new_width}\n/video_serie/{quality}/{episode_id}\n"
            file.append(m3u8_line)
    last_line = f"#EXT-X-STREAM-INF:BANDWIDTH={width*height},SUBTITLES=\"SUBS\",RESOLUTION={width}x{height}\n/video_serie/{episode_id}\n"
    file.append(last_line)
    file = file[::-1]
    file = "".join(file)
    m3u8_file += file
            ...
            all_captions.append(
                {
                    ...
                    "languageCode": language,
                    "language": new_language,
                    "url": f"/chunk_caption_serie/{language}/{index}/{movie_id}.vtt",
                    "name": title_name,
                }
            )
