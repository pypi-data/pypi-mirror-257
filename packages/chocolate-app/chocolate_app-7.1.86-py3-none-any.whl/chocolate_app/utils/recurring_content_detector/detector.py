...
def detect(videos, feature_vector_function="CH", annotations=None, artifacts_dir=None, framejump=3, percentile=10,
    ...
    artifacts_dir : str
        ...
        defined with the videos parameter.
    framejump : int
    ...
    video_path = videos
    if isinstance(videos, str) and os.path.isdir(videos):
    ...
    if artifacts_dir is None and os.path.isdir(video_path):
        artifacts_dir = video_path
    ...
    for file in videos:
        ...
        file_full = os.path.join(video_path, file)
    ...
    for video, result in results:
        framerate = video_functions.get_framerate(os.path.join(video_path, video))
