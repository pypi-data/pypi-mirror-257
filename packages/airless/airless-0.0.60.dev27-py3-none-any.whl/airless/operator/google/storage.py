                    ...
                    if tables[key]['size'] > threshold['size_medium']:
                        ...
                        if (len(tables[key]['files']) > threshold['file_quantity']):
                            if tables[key]['size'] < threshold['size_small']:
                                self.send_to_process(from_bucket=bucket, to_bucket=bucket, directory=key, files=tables[key]['files'], size=ProcessTopic.SMALL)
                            else:
                                self.send_to_process(from_bucket=bucket, to_bucket=bucket, directory=key, files=tables[key]['files'], size=ProcessTopic.MEDIUM)
        ...
        for directory, v in tables.items():
            if v is not None:
                if (len(v['files']) == 1) and (directory not in partially_processed_tables):
                    self.send_to_process(from_bucket=bucket, to_bucket=get_config('GCS_BUCKET_RAW_ZONE'), directory=key, files=[filename], size=ProcessTopic.MEDIUM)  # only have one file that is lower than best performnce partition size in this directory
                    ...
                    self.send_to_process(from_bucket=bucket, to_bucket=bucket, directory=directory, files=v['files'], size=ProcessTopic.SMALL)
                elif ((directory in partially_processed_tables) or (v['min_time_created'].strftime('%Y-%m-%d %H:%M') < time_threshold)):
                    self.send_to_process(from_bucket=bucket, to_bucket=bucket, directory=directory, files=v['files'], size=ProcessTopic.MEDIUM)
