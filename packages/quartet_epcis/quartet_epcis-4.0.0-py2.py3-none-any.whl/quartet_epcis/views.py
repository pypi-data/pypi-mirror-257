...
class FormatHelperMixin:
    ...
    def get_formatted_data(self, request: Request, template_event, format=None):
        if (
            "xml" in request.content_type.lower()
            or "xml" in request.query_params.get("format", "")
            or format == "xml"
        ):
...
class EventDetailView(views.APIView, FormatHelperMixin):
    ...
    def get(self, request: Request, format=None, event_id: str = None):
        ...
        try:
            ...
            response = Response(
                "Event with id %s could not be " "found" % event_id,
...
class EntryEventHistoryView(views.APIView, FormatHelperMixin):
    ...
    def get(
        ...
        """
        args = {"epc_pk": entry_pk} if entry_pk else {"epc": entry_identifier}
        ...
        if len(events) > 0:
            ...
            response_data = self.get_formatted_data(request, epcis_document, format)
            return Response(response_data, status.HTTP_200_OK)
        else:
            raise NotFound(_("The entry with id %s could not be found." % str(args)))
...
class EventsByILMDView(views.APIView, FormatHelperMixin):
    ...
    def get(self, request: Request, format=None, ilmd_name=None, ilmd_value=None):
        pyyes_events = proxy.get_events_by_ilmd(ilmd_name, ilmd_value)
        if len(pyyes_events) > 0:
            epcis_document = template_events.EPCISEventListDocument(pyyes_events)
            response_data = self.get_formatted_data(
                request, epcis_document, format=format
            )
            return Response(response_data, status.HTTP_200_OK)
        else:
            msg = _(
                "No events could be found that match name %s "
...
class MessageDetail(views.APIView, FormatHelperMixin):
    ...
    def get(self, request: Request, format=None, message_id=None):
        ...
        try:
            ...
            found_message_id = (
                "The message with id %s could not " "be found" % message_id
            )
            logger.debug()
