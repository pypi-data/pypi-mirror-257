...
class BusinessEPCISParser(QuartetParser):
    ...
    def _handle_aggregation_entries(
        ...
        if epcis_event.action == events.Action.add.value:
            ...
            db_entries, count = self._get_entries_for_aggregation(epcis_event)
            ...
            if not count == len(epcis_event.child_epcs):
                ...
                raise errors.InvalidAggregationEventError(
                    _(
                        "The aggregation event with parent %s "
                        "contained children "
                ...
                self._update_aggregation_entries(
                    db_entries, parent, db_event, epcis_event
                )
        else:
            self.handle_entries(db_event, epcis_event.child_epcs, epcis_event)

    def _get_error_list(
        self, epcis_event: events.AggregationEvent, db_entries: EntryList
    ):
        """
    ...
    def _get_entries_for_aggregation(self, epcis_event: events.AggregationEvent):
        """
        ...
        if count < len(epcis_event.child_epcs):
            ...
            db_entries = entries.Entry.objects.select_for_update().filter(**kwargs)
    ...
    def _update_aggregation_entries(
        ...
        for db_entry in db_entries:
            ...
            if db_entry.is_parent:
                ...
                self._update_aggregation_entries(
                    self._get_child_entries(db_entry), db_entry, db_event, epcis_event
                )
    ...
    def _update_event_entries(
        self, db_entries, db_event: db_events.Event, epcis_event: events.EPCISEvent
    ):
        """
        Will update the value of the entries and add them to the cache if they
        were not there already or just update the cache entries.  If a list
        ...
        if isinstance(db_entries, list):
            ...
            if count == 0:
                raise errors.EntryException(_("No Entry records were updated."))
            # then update the local cache if the db update was successful
            for entry in db_entries:
                self.entry_cache[entry.identifier] = entry

    def _handle_aggregation_parent(
        self, db_event: db_events.Event, epcis_event: events.AggregationEvent
    ):
        """
        ...
        try:
            ...
            raise errors.EntryException(
                _(
                    "The parent entry with identifer %s was either "
    ...
    def _handle_aggregation_delete_action(
        self, db_event: db_events.Event, epcis_event: events.AggregationEvent
    ):
        ...
        if epcis_event.parent_id and len(epcis_event.child_epcs) > 0:
            ...
            db_entries = self._get_entries(epcis_event.child_epcs)
            ...
            lower_entries.update(top_id=None)
        self._create_parent_entry_event(db_event, epcis_event)

        self._update_aggregation_entries(db_entries, None, db_event, epcis_event)
    ...
    def _get_entry(self, epc: str):
        ...
        if entry and entry.decommissioned:
            raise errors.DecommissionedEntryException(
                "The entry with identifier %s has been decommissioned.", epc
            )
        if not entry:
            try:
                entry = entries.Entry.objects.get(identifier=epc, decommissioned=False)
                # add it to the cache
                self.entry_cache[epc] = entry
            except entries.Entry.DoesNotExist:
                raise errors.EntryException(
                    _(
                        "The entry with identifier %s could "
    ...
    def _get_entries(self, epcs: list):
        ...
        for epc in epcs:
            ...
            if epc:
                db_entries.append(epc)
        if len(db_entries) != len(epcs):
            db_entries = db_proxy.get_entries_by_epcs(epcs, select_for_update=False)
            if db_entries.count() != len(epcs):
                raise errors.EntryException(
                    _(
                        "Invalid Entry in %s.  One of the values in the "
                        "event has either "
    ...
    def _recursive_child_update(self, parents: list):
        ...
        parents = [
            entry for entry in parents if entry.is_parent and entry.is_top is False
        ]
        for entry in tops:
        ...
        for entry in parents:
            if entry.top_id not in tops:
                children = db_proxy.get_entries_by_parent(
                    entry, select_for_update=False
                )
                children.all().update(
                    last_event=entry.last_event,
                    last_event_time=entry.last_event_time,
    ...
    def _child_update(self, tops: EntryList):
        ...
        """
        for entry in tops:
            entries.Entry.objects.filter(top_id__in=tops).update(
                last_event=entry.last_event,
                last_event_time=entry.last_event_time,
                last_disposition=entry.last_disposition,
            )
    ...
    def clear_cache(self):
        ...
        if self.recursive_child_update:
            if self.child_update_from_top:
                tops = [entry for entry in self.entry_cache.values() if entry.is_top]
                self._child_update(tops)
            else:
                parents = [
                    entry for entry in self.entry_cache.values() if entry.is_parent
                ]
                self._recursive_child_update(parents)
        # clear the event cache
        self.event_cache.clear()
        decommissioned_entries = list(self.decommissioned_entry_cache.values())
