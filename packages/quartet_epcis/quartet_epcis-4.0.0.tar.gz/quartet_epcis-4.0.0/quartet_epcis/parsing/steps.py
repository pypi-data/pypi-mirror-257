...
def create_rule():
    ...
    rule = Rule.objects.create(
        name=_("EPCIS"),
        description=_(
            "Will capture and parse all properly formed inbound "
    ...
    step = Step.objects.create(
        name=_("Parse XML"),
        description=_(
            "Parse EPCIS data and save to database. To set loose "
    ...
    StepParameter.objects.create(
        ...
        value="False",
        description=_(
            "If set to true, QU4RTET will capture all properly "
...
class EPCISParsingStep(RuleStep):
    ...
    def __init__(self, db_task: models.Task, **kwargs):
        ...
        self.loose_enforcement = self.get_boolean_parameter("LooseEnforcement", False)
        self.format = self.get_parameter("Format", "XML")
        self.recursive_child_update = (
    ...
    def declared_parameters(self):
        return {
            "LooseEnforcement": "Whether or not the parsing step should "
            ...
            "Recursive Child Update": "Boolean, whether or not to recursively update all child"
            " entries during object events to reflect the state of "
            "their parent.",
            "Use Top For Child Update": "Boolean, or not to use the top hierarchical item for "
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        if self.format.upper() == "XML":
            parser_type = (
                QuartetParser if self.loose_enforcement else BusinessEPCISParser
            )
        else:
            parser_type = JSONParser
        self.info(
            "Loose Enforcement of busines rules set to %s", self.loose_enforcement
        )
        ...
        try:
            ...
            try:
                ...
                self.error(
                    "Could not convert the data into a format that " "could be handled."
                )
                raise
        # add the message id to the context
        message_id = parser.parse()
        self.info(
            "Adding Message ID %s to the context under " "key MESSAGE_ID.", message_id
        )
        rule_context.context[ContextKeys.EPCIS_MESSAGE_ID_KEY.value] = message_id
