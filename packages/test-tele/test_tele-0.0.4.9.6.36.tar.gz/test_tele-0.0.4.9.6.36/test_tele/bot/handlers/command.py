...
from test_tele.bot.const import TTLOLI_START, TTLOLI_HELP
...
async def start_command_handler(event):
    ...
    try:
        ...
        await event.respond(TTLOLI_START)
    except:
        pass
    finally:
...
async def help_command_handler(event):
    ...
    await event.respond(TTLOLI_HELP)
    ...
    **Example** 
    ...
    try:
        ...
        if match:
            ...
            user_lang = query.read_datas('settings', ['lang'], 'chat_id = %s', [event.message.chat_id])
            if user_lang:
                tm = await translate(tm, user_lang[0][0])
            caption = tm.text + f"\n\n👉 {BOT_CONFIG.bot_name} 👈"
