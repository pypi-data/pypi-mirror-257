...
async def inline_realperson(client, inline_query):
    ...
    logging.warning(f"lama proses gambar : {time.time() - start}")
    lists = await get_nude_list(keywords, pid, rp_mode)
    logging.warning(f"lama ambil gambar : {time.time() - start}")
    ...
    logging.warning(f"lama ngurus hasil : {time.time() - start}")
    try:
        ...
        logging.warning(f"lama seluruh proses : {time.time() - start}")
