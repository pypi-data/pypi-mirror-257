...
async def get_images_list(query, chunk, max_slice):
    if query.startswith('.px'):
        img_lists = [lists['img_urls']['img_original'].replace('600x1200_90', '768x1200_80') for lists in chunk[:max_slice]]
    elif query.startswith('.rp'):
...
async def upload_images_to_tele(as_file, event, img_lists):
    ...
    try:
        ...
        await event.client.send_file(event.input_chat, img_lists)
        return None
