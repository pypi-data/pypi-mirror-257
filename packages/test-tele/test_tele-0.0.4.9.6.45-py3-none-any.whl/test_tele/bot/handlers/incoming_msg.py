...
async def download_media_url_handler(event, is_premium: bool=False):
    ...
    try:
        ...
        if isinstance(gallery_dl_result, dict):
            ...
            if category == 'twitter':
                file_path_list = gallery_dl_result.split("\r\n")
                for file_path in file_path_list:
                    file_path = file_path[4:]#.replace("#", "").replace(".", "").replace("\\", "/").strip()
        ...
        if category == 'twitter':
            ...
            time.sleep(1800)
            remove_files = [cleanup(img) for img in enumerate(lists)]
            await asyncio.gather(*remove_files)
