...
async def inline_pixiv(client, inline_query):
    ...
    logging.warning("selesai cleanup : %s", time.time() - start)
    px_mode = await get_user_setting(inline_query, px_mode)
    logging.warning("selesai ambil data user : %s", time.time() - start)
    lists = await get_pixiv_list(keywords, pid, px_mode)
    logging.warning("selesai ambil list pixiv : %s", time.time() - start)
    ...
    logging.warning("selesai buat inline query result : %s", time.time() - start)

    try:
        next_offset = None
        if 'illust_detail' not in px_mode or ('illust_detail' in px_mode and len(lists) == 50):
            next_offset = str(pid + limit)
        logging.warning("selesai cek mode : %s", time.time() - start)
        ...
        logging.warning("selesai seluruh proses : %s", time.time() - start)
