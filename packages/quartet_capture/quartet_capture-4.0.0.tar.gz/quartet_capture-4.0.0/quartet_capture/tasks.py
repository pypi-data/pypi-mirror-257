...
from quartet_capture.models import (
    Task as DBTask,
    Rule as DBRule,
    TaskHistory,
    Filter,
...
def execute_queued_task(task_name: str, user_id: int = None, raise_exception=False):
    """
    ...
    try:
        ...
        logger.exception(
            "The task exceeded the configured time limit "
...
def create_and_queue_task(
    ...
    try:
        ...
        try:
            ...
            logger.warning(
                "There was a task name conflict trying to generate " "a new one..."
            )
            task.save()
        ...
        if run_immediately:
            ...
            execute_queued_task(
                task_name=task.name, user_id=user_id, raise_exception=True
            )
        else:
        ...
        raise RuleNotFound(
            _(
                "The Rule with name %s could not be found.  Please check "
...
def get_rules_by_filter(
    filter_name: str, message: str, return_all: bool = True
) -> StringList:
    """
    ...
    filter = Filter.objects.prefetch_related("rulefilter_set").get(name=filter_name)
    ret = []
    match_found = False
    for rule_filter in filter.rulefilter_set.all():
        if not match_found and rule_filter.default:
            ...
            match = rule_filter.search_value in message and not rule_filter.reverse
            if match:
                match_found = True
                ret.append(rule_filter.rule.name)
                if not return_all or rule_filter.break_on_true:
                    break
        elif rule_filter.search_type == "regex":
            match = (
                re.search(rule_filter.search_value, message) and not rule_filter.reverse
            )
            if match:
                match_found = True
                ret.append(rule_filter.rule.name)
