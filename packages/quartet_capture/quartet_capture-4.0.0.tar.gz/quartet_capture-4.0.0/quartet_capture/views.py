...
from quartet_capture.tasks import (
    execute_queued_task,
    ...
    def get(self, request: Request, task_name: str = None, format=None):
        if task_name:
            ...
            try:
                ...
                logger.error("There was an unexpected error executing the " "task")
                ret = Response(
...
class CloneRuleView(APIView):
    ...
    schema = ManualSchema(
        fields=[
            coreapi.Field(
                "rule_name", required=True, location="path", schema=coreschema.String()
            ),
            coreapi.Field(
                "new_rule_name",
                required=True,
        ...
        if rule_name:
            try:
                ...
                if getattr(settings, "DEBUG", False):
                    logger.exception(
                        "Error trying to clone rule with name." % rule_name
                    )
                    ret = Response(
...
class CaptureInterface(APIView):
    ...
    def post(self, request: Request, format=None, epcis=False):
        logger.info(
            "Message from %s",
            getattr(request.META, "REMOTE_HOST", "Host Info not Available"),
        )
        ...
        if len(files) == 0:
            ...
            raise exceptions.APIException(
                "Only one file may be posted at a time.", status.HTTP_400_BAD_REQUEST
            )
        for file, message in files.items():
            ...
            if filter_name:
                logger.debug("Grabbing a list of rules for filter %s.", filter_name)
                return_all_rules = (
                    request.query_params.get("return-all-rules")
                    or DEFAULT_RETURN_ALL_RULES
                )
                try:
                    rules = get_rules_by_filter(filter_name, message, return_all_rules)
                    ...
                    exc = exceptions.APIException(
                        ...
                        " the name of a valid filter on this instance." % filter_name
                    )
            ...
            run = (
                True
                if request.query_params.get("run-immediately") in ["true", "True"]
                else False
            )
            ...
            for rule_name in rules:
                ...
                if not self._rule_exists(rule_name):
                    exc = exceptions.APIException(
                        ...
                        "not exist in the system." % rule_name
                    )
                    exc.status_code = status.HTTP_400_BAD_REQUEST
                    raise exc
                ...
                try:
                    ...
                    exc = exceptions.APIException(
                        "Error in rule %s: %s" % (rule_name, args)
                    )
                    exc.status_code = status.HTTP_500_INTERNAL_SERVER_ERROR
                    raise exc
                return self._determine_return_status(request, task)
            raise exceptions.APIException(
                "No task was created.  This is most "
    ...
    def _inspect_data(self, request: Request):
        ...
        if len(request.data) > 0:
            ...
            raise exceptions.APIException(
                "No data was posted.", status.HTTP_400_BAD_REQUEST
            )
    ...
    def _queue_task(
        self, message, rule, file_store: storage.Storage, request: HttpRequest
    ):
        """
    ...
    def _get_task_parameters(self, request: Request, ignore=None):
        """
        ...
        for k, v in params.items():
            if k not in ignore:
                ret.append(TaskParameter(name=k, value=v))
        return ret
