...
class TaskMessaging:
    ...
    def debug(self, *args: object, task: models.Task = None):
        ...
        """
        self._create_task_message(
            *args, task=task or self.task, level=TaskMessageLevel.DEBUG
        )
    ...
    def warning(self, *args: object, task: models.Task = None):
        ...
        :param task: The associated task.
        """
        self._create_task_message(*args, task=task, level=TaskMessageLevel.WARNING)
    ...
    def error(self, *args: object, task: models.Task = None):
        ...
        :param task: The associated task.
        """
        self._create_task_message(*args, task=task, level=TaskMessageLevel.ERROR)
        ...
        *args: object,
        ...
        try:
            ...
            models.TaskMessage.objects.create(
                message=message, task=task or self.task, level=level.value
            )
        except:
    ...
    def get_required_context_variable(self, key: str):
        ...
        if not ret:
            raise errors.ExpectedContextVariableError(
                _(
                    "The context variable with key %s could not be located "
...
class DependencyMixin:
    def get_running_tasks(self):
        ...
        """
        running_tasks = models.Task.objects.filter(status="RUNNING").values_list(
            "name", flat=True
        )
    ...
    def monitor_tasks(self):
        ...
        task_interval = int(
            getattr(settings, "QUARTET_CAPTURE_WAIT_CYCLE_INTERVAL"), 10
        )
        cycles = int(getattr(settings, "QUARTET_CAPTURE_MAX_WAIT_CYCLES", 1080))
        while tasks.count() > 0 or cycles == 0:
            # refresh the tasks
            tasks = models.Task.objects.filter(name__in=running_tasks)
...
class Rule(TaskMessaging):
    ...
    def __init__(self, rule: models.Rule, task: models.Task):
        ...
        self.context.context["RULE_PARAMETERS"] = {
            p.name: p.value for p in self.db_rule.ruleparameter_set.all()
        }
    ...
    def _load_step(self, db_step: models.Step):
        ...
        params = {p.name: p.value for p in db_step.stepparameter_set.all()}
...
class Step(TaskMessaging, metaclass=ABCMeta):
    ...
    def get_parameter(
        self, parameter_name: str, default: str = None, raise_exception: bool = False
    ):
        """
    ...
    def get_integer_parameter(
        self, parameter_name: str, default: str, raise_exception: bool = False
    ):
        """
    ...
    def get_boolean_parameter(
        self, parameter_name: str, default: bool = False, raise_exception: bool = False
    ):
        """
    ...
    def get_or_create_parameter(
        self, name: str, default: str, description: str = "Default value."
    ):
        """
        ...
        *default* and returns that.
        ...
        """
        try:
            param = models.StepParameter.objects.get(name=name, step=self.db_step)
        except models.StepParameter.DoesNotExist:
            param = models.StepParameter.objects.create(
                name=name, value=default, step=self.db_step, description=description
            )
        return param.value
    ...
    def get_task_parameters(self, rule_context: RuleContext) -> dict:
        ...
        """
        ret = {}
        params = models.TaskParameter.objects.filter(task__name=rule_context.task_name)
        for param in params:
    ...
    def _check_parameters(self, declared: dict, parameters: dict):
        for parameter in parameters.keys():
            if not declared.get(parameter):
                raise self.ParameterNotFoundError(
                    _(
                        "A parameter with name %s was found in the parameters "
...
def clone_rule(rule_name: str, new_rule_name: str):
    ...
    *This Rule 1*.
    ...
    db_rule = models.Rule.objects.prefetch_related(
        "step_set", "step_set__stepparameter_set", "ruleparameter_set"
    ).get(name=rule_name)
