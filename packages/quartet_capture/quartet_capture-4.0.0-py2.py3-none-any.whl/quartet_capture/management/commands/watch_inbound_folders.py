...
class ProcessInboundFiles(FileSystemEventHandler):
    ...
    def create_task_for_inbound_file(self, filepath: str, rule_name: str):
        ...
        with open(filepath, "rb") as f:
            data = f.read()
            logging.info(
                "Creating task for file %s and rule %s" % (filepath, rule_name)
            )
    ...
    def on_created(self, event):
        ...
        try:
            ...
            if self.check_for_file_expansion(event.src_path):
                ...
                raise self.FileIncompleteException(
            ...
            logging.warning(
                "An exception occurred while processing creation event, recovering. %s"
                % str(e)
            )
            reset_queries()

    def check_for_file_expansion(
        self, file_path, tries=120, try_interval=0.5, success_tries=6
    ):
        """
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.create_folders_for_rules(inbound_file_directory_processed, group_name)
        ...
        try:
            while True:
                self.create_folders_for_rules(inbound_file_directory, group_name)
                self.create_folders_for_rules(
                    inbound_file_directory_processed, group_name
                )
                reset_queries()
                time.sleep(120)
        except:
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "inbound_dir",
            help="The inbound directory to" " monitor.",
        )
        parser.add_argument(
            "processed_dir",
            help="The directory to store " "files in after " "processing",
        )
        parser.add_argument(
            "group",
            help="The default group to assign " "ownership of new directories " "to",
        )
