<script lang="ts">
  ...
  async function handleShare() {
    const client = await getClient();
    // Component is hosted in an iframe so window.location.href will have
    // the deployed application URL, e.g. https://lastmileai-gradio-notebook-template.hf.space/
    // instead of the space URL, e.g. https://huggingface.co/spaces/lastmileai/gradio-notebook-template
    // Try to get the workspace URL from the parent window if it exists
    // See https://stackoverflow.com/a/7739035 for details
    let workspaceUrl = window.location.href;
    if (window.location != window.parent.location) {
      const parentUrl = document.referrer;
      // Simple check to make sure it's a space url since referrer could be a different domain
      // if redirects are used, e.g. for authentication
      if (parentUrl && parentUrl.includes("huggingface.co/spaces")) {
        workspaceUrl = parentUrl;
      }
    }
