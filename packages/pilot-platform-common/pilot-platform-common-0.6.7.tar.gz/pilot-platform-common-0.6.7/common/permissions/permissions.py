...
async def has_file_permission(
    auth_url: str, metadata_url: str, file_entity: dict, operation: str, current_identity: dict
) -> bool:
    ...
    folder_path_split = file_entity.get(path_for_permissions, '').split('/')
    ...
    if root_folder == 'shared':
        ...
        if not await has_permission(
            auth_url, project_code, f'file_shared_{project_folder_id}', zone, operation, current_identity
        ):
            return False
        else:
            return True
