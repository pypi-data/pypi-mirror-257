...
class BatteryStorageData(UnitModelBlockData):
    ...
    def build(self):
        ...
        self.initial_energy_throughput = Var(
            within=NonNegativeReals,
            initialize=0.0,
        ...
        def accumulate_energy_throughput(b, t):
            return (
                b.energy_throughput[t]
                == b.initial_energy_throughput
                + b.dt * (b.elec_in[t] + b.elec_out[t]) / 2
            )
        ...
        def state_of_charge_bounds(b, t):
            return (
                b.state_of_charge[t]
                <= b.nameplate_energy - b.degradation_rate * b.energy_throughput[t]
            )
    ...
    def report(self, time_point=0, dof=False, ostream=None, prefix=""):
        ...
        stream_attributes["kWh"]["initial_state_of_charge"] = value(
            self.initial_state_of_charge
        )
        stream_attributes["kWh"]["initial_energy_throughput"] = value(
            self.initial_energy_throughput
        )
        stream_attributes["kWh"]["state_of_charge"] = value(
            self.state_of_charge[time_point]
        )
        stream_attributes["kWh"]["energy_throughput"] = value(
            self.energy_throughput[time_point]
        )
