...
def ssc_cmod(ssc, dat, name):
    ...
    ssc_table_dict = ssc_table_to_dict(ssc, cmod, dat)
    ssc.module_free(cmod)
    return [True, ssc_table_dict]
    ...
    def value(self, name, val=None):
        if val is None:
            if name in self.params.keys():
                ...
                raise Exception(f"'{name}' is not a model parameter or output.")
        else:
            self.params[name] = val

    def execute(self):
        results = ssc_sim_from_dict(self, self.params)
