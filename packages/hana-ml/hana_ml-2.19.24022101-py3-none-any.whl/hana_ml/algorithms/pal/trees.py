...
class RDTClassifier(_RDTBase):#pylint: disable=too-many-instance-attributes
    ...
    Bagging: The sampling size is n, and each one is drawn from the original data set with replacement.
...
class DecisionTreeClassifier(_DecisionTreeBase):
    ...
    4. As for ordered independent variable, C45 and CHAID firstly discretise it, whereas CART uses predicate {is xm ≤ c?} instead of {is xm ∈ s?} to handle it. For large data set with a number of ordered independent variables, consequently, CART is more efficient than the other two.
