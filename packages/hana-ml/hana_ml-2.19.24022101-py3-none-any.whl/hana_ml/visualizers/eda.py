...
#pylint: disable=bare-except, misplaced-bare-raise
import logging
import operator
import sys
import math
    ...
    for pkg in list(installed_packages):
        if 'plotly' in pkg.key:
            if version_compare(plotly.__version__, "4.14.3"):
                return True
    logger.error("plotly (>=4.16.3) has not been installed.")
    raise
...
def hist(data, columns, bins=None, debrief=False, x_axis_fontsize=10,
         ...
         rounding_precision=3, replacena=0, enable_plotly=True, **kwargs):
    """
...
class EDAVisualizer(Visualizer):
    ...
    def __init__(self, ax=None, size=None, cmap=None, enable_plotly=True, fig=None, no_fig=False):
