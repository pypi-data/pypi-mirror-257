...
class TaskManager(threading.Thread):
    def __init__(self, automatic_obj_proxy: AutomaticObjProxy, update_ui_interval, fetch_table_interval, runtime_platform):
        ...
        if runtime_platform == 'console':
            ...
            if runtime_platform != 'jupyter':
                self.should_print_special_log = True
...
class UpdateProgressStatusToConsoleTask(object):
    def __init__(self, manager: TaskManager, update_ui_interval):
        ...
        runtime_env = str(sys.__dict__['displayhook'])
        self.is_cmd_line_env = False
        if runtime_env.find('<built-in') >= 0:
            self.is_cmd_line_env = True
        # windows: nt  |  linux/mac: posix
        self.clear_screen_cmd = 'cls' if os.name == 'nt' else 'clear'

    def clear_log(self):
        if self.is_cmd_line_env:
    ...
    def display_progress_report(self):
        ...
        frame_id = '{}'.format(uuid.uuid4()).replace('-', '_').upper()
        html_str = TemplateUtil.get_template('pipeline_progress.html').render(
            executionId=self.manager.automatic_obj_proxy.progress_indicator_id,
            frameId=frame_id,
            highlighted_metric_name=self.manager.automatic_obj_proxy.highlight_metric,
            is_simple_mode='false',
    ...
    def __task(self):
        ...
        if self.manager.fetch_table_task.is_read_completed():
            ...
            if self.generation_num == self.current_generation:
                self.current_generation = -1
            self.print_main_info()
            if self.is_cmd_line_env is False:
