...
def _data_summary(data, select_stat=None):
    param_template = "param_name[{pos}] := N'{param_name}';\nint_value[{pos}] := {int_value};\ndouble_value[{pos}] := {double_value};\nstring_value[{pos}] := {string_value};"
    ext_params = [param_template.format(pos=2, param_name='CATEGORICAL_OUTPUT_FORMAT', int_value=1, double_value='NULL', string_value='NULL')]
    if select_stat:
        ext_params.append(param_template.format(pos=3, param_name='SELECT_STAT_STATE', int_value=1, double_value='NULL', string_value='NULL'))
        if isinstance(select_stat, str):
            select_stat = [select_stat]
        if isinstance(select_stat, (list, tuple)):
            for idx, stat in enumerate(select_stat, start=4):
                ext_params.append(param_template.format(pos=idx, param_name='SELECT_STAT', int_value='NULL', double_value='NULL', string_value="N'{}'".format(stat.upper())))
    unique_id = str(uuid.uuid1()).replace('-', '_').upper()
    tables = ['RESULT']
    tables = ['#PAL_DATASUMMARY_{}_TBL_{}'.format(name, unique_id) for name in tables]
    execute_sql = "DO\nBEGIN\nDECLARE param_name VARCHAR(5000) ARRAY;\nDECLARE int_value INTEGER ARRAY;\nDECLARE double_value DOUBLE ARRAY;\nDECLARE string_value VARCHAR(5000) ARRAY;\nparam_name[1] := N'HAS_ID';\nint_value[1] := 0;\ndouble_value[1] := NULL;\nstring_value[1] := NULL;\n{2}\nparams = UNNEST(:param_name, :int_value, :double_value, :string_value);\nin_0 = {0};\nCALL _SYS_AFL.PAL_UNIVARIATE_ANALYSIS(:in_0, :params, out_0, out_1);\nCREATE LOCAL TEMPORARY COLUMN TABLE {1} AS (SELECT * FROM :out_0);\nEND;".format(data.select_statement, tables[0], '\n'.join(ext_params))
    with data.connection_context.connection.cursor() as cur:
        cur.execute(execute_sql)
        logger.info(execute_sql)
        data.connection_context.last_execute_statement = execute_sql
    return data.connection_context.table(tables[0])
...
restricted by the keys of ``to_replace`` and ``value``.
    ...
    def auto_cast(self, type_convert):
        ...
        for kkey, vval in self.get_table_structure().items():
            if vval in type_convert:
                new_cast[kkey] = type_convert[vval]
