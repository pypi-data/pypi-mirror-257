...
def condition_index(data, key=None, col=None, scaling=True,
    ...
    r"""
    Detect collinearity problem between independent
...
def ftest_equal_var(data_x, data_y, test_type=None):
    r"""
    Test the equality of two random
    ...
    if len(data_x.columns) * len(data_y.columns) != 1:
        ...
        logger.error(msg)
        raise ValueError(msg)
    if not all(var.dtypes(var.columns[0])[0][1] in ('INT', 'FLOAT', 'DOUBLE') for var in (data_x, data_y)):
...
def interval_quality(data, significance_level, score_type=None, ave_abs_error=None,
    ...
    res_tbl, stats_tbl = tables
    if not (check_pal_function_exist(conn,
                                     '%INTERVAL_QUALITY%',
