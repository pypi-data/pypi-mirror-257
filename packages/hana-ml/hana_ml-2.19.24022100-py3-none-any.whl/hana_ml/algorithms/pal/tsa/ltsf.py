...
#pylint: disable= c-extension-no-member, super-with-arguments, too-many-statements, invalid-name, consider-using-f-string
...
class LTSF(PALBase):
    ...
    explainer_ : DataFrame
        ...
        The attribute only appear when ``show_explainer=True`` and ``network_type`` is 'XLinear' in the `predict()`.
...
#pylint: disable=too-many-arguments, too-many-branches, too-many-statements
    ...
    def predict(self, data, key=None, endog=None, allow_new_index=True, show_explainer=False, reference_dict=None):
        ...
        param_rows = []
        unique_id = str(uuid.uuid1()).replace('-', '_').upper()
        forecast_tbl = "#PAL_LTSF_FORECAST_RESULT_TBL_{}_{}".format(self.id, unique_id)
        ...
        if show_explainer is True:
            ...
            decompose_tbl = '#PAL_LTSF_DECOMPOSE_TBL_{}_{}'.format(self.id, unique_id)
