...
.. code-block:: python
    ...
    from gcloud.rest.auth import IapToken
...
* ``session``: an ``aiohttp.ClientSession`` instance to be used for all
  ...
  manager (``with IapToken(..) as token:``) or explicitly calling the
...
* ``session``: an ``aiohttp.ClientSession`` instance to be used for all
  ...
  manager (``with Token(..) as token:``) or explicitly calling the
...
__version__ = importlib.metadata.version('gcloud-rest-auth')
__all__ = [
    'SyncSession',
    'BUILD_GCLOUD_REST',
    'IamClient',
