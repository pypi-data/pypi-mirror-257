...
def noscbank_cell_no_sum(lvix, chans, nw, p_inc, i_inv, rg):
    """
    ...
    return (lastval, index), o


def noscbank_cell(lvix, chans, nw, p_inc, i_inv, rg):
    a, b = noscbank_cell_no_sum(lvix, chans, nw, p_inc, i_inv, rg)
    return a, jnp.sum(b, axis=0)


def noscbank(lvix, chans, nw, p_inc, i_inv, rg, cell=noscbank_cell):
    return jax.lax.scan(
        partial(cell, nw=nw, p_inc=p_inc, i_inv=i_inv, rg=rg), lvix, chans
    )
