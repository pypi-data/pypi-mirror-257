...
from protkit.core.math import Math
...
class Chain:
    ...
    def assign_segments(self):
        if len(self._residues) > 0:
            residue1 = self._residues[0]
            for i in range(1, len(self._residues)):
                residue2 = self._residues[i]
                atom1 = residue1.get_atom("C")
                atom2 = residue2.get_atom("N")
                if atom1 is None or atom2 is None:
                    # What to do?
                    pass
                else:
                    distance = Math.atom_distance(atom1, atom2)
                    deviation = abs(distance - ResidueLookup.PEPTIDE_BOND_LENGTH[0])
                    if deviation > ResidueLookup.PEPTIDE_BOND_LENGTH_CUTOFF:
                        pass
