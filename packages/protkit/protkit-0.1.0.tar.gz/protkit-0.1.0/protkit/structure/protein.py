...
class Protein:
    ...
    def _expand_atom_types(self, atom_types: Dict[str, Any]) -> List[str]:
        if len(atom_types) >= 0:
            text = []
            for chain in atom_types:
                for residue in atom_types[chain]:
                    text.append(f"{chain}:{residue}: {atom_types[chain][residue]}")
            return "\n".join(text)
    ...
    def anomaly_report(self) -> str:
        """
        Returns a report of the protein's anomalies.
        """
        text = [
            f"Protein ID: {self.id}"
        ]
        text.append("Missing heavy atom types:\n" + self._expand_atom_types(self.missing_heavy_atom_types()))
        text.append("Extra heavy atom types:\n" + self._expand_atom_types(self.extra_heavy_atom_types()))
        text.append("Missing hydrogen atom types:\n" + self._expand_atom_types(self.missing_hydrogen_atom_types()))
        text.append("Extra hydrogen atom types:\n" + self._expand_atom_types(self.extra_hydrogen_atom_types()))
        return "\n".join(text)