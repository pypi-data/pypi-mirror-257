...
chemical(a, rin, 30). chemical(b, rin, 60). chemical(c, rin, 10).
nutrient(c, rin, 30).

entity(E, factory) :- factory(E).
entity(E, farm) :- farm(E).

polluted_river(River) :- 
    river(River), 
    % #sum{P1: entity(E, T), chemical(E, River, P1); P2: entity(E, T), nutrient(E, rin, P2)}=Sum, 
    #sum{P: __aggregate_set(P,River)}=Sum, 
    Sum = 51..100.
__aggregate_set(P1,River) :- entity(E, T), chemical(E, River, P1).
__aggregate_set(P2,River) :- river(River), entity(E, T), nutrient(E, rin, P2).
        """.strip()
    print(Model.of_program(program).as_facts)
    explain = Explain.the_program(
        program,
        the_answer_set=Model.of_program(program),
        the_atoms_to_explain=Model.of_atoms("polluted_river(rin)"),
        # the_additional_atoms_in_the_base=Model.of_atoms("q"),
    )
    ...
    explain.compute_igraph()  # it can also be omitted
    print(explain.show_navigator_graph())
