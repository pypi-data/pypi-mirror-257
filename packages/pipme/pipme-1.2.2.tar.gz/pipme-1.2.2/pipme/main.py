...
def run_cli(shell_path:str=os.getcwd()):
    ...
    ask_user = input(f"Using {shell_path} as root directory. \nIf this is not the correct root directory, press 'n' else anything: ")
    if ask_user=="" or ask_user.lower().split()=='n':
        return 


    # ask if they registered at pypi.org and have an access token ready or not ------------------------------------
    ask_user = input(f"Are you registered at https://pypi.org/ and have an access token (y/N): ")
    if ask_user=='' or ask_user.lower().split()=='n':
        return 
    ...
    print(f"Package name used: {valid_user_package_name}")


    # ask user for the version they wish to make it ----------------------------------------------------------------
    ask_user_version = input("What's the version you wish to give(default 1.0.0): ")
    # check user input to version's authneticity
    if ask_user_version=="":
        ask_user_version = "1.0.0"
    while not is_valid_version(ask_user_version):
        print("Unauthentic version syntax, must be of type num.num.num")
        ask_user_version = input("What's the version you wish to give(default 1.0.0): ")
        if ask_user_version=="":
            ask_user_version = "1.0.0"
    ...
    print(f"Migrating files from {shell_path} to {package_path}")
    ...
    print("Migrated shell path to:",os.getcwd())
    ...
    print("Running command to create distwheel...")
    ...
    # get access token
    access_token = input(f"Enter the access token: ")
    if len(access_token)<150:
        print("The access token entered seems too short to be precise")
        return
    # run pip upload command
    print("Running command to upload to pip using twine and access token provided...")
