...
GLOBAL_MEM int *data)
{
    ...
    int offset = pos * box_size_x * box_size_y;

    // Iterate over the relevant region around the peaks
    for (int j = ds_y; j < box_size_y - ds_y; j++)
    {
        for (int i = ds_x; i < box_size_x - ds_x; i++)
        {
            // Calculate index
            int idx = offset + box_size_x * j + i;

            // Find first peak
            float value = input[idx].x;
            ...
            // Find second peak
            if (value > max_peak2 && abs(i - idcol1) > lm && abs(j - idrow1) > lm)
            {
                max_peak2 = value;
                idrow2 = j;
                idcol2 = i;
    ...
    // Calculate the sum of neighboring pixels for both peaks
    ...
    // Determine the peak with the highest sum of neighboring pixels
    if (temp1 > temp2)
    {
        idx = idx1;
        idrow = idrow1;
