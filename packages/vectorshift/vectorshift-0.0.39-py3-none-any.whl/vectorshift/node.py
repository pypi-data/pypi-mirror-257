...
class OutputNode(NodeTemplate):
    ...
    def _from_json_rep(json_data:dict) -> 'OutputNode':
        return OutputNode(
            ...
            output_type=json_data['outputType'].lower(),
            input=None,
            skip_typecheck=True
        )
...
class TextNode(NodeTemplate):
    ...
    def _from_json_rep(json_data:dict) -> 'TextNode':
        text_inputs = {}
        for name in json_data.get('inputNames', []):
...
class FileNode(NodeTemplate):
    ...
    def _from_json_rep(json_data:dict) -> 'FileNode':
        file_names = [
            file_data['name'] \
                 for file_data in json_data.get('selectedFiles', [])
        ]
        return FileNode(
            file_names=file_names,
            process_files=json_data['processFiles'],
            chunk_size=json_data['chunkSize'],
            chunk_overlap=json_data['chunkOverlap'],
            skip_typecheck=True
        )
...
class PipelineNode(NodeTemplate):
    ...
    def _from_json_rep(json_data:dict) -> 'PipelineNode':
        inputs = {}
        for i in json_data['pipeline'].get('inputs', {}).values():
            inputs[i['name']] = [None]
        return PipelineNode(
            pipeline_id=json_data['pipeline']['id'],
...
class IntegrationNode(NodeTemplate):
    ...
    def __init__(self, integration_type:str, integration_name:str, 
            action:str, inputs:dict[str, list[NodeOutput]],
            public_key:str=None, private_key:str=None, **kwargs):
        super().__init__()
        ...
        if action not in INTEGRATION_PARAMS[integration_type].keys():
            raise ValueError(f'IntegrationNode: invalid action {action} for integration {integration_type}.')
        ...
        if self.integration_type != 'Notion' or self.action != 'write_to_database':
            if sorted(list(inputs.keys())) != sorted(input_names):
    ...
    def outputs(self):
        os = {}
        for o in self.action_params['outputs']:
            output_field = o['name']
            os[output_field] = NodeOutput(
                source=self, 
                output_field=output_field,
    ...
    def _from_json_rep(json_data:dict) -> 'IntegrationNode':
        return IntegrationNode(
            integration_type=json_data['integration']['type'],
            integration_name=json_data['integration']['name'],
            action=json_data['function']['name'],
            ...
            inputs={ 
                i['name']: [None] 
                for i in json_data['function'].get('inputs', [])
            },
            skip_typecheck=True
        )
...
class PineconeIntegrationNode(IntegrationNode):
    ...
    def __init__(self, integration_name:str, action:str, 
...
class OpenAILLMNode(NodeTemplate):
    ...
    def _from_json_rep(json_data:dict) -> 'OpenAILLMNode':
        text_inputs = {}
        for name in json_data.get('systemInputNames', []):
            text_inputs[name] = None
        for name in json_data.get('promptInputNames', []):
            text_inputs[name] = None
        return OpenAILLMNode(
            model=json_data['model'],
            system_input=json_data['system'],
            prompt_input=json_data['prompt'],
            text_inputs=text_inputs,
            max_tokens=json_data['maxTokens'],
            temperature=float(json_data['temperature']),
            top_p=float(json_data['topP']),
            stream_response=json_data['stream'],
            json_response=json_data['jsonResponse'],
            personal_api_key=json_data['apiKey'] if json_data['usePersonalAPIKey'] else None,
            skip_typecheck=True
        )
...
class ImageToTextNode(OpenAILLMNode):
    ...
    def __init__(self, model:str, 
        ...
        stream_response:bool = False, json_response:bool = False, 
        personal_api_key:str = None, **kwargs):
        super().__init__(model, system_input, prompt_input, text_inputs, max_tokens, temperature, top_p, stream_response, json_response, personal_api_key)
        self.node_type = 'llmOpenAIVision'
        self.task_name = 'image_to_text'
        if model not in SUPPORTED_IMAGE_TO_TEXT_MODELS:
            raise ValueError(f'OpenAILLMNode (ImageToTextNode): invalid model {model}.')
        if not 'skip_typecheck' in kwargs or not kwargs['skip_typecheck']:
            check_type(f'OpenAILLMNode (ImageToTextNode) input image', image_input, IMAGE_FILE_TYPE)
...
class ApiLoaderNode(NodeTemplate):
    ...
    def _from_json_rep(json_data:dict) -> 'ApiLoaderNode':
        return ApiLoaderNode(
            ...
            url=json_data['url'],
            headers=[(h['key'], h['value']) for h in json_data['headers']],
            param_type=json_data['param'],
            params=[(p['key'], p['value']) for p in json_data['params']],
            skip_typecheck=True
        )
...
class VectorQueryNode(NodeTemplate):
    ...
    def _from_json_rep(json_data:dict) -> 'VectorQueryNode':
        return VectorQueryNode(
            ...
            func=json_data['function'],
            max_docs_per_query=json_data['maxDocsPerQuery'],
            enable_filter=json_data['enableFilter'],
            filter_input=json_data['filter'],
            rerank_documents=json_data['rerankDocuments'],
            skip_typecheck=True
        )
...
class VectorStoreNode(NodeTemplate):
    ...
    def _from_json_rep(json_data:dict) -> 'VectorStoreNode':
        ...
        return VectorStoreNode(
            query_input=None,
            vectorstore_id=json_data['vectorStore']['id'],
            vectorstore_name=json_data['vectorStore']['name'], 
            max_docs_per_query=json_data['maxDocsPerQuery'],
            enable_filter=json_data['enableFilter'],
            filter_input=json_data['filter'],
            rerank_documents=json_data['rerankDocuments'],
            alpha=json_data.get('alpha', None),
            skip_typecheck=True
        )
...
class LogicConditionNode(NodeTemplate):
    ...
    def _from_json_rep(json_data:dict) -> 'LogicConditionNode':
        ...
        return LogicConditionNode(
            inputs=[(name, None) for name in json_data.get('inputNames', [])],
...
class SplitTextNode(NodeTemplate):
    ...
    def _from_json_rep(json_data:dict) -> 'SplitTextNode':
        delimiter_name = json_data['delimiter']
        delimiter_chars = json_data['character']
...
class ChatMemoryNode(NodeTemplate):
    ...
    def _from_json_rep(json_data:dict) -> 'ChatMemoryNode':
        ...
        n.memory_window = json_data['memoryWindow']
        return n
