...
from sermonaudio import APIException
from sermonaudio.user import BroadcasterUserRoles, UserRoles
...
def raise_api_exception(response: Response):
    try:
        json = response.json()
        ex = APIException(json, json.get("errors", {}).get("code"))
    except:  # noqa
...
class SiteUser(Model):
    def __init__(self, obj: dict):
        ...
        self.broadcaster_roles = obj["broadcasterRoles"]

    def has_role(self, role: UserRoles) -> bool:
        """Returns `True` if this user has specified `role`, or is an administrator"""

        return any(r["roleID"] for r in self.roles or [] if r["roleID"] in [role.value, UserRoles.ADMINISTRATOR.value])

    def has_broadcaster_role(
        self,
        role: Optional[BroadcasterUserRoles] = None,
        broadcaster_id: Optional[str] = None,
    ) -> bool:
        """Returns `True` if this user has specified `role` for broadcaster

        Also returns `True` if user has broadcaster admin role.

        If `broadcaster_id` is not specified, returns `True` if user has
        specified `role` for any broadcaster

        If `role` is not specified, returns `True` if user has any role for specified broadcaster
        """

        return any(
            r["broadcasterRoleID"]
            for r in self.broadcaster_roles or []
            if (broadcaster_id is None or r["broadcasterID"] == broadcaster_id)
            and (role is None or r["broadcasterRoleID"] in [role.value, BroadcasterUserRoles.ADMINISTRATOR.value])
        )
