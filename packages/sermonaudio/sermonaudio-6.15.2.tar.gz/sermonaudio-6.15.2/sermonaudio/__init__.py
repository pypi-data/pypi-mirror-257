...
class API:
    ...
    def _get_request_headers(
        ...
        if use_cookie_auth:
            result["X-Auth-With-Cookie"] = "true"
    ...
    def _request(
        ...
        **kwargs,
        ...
        :param session: The request.Session object to use when making the request. Don't use without a good reason.
        ...
        :param use_cookie_auth: If present, adds a header to the request instructing the API to use any authentication
                                cookie that may be present in the request to authenticate the request
        ...
        headers = headers or {}
        base_headers = cls._get_request_headers(
            preferred_language_override, show_content_in_any_language, use_cookie_auth
        )
...
class APIErrorCode(IntEnum):
    """Note: The following codes are from apierror.py in our API implementation project."""

    # User authorization token is no longer valid because a logout has invalidated it
    AUTH_TOKEN_STALE = 101
    # User has signed up but not completed verification process
    USER_NOT_VERIFIED = 102
    # User authorization token has expired and must be refreshed
    AUTH_TOKEN_EXPIRED = 103
    # Invalid site user login but valid legacy plus user login
    IS_LEGACY_USER = 104


class APIException(Exception):
    def __init__(self, msg: Optional[str] = None, code: Optional[int] = None):
        super().__init__(msg)
        self.code = code
