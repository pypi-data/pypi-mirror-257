...
class BaseConversionClass(Step):
    ...
    def _get_parameter_values(self):
        ...
        context_key = self.get_or_create_parameter(
            "Context Key", "NUMBER_RESPONSE", self.declared_parameters["Context Key"]
        )
        use_context_key = self.get_or_create_parameter(
            "Use Context Key", "False", self.declared_parameters["Use Context Key"]
        )
...
class ListBarcodeConversionStep(BaseConversionClass):
    ...
    def execute(self, data, rule_context: RuleContext):
        self.info("Task parameters: %s", str(self.get_task_parameters(rule_context)))
        ...
        if isinstance(to_process, list):
            ...
            if data:
                self.info("Inbound data was converted.  Returning back " "to rule.")
                return converted
            else:
                self.info(
                    "The information in context key %s was converted.", self.context_key
                )
    ...
    def convert(self, data):
        ...
        prop_val = BarcodeConverter(
            data, self.company_prefix_length, self.serial_number_length
...
class ListURNConversionStep(ListBarcodeConversionStep):
    ...
    def convert(self, data):
        ...
        :return: An EPC URN based on the inbound data.
        """
        prop_val = URNConverter(data).__getattribute__(self.prop_name)
        return prop_val if isinstance(prop_val, str) else prop_val()
...
class XMLBarcodeConversionStep(BaseConversionClass):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        if barcode_xml:
            ...
            self.info("Barcode data has been filtered and replaced where " "possible.")
            ...
            self.warning(
                "No XML was found in the Rule Context under the "
                "context key %s or the rule had no inbound data." % self.context_key
            )
