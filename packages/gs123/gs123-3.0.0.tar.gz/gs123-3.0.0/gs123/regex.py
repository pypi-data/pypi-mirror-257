...
_NO_PARENS_NUMERIC_GS1_01_21 = (
    r"^01(?P<gtin14>[0-9]{14})21(?P<serial_number>" r"[0-9,A-Z]{1,20})$"
)
...
NUMERIC_GS1_01_21_OPTIONAL_17_10 = re.compile(_NUMERIC_GS1_01_21_OPTIONAL_17_10)

NO_PARENS_NUMERIC_GS1_01_21 = re.compile(_NO_PARENS_NUMERIC_GS1_01_21)

_ALPHA_01_21_GTIN_NO_PARENS = (
    r"^01(?P<gtin14>[0-9]{14})21(?P<serial_number>[0-9,a-z,A-Z]{10,20})$"
)
...
NO_PARENS_NUMERIC_GS1_01_21_IN_DOC = (
    r"01(?P<gtin14>[0-9]{14})21(?P<serial_number>[0-9,A-Z]{1,20})"
)
...
SSCC_URN = (
    r"urn:epc:id:sscc:(?P<company_prefix>[0-9]{4,12})\.(?P<serial_number>[0-9]{1,10})"
)

urn_patterns = [re.compile(SGTIN_URN), re.compile(SSCC_URN)]

patterns = [GTIN14, SSCC]
...
def match_pattern(barcode_val: str, max_serial_number_length=14):
    ...
    if barcode_val.startswith("(01)"):
        match = SGTIN_SN_10_13_ALPHA.match(barcode_val)
    elif barcode_val.startswith("01"):
        if len(barcode_val) <= 18 + max_serial_number_length:
            match = ALPHA_01_21_GTIN_NO_PARENS.match(barcode_val)
    elif barcode_val.startswith("(00)") or barcode_val.startswith("00"):
        match = SSCC.match(barcode_val)
    if not match:
        if not match:
            match = NO_PARENS_NUMERIC_GS1_01_21.match(barcode_val)
        if not match:
            match = get_no_parens_numeric_gs1_01_21_optional_17_10().match(barcode_val)
        if not match:
            match = FNC1_SERIAL.match(barcode_val)
    return match
