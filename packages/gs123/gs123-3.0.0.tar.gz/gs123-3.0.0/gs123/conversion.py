    ...
    def epc_urn(self) -> str:
        ...
        if self._gtin14:
            ...
            ret = self._sscc_pattern.format(
                self.company_prefix, self.extension_digit, self.serial_number_field
            )
        return ret
...
class URNConverter(BarcodeConverter):
    ...
    def _handle_sgtin_urn(self, groups, urn_value):
        ...
        self._gtin14 = "{0}{1}{2}".format(
            self._indicator_digit, self._company_prefix, self._item_reference
        )
        if len(self._gtin14) != 13:
            raise URNNotValid(
                "The urn %s is not valid. The parts representing the "
                "GTIN-14 should be 13 digits in length total." % urn_value
            )
    ...
    def _get_gtin_barcode_val(
        ...
        if serial_number_padding:
            serial_number = self.serial_number.rjust(
                serial_number_length, padding_character
            )
        else:
            serial_number = self.serial_number
        format_string = "(01){0}(21){1}" if parenthesis else "01{0}21{1}"
        barcode = format_string.format(self.gtin14, serial_number)
