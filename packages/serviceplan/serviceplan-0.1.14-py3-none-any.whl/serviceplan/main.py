...
class ServerPlan:
    def __init__(self, dbhost=None,dbuser=None,dbpassword=None,dbdatabase=None,sql_query=None, url=None,myproxy=None,timeout=10,smtp_host=None,mailpassword=None,sender=None,subject=None,mailto=None,cc=None):
        self.host = dbhost
        self.user = dbuser
    ...
    def push_mail_message(self, message, headers):
        ...
        dlmail = DlMail(password=mailpassword, smtp_host=smtp_host,sender=sender)
    ...
    def serverplan(self):
        ...
        message2 += "<table border='1'><tr><th> </th><th>项目</th><th>名称</th><th>在线算路</th><th>DBU</th></tr>"
        for row in results:
            j = j + 1
            project_name_info, alias, ocserver_info, rds_info = row
            row_color1 = "red" if ocserver_info == "stopped" else "green"
            row_color2 = "red" if rds_info == "stopped" else "green"
            #ocserver_info = "已关闭" if ocserver_info == "stopped" else "正在运行"
            ocserver_info = "正在运行" if ocserver_info == "RUNNING" else ("已关闭" if ocserver_info == "stopped" else ("无" if ocserver_info == "null" else ocserver_info))
            rds_info = "正在运行" if rds_info == "available" else ("已关闭" if rds_info == "stopped" else ("无" if rds_info == "null" else rds_info))
            #message2 += f"{j}、{project_name_info}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{alias}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{ocserver_info}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{rds_info}</br>"
            message2 += f"<tr><td>{j}</td><td>{project_name_info}</td><td>{alias}</td><td style='color:{row_color1}'>{ocserver_info}</td><td style='color:{row_color2}'>{rds_info}</td></tr>"
        message2 += "</table>"
        print(message2)
        cursor.close()
