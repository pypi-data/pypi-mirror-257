...
class boxdimensions(f90wrap.runtime.FortranDerivedType):
    ...
    def __init__(self, handle=None):
        ...
        result = libqepy_modules.f90wrap_boxdimensions_initialise()
    ...
    def __del__(self):
        ...
        """
        if self._alloc:
            libqepy_modules.f90wrap_boxdimensions_finalise(this=self._handle)
...
def cell_base_init(ibrav_, celldm_, a_, b_, c_, cosab_, cosac_, cosbc_, trd_ht, \
    ...
    """
    libqepy_modules.f90wrap_cell_base_init(ibrav_=ibrav_, celldm_=celldm_, a_=a_, \
        b_=b_, c_=c_, cosab_=cosab_, cosac_=cosac_, cosbc_=cosbc_, trd_ht=trd_ht, \
...
def ref_cell_base_init(ref_alat, rd_ref_ht, ref_cell_units):
    ...
    --------------------------------------------------------------------
     Initialize cell_base module variables, set up crystal lattice
    """
    libqepy_modules.f90wrap_ref_cell_base_init(ref_alat=ref_alat, \
...
def gethinv(self):
    ...
    """
    libqepy_modules.f90wrap_gethinv(box=self._handle)
...
def get_volume(hmat):
    ...
    """
    get_volume = libqepy_modules.f90wrap_get_volume(hmat=hmat)
    return get_volume
...
def pbc(rin, box, nl=None):
    ...
    """
    rout = libqepy_modules.f90wrap_pbc(rin=rin, box=box._handle, nl=nl)
    return rout
...
def get_cell_param(self, cell, ang=None):
    ...
    """
    libqepy_modules.f90wrap_get_cell_param(box=self._handle, cell=cell, ang=ang)
...
def set_h_ainv():
    ...
    """
    libqepy_modules.f90wrap_set_h_ainv()
...
def cell_dyn_init(trd_ht, rd_ht, wc_, total_ions_mass, press_, frich_, greash_, \
    ...
    """
    libqepy_modules.f90wrap_cell_dyn_init(trd_ht=trd_ht, rd_ht=rd_ht, wc_=wc_, \
...
def init_dofree(cell_dofree):
    ...
    """
    libqepy_modules.f90wrap_init_dofree(cell_dofree=cell_dofree)
...
def cell_base_reinit(ht):
    ...
    """
    libqepy_modules.f90wrap_cell_base_reinit(ht=ht)
...
def cell_steepest(hnew, h, delt, iforceh, fcell):
    ...
    """
    libqepy_modules.f90wrap_cell_steepest(hnew=hnew, h=h, delt=delt, \
...
def cell_verlet(hnew, h, hold, delt, iforceh, fcell, frich, tnoseh, hnos):
    ...
    """
    libqepy_modules.f90wrap_cell_verlet(hnew=hnew, h=h, hold=hold, delt=delt, \
        iforceh=iforceh, fcell=fcell, frich=frich, tnoseh=tnoseh, hnos=hnos)
...
def cell_hmove(h, hold, delt, iforceh, fcell):
    ...
    """
    libqepy_modules.f90wrap_cell_hmove(h=h, hold=hold, delt=delt, iforceh=iforceh, \
        fcell=fcell)
...
def cell_force(fcell, ainv, stress, omega, press, wmassin=None):
    ...
    """
    libqepy_modules.f90wrap_cell_force(fcell=fcell, ainv=ainv, stress=stress, \
        omega=omega, press=press, wmassin=wmassin)
...
def cell_move(hnew, h, hold, delt, iforceh, fcell, frich, tnoseh, vnhh, velh, \
    ...
    """
    libqepy_modules.f90wrap_cell_move(hnew=hnew, h=h, hold=hold, delt=delt, \
        iforceh=iforceh, fcell=fcell, frich=frich, tnoseh=tnoseh, vnhh=vnhh, \
        velh=velh, tsdc=tsdc)
...
def cell_gamma(hgamma, ainv, h, velh):
    ...
    """
    libqepy_modules.f90wrap_cell_gamma(hgamma=hgamma, ainv=ainv, h=h, velh=velh)
...
def cell_update_vel(self, ht0, htm, delt, velh):
    ...
    """
    libqepy_modules.f90wrap_cell_update_vel(htp=self._handle, ht0=ht0._handle, \
...
def cell_kinene(temphh, velh):
    ...
    """
    ekinh = libqepy_modules.f90wrap_cell_kinene(temphh=temphh, velh=velh)
    return ekinh
...
def cell_alat():
    ...
    """
    cell_alat = libqepy_modules.f90wrap_cell_alat()
    return cell_alat
...
def _cell_init_ht(what, box, hval):
    ...
    """
    libqepy_modules.f90wrap_cell_init_ht(what=what, box=box._handle, hval=hval)
...
def _cell_init_a(alat, at, box):
    ...
    """
    libqepy_modules.f90wrap_cell_init_a(alat=alat, at=at, box=box._handle)
...
def _pbcs_components(x1, y1, z1, m):
    ...
    """
    x2, y2, z2 = libqepy_modules.f90wrap_pbcs_components(x1=x1, y1=y1, z1=z1, m=m)
    return x2, y2, z2
...
def _pbcs_vectors(v, w, m):
    ...
    """
    libqepy_modules.f90wrap_pbcs_vectors(v=v, w=w, m=m)
...
def _s_to_r1(s, r, box):
    ...
    """
    libqepy_modules.f90wrap_s_to_r1(s=s, r=r, box=box._handle)
...
def _s_to_r1b(s, r, h):
    ...
    """
    libqepy_modules.f90wrap_s_to_r1b(s=s, r=r, h=h)
...
def _s_to_r3(s, r, nat, h):
    ...
    """
    libqepy_modules.f90wrap_s_to_r3(s=s, r=r, nat=nat, h=h)
...
def _r_to_s1(r, s, box):
    ...
    """
    libqepy_modules.f90wrap_r_to_s1(r=r, s=s, box=box._handle)
...
def _r_to_s1b(r, s, hinv):
    ...
    """
    libqepy_modules.f90wrap_r_to_s1b(r=r, s=s, hinv=hinv)
...
def _r_to_s3(r, s, nat, hinv):
    ...
    """
    libqepy_modules.f90wrap_r_to_s3(r=r, s=s, nat=nat, hinv=hinv)
