...
class LocalStorage:
    ...
    def get_file_content(
        self, path: str, selection_strategy: SelectionStrategy
    ) -> FileObject | None:
...
class StorageBackend:
    ...
    def write_buffer(
        ...
        *,
        ...
        filename = self.format_path(
            filename, name=name, file_type=file_type, compression=compression
        )
    ...
    def format_path(
        ...
        *segments: str,
        ...
        full_extension = Compressor.get_with_name(compression).compose_filetype(
            base_extension
        )
        ...
        result = filename_template.format(
            table=name, timestamp=timestamp, ext=full_extension, date=date
        )
    ...
    def path_exists(
        ...
        *,
        ...
        full_path = self.format_path(
            path, name=name, file_type=file_type, compression=compression
        )
    ...
    def get_file_content(
        ...
        *,
        ...
        if (
            self.manifest
            and self.perform_writes
            and file_type == file_type.data
            and name
            and file_object
        ):
...
class FileSelectionStrategy:
    ...
    def use_metadata_strategy(available_objects: Iterable[FileStat]) -> FileStat | None:
        objects_by_last_modified_date = {
            s3_object.last_modified: s3_object for s3_object in available_objects
        }
        ordered_last_modified_dates = sorted(
            objects_by_last_modified_date.keys(), reverse=True
        )
