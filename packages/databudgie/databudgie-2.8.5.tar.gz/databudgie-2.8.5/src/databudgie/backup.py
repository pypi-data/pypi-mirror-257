    ...
    *,
    ...
    with Progress(console) as progress:
        ...
        for schema_op in schemas:
            progress.update(
                task, description=f"Backing up schema DDL: {schema_op.name}"
            )
    ...
    if table_names:
        ...
        with io.BytesIO(manifest_data) as buffer:
            filename = storage.write_buffer(
                backup_config.ddl.full_path(), buffer, file_type=FileTypes.manifest
            )
    ...
    *,
    ...
    with Progress(console) as progress:
        ...
        for table_op in table_ops:
            ...
            progress.update(
                task,
                description=f"Backing up sequence position: {table_op.pretty_name}",
            )
...
def backup_tables(
    ...
    *,
    ...
    with Progress(console) as progress:
        ...
        for table_op in table_ops:
            progress.update(
                task, description=f"Backing up table: {table_op.pretty_name}"
            )
