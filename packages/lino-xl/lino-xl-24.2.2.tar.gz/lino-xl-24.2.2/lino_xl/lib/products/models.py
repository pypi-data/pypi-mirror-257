# -*- coding: UTF-8 -*-
# Copyright 2008-2023 Rumma 6 Ko Ltd
...
class Product(BabelNamed, BabelPreviewable, Duplicable, MemoReferrable,
              Storable):
    ...
    def full_clean(self):
        ...
        if drv and self.barcode:
            drv.write_svg_file(self.barcode)
        super().full_clean()

    @dd.htmlbox()
    def barcode_svg(self, ar=None):
        if dd.plugins.products.barcode_driver and self.barcode:
            return '<img src="{}" alt="{}"/>'.format(
                settings.SITE.build_media_url('barcodes',
                                              self.barcode + ".svg"),
                self.barcode)
        return "No barcode"
