...
class Ticket(UserAuthored, mixins.CreatedModified, TimeInvestment, Assignable,
    ...
    def get_user_queryset(cls, user):
        ...
        if user.is_anonymous:
            ...
            qs = qs.filter(
                Q(group__members__user__id=user.pk) | Q(user=user)
                |  # Q(end_user=user)|
                Q(assigned_to=user)
                | Q(private=False, site__private=False)).distinct()
        return qs

    # write_comment_on_create = True

    # def get_create_comment(self, ar):
    #     # causes Commentable.save_new_instance() to create a comment
    #     p = dd.plugins.memo.parser
    #     txt = _("{user} created new {model} {row}").format(
    #             model=self.__class__._meta.verbose_name,
    #             user=p.obj2memo(ar.get_user()),
    #             row=p.obj2memo(self))
    #     if self.parent is not None:
    #         txt += " " + _("from {row}").format(row=str(self.parent))
    #     txt += "."
    #     return txt
    ...
    def get_comments_filter(cls, user):
        ...
        if user.user_type.has_required_roles([PrivateCommentsReader]):
            return None
        flt = Q(ticket__group__members__user__id=user.pk)
    ...
    def __str__(self):
        ...
        if self.group and self.group.ref:
            num += "@" + self.group.ref
        return "#{} ({})".format(num, self.summary)
...
def setup_memo_commands(sender=None, **kwargs):
    ...
    mp.add_suggester("#", Ticket.objects.order_by('id'), 'id')
