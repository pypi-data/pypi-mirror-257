...
from etgen.html import E, join_elems, forcetext, tostring
...
def show_birthdays(ar, today=None):
    ...
    def fmt(obj):
        return "{} {}".format(
            obj.birth_date,
            ar.obj2htmls(obj, obj.get_full_name(salutation=False)))

    if todayqs.count():
        yield ("<p>{}: ".format(_("Birthdays today")) +
               ", ".join([fmt(obj) for obj in todayqs]) + "</p>")
    if recent.count():
        yield ("<p>{}: ".format(_("Recent birthdays")) +
               ", ".join([fmt(obj) for obj in recent]) + "</p>")
    if coming.count():
        yield ("<p>{}: ".format(_("Upcoming birthdays")) +
               ", ".join([fmt(obj) for obj in coming]) + "</p>")
