...
def objects():
    ...
    img = make_file("crossroads.jpg", "A House beside crossroads",
                    "2010-2017 rarestohanean (pixabay.com user)")
    # source: https://pixabay.com/photos/crossroads-little-house-park-1951949/
    yield img

    title = "At the fork of the cross roads"
    cmd = "[upload {} thumb|left|]".format(img.pk)
    ...
    img = make_file("bleeding_words.jpg", "Bleeding Words",
...
admire.</p><p>Also, they never end.</p></div>""".format(cmd)
    ...
    img = make_file("History_of_PDF.pdf", "History of PDF", "Wikipedia")
    # source:
    yield img
    cmd = "[upload {}]".format(img.pk)
    title = "A PDF file"
