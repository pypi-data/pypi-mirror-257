...
class Client(contacts.Person, BiographyOwner, BeIdCardHolder, DupableClient,
             Coachable, Notable, UploadController, DateRangeObservable):
    ...
    in_belgium_since = models.DateField(_("Lives in Belgium since"),
                                        blank=True,
                                        null=True)
    residence_until = models.DateField(_("Residence until"),
                                       blank=True,
                                       null=True)
    ...
    needs_residence_permit = models.BooleanField(_("Needs residence permit"),
                                                 default=False)
    ...
    def disabled_fields(self, ar):
        ...
        if not ar.get_user().user_type.has_required_roles([NewcomersOperator]):
    ...
    def get_first_meeting(self, today=None):
        if today is None:
            today = dd.today()
        qs = SpecialTypes.first_meeting.get_notes(project=self,
                                                  date__lte=today).order_by(
                                                      'date', 'time')
    ...
    def __str__(self):
        if self.is_obsolete:
            return "%s %s (%s*)" % (self.last_name.upper(), self.first_name,
                                    self.pk)
        return "%s %s (%s)" % (self.last_name.upper(), self.first_name,
                               self.pk)
    ...
    def get_overview_elems(self, ar):
        ...
        notes = []
        for note in rt.models.notes.Note.objects.filter(project=self,
                                                        important=True):
    ...
    def update_reminders(self, ar):
        ...
        if user:
            with translation.override(user.language):
                ...
                update_reminder(1, self, user, self.card_valid_until,
                                _("eID card expires in 2 months"), 2, M)
                update_reminder(2, self, user, self.unavailable_until,
                                _("becomes available again in 1 month"), 1, M)
                update_reminder(3, self, user,
                                self.work_permit_suspended_until,
                                _("work permit suspension ends in 1 month"), 1,
                                M)
    ...
    if not dd.is_installed("art60"):
        ...
        @dd.virtualfield(
            dd.ForeignKey('contacts.Company', verbose_name=_("Working at ")))
        ...
        if n > 0:
            ...
            for row in res:
                if row.type in SHOWN_TYPES:
                    chunks = []
                    if row.counter == 1:
                        chunks += ['IT%s (' % row.type, E.b(row.group), ') ']
...
"{}{}{}", _("Extended filter criteria, e.g.:"), "<br/>",
_("Active: All those who have some active coaching."))),
            only_primary=models.BooleanField(_("Only primary clients"),
                default=False,
                help_text="""\
Nur Klienten, die eine effektive <b>primäre</b> Begleitung haben.""")
    ...
    def get_request_queryset(self, ar):
        ...
        if pv.coached_by:  # or pv.start_date or pv.end_date:
            qs = add_coachings_filter(qs, pv.coached_by, period,
                                      pv.only_primary)
            if pv.and_coached_by:
                qs = add_coachings_filter(qs, pv.and_coached_by, period, False)
    ...
    def get_title_tags(self, ar):
        ...
        if pv.start_date is None or pv.end_date is None:
            ...
            period = daterange_text(pv.start_date, pv.end_date)

        if pv.coached_by:
            s = str(self.parameters['coached_by'].verbose_name) + \
                ' ' + str(pv.coached_by)
            if pv.and_coached_by:
                s += " %s %s" % (str(_('and')), pv.and_coached_by)
...
class Dispense(dd.Model):
    ...
    end_date = models.DateField(blank=True, null=True, verbose_name=_("until"))
...
class Exclusion(dd.Model):
    ...
    type = dd.ForeignKey("pcsw.ExclusionType",
                         ...
                         blank=True,
                         null=True)
    excluded_from = models.DateField(blank=True,
                                     null=True,
                                     verbose_name=_("Excluded from"))
    excluded_until = models.DateField(blank=True,
                                      null=True,
