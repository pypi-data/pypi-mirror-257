...
class BackTester(DataBaseConnection):
    ...
    def _build_option_chain_skeleton(
        ...
        return df_exploded.astype(
            {"strike": "int", "call_strike": "int", "put_strike": "int"}
        )
    ...
    def build_trade_df(
        ...
        | float = 1.1,  # A decimal on purpose because last timestamp in the database is 15:29
        ...
        df["spot_exit"] = df.square_up_timestamp.apply(
            lambda x: underlying_prices.open.get(x, np.nan)
        )
...
def store_atm_ivs(*args, **kwargs):
    ...
    ivs.to_csv(
        f"{underlying}_atm_ivs_{from_timestamp.strftime('%Y-%m-%d')}_{to_timestamp.strftime('%Y-%m-%d')}.csv"
    )
