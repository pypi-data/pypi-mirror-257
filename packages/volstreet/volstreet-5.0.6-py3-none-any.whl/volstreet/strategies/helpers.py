...
class DeltaPosition:
    ...
    def set_hedge_options(self):
        self.hedge_call_option = min(
            self.prospective_calls, key=lambda x: abs(x.cached_greeks.delta - 0.5)
        )
        self.hedge_put_option = min(
            self.prospective_puts, key=lambda x: abs(x.cached_greeks.delta + 0.5)
        )
    ...
    def recommend_delta_action(
        ...
        hedge_call_delta = self.hedge_call_option.cached_greeks.delta
        hedge_put_delta = self.hedge_put_option.cached_greeks.delta
        if (
