...
class grpc_iter_producer (iter_producer):
    def __init__ (self, data):
        self.data = data
        self.proxsize = -1
        self._done = False
    ...
    def more (self):
        try:
            data = next (self.data)
            return grpc_serialize (data, True)
        except StopIteration:
            self._done = True
            return b""


class thread_producer (iter_producer):
    def __init__ (self, producer, max_size = 0, wait_timeout = 8, thread_pool = None):
        self.producer = producer
        self.wait_timeout = wait_timeout
        if max_size:
            max_size = max (2, max_size)
        self.q = queue.Queue (max_size)
        self._last_ready = time.time ()
        self._done = False
        self.proxsize = -1
        if thread_pool:
            thread_pool.submit (producer, self.q)
        else:
            threading.Thread (target = producer, args = (self.q,)).start ()

    def ready (self):
        isready = self.q.qsize ()
        if isready:
            self._last_ready = time.time ()
            return True

        if time.time () - self._last_ready > self.wait_timeout:
            self.q.put (None)
            self._last_ready = time.time ()
            return True
    ...
    def more (self):
        data = self.q.get ()
        if data is None:
            hasattr (self.producer, 'close') and self.producer.close ()
            self._done = True
            return b''
        if strutil.is_encodable (data):
            return data.encode ("utf8")
...
class default_globbing_producer:
    """
...
class Sendable:
    def __init__ (self, data, callback = None):
        ...
        self._exec_when_done = None
        data and self.send (data)

    def execute_when_done (self, func):
        self._exec_when_done = func ()

    def close (self):
        self.done = True
        self._exec_when_done and self._exec_when_done ()


class sendable_producer (Sendable):
    def send (self, data):
        if strutil.is_encodable (data):
    ...
    def ready (self):
        return self.data or self.done

    def exhausted (self):
        return self.done and not self.data
...
class ready_globbing_producer (default_globbing_producer):
    # collect data for one que pushing
    def __init__ (self, producer, buffer_size = SIZE_BUFFER):
        self.producer = producer
...
def globbing_producer (producer, buffer_size = SIZE_BUFFER):
    if hasattr (producer, 'ready'):
        return ready_globbing_producer (producer, buffer_size)
    return default_globbing_producer (producer, buffer_size)

class hooked_producer (default_globbing_producer):
    """
    ...
    def more (self):
        if self.producer:
            ...
            if not result or self.producer.exhausted ():
                self.producer = None
