...
def aggregate_country(country: dict, product: dict, cycles: list, source: dict, start_year: int, end_year: int) -> list:
    ...
    cycles = non_empty_list(map(_update_cycle(country, start_year, end_year, source), cycles))
    if len(cycles) == 0:
        return []
    ...
    return filter_min_score(map(calculate_score, cycles + country_cycles))
...
def aggregate_global(country: dict, product: dict, cycles: list, source: dict, start_year: int, end_year: int) -> list:
    ...
    return filter_min_score([calculate_score(cycle, countries) for cycle in cycles])
