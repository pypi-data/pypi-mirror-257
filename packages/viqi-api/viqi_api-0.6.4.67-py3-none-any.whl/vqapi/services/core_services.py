...
class DirProxy(FuturizedServiceProxy):
    ...
    def touch(self, path, __xattrs=None, **kw):
        ...
        xattrs = __xattrs or {}
        if xattrs:
            res = self.put(path, json=xattrs, **kw)
            code_to_exception(res)
            return res.doc()

    def refresh(self, path, **kw) -> requests.Response:
        """Force a refresh on path and all descendents
        Args:
        """
        params = {**{"refresh": "true", "view": "deep,count"}, **kw.pop("params", {})}
        res = self.get(path, params=params, **kw)
        return res

    def find(self, path: str, depth=None, patterns: list[str] | None = None, **kw) -> requests.Response:
