...
def dfToDB2(CREDS,table,df,chunkSize,logger):
    try:
        ...
        return 1
'''
def getVaultSecret0(CREDS,vaultName,secretName,logger):
    try:
        ...
        credential = ManagedIdentityCredential(client_id="test")
...
def getVaultSecret(vault_name,secret_name,logger=None):
    try:
        credential = DefaultAzureCredential()
        ...
        if logger is not None:
            logger.error("mstAst.getVaultSecret Failed, error: {e1}[:500]")
        return None
