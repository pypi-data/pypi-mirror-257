...
class DeltaWriter:
    ...
    def transaction_exists(self, is_overwrite: bool, metadata: metadata_pb2.TectonDeltaMetadata) -> bool:
        ...
        :param is_overwrite: whether the task overlaps with a previous task
        :param metadata: transaction metadata
        :return: whether the same transaction has been executed before
        """
        return not is_overwrite and self._transaction_writer().has_commit_with_metadata(metadata)

    def maybe_delete_time_range(self, interval: TimeInterval, is_overwrite: bool) -> None:
        ...
        if is_overwrite:
            print(
                f"Materialization interval overlaps with another task; clearing prior data in range {interval.start} - {interval.end}"
            )
