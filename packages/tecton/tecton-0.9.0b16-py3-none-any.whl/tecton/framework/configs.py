...
class LocalSecret(Secret):
    """A reference to a local secret for use in Local Development.

    The secret will be stored in memory instead of Tecton Secrets Service.
    ...
    value: str = pydantic_v1.Field(repr=False)  # Do not print secret values by default.

    def to_proto(self) -> SecretReference:
        return SecretReference(scope=self.scope, key=self.key, is_local=True)


def _convert_secret_to_sanitized_reference(secret: Union[str, Secret, LocalSecret]) -> SecretReference:
    ...
    if isinstance(secret, str):
        ...
        secret = LocalSecret(scope="LOCAL_SECRET", key=generated_key, value=secret)
    if isinstance(secret, LocalSecret):
        set_local_secret(scope=secret.scope, key=secret.key, value=secret.value)
        return SecretReference(scope=secret.scope, key=secret.key, is_local=True)
...
class PandasBatchConfig(BaseBatchConfig):
    ...
    **Do not instantiate this class directly.** Use :py:func:`tecton.pandas_batch_config` instead.
    ...
    def __init__(
        ...
        self._args.supports_time_filtering = supports_time_filtering
        if secrets:
            self._args.secrets.extend([_convert_secret_to_sanitized_reference(s) for s in secrets])
        self._data_delay = data_delay
        self.data_source_function = data_source_function
