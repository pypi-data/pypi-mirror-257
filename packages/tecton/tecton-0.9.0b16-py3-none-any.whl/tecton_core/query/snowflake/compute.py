...
def _get_user_and_password(
    ...
    for source in sources:
        if source.user and source.password:
            ...
            secret_context = secret_resolver.resolve([source.user, source.password])
            user = secret_context.get(scope=source.user.scope, key=source.user.key)
            password = secret_context.get(scope=source.password.scope, key=source.password.key)
