...
class MaterializedFeatureView(FeatureView):
    ...
    def run_transformation(
        ...
        if any(ds._data_source_type == DataSourceType.PUSH_NO_BATCH for ds in self.sources) and len(mock_inputs) == 0:
            msg = "The `run_transformation()` method is currently unsupported for Feature Views backed by a PushSource without a batch_config unless mock_inputs are provided."
            raise TectonValidationError(msg)
    ...
    def run(
        ...
        **mock_inputs_kwargs: Union[pandas.DataFrame, pyspark_dataframe.DataFrame],
        ...
        if (
            ...
            msg = "The `run()` method is currently unsupported for Feature Views backed by a PushSource without a batch_config unless mock_inputs are provided."
            raise TectonValidationError(msg)
