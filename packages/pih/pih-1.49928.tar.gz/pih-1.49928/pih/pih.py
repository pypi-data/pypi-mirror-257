...
class PIH:
    ...
    class ERROR:
                    ...
                    if PIH.ERROR.notify_about_error:
                        PIH.EVENT.send(
                            ...
                            (
                                str(exception),
                                details,
                                DataTool.filter(lambda item: isinstance(item, str), exception.args),
                            ),
                        )
            raise Error(exception.details(), code) from None

        @staticmethod
        def global_except_hook(exception: BaseException) -> None:
            exctype, traceback_value = type(exception), exception.__traceback__
            details_list: list[str] = []
            for item in exception.args:
                if isinstance(item, str):
                    details_list.append(item)
            traceback_repr: str = jnl(traceback.format_exception(exception))
            if PIH.ERROR.notify_about_error:
                PIH.EVENT.send(
                    Events.ERROR,
                    (
                        str(exception),
                        PIH.ERROR.create_header(traceback_repr),
                        details_list,
                    ),
                )
            sys.__excepthook__(exctype, exception, traceback_value)

        sys.excepthook = (
            lambda _, exception, __: PIH.ERROR.global_except_hook(
                exception
            )
        )
        ...
        class USER:
            @staticmethod
            def not_found_error(title: str, active: bool, value: str) -> str:
                start: str | None = None
                if n(active):
                    start = "Пользователь"
                elif active:
    ...
    class UPDATER:
        ...
        def install_package(
            ...
            return PIH.UPDATER.package_operation(
                name, version, host, local_location, show_output, True
            )
        ...
        def uninstall_package(
            ...
            return PIH.UPDATER.package_operation(
                name, version, host, local_location, show_output, False
            )
            ...
            if from_local_location:
                if nn(version):
                    ...
                    if PathTool.exists(path):
                        ...
                        from_local_location = False
                else:
                    from_local_location = False
            virtual_environment: bool = PIH.SYS.is_virtual_environment()
            if virtual_environment and install_operation:
                if is_pih_package:
                    return None
    ...
    class RESULT(ResultTool):
        ...
        class FILES:
            ...
            def execute_some(
                ...
                file: File | None = None
                result: list[str] = []
                file_list: list[File] = PIH.RESULT.FILES.get(file_search_request).data  # type: ignore
                if e(file_list):
                    return None
        ...
        class WORKSTATION:
            ...
            def by_internal_telephone_number(value: int) -> Result[Workstation]:
                ...
                if result_worksation is not None:
                    ...
                    raise PIH.ERROR.USER.not_found_error(
        ...
        class USER:
            ...
            def by_login(
                ...
                try:
                    ...
                    raise PIH.ERROR.USER.not_found_error("логином", active, value)
            ...
            def by_name(
                ...
                if e(result):
                    raise PIH.ERROR.USER.not_found_error("именем", active, value)
                return result
...
def thread(
    value: Callable[[Callable[[], None]], None],
