...
class FieldToTypeMatcherVisitor(Visitor):
    ...
    def enter_field(self, node: FieldNode, *_):
        ...
        if not graphql_type:
            raise ValueError(f"{node} does not have a graphql type")
        directed = any(item.name.value in ["include", "skip"] for item in node.directives)
        field_type = self._parse_type(graphql_type=graphql_type, is_directed=directed)
    ...
    # Custom Functions
    def _parse_type(self, graphql_type: GraphQLOutputType, is_directed: bool = False) -> ParsedFieldType:
        ...
        if (len(unwrapper_result.wrapper_stack) == 0 or unwrapper_result.wrapper_stack[
            -1] != WrapperType.OPTIONAL) and is_directed:
            wrapped_type = f"Optional[{wrapped_type}]"
        ...
        while len(processed) < len(definitions):
            for definition in current_to_process:
                ...
                assembled_definition = "\n\n".join(
                    sorted(
                        list(
                            self._assemble_definition(definition, {})
                        )
                    )
                )
