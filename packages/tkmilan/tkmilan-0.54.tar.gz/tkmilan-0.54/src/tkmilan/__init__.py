...
class RootWindow(tk.Tk, mixin.ContainerWidget):
    ...
    def __init__(self, *args,
        ...
        if __debug__:
            self.after_idle(self.__sanity_check)  # No need for a `TimeoutIdle` here
    ...
    def register_styleID(self, argument: typing.Optional[str],
        ...
        :tk:`style <ttk_style.html>` documentation.
    ...
    def start_eventloops(self, **kwargs) -> None:
        ...
        for elID, el in self.get_eventloops(**kwargs):
            if __debug__:
                logger.debug('Starting "%s" tName=%r', elID, el.tName)
            el.start()
...
class FrameStateful(ttk.LabelFrame, mixin.ContainerWidget):
    ...
    def state_get(self, *args, vid_upstream: typing.Optional[typing.Set[str]] = None, **kwargs) -> model.WState[bool, typing.Any]:
        ...
        if __debug__:
            logger_autostate.debug('%s: cstate=%s', self, cstate)
        return model.WState(
            cstate,
...
class Tree(ttk.Treeview, mixin.SingleWidget):
    ...
    def __init__(self, parent: mixin.ContainerWidget, *,
        ...
        if self.wstyle.altbg:
            self.__etags[Tree.__lines_alt] = model.TreeRow(background=self.wstyle.colour_altbg)
    ...
    def etag(self, wid: str,
        ...
        if __debug__:
            delta_tags = new_tags - set(self.__etags)
    ...
    def wsel(self, wid: typing.Optional[str], *, see: bool = True) -> typing.Optional[str]:
        ...
        Defaults to ensuring the selected element is visible, by scrolling the
        view. This is controlled by the ``see`` argument.
...
class Scrolled(ttk.Frame, mixin.ProxyWidget):
    ...
    def __gweights(self) -> typing.Tuple[int, int]:
        ...
        if (wgrid := self.wgrid) is not None:
            ...
            assert isinstance(wparent, (Notebook)), f'{self}: Bad parent widget type `{wparent.__class__.__qualname__}`'
            return (1, 1)
...
class RadioFrameUnlabelled(FrameUnlabelled):
    ...
    def setup_widgets(self, *,
        ...
        if __debug__:
            ...
            assert all(n not in (radioArgs or {}) for n in badkeys), f'Invalid "radioArgs": invalid key @ {badkeys}'
        assert len(stateRadio) > 0
        rvar: var.StaticMap = typing.cast(var.StaticMap,
                                          self.varSpec(var.StaticMap, stateRadio))
        assert isinstance(rvar, var.StaticMap)
        widgets: typing.MutableMapping[str, mixin.MixinWidget] = {}
        for lbl, idx in rvar.container.items():
            # Use `lbl` as "value", the validation takes care of returning the idx
            widgets[idx] = w_radio(self, rvar,
                                   value=lbl, label=lbl,
                                   **(radioArgs or {}))
        return widgets
...
class RadioFrameLabelled(FrameLabelled):
    ...
    wstate_single = 'raw'
    '''Default value for `wstate_single <mixin.MixinStateContainer.wstate_single>`.'''
    variable_raw: var.Variable
    '''The internal variable exposed as the state.'''
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)  # FrameUnlabelled
        self.variable_raw = list(self.raw.widgets.values())[0].variable
...
class RadioFrameStateful(FrameStateful):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)  # FrameStateful
        self.variable_raw = list(self.raw.widgets.values())[0].variable
...
class Canvas(tk.Canvas, mixin.SingleWidget):
    ...
    There is no Python documentation, see ``Tk`` :tk:`tk.Canvas <canvas.html>`
    documentation.
...
class Separator(ttk.Separator, mixin.SingleWidget):
    ...
    <ttk_separator.html>` documentation.
...
# TODO: Support delay (use a cancellable Timeout for scheduling)
class Tooltip(SecondaryWindow):
    ...
    hovering over it with the mouse. It is hidden when the mouse moves away, or
    when the mouse touches the tooltip itself.
    ...
    Does not contribute to the parent layout (being a `SecondaryWindow`), but
    neither does contribute to the parent state.
    ...
    def adjustGeometry(self, widget: mixin.MixinWidget, *,
        ...
        if sposition == model.CP.N:
            ...
            raise NotImplementedError(f'Missing Support for CP: {position}')
    ...
    def enable(self, *, immediate: bool = True):
        ...
        assert _benter.sequence == '<Enter>' and _benter.action == self.tryschedule
        ...
        if _benter_changed and immediate:
            ...
            if self.wparent == self.winfo_containing(*mxy):
                ...
                assert self.wparent is not None
                self.after_idle(_bcfg.action)  # No need for a `TimeoutIdle` here
    ...
    def disable(self, *, immediate: bool = True):
        ...
        assert _benter.action == self.tryschedule
...
class TooltipValidation(Tooltip):
    ...
    invalid, and hidden when the attached widget is valid. This is static
