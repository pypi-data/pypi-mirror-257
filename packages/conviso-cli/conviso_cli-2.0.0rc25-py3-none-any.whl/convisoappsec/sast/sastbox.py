...
class SASTBox(object):
    ...
    def _scan_diff(self, code_dir, current_commit, previous_commit, log, token):
        ...
        command = (
            ''' -c {code_dir} --diff={previous_commit},{current_commit} -a -o {report} -v --dedup'''.format(
                code_dir=self.CONTAINER_CODE_DIR, previous_commit=previous_commit, current_commit=current_commit, report='/tmp/output.sarif')
        )
