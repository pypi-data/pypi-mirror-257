...
class GitAdapter(object):
    ...
    def diff(self, version, another_version):
        ...
        commit = self.find_commit_by_hash(self._repo, version)
        ...
        if commit:
            ...
            self._git_client.diff(None, None, output_stream=diff_file)

        return diff_file

    @staticmethod
    def find_commit_by_hash(repo, version):
        try:
            commit = repo.rev_parse(version)
            return commit
        except:
            return None
    ...
    def diff_stats(self, version, another_version):
        ...
        if commit:
            ...
            self._git_client.diff(None, None, '--numstat', output_stream=stats_output)
    ...
    def get_commits_by_range(self, start_commit, end_commit):
        ...
        commit = self.find_commit_by_hash(self._repo, start_commit)
        ...
        if commit:
            ...
            self._git_client.rev_list('--all', output_stream=tmp_commits)
