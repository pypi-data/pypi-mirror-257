...
class MarcJsonReader:
    ...
    @staticmethod
    def from_json_data(json_data) -> list[Record]:
        reader = MarcJsonReader(None)
        reader.json = json_data
        if not isinstance(reader.json, list):
            reader.json = [reader.json]
        return list(reader)

    @staticmethod
    def from_json_string(json_string) -> list[Record]:
        reader = MarcJsonReader(None)
        reader.json = json.load(json_string)
        if not isinstance(reader.json, list):
            reader.json = [reader.json]
        return list(reader)
