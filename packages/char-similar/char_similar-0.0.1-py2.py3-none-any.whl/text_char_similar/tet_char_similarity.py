...
from text_char_similar.const_dict import load_json, save_json
...
def cal_sim_by_w2v(char1, char2, rounded=4, code=4, rate_fourangle=7,
    ...
    # 笔顺(实际为最小的集合), 集合的与 / 集合的并
    score_order = sim_order(char1, char2)
    # 四角码(code=4, 共4位), 统计四个数字中的相同数(拼音/声母/韵母/声调)/4
...
def cal_sim(char1, char2, rounded=4, kind="shape"):
    """ 计算两汉字相似度(字形-拼音-语义)
    calculate similarity of two chars, by char shape
...
def calculate_char_similarity(rate=0.5):
        ...
        with open(path_confusion, mode="r", encoding="utf-8") as fj:
            ...
            for k, v in tqdm(model_json.items(), desc="data"):
                ...
                for vi in v:
                    try:
                        ...
                        # score_sim = cal_sim_by_pinyin(k, vi)
                        # score_sim = cal_sim_by_w2v(k, vi)
                        score_sim = cal_sim_by_all(k, vi)
            ...
            save_json(model_json_new, "confusion_merge_all_fix_dict.sort_all.json")
...
def tet_2w_to_2w_v1():
    ...
    list_chat_all = list(dict_char_fourangle.keys())
    res_dict_128 = {}
    res_dict = {}
    kind = "all"  # "shape"  # "all"  # "shape" # "all"   "w2v"  "pinyin"
    for k in tqdm(dict_char_fourangle.keys(), desc="data"):
        v_score = []
        for v in list_chat_all:
...
def tet_2w_to_2w_line_by_line_v2():
...
if __name__ == "__main__":
    ...
    # # 计算已知字典的相似度, 最多取得top20, 或rate>0.4
    # calculate_char_similarity()

    # # 废弃, 2w数据一一比对, 获取最相似的, 全量写入会OOM
    # tet_2w_to_2w_v1()

    # # 2w数据一一比对, 获取最相似的, 单行数据line写入
    tet_2w_to_2w_line_by_line_v2()
...
tet_2w_to_2w_v1
内存泄漏卡住直接杀掉
[19871398.193232] Out of memory: Killed process 1601460 (python) total-vm:151812064kB, anon-rss:133178256kB, file-rss:0kB, shmem-rss:0kB, UID:1012 pgtables:287148kB oom_score_adj:0
[19871408.007256] oom_reaper: reaped process 1601460 (python), now anon-rss:0kB, file-rss:0kB, shmem-rss:0kB
