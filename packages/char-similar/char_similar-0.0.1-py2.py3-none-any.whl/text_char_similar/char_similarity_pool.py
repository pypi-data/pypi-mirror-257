...
def pool_cal_sim_by_shape(char1, char2, rounded=4, code=4, rate_fourangle=7,
    rate_component=2, rate_frequency=5, rate_number=6, rate_stroke=4,
    rate_struct=8, rate_order=3, processes=7, timeout=1):
    """ 计算两汉字相似度(字形重点)
    ...
    """
    executor = Pool(max_workers=processes)
    future_1 = executor.submit(lambda p: sim_fourangle(*p),
                                      (char1, char2, code))
    future_2 = executor.submit(lambda p: sim_component(*p),
                                      (char1, char2))
    future_3 = executor.submit(lambda p: sim_frequency(*p),
                                      (char1, char2))
    future_4 = executor.submit(lambda p: sim_number(*p),
                                   (char1, char2))
    future_5 = executor.submit(lambda p: sim_stroke(*p),
                                   (char1, char2))
    future_6 = executor.submit(lambda p: sim_struct(*p),
                                   (char1, char2))
    future_7 = executor.submit(lambda p: sim_order(*p),
                                  (char1, char2))
...
def pool_cal_sim_by_pinyin(char1, char2, rounded=4, code=4, rate_fourangle=7,
    rate_component=2, rate_frequency=5, rate_number=6, rate_stroke=4,
    ...
    future_7 = executor.submit(lambda p: sim_order(*p),
                                  (char1, char2))
    future_8 = executor.submit(lambda p: sim_pinyin(*p),
                               (char1, char2, code))
...
def pool_cal_sim_by_w2v(char1, char2, rounded=4, code=4, rate_fourangle=7,
    rate_component=2, rate_frequency=5, rate_number=6, rate_stroke=4,
    ...
    future_7 = executor.submit(lambda p: sim_order(*p),
                                  (char1, char2))
    future_8 = executor.submit(lambda p: sim_w2v(*p),
                               (char1, char2))
...
def pool_cal_sim_by_all(char1, char2, rounded=4, code=4, rate_fourangle=7,
    rate_component=2, rate_frequency=5, rate_number=6, rate_stroke=4,
    ...
    future_8 = executor.submit(lambda p: sim_pinyin(*p),
                               (char1, char2, code))
    future_9 = executor.submit(lambda p: sim_w2v(*p),
                               (char1, char2))
    concurrent.futures.wait([future_1, future_2, future_3, future_4,
...
def pool_cal_sim(char1, char2, rounded=4, kind="shape"):
    ...
    if kind.upper() == "PINYIN":
        result = pool_cal_sim_by_pinyin(char1, char2, rounded)
    elif kind.upper() == "W2V":
        result = pool_cal_sim_by_w2v(char1, char2, rounded)
    elif kind.upper() == "ALL":
        result = pool_cal_sim_by_all(char1, char2, rounded)
    else:
