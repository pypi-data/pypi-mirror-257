...
def get_similar_by_shape(char1, char2, rounded=4):
    """   计算字形相似度   """
    # 获取四码和汉字笔画数
    structure1 = StructureDict.get(char1, None)
    structure2 = StructureDict.get(char2, None)
    stroke1 = StrokeDict.get(char1, None)
    stroke2 = StrokeDict.get(char2, None)
    code1 = FourAngleDict.get(char1, None)
    code2 = FourAngleDict.get(char2, None)
...
def calculate_char_similarity():
    ...
    with open(path_confusion, mode="r", encoding="utf-8") as fj:
        ...
        for k, v in tqdm(model_json.items(), desc="data"):
            ...
            for vi in v:
                try:
                    score_sim = get_similar_by_shape(k, vi)
