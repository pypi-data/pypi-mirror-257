...
# @author  : Mo
# @function: logger of text_char_similar
...
# 定义一个日志记录器
logger = logging.getLogger("text-char-similar")
logger.setLevel(level = logger_level)
# 日志文件名,为启动时的日期
log_file_name = time.strftime('text-char-similar-%Y-%m-%d', time.localtime(time.time())) + ".log"
...
def get_logger_root(name="text-char-similar"):
    """
        全局日志引用
