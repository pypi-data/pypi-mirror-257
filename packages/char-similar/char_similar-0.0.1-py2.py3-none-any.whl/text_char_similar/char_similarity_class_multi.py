...
from text_char_similar.char_similarity import sim_fourangle, sim_pinyin, sim_component
from text_char_similar.char_similarity import sim_frequency, sim_number, sim_stroke
from text_char_similar.char_similarity import sim_struct, sim_order, sim_w2v


class MultiCalSimByShape:
    def __init__(self, processes=7):
        self.executor = Pool(max_workers=processes)

    def multi_cal_sim_by_shape(self, char1, char2, rounded=4, code=4, rate_fourangle=7,
...
class MultiCalSimByPinyin:
    def __init__(self, processes=8):
        self.executor = Pool(max_workers=processes)

    def multi_cal_sim_by_pinyin(self, char1, char2, rounded=4, code=4, rate_fourangle=7,
...
class MultiCalSimByW2v:
    def __init__(self, processes=8):
        self.executor = Pool(max_workers=processes)

    def multi_cal_sim_by_w2v(self, char1, char2, rounded=4, code=4, rate_fourangle=7,
        rate_component=2, rate_frequency=5, rate_number=6, rate_stroke=4,
...
class MultiCalSimByAll:
    def __init__(self, processes=9):
        self.executor = Pool(max_workers=processes)

    def multi_cal_sim_by_all(self, char1, char2, rounded=4, code=4, rate_fourangle=7,
        rate_component=2, rate_frequency=5, rate_number=6, rate_stroke=4,
...
class MultiCalSim:
    def __init__(self, kind="shape"):
        ...
        if kind.upper() == "PINYIN":
            self.MultiCalSimByPinyin_1 = MultiCalSimByPinyin()
        elif kind.upper() == "W2V":
            self.MultiCalSimByW2v_1 = MultiCalSimByW2v()
        elif kind.upper() == "ALL":
            self.MultiCalSimByAll_1 = MultiCalSimByAll()
        else:
            self.MultiCalSimByShape_1 = MultiCalSimByShape()

    def multi_cal_sim(self, char1, char2, rounded=4):
        """ 计算两汉字相似度(字形-拼音-语义)
        calculate similarity of two chars, by char shape
        ...
        if self.kind.upper() == "PINYIN":
            result = self.MultiCalSimByPinyin_1.multi_cal_sim_by_pinyin(char1, char2, rounded)
        elif self.kind.upper() == "W2V":
            result = self.MultiCalSimByW2v_1.multi_cal_sim_by_w2v(char1, char2, rounded)
        elif self.kind.upper() == "ALL":
            result = self.MultiCalSimByAll_1.multi_cal_sim_by_all(char1, char2, rounded)
        else:
            # result = pool_cal_sim_by_shape(char1, char2, rounded)
            result = self.MultiCalSimByShape_1.multi_cal_sim_by_shape(char1, char2, rounded)
        return result
...
def multi_cal_sim(char1, char2, rounded=4, kind="shape"):
    ...
    if kind.upper() == "PINYIN":
        result = MultiCalSimByPinyin_1.multi_cal_sim_by_pinyin(char1, char2, rounded)
    elif kind.upper() == "W2V":
        result = MultiCalSimByW2v_1.multi_cal_sim_by_w2v(char1, char2, rounded)
    elif kind.upper() == "ALL":
        result = MultiCalSimByAll_1.multi_cal_sim_by_all(char1, char2, rounded)
    else:
        # result = Multi_cal_sim_by_shape(char1, char2, rounded)
        result = MultiCalSimByShape_1.multi_cal_sim_by_shape(char1, char2, rounded)
    return result


# 初始化类class, 主要是Multi进程池
MultiCalSimByPinyin_1 = MultiCalSimByPinyin()
MultiCalSimByShape_1 = MultiCalSimByShape()
MultiCalSimByW2v_1 = MultiCalSimByW2v()
MultiCalSimByAll_1 = MultiCalSimByAll()
...
if __name__ == '__main__':
    ...
    # MultiCalSim_1 = MultiCalSim(kind=kind)

    time_start = time.time()
    # res = MultiCalSim_1.multi_cal_sim(char1, char2, rounded=rounded)
    res = multi_cal_sim(char1, char2, rounded=rounded)
    time_end = time.time()
