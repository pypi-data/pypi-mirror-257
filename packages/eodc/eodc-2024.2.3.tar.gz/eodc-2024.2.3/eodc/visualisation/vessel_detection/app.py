...
def get_info(target_row=None):
    header = [html.H4("Nearest Vessel")]
    if isinstance(target_row, type(None)):
        return header + [html.P("Hover over an OpenEO vessel result")]
...
def add_vessel_layer(
    ...
    if query_via_item:
        ...
        matching_item = next(
            (item for item in item_collection.items if item.id == openeo_item), None
        )

        features = pygeoapi_from_item(matching_item)
        if not features:
        ...
        new_layer = [
            dl.Overlay(
                dl.GeoJSON(
                    url=features,
                    cluster=False,
                    id={"type": "pygeoapi-layer", "index": n},
                ),
                name=f"{openeo_item}: Marine Traffic Overlay",
                checked=True,
            )
        ]
...
def display_output(values, children):
    ...
    if tigger["value"]:
        ...
        df = pd.DataFrame(
            {
                "Keys": [key for key in data["properties"].keys()],
                "Values": [value for value in data["properties"].values()],
            }
        )
...
def render_item(n, asset_title, item_id, canonical_url, map_layers, data, date):
    if n:
        ...
        if "data" not in data:
            ...
            # Really ugly. Filter dates out of geojson, and then convert back to json,
            # and load into dict repalce T in string for dataframe filter
            ...
            layer_name = f"{item_id}: {asset_title}: {str(data['dates'][date])[:-3]}"
        ...
        new_layer = [dl.Overlay(geojson, name=layer_name, checked=True)]
