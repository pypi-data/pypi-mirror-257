...
def dq_generated_zip(table_name=None):
    ...
    src_path = os.path.join('data_quality_rules', 'data_mvp', "dql", uuaa_master, table_name)
    archive_name = f'{table_name}.zip'
...
def kirby_generated_zip(table_name=None):
    ...
    uuaa_master = "".join(table_name.split("_")[1])
    src_path = os.path.join('data_quality_rules', 'data_kirby', "kirby", uuaa_master, table_name)
    archive_name = f'{table_name}.zip'
...
def dq_read_schema_artifactory(dir_filename=None, columns_primary_key=None):
    ...
    print(rs.get("table_name"), rs.get("key_columns_list"))
    return rs
...
def dq_generated_dataframe_json(hamu_type=None,
    ...
    if hamu_type == "raw":
        ...
        table_dict["kind"] = "processing"
        table_dict["params"] = dict()
        repo_config = os.path.join(repo_path_per, uuaa_master, "raw", table_master_name, repo_version, f"{table_master_name}-01.conf")
        table_dict["params"]["configUrl"] = repo_config
        table_dict["params"]["sparkHistoryEnabled"] = "true"
        table_dict["runtime"] = "hammurabi-lts"
        table_dict["size"] = "M"
    ...
    if hamu_type == "master":
        ...
        table_dict["kind"] = "processing"
        table_dict["params"] = dict()
        repo_config = os.path.join(repo_path_per, uuaa_master, "master", table_master_name, repo_version, f"{table_master_name}-01.conf")
        table_dict["params"]["configUrl"] = repo_config
        table_dict["params"]["sparkHistoryEnabled"] = "true"
        table_dict["runtime"] = "hammurabi-lts"
        table_dict["size"] = "M"
    ...
    print(f"{get_color('HOCON JSON CREATE:')} {get_color_b(_filename_text)}")
...
def dq_generated_dataframe_conf(namespace=None,
    ...
    if table_name not in table_dict.keys():
        ...
        table_dict[table_name]["hammurabi"]["dataFrameInfo"]["cutoffDate"] = "${?CUTOFF_ODATE}"
        ...
        if hamu_type == "staging":
            ...
            table_dict[table_name]["hammurabi"]["input"]["options"] = dict(delimiter="|", castMode="notPermissive", charset="UTF-8", header=False)
    ...
    _filename_text = dir_hocons_mvp_filename.split("/")[-5:]
    _filename_text = "/".join(_filename_text)
    print(f"{get_color('HOCON CONF CREATE:')} {get_color_b(_filename_text)}")
...
def dq_generated_mvp(table_master_name=None,
    ...
    for index, rule_id in enumerate(rule_ids_raw):
        ...
        hamu_dict["config"]["dataValues"]["options"]["header"] = False
    ...
    generate_kirby_conf(table_raw_name=table_name_raw, table_master_name=table_name_master, kirby_type="raw")
    generate_kirby_conf(table_raw_name=table_name_raw, table_master_name=table_name_master, kirby_type="master")
    kirby_generated_zip(table_name=table_name_master)


def generate_kirby_conf(table_raw_name=None, table_master_name=None, kirby_type=None):
    import os
    import json
    from pyhocon import ConfigFactory
    from pyhocon import HOCONConverter
    ...
    if kirby_type == "raw":
        ...
        dir_hocons_kirby_raw_filename_json = f"{table_master_name}.json"
        dir_hocons_kirby_raw_filename_conf = f"{table_master_name}.conf"
        pj_dq_dir_name = os.path.join(pj_dir_workspace, pj_dq_dir_name)
        pj_dq_dir_table_raw_json = os.path.join(pj_dir_workspace, pj_dq_dir_name, "data_kirby", "kirby", f"{uuaa_master}", "raw", f"{dir_hocons_kirby_raw_filename_json}")
        pj_dq_dir_table_raw_conf = os.path.join(pj_dir_workspace, pj_dq_dir_name, "data_kirby", "kirby", f"{uuaa_master}", "raw", f"{dir_hocons_kirby_raw_filename_conf}")
        ...
        table_dict_conf[table_raw_name]["kirby"]["input"]["path"] = [f"/in/staging/datax/{uuaa_raw}/" + "#SDA_MYFILENAME_${?PERIOD}.csv"]
        ...
        table_dict_conf[table_raw_name]["kirby"]["output"] = dict(mode="", force="", options=dict(), partition="", path="",
                                                                  schema=dict(), type="", dropLeftoverFields="")
        table_dict_conf[table_raw_name]["kirby"]["output"]["mode"] = "overwrite"
        table_dict_conf[table_raw_name]["kirby"]["output"]["force"] = "true"
        table_dict_conf[table_raw_name]["kirby"]["output"]["options"] = dict(partitionOverwriteMode=dict())
        ...
        pj_dq_dir_name = os.path.join(pj_dir_workspace, pj_dq_dir_name)
        pj_dq_dir_table_master_json = os.path.join(pj_dir_workspace, pj_dq_dir_name, "data_kirby", "kirby", f"{uuaa_master}", "master", f"{dir_hocons_kirby_master_filename_json}")
        pj_dq_dir_table_master_conf = os.path.join(pj_dir_workspace, pj_dq_dir_name, "data_kirby", "kirby", f"{uuaa_master}", "master", f"{dir_hocons_kirby_master_filename_conf}")
        pj_dq_dir_table_raw_schema = os.path.join(pj_dir_workspace, pj_dq_dir_name, "data_schema", f"{table_master_name}", "raw", f"{table_raw_name}.output.schema")
        pj_dq_dir_table_master_schema = os.path.join(pj_dir_workspace, pj_dq_dir_name, "data_schema", f"{table_master_name}", "master", f"{table_master_name}.output.schema")
        ...
        table_dict_conf[table_master_name]["kirby"]["input"]["options"]["applyConversions"] = False
        table_dict_conf[table_master_name]["kirby"]["input"]["path"] = [f"/data/raw/{uuaa_raw}/data/{table_raw_name}"]
        ...
        table_dict_conf[table_master_name]["kirby"]["output"]["options"] = dict(partitionOverwriteMode=dict())
        ...
        table_dict_conf[table_master_name]["kirby"]["output"]["path"] = f"/data/master/{uuaa_raw}/data/{table_master_name}"
        ...
        table_dict_conf[table_master_name]["kirby"]["output"]["type"] = "avro"
        ...
        for field in txt_json["fields"]:
            ...
            if name not in ("audtiminsert_date", "cutoff_date"):
                ...
                if str(logical_format).startswith("DECIMAL"):
        ...
        if len(decimal_columns_list) > 0:
            decimal_columns = "|".join(decimal_columns_list)
            formatter_decimal_dict["type"] = "formatter"
            formatter_decimal_dict["field"] = decimal_columns
