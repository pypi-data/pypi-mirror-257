...
def run():
    ...
    parser = argparse.ArgumentParser(
        description="This script takes a product name and a directory, and checks for newly available imagery or gaps in the archive. Imagery is then downloaded and written to disk to complete the archive.")
    parser.add_argument('product',
                        type=str,
                        default='MOD09CMG',
                        help='Name of desired imagery product')
    parser.add_argument('directory',
                        type=str,
                        default='.',
                        help='Full path to imagery storage directory')
    parser.add_argument('-vi',
                        type=str,
                        default='ndvi',
                        help='Which vegetation index to process: NDVI, GCVI')
    parser.add_argument('-start_year',
                        type=int,
                        default=2000,
                        help='Override default start year (first existing file)')
    parser.add_argument("-scale_glam",
                        action='store_true',
                        help='Use GLAM system scaling (NDVI * 10000)')  # default is False
    parser.add_argument("-scale_mark",
                        action='store_true',
                        help="Use Mark's scaling ((NDVI * 200) + 50)")  # default is False
    parser.add_argument("-print_missing",
                        action='store_true',
                        help="Print missing dates (+status) and exit")

    args = parser.parse_args()

    ## validate arguments
    if args.scale_glam:
        assert not args.scale_mark, "Set exactly one of -scale_glam or -scale_mark!"
    else:
        assert args.scale_mark, "Set exactly one of -scale_glam or -scale_mark!"
        assert args.vi == 'ndvi', "MARK scaling should only be used with NDVI"

    ## clean and collect existing files
    # remove running composites
    runningComposites = glob.glob(os.path.join(args.directory, "*.running_composite.tif"))
    for c in runningComposites:
        os.remove(c)
    # get list of existing full files
    extantFiles = glob.glob(os.path.join(args.directory, f"*.tif"))

    ## get missing dates
    # first and last year
    if args.start_year:
        firstYear = int(args.start_year)
    else:
    ...
    for y in tqdm(years, desc='year'):
        for doy in tqdm(doys, desc='doy', leave=False):
            if not os.path.exists(os.path.join(args.directory, generateFileName(args.product, args.vi, y, doy))):
                formattedDate = datetime.strptime(f"{y}.{doy}", "%Y.%j").strftime("%Y-%m-%d")
                # is the image in the future?
                if datetime.strptime(f"{y}.{doy}", "%Y.%j") >= datetime.now() or datetime.strptime(f"{y}.{doy}",
                                                                                                   "%Y.%j") < datetime.strptime(
                    earliestDataDict.get(args.product, "2000.049"), "%Y.%j"):
                    continue
                # does the image exist at all?
                if len(octvi.url.getDates(args.product, formattedDate)) == 0:  # if there is valid imagery for the date
                    dates[formattedDate] = "No imagery"
                # are there missing files in the compositing period?
                elif not check8DayValidity(args.product, formattedDate):
    ...
    if args.vi == 'ndvi':
        ...
        for d in dates.keys():
            if dates[d] == "Available":
                ...
                outPath = os.path.join(args.directory, generateFileName(args.product, args.vi, date=d))
                octvi.globalVi(args.product, d, outPath)
                if args.scale_mark:
        ...
        for d in dates.keys():
            if dates[d] == "Available":
                ...
                log.info(f"Creating Composite for {d}")
                outPath = os.path.join(args.directory, generateFileName(args.product, args.vi, date=d))
                octvi.globalVi(args.product, d, outPath, vi="GCVI")
