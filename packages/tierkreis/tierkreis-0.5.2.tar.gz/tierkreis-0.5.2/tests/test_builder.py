...
async def main():
    ...
    import tierkreis.worker as tk_wo

    wo_root = tk_wo.Namespace()
    wo_fl = wo_root["fairlight"]


    @wo_fl.function()
    async def echo_str() -> str:
        return "Hello world!"
    from tierkreis.builder import Output, Const, graph
    from tierkreis.pyruntime import PyRuntime
    from tierkreis.builder import Namespace

    cl = PyRuntime([wo_root])
    ...
    res = await cl.run_graph(main)

    print(res["value"].to_python(str))
