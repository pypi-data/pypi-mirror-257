...
class OpenAIDataExtractor:
                    ...
                    if not ent_code and not ent_name:
                        ...
                        try:                                                        # Load modules for template fetching
                            ...
                            if not isinstance(resultdata, bool) and resultdata != "":
                                ymlfilename = self.template_folder + "/" + (resultdata["ent_name"]).replace(" ","_") + "_" + str(resultdata["ent_code"]) + ".yml"
    ...
    def getlayouttextaidata(self):
        ...
        try:
            ...
            if 'keyword_1' in jsonData.keys():
                template_Keyword_1 = jsonData['keyword_1']
                logger.log(f"template_Keyword_1::  {template_Keyword_1}")

            if 'keyword_2' in jsonData.keys():
                template_Keyword_2 = jsonData['keyword_2']
            ...
            # if len(template_Keyword_1)>0 and len(template_Keyword_2)>0:
            #     logger.log(f"keyword both are not empty CASE")
            #     ymlfilename = ent_name +  "_" + template_Keyword_1 + "_" + template_Keyword_2
            # elif len(template_Keyword_1) == 0 and len(template_Keyword_2)>0:
            #     logger.log(f"keyword1 is empty and 2 is not empty CASE")
            #     ymlfilename=ent_name + "_" + template_Keyword_2
            # elif len(template_Keyword_1) > 0 and len(template_Keyword_2)==0:
            #     logger.log(f"keyword2 is empty and 1 is not empty CASE")
            #     ymlfilename=ent_name + "_" + template_Keyword_1
            # else:
            #     logger.log(f"keyword1 is empty")
            #     ymlfilename=ent_name
            ...
            if ai_proc_templ:
                ymlfilepath = "/"+(given_temp_path)+"/"+str(ymlfilename).strip().replace(" ","_").replace(".","").replace("/","")+".yml"
                ...
                if ent_name.strip() and ((isinstance(ent_code, str) and ent_code.strip()) or isinstance(ent_code, int)):  
                    ...
                    if ent_name in OCR_Text:
                        ...
                        templatecreation.generateHeaderTemplate(ymlfilepath,ent_name,ent_code,ent_type,ai_proc_variables,OCR_Text,template_Keyword_1,template_Keyword_2)
