...
from ipyoptimade.logger import LOG_DIR, LOGGER, REPORT_HANDLER, WIDGET_HANDLER
from ipyoptimade.utils import __optimade_version__, ButtonStyle, CACHE_DIR
from ipyoptimade.version import __version__ as __ipyoptimade_version__


STATICS_DIR = Path(__file__).parent.joinpath("statics")
SOURCE_URL = "https://github.com/aiidalab/ipyoptimade/"
...
"""
    ...
    def __init__(
        ...
        logo = self._get_file(str(STATICS_DIR.joinpath(logo)))
...
class OptimadeLog(ipw.Accordion):
    ...
    def __init__(self, **kwargs):
        self._debug = bool(os.environ.get("ipyoptimade_DEBUG", False))
