...
try:
    DEVELOPMENT_MODE = bool(int(os.getenv("ipyoptimade_DEVELOPMENT_MODE", "0")))
except ValueError:
    LOGGER.debug(
        (
            "ipyoptimade_DEVELOPMENT_MODE found, but cannot be parsed as a bool of an int. "
            "Setting it to False. Found value: %s"
        ),
        os.getenv("ipyoptimade_DEVELOPMENT_MODE"),
    )
