...
class Clea(Lea):
    ...
    def __init__(self, *args):
        Lea.__init__(self)
        self._lea_args = tuple(coerce(arg) for arg in args)
    ...
    ## this overwrites Lea..__getitem__
    ## - returns a Clea instance in case of slicing
    ## - optimization, to avoid creation of a Flea2 instance
    def __getitem__(self, key):
        res = getitem(self._lea_args, key)
        if isinstance(key, slice):
            return Clea(*res)
        return res

    def __len__(self):
        """ returns the number of inner Lea instances jointed in self
            called on evaluation of "len(self)";
        """
        return len(self._lea_args)

    def __iter__(self):
        """ generates one by one the inner Lea instances jointed in self,
            in the order used at instantiation of self
            called on evaluation of "iter(self)", "tuple(self)", "list(self)"
            or on "for x in self";
        """
        return iter(self._lea_args)
    ...
    def _em_step(self, model_lea, cond_lea, obs_pmf_tuple, conversion_dict):
        return Clea(
            *(lea_arg.em_step(model_lea, cond_lea, obs_pmf_tuple, conversion_dict) for lea_arg in self._lea_args))
