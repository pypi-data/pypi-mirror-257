...
from .lea import Lea, reduce_all
from .exceptions import LeaError
from .alea import coerce, vals, pmf
...
class Blea(Lea):
    ...
    given distribution <Vi,p(Vi|C)>, assuming a given condition C is verified,
    ...
    @staticmethod
    def build(*clauses, prior_lea=None, auto_else=False, check=True):
        ...
        if len(else_clause_results) == 1:
            ...
            else_clause_result = vals(*frozenset(val for (cond, result) in clauses for val in coerce(result).support()))
        else:
            else_clause_result = None
        # get clause conditions and results, excepting 'else' clause, after coercion to Lea instances
        norm_clauses = ((coerce(cond), coerce(result)) for (cond, result) in clauses if cond is not None)
        ...
        or_conds_lea = reduce_all(or_, cond_leas, True)
        if prior_lea is not None:
            ...
            norm_alea_dict = dict(vals(*res_leas).flat().get_alea()._gen_vp())
            ...
            for value in values_set:
                ...
                if not (0 <= p <= p_false):
                    ...
                    raise LeaError("prior probability of '%s' is %s, outside the range [ %s , %s ]" % (
                        value, prior_p, lower_p, upper_p))
                vps.append((value, p))
            else_clause_result = pmf(vps)
        ...
        if else_clause_result is not None:
            ...
            ## other equivalent statement: else_cond_lea = reduce_all(and_,(~cond_lea for cond_lea in cond_leas))
            else_clause_result = coerce(else_clause_result)
    ...
    def _gen_one_random_mc(self):
        ...
        # the first for loop binds a random value on each Alea instances referred in CPT conditions
