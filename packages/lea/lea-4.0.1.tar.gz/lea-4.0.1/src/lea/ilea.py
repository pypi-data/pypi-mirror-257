...
class Ilea(Lea):
    """
    Ilea is a Lea subclass, which instance represents a conditional probability distribution
    obtained by filtering the values Vi of a given Lea instance that verify a boolean
    condition C, which is the AND of given boolean conditions Cj(Vi).
    Beside explicit creations, Ilea instances can be created automatically to enforce
    global conditions expressed in "evidence contexts" (see EvidenceCtx class).
    ...
    def _gen_vp_mclw(self, nb_subsamples, exact_vars_lea, nb_tries=None):
        """ generates tuples (v,p) where v is a value of the current probability distribution
            ...
            (Monte-Carlo Likelihood Weighting): exact inference on the condition,
