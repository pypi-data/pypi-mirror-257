...
def detection(image_path, masked_image, label_list, bboxes=None, points=None, neg_points=None,labels=None, height=800, width=512, line_width=5.0, key=None, color_map=None) -> CustomComponent:
    ...
    bbox_info = [{'bbox':[b/scale for b in item[0]], 'label_id': item[1], 'label': color_map[item[1]]} for item in zip(bboxes, labels)]
    points_info = [{'point':[b/scale for b in item[0]], 'label_id': item[1], 'label': color_map[item[1]]} for item in zip(points, labels)]
    neg_points_info = [{'neg_point':[b/scale for b in item[0]], 'label_id': item[1], 'label': color_map[item[1]]} for item in zip(neg_points, labels)]
    component_value = _component_func(image_url=image_url, image_size=image.size, label_list=color_map, bbox_info=bbox_info, points_info=points_info, neg_points_info=neg_points_info,color_map=color_map, line_width=line_width, key=key)
