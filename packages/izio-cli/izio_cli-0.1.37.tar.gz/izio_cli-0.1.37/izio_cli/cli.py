...
from izio_cli.helper.console_helper import create_table, updateGradle
from izio_cli.helper.loyalty_helper import (
    changeAppIcon,
    changeOnesignalIcons,
    getEnv,
    setupEnvFile,
    setupFirebase,
)
...
def new_project(
    ...
    if not path:
        path = getProjectPath(projectName, solution=solution, type=type)
    else:
        path = getProjectPath(projectName, path, type=type, solution=solution)
    ...
    if (
        path
        != f"{os.sep}Users{os.sep}saulosenoski{os.sep}Development{os.sep}Izio{os.sep}izio_cli"
    ):
        if type == "flutter":
            setupVsCode(path, projectName, console)
...
def loyaltyFlavor(
    ...
    # get env file data
    flavor, bundleId, appName, appIconPath = getEnv(flavor, root)

    with console.status(
        "Updating android settigns", spinner="arc", spinner_style="bold green"
    ) as status:
        updateGradle(root, bundleId, appName)
        status.update("Updating app icon")
        changeAppIcon(root, appIconPath.replace("/", os.sep))
        status.update("Updating onesignal icons")
        changeOnesignalIcons(root)
        status.update("Updating firebase settings")
        setupFirebase(flavor, root)
        status.update("setup envfile")
        setupEnvFile(flavor, root)
        status.stop()
...
def testCommand():
    root = f"{os.sep}Users{os.sep}saulosenoski{os.sep}Development{os.sep}Izio{os.sep}mb_izio.flutter.apployalty2_0{os.sep}loyalty2_0"
    print(root)
