    ...
    with console.status(
        ...
        if projects:
            ...
            create_file(
                path,
                filename=".env",
                payload=f"//ToDo: change to your APM credentials \ELASTIC_APM_SERVICE_NAME={projectName}\ELASTIC_APM_SERVER_URLS=http://localhost:8200\ELASTIC_APM_SECRET_TOKEN=hashToken",
            )
...
def addIzioLib(projectName, path):
    ...
    run_command(
        [
            "dotnet",
            "add",
            f"{projectName}.DataAccess{os.sep}{projectName}.DataAccess.csproj",
            "package",
...
def createSubmodules(path, projectName):
    ...
    """
    subMobules = [
        projectName + f".Application{os.sep}Extensions",
        projectName + f".Application{os.sep}Services",
        projectName + f".Contracts{os.sep}DTOs",
        projectName + f".Contracts{os.sep}Validators",
        projectName + f".DataAccess{os.sep}Services",
        projectName + f".Domain{os.sep}Entities",
        projectName + f".Domain{os.sep}Interfaces",
        projectName + f".Domain{os.sep}Interfaces{os.sep}Business",
        projectName + f".Domain{os.sep}Interfaces{os.sep}DataAccess",
    ]
