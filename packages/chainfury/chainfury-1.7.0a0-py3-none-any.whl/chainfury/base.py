...
def pyannotation_to_json_schema(
    ...
    *,
    ...
    if isinstance(x, type):
        ...
        if x == Any:
            raise ValueError(
                f"i0: There is a 'Any' datatype in the annotation, chainfury will never support this."
            )
        ...
        if x == Exception and allow_exc:
            ...
            if is_return:
                raise ValueError(
                    f"i0: Unsupported type: {x}. Is your output annotated? Write like ... foo() -> Dict[str, str]"
                )
            else:
                raise ValueError(
...
def extract_jinja_indices(
    data: Union[str, List, Dict[str, Any]], current_index=(), indices=None
) -> List:
    """
...
class Chain:
    ...
    def to_id(
        ...
        if self.chain_id is None:
            ...
            if name is None:
                raise ValueError("Name is required for creating a new chain")
...
class ThreadChain:
    ...
    # Expand into thread management (koromandal)
    ...
    def thread(
        ...
        from chainfury.tune import Message
        from chainfury.core import ai_actions_registry, AIAction, model_registry

        if model:
            model_o = model_registry.get(model)
            if model_o is None:
                raise ValueError(f"Model {model} not found")
        else:
            model_o = model_registry.get_any_model()

        def _thread(fn) -> Callable:
            # fn must be of type: fn[Message, str, int, ...] -> [Chat]

            # check if the action is in the registry, if not then create one
            file_id = fn.__file__.replace(".py", "").replace("/", ".")
            node_id = id or f"{file_id}__{fn.__name__}"
            node = ai_actions_registry.get(node_id)
            if node is None:
                self.is_compiled = False
                setattr(fn, AIAction.FNTYPE, "thread")
                node = ai_actions_registry.register(
                    node_id=node_id,
                    fn=fn,
                    model_id=model_o.id,
                    model_params={},
            ...
            def llm_call(prompt: Message) -> Message:
                chat = fn(prompt)
                return prompt
