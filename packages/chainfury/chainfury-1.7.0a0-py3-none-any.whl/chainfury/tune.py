...
### Aliases
user = partial(Message, role=Message.HUMAN)
system = partial(Message, role=Message.SYSTEM)
gpt = partial(Message, role=Message.GPT)
value = partial(Message, role=Message.VALUE)
fn = partial(Message, role=Message.FUNCTION)
fn_resp = partial(Message, role=Message.FUNCTION_RESPONSE)
...
class TuneDataset:
    ...
    @classmethod
    def from_ddict_list(cls, items: List["TuneDataset"]):
        train_ds = TuneChats()
        eval_ds = TuneChats()
        for item in items:
...
#
# TuneChain: The code behind our internal chaining engine
#
...
class TuneChain:
    ...
    def compile(self):
        if self.is_compiled:
            ...
            # super().__init__(nodes=[], edges=[], main_in="", main_out="")
