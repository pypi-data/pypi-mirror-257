...
class SalureConnect:
    def __init__(self, customer: str = None, api_token: str = None, staging: str = 'prod', slug: str = None, task_id: int = None):
    ...
    def get_mappings(self, task_id: int) -> dict:
        ...
        """
        response = requests.get(url=f'{self.url}interfaces/{task_id}/data-mapping', headers=self._get_brynq_headers())
    ...
    def get_system_credential(self, system: str, label: Union[str, list], test_environment: bool = False) -> json:
        ...
        """
        response = requests.get(url=f'{self.url}interfaces/{system}', headers=self._get_brynq_headers())
    ...
    def refresh_system_credential(self, system: str, system_id: int) -> json:
        ...
        :return json response from BrynQ
        """
        response = requests.post(url=f'{self.url}interfaces/{system}/{system_id}/refresh', headers=self._get_brynq_headers())
        response.raise_for_status()
        credentials = response.json()
    ...
    def get_user_data(self):
        ...
        """
        return requests.get(url=f'{self.url}users', headers=self._get_brynq_headers())
    ...
    def get_role_data(self):
        ...
        """
        return requests.get(url=f'{self.url}roles', headers=self._get_brynq_headers())
    ...
    def create_user(self, user_data: dict) -> requests.Response:
        ...
        return requests.post(url=f'{self.url}users', headers=self._get_brynq_headers(), json=data)
    ...
    def update_user(self, user_id: str, user_data: dict) -> requests.Response:
        ...
        return requests.put(url=f'{self.url}users/{user_id}', headers=self._get_brynq_headers(), json=data)
    ...
    def delete_user(self, user_id: str) -> requests.Response:
        ...
        :return: A response object
        """
        return requests.delete(url=f'{self.url}users/{user_id}', headers=self._get_brynq_headers())
    ...
    def overwrite_user_roles(self, user_id: int, roles: list) -> requests.Response:
        ...
        return requests.put(url=f'{self.url}users/{user_id}/roles', headers=self._get_brynq_headers(), json=data)
    ...
    def get_source_system_entities(self, system: str) -> requests.Response:
        ...
        :return: A response object
        """
        return requests.get(url=f'{self.url}source-systems/{system}/entities', headers=self._get_brynq_headers())
    ...
    def get_layers(self) -> requests.Response:
        ...
        :return: A response object
        """
        return requests.get(url=f'{self.url}organization-chart/layers', headers=self._get_brynq_headers())
