...
def init(
    ...
    if skip_prompts:
        ...
        if not domain_to_use:
            console.print(
                "[red]You must pass a domain or set the REGSCALE_DOMAIN environment variable"
            )
            sys.exit(1)
        ...
        about_display()
        console.print(
            f'Successfully initialized with Domain [green]{app.config["domain"]}.'
        )
...
def env_info():
    ...
    print(
        f"Read & Write Permissions: {os.access('.', os.R_OK) and os.access('.', os.W_OK)}"
    )
    ...
    print(
        f"Running in an official RegScale container: {os.getenv('REGSCALE_CONTAINER', 'No')}"
    )
