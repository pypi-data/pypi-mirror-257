...
def fetch_inventory(
    ...
    existing_asset_data = Asset.get_all_by_parent(
...
def create_wiz_assets(
    ...
    for _, row in dataframe.iterrows():
        ...
        r_asset = Asset(
            ...
            otherTrackingNumber=str(provider_id),
            wizId=external_id,
            wizInfo=None,
            parentId=parent_id,
...
def get_report_id_from_response(
    ...
    try:
        ...
        resource_type = (
            report_variables["input"]["type"] if "input" in report_variables else ""
        )
        if resource_type:
            logger.warning(
                "Unable to process report for %s", report_variables["input"]["type"]
            )
        wiz_report_id = None
    return wiz_report_id
