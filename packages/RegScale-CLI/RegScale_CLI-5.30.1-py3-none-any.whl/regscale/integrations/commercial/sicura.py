...
from regscale.models import regscale_id as reg_id, regscale_module as reg_module
...
def sync_sicura_and_regscale(
    regscale_id: int, regscale_module, node_id: Optional[int] = None
):
    """Sync nodes from Sicura to RegScale as assets"""
    sync_sicura_nodes_and_regscale_assets(regscale_id, regscale_module, node_id)


def sync_sicura_nodes_and_regscale_assets(
    regscale_id: int, regscale_module: str, node_id: Optional[int] = None
):
    """
    ...
    existing_assets = Asset.get_by_parent(regscale_id, regscale_module)
    ...
    if update_assets := [
        asset for asset in existing_assets if asset.sicuraId in sicura_node_ids
    ]:
    ...
    for asset in track(
        ...
        existing_checks = {
            check.vulnerabilityId: check
            for check in Checklist.get_checklists_by_asset(api, asset.id)
        }
        results = sync_sicura_and_regscale_checks(
            app, asset, sicura_node_ids[asset.sicuraId]["scans"], existing_checks
        )
...
def bulk_asset_operation(method: str, app: Application, assets: list[Asset]) -> dict:
    ...
    if method == "insert":
        ...
        error_and_exit(
            f"Invalid method provided: {method}. Must be 'insert' or 'update'."
        )
    method = f"{method}d" if method == "update" else "created"
    failures = [
        f"{res.status_code}: {res.reason}-{res.text}" for res in results if not res.ok
    ]
    ...
    if failures:
        app.logger.error(
            "Failed to %s %i asset(s) in RegScale.", method[:-1], len(failures)
        )
        app.logger.warning("Failure message(s):\n%s", "\n".join(failures))
    return successes


def get_most_recent_scan(
    scans: list[dict], date_field: Optional[str] = "scan_date"
) -> dict:
    """
...
def sync_sicura_and_regscale_checks(
    ...
    for sicura_scan in recent_scan["scans"]:
        scan_date = remove_timezone_from_date_str(
            recent_scan["scan_metadata"]["scan_date"]
        )
        new_checks.append(
            Checklist(
    ...
    return Checklist.analyze_and_batch_process(
        app, new_checks, [check for check in existing_checklists.values()]
    )
...
def convert_string_to_int(string: Optional[str] = "0") -> int:
    ...
    :param str string: String to convert to an int, defaults to "0"
...
def map_asset(
    sicura_node: dict, parent_id: int, parent_module: str, user_id: str
) -> Asset:
    """
    ...
    if properties := sicura_node.get("properties"):
        ...
        new_asset.ram = (
            convert_string_to_int(properties.get("memorysize"))
            if properties.get("memorysize")
            else 0
        )
        new_asset.diskStorage = (
            convert_string_to_int(properties.get("mountpoints./.size"))
            if properties.get("mountpoints./.size")
            else 0
        )
...
def get_scans_from_sicura(api: Api, node_id: int) -> dict:
    ...
    if not response.ok:
        error_and_exit(
            f"Unable to fetch scan info from {response.url}.\n{response.status_code} "
            f"{response.reason}"
        )
    try:
