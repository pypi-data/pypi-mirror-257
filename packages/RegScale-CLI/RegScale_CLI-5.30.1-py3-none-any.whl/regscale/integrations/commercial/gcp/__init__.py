...
class GCPScannerIntegration(ScannerIntegration):
    ...
    def get_passed_findings(self) -> List[IntegrationFinding]:
        ...
        for control_id, categories in self.gcp_control_tests.items():
            for category, control_test in categories.items():
                if control_test.get("status", "") == "Failed":
                    logger.debug(
                        f"Control {control_id} had findings in category {category}, "
                        f"skipping passed control test creation"
                    )
                    continue
                if ci_id := self.ci_map.get(control_id.lower()):
                    ...
                    logger.warning(
                        f"Control {control_id} not found in RegScale SSP {self.plan_id}."
                    )
    ...
    def fetch_findings(self) -> List[IntegrationFinding]:
        ...
        failed_findings = list(
            filter(
                None, [self.parse_finding(finding.finding) for finding in gcp_findings]
            )
        )
    ...
    def parse_finding(self, gcp_finding: Finding) -> Optional[IntegrationFinding]:
        ...
        if any(
            control_labels := [
                c.ids for c in gcp_finding.compliances if c.standard == "nist"
            ]
        ):
            control_ids = [
                self.ci_map.get(str(cid).lower(), None)
                for cid in control_labels[0]
                if self.ci_map.get(str(cid).lower(), None)
            ]

            # Set control test status to failed since we found a finding for it
            for control_id in control_ids:
                if self.gcp_control_tests.get(control_id, {}).get(gcp_finding.category):
                    self.gcp_control_tests[control_id][gcp_finding.category][
                        "status"
                    ] = "Failed"
            ...
            return IntegrationFinding(
                ...
                impact=self.severity_map.get(
                    gcp_finding.severity, regscale_models.IssueSeverity.Low
                ),
                recommendation_for_mitigation=gcp_finding.source_properties.get(
                    "Recommendation", ""
                ),
            )
    ...
    def fetch_assets(self) -> List[IntegrationAsset]:
        ...
        :return: A list of parsed assets
    ...
    def parse_asset(self, asset: asset_v1.Asset) -> IntegrationAsset:
        ...
        :param asset: The GCP asset to parse
