...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~geezram.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~geezram.Client.sign_in` or :meth:`~geezram.Client.sign_up`).

        next_type (:obj:`~geezram.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~geezram.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~geezram.Client.resend_code`.
    """
