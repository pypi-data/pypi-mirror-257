...
class Object:
    def __init__(self, client: "geezram.Client" = None):
        self._client = client

    def bind(self, client: "geezram.Client"):
        """Bind a Client instance to this and to all nested geezram objects.

        Parameters:
            client (:obj:`~geezram.types.Client`):
                The Client instance to bind this object with. Useful to re-enable bound methods after serializing and
                deserializing geezram objects with ``repr`` and ``eval``.
        """
        self._client = client
