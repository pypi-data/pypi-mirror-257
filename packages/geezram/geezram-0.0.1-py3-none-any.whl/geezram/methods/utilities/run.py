...
import geezram
from geezram.methods.utilities.idle import idle
...
class Run:
    def run(
        ...
        :meth:`~geezram.Client.start`, :meth:`~geezram.idle` and :meth:`~geezram.Client.stop` in sequence.
        ...
        operation. This is almost the same as :py:obj:`asyncio.run` except for the fact that geezram's ``run`` uses the
        current event loop instead of a new one.

        If you want to run multiple clients at once, see :meth:`geezram.compose`.
