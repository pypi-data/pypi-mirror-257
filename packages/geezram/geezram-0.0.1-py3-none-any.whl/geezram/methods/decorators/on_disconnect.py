...
class OnDisconnect:
    def on_disconnect(self=None) -> Callable:
        ...
        :obj:`~geezram.handlers.DisconnectHandler`.
        """

        def decorator(func: Callable) -> Callable:
            if isinstance(self, geezram.Client):
                self.add_handler(geezram.handlers.DisconnectHandler(func))
            else:
                if not hasattr(func, "handlers"):
                    func.handlers = []

                func.handlers.append((geezram.handlers.DisconnectHandler(func), 0))
