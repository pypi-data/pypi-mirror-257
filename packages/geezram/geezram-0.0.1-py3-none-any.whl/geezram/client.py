...
import geezram
from geezram import __version__, __license__
from geezram import enums
from geezram import raw
from geezram import utils
from geezram.crypto import aes
from geezram.errors import CDNFileHashMismatch
from geezram.errors import (
    SessionPasswordNeeded,
    VolumeLocNotFound, ChannelPrivate,
    AuthBytesInvalid, BadRequest, ListenerCanceled
)
from geezram.handlers.handler import Handler
from geezram.methods import Methods
from geezram.session import Auth, Session
from geezram.storage import FileStorage, MemoryStorage, MongoStorage, Storage
from geezram.types import User, TermsOfService
from geezram.utils import ginput
...
class Client(Methods):
    """geezram Client, the main means for interacting with Telegram.
    ...
    Parameters:
        ...
        in_memory (``bool``, *optional*):
            ...
            :meth:`~geezram.Client.export_session_string` before stopping the client to get a session string you can
        ...
        storage (:obj:`~geezram.storage.Storage`, *optional*):
        ...
        workdir (``str``, *optional*):
            ...
            The working directory is the location in the filesystem where geezram will store the session files.
    ...
    APP_VERSION = f"Ky-Mang {__version__}"
    ...
    async def authorize(self) -> User:
        ...
        print(f"Welcome to Ky-Mang (version {__version__})")
        print(f"Ky-Mang is free software and comes with ABSOLUTELY NO WARRANTY. Licensed\n"
        ...
        while True:
            try:
                if not self.phone_number:
                    while True:
                        value = await ginput("Enter phone number or bot token: ")

                        if not value:
                            continue

                        confirm = (await ginput(f'Is "{value}" correct? (y/N): ')).lower()
        ...
        while True:
            if not self.phone_code:
                self.phone_code = await ginput("Enter confirmation code: ")
            ...
            try:
                ...
                while True:
                    ...
                    if not self.password:
                        self.password = await ginput("Enter password (empty to recover): ", hide=self.hide_password)

                    try:
                        if not self.password:
                            confirm = await ginput("Confirm password recovery (y/n): ")
                            ...
                            if confirm == "y":
                                ...
                                while True:
                                    recovery_code = await ginput("Enter recovery code: ")
        ...
        while True:
            first_name = await ginput("Enter first name: ")
            last_name = await ginput("Enter last name (empty to skip): ")
    ...
    def set_parse_mode(self, parse_mode: Optional["enums.ParseMode"]):
        ...
        Parameters:
            parse_mode (:obj:`~geezram.enums.ParseMode`):
        ...
        if session_empty:
            if not self.api_id or not self.api_hash:
                raise AttributeError("The API key is required for new authorizations. "
                                     "More info: https://docs.geezram.org/start/auth")
        ...
        try:
            ...
            if isinstance(r, raw.types.upload.File):
                ...
                try:
                    ...
                    raise e
        except geezram.StopTransmission:
            raise
        except Exception as e:
            log.error(e, exc_info=True)
