...
if __name__ == "__main__":
    # # 生成类实例对象
    om = OperateMysql('show', 'xcj')
    select_sql = ('SELECT a.`org_id`,a.`org_name`,a.`linkman`,a.`linkman_name` FROM xcjorg.`unit_stock_repo` a '
                  'WHERE a.`repo_id`=2003 AND a.`linkman` IS NOT NULL AND a.`linkman_name` IS NOT NULL '
                  'ORDER BY RAND() LIMIT 1')
    # # 执行sql
    om = OperateMysql('show', 'xcj')
    sql_res = om.select_all_data(select_sql)
    print(sql_res)
    # #
    # print(a)
    # # 关闭数据库连接l
