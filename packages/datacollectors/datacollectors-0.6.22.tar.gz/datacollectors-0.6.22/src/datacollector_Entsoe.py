...
parms_path = f'{Path(__file__).parent.parent / 'parms'}/'
...
class lib:
    _max_workers = 24
    _max_requests_per_minute = 400
    _entsoe_table_parms = utils.load_csv(f'{parms_path}entsoe_table_parms.csv')

    @staticmethod
    def show_code_areas(entsoe_data_type):
        entsoe_table = ''
        if entsoe_data_type.lower() in ['a73', 'production_by_asset']:
            entsoe_table = 'A73'
            ...
            entsoe_table = 'A44'
        if entsoe_table:
            entsoe_codes = utils.load_csv(f'{parms_path}{entsoe_table}.csv')
    ...
    def _create_tasks(cls, doc_url_base, start_date, end_date):
        ...
        for datestep in range(0, (end_date - start_date).days + 1, _request_max_days):
            ...
            step_end = (min(step_start + timedelta(days=_request_max_days - 1), end_date)
                        + timedelta(hours=(int(_entsoe_table['last_hour']))))
    ...
    def _load_tasks(cls, tasks):
        ...
        for batch in range(_batches + 1):
            ...
            with ThreadPoolExecutor(max_workers=cls._max_workers) as executor:
                _indata_list += list(executor.map(cls._load_single_xml,
                                                  tasks[batch * cls._max_requests_per_minute:(batch + 1) * cls._max_requests_per_minute]))
            duration = datetime.today().timestamp() - st
            if duration < 60 and batch < _batches:
    ...
    def _read_xml(cls, indata_xml):
        ...
        for obs in timeseries:
            ...
            if obs['Period']['resolution'] == 'PT60M':
                ...
                for point in data_points:
                    obs_dict = cls._read_point(obs, point, ts_start, time_resolution, flow_direction)
...
class transmission(lib):
    ...
    def _get_dayahead_prices(cls):
        ...
        entsoe_codes = utils.load_csv(f'{parms_path}{cls._entsoe_table}.csv')
...
class generation(lib):
    ...
    def _get_production_A75(cls):
        ...
        entsoe_codes = [c for c in entsoe_codes if c['area'] == cls._area]
        out_list = []
        psr_codes = {c['Code']: c['Type'] for c in utils.load_csv(f'{parms_path}/psr_types.csv')}
        for segment in entsoe_codes:
            code = segment['code']
            doc_url_base = f'documentType={cls._entsoe_table}&processType=A16&in_Domain={code}'
            indata_list = cls._load_entsoe_table(doc_url_base, segment)
