...
class ECB():
    ...
    def exrates(currency, denominator, start_date, end_date, decimals=4):
        ...
        _exrates = [{'date': obs, f'{currency} per {denominator}': _exrates[obs]} for obs in _exrates]
        return _exrates

    @staticmethod
    def exrates_df(currency, denominator, start_date, end_date, decimals=4):
        import pandas as pd
        exrates = ECB.exrates(currency, denominator, start_date, end_date, decimals)
        df = pd.DataFrame(exrates)
