...
@LastEditTime: 2024-02-05 15:36:42
...
class UserBaseModel(FrameBaseModel):
    ...
    def save_user_by_openid(self, app_id, act_id, open_id, user_nick, avatar, union_id="", login_ip="", is_update_user_nick=True, mix_nick='', plat_type=0):
        """
        ...
        if SevenHelper.is_continue_request(f"continue_request:save_user_by_openid:{app_id}_{open_id}", expire=continue_request_expire * 1000) == True:
            invoke_result_data.success = False
    ...
    def get_user_address_list(self, app_id, act_id, user_id, is_cache=True):
        ...
        if is_cache:
            user_address_dict_list = user_address_model.get_cache_dict_list(condition, limit="100", params=params, dependency_key=DependencyKey.user_address_list(act_id,user_id))
        else:
            user_address_dict_list = user_address_model.get_dict_list(condition, limit="100", params=params)

        # 敏感字段加密
        user_address_dict_list, status = self.sensitive_decrypt(user_address_dict_list, ["real_name","telephone","province","city","county","street","address"])
