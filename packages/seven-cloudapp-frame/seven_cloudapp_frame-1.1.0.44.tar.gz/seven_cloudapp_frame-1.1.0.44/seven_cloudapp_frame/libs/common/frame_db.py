...
@LastEditTime: 2024-02-20 11:11:24
...
class FrameDbModel(BaseModel):
    ...
    def relation_and_merge_dict_list(primary_dict_list, relation_db_model, relation_key_field, dependency_key, cache_expire, field="*", primary_key_field="id", is_cache=True):
        """
        ...
        if is_cache == True:
            ...
            relation_dict_list = relation_db_model.get_cache_dict_list(where,field=field)
        dict_list = SevenHelper.merge_dict_list(primary_dict_list, primary_key_field, relation_dict_list, relation_key_field)
        return dict_list
    

    def relation_and_merge_dict(primary_dict, relation_db_model, dependency_key, cache_expire, field="*", primary_key_field="id", is_cache=True):
        """
        :description: 根据给定的主键表字典合并关联表字典。
        :param primary_dict: 主表字典列表
        :param relation_db_model: 关联表关联model
        :param dependency_key: 缓存依赖键
        :param cache_expire: 缓存过期时间（秒）
        :param field: 关联表查询字段
