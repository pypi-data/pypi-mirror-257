    ...
    for debtor_l in debtors_list:  # Перебираем список договоров
        if debtor_l['name'] == client_name:  # Нашли договор организации
            ...
            if (ban_reason and isinstance(ban_reason, str)
                    and 'прекращен' in ban_reason.lower() and
                    debtor_l['summ_debt'] and int(debtor_l['summ_debt']) < 0):
                # Есть прекращенный договор с долгом - сразу запрет
                aborted_with_debt = True
                #return ban_reason
            if (ban_reason and isinstance(ban_reason, str)
                    and 'прекращен' in ban_reason.lower()):
                # Если есть прекращенный договор, но долга нет
            ...
            if not banned:
                ...
                actual_contract = True
                #status_special = True
                if debtor_l["status"].lower().strip() == "особый":
                    status_special = True
                    return
    # Итак если мы дошли до этого момента, уже вылетели те, у кого были
    # приостановленные договора
    if aborted_with_debt and status_special:
        # Если есть договор с долгом, но статус особый = пропускаем
        return
    if aborted_contract and actual_contract:
        # Проверяем, если у орагнизации есть прекращенный договор без долгов + актуальный, все ок
        return
    if banned or aborted_with_debt:
        # Если же мы дошли сюда, и есть причина запрета - выводим ее
        return ban_reason
