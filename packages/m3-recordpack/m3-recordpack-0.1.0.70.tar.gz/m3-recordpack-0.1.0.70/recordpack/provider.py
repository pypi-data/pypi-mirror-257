...
class DjangoModelProvider(BaseRecordProvider):
    ...
    def get_record(self, key, query_object=None):
        ...
        try:
            ...
            record = self.get_record_from_base_query(base_query, key)
            ...
            return self._do_on_multiple_objects_return(key)

        else:
            return record

    def get_record_from_base_query(self, base_query, key):
        """Получает запись по ключу.

        Args:
            base_query: основной кверисет провайдера
            key: значение первичного ключа

        Returns:
            Запись, найденная по ключу

        Raises:
            DoesNotExist: если запись не найдена
            MultipleObjectsReturned: если найдено более одной записи
        """
        return base_query.get((self.key_field, key))

    def _do_on_multiple_objects_return(self, key):
        """Возвращает поведение при обноружении нескольких записей.

        Args:
            key: значение первичного ключа.

        Raises:
            MultipleObjects: множественные данные вместо единичной записи.
        """
