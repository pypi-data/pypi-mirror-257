...
def _get_asset_parent_name(value: ModelVersionRepresentation | DatasetVersionRepresentation):
    return (
        value.model_representation_instance.name
        if isinstance(value, ModelVersionRepresentation)
        else value.dataset_representation_instance.name
    )
...
def assign_asset_version_logging(
    ...
    asset_id = value._last_version.id if is_parent else value.id  # pyright: ignore [reportPrivateUsage]
