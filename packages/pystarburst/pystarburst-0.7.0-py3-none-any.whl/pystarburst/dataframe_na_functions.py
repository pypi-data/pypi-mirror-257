...
from pystarburst._internal.type_utils import (
    ...
    LiteralType,
    python_type_to_pystarburst_type,
)
from pystarburst.functions import iff, lit, when
...
def _is_value_type_matching_for_na_function(value: LiteralType, datatype: DataType) -> bool:
    # Python `int` can match into FloatType/DoubleType,
    # but Python `float` can't match IntegerType/LongType.
    # None should be compatible with any Trino type.
    return (
        value is None
        or (isinstance(value, int) and isinstance(datatype, (IntegerType, LongType, FloatType, DoubleType)))
        or (isinstance(value, float) and isinstance(datatype, (FloatType, DoubleType)))
        or (isinstance(value, datetime) and isinstance(datatype, (TimestampType, TimestampNTZType)))
        or (isinstance(value, time) and isinstance(datatype, (TimeType, TimeNTZType)))
        or isinstance(datatype, type(python_type_to_pystarburst_type(type(value))[0]))
    )
...
class DataFrameNaFunctions:
    ...
    def __init__(self, df: "pystarburst.dataframe.DataFrame") -> None:
        self._df = df
    ...
    def drop(
    ...
    ) -> "pystarburst.dataframe.DataFrame":
        """
        ...
        # drop a row only if all its values are null
            ...
            df_col_type_dict = {quote_name(field.name): field.datatype for field in self._df.schema.fields}
            ...
            for normalized_col_name in normalized_col_name_set:
                if normalized_col_name not in df_col_type_dict:
                    raise PyStarburstClientExceptionMessages.DF_CANNOT_RESOLVE_COLUMN_NAME(normalized_col_name)
                col = self._df.col(normalized_col_name)
                if isinstance(df_col_type_dict[normalized_col_name], (FloatType, DoubleType)):
                    # iff(col = 'NaN' or col is null, 0, 1)
                    is_na = iff((pystarburst.functions.is_nan(col)) | col.is_null(), 0, 1)
                else:
                    # iff(col is null, 0, 1)
    ...
    def fill(
        ...
        if not value_dict:
            new_df = copy.copy(self._df)
            return new_df
        if not all([isinstance(v, VALID_PYTHON_TYPES_FOR_LITERAL_VALUE) for v in value_dict.values()]):
            raise ValueError("All values in value should be in one of " f"{VALID_PYTHON_TYPES_FOR_LITERAL_VALUE} types")
            ...
            if col_name in normalized_value_dict:
                ...
                if _is_value_type_matching_for_na_function(value, datatype):
                    if isinstance(datatype, (FloatType, DoubleType)):
                        ...
                        res_columns.append(iff(pystarburst.functions.is_nan(col) | col.is_null(), value, col).as_(col_name))
                    else:
    ...
    def replace(
        ...
        if isinstance(to_replace, dict):
            ...
            if isinstance(value, (list, tuple)):
                if len(to_replace) != len(value):
                    raise ValueError(
                        "to_replace and value lists should be of the same length." f"Got {len(to_replace)} and {len(value)}"
                    )
                else:
        ...
        if not all(
            [
                isinstance(k, VALID_PYTHON_TYPES_FOR_LITERAL_VALUE) and isinstance(v, VALID_PYTHON_TYPES_FOR_LITERAL_VALUE)
                for k, v in replacement.items()
            ]
        ):
            raise ValueError(
                "All keys and values in value should be in one of " f"{VALID_PYTHON_TYPES_FOR_LITERAL_VALUE} types"
            )
        ...
        if not any(
            all_of_type(replacement.keys()) and all_of_type(x for x in replacement.values() if x is not None)
        ...
        for col_name, datatype in df_col_type_dict.items():
            ...
            if col_name in normalized_col_name_set:
                ...
                for key, value in replacement.items():
                    if _is_value_type_matching_for_na_function(key, datatype) and _is_value_type_matching_for_na_function(
                        value, datatype
                    ):
                        if key is None:
                            cond = col.is_null()
                        elif isinstance(key, Number) and math.isnan(key):
                            cond = col.equal_nan()
                        else:
                            cond = col == lit(key)
                        replace_value = lit(value)
                        case_when = case_when.when(cond, replace_value) if case_when is not None else when(cond, replace_value)
                    else:
                        _logger.warning(
