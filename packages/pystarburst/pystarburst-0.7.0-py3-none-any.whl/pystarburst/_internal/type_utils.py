...
import pystarburst.types  # type: ignore
...
PYTHON_TO_TRINO_TYPE_MAPPINGS = {
    ...
    bytes: BinaryType,
    UUID: UuidType,
    relativedelta: YearMonthIntervalType,
    datetime.timedelta: DayTimeIntervalType,
}


VALID_PYTHON_TYPES_FOR_LITERAL_VALUE = (
    *PYTHON_TO_TRINO_TYPE_MAPPINGS.keys(),
    list,
    tuple,
    dict,
)
VALID_PYSTARBURST_TYPES_FOR_LITERAL_VALUE = (
    *PYTHON_TO_TRINO_TYPE_MAPPINGS.values(),
    _NumericType,
    ArrayType,
    MapType,
    StructType,
...
def infer_type(obj: Any) -> DataType:
    ...
    datatype = PYTHON_TO_TRINO_TYPE_MAPPINGS.get(type(obj))
    if datatype is DecimalType:
        # the precision and scale of `obj` may be different from row to row.
        return DecimalType(38, 18)
    elif datatype is TimestampType:
        if not hasattr(obj, "tzinfo") or not obj.tzinfo:
            return TimestampNTZType()
        return TimestampType()
    elif datatype is TimeType:
        if not hasattr(obj, "tzinfo") or not obj.tzinfo:
            return TimeNTZType()
        return TimeType()
    ...
    if isinstance(obj, tuple):
        ...
        for v in obj:
            if v is not None:
                element_type = infer_type(v)
                array_types.add(element_type)
                if len(array_types) > 1:
                    raise ValueError("All ArrayType elements must be the same type")
        if array_types:
            return ArrayType(array_types.pop())
...
def infer_schema(row: Union[Dict, List, Tuple], names: Optional[List] = None) -> StructType:
...
def merge_type(a: DataType, b: DataType, name: Optional[str] = None) -> DataType:
    ...
    if isinstance(a, StructType):
        ...
        return ArrayType(merge_type(a.element_type, b.element_type, name="element in array %s" % name))
...
def python_type_to_pystarburst_type(tp: Union[str, Type]) -> Tuple[DataType, bool]:
    """Converts a Python type or a Python type string to a pystarburst type.
    Returns a pystarburst type and whether it's nullable.
    """
    ...
    elif tp in PYTHON_TO_TRINO_TYPE_MAPPINGS:
        return PYTHON_TO_TRINO_TYPE_MAPPINGS[tp](), False

    tp_origin = get_origin(tp)
    tp_args = get_args(tp)

    # only typing.Optional[X], i.e., typing.Union[X, None] is accepted
    if tp_origin and tp_origin == Union and tp_args and len(tp_args) == 2 and tp_args[1] == NoneType:
        return python_type_to_pystarburst_type(tp_args[0])[0], True
    ...
    if tp in list_tps or (tp_origin and tp_origin in list_tps):
        element_type = python_type_to_pystarburst_type(tp_args[0])[0] if tp_args else StringType()
        return ArrayType(element_type), False

    # typing.Dict, dict
    dict_tps = [dict, Dict]
    if tp in dict_tps or (tp_origin and tp_origin in dict_tps):
        key_type = python_type_to_pystarburst_type(tp_args[0])[0] if tp_args else StringType()
        value_type = python_type_to_pystarburst_type(tp_args[1])[0] if tp_args else StringType()
...
# Add additional mappings to match Trino db data types
...
DECIMAL_RE = re.compile(r"^\s*decimal\s*\(\s*(\s*)(\d*)\s*,\s*(\d*)\s*\)\s*$")
...
def get_number_precision_scale(type_str: str) -> Optional[Tuple[int, int]]:
    ...
    if decimal_matches:
        return int(decimal_matches.group(2)), int(decimal_matches.group(3))
...
# Type hints
ColumnOrName = NewType("ColumnOrName", Union["pystarburst.column.Column", str])
ColumnOrLiteralStr = NewType("ColumnOrLiteralStr", Union["pystarburst.column.Column", str])
ColumnOrSqlExpr = NewType("ColumnOrSqlExpr", Union["pystarburst.column.Column", str])
LiteralType = NewType("LiteralType", Union[VALID_PYTHON_TYPES_FOR_LITERAL_VALUE])
ColumnOrLiteral = NewType("ColumnOrLiteral", Union["pystarburst.column.Column", LiteralType])
