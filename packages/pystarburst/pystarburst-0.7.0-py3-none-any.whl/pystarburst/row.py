...
class Row(tuple):
    ...
    def __new__(cls, *values: Any, **named_values: Any):
        ...
        # Trino can return duplicate column names, for instance, "select a, a from a_table."
    ...
    def __call__(self, *args, **kwargs):
        ...
        if args and kwargs:
            raise ValueError("A Row instance should have either values or field-value pairs but not both.")
        ...
        if self._named_values:
            if args:
                raise ValueError(
                    "The Row object can't be called with a list of values" "because it already has fields and values."
                )
            new_row = Row(**self._named_values)
            for input_key, input_value in kwargs.items():
                if input_key not in self._named_values:
                    raise ValueError(f"Wrong keyword argument {input_key} for Row f{self}")
                new_row._named_values[input_key] = input_value
            return new_row
        elif self._fields and self._check_if_having_duplicates():
            raise ValueError("The Row object can't be called because it has duplicate fields")
        else:
            if kwargs:
                raise ValueError(
                    "The Row object can't be called with field-value pairs " "because it doesn't have field-value pairs"
                )
            if len(args) != len(self) or any(not isinstance(value, str) for value in self):
                raise ValueError("The called Row object with and input values must have the same size.")
    ...
    def __repr__(self):
        if self._fields:
            return "Row({})".format(", ".join(f"{k}={v!r}" for k, v in zip(self._fields, self)))
        elif self._named_values:
            return "Row({})".format(", ".join(f"{k}={v!r}" for k, v in self._named_values.items()))
        else:
    ...
    def as_dict(self, recursive: bool = False) -> Dict:
        ...
        if not self._named_values:
            raise TypeError("Cannot convert a Row without key values or duplicated keys to a dict.")
        if not recursive:
            return dict(self._named_values)
        return self._convert_dict(self._named_values)

    def _convert_dict(self, obj: Union["Row", Dict, Iterable[Union["Row", Dict]]]) -> Union[Dict, Iterable[Dict]]:
    ...
    def _populate_named_values_from_fields(self) -> None:
        ...
        if self._named_values is None and self._fields and not self._check_if_having_duplicates():
    ...
    def _check_if_having_duplicates(self) -> bool:
        if self._has_duplicates is None:
            ...
            self.__dict__["_has_duplicates"] = bool(len(set(self._fields)) != len(self._fields))
