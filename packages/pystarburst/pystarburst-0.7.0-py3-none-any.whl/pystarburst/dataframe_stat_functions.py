...
from pystarburst._internal.type_utils import ColumnOrName, LiteralType
from pystarburst.functions import (
    _to_col_if_str,
    approx_percentile_accumulate,
    approx_percentile_estimate,
)
from pystarburst.functions import corr as corr_func
from pystarburst.functions import count, count_distinct, covar_samp
...
class DataFrameStatFunctions:
    ...
    def __init__(self, df: "pystarburst.DataFrame") -> None:
        self._df = df
    ...
    def approx_quantile(
        ...
        *,
        ...
        if isinstance(col, (Column, str)):
            ...
            accumate_cols = [approx_percentile_accumulate(col_i).as_(f"{temp_col_name}_{i}") for i, col_i in enumerate(col)]
            output_cols = [
                approx_percentile_estimate(f"{temp_col_name}_{i}", p) for i in range(len(accumate_cols)) for p in percentile
            ]
            percentile_len = len(output_cols) // len(accumate_cols)
            df = self._df.select(accumate_cols).select(output_cols)
            res = df._internal_collect(statement_properties=statement_properties)
            return [[x for x in res[0][j * percentile_len : (j + 1) * percentile_len]] for j in range(len(accumate_cols))]
        else:
            raise TypeError("'col' must be a column name, a column object, or a list of them.")
