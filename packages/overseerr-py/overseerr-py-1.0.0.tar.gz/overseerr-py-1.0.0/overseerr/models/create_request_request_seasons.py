...
class CreateRequestRequestSeasons(BaseModel):
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = CreateRequestRequestSeasons.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in CreateRequestRequestSeasons with oneOf schemas: List[float], str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in CreateRequestRequestSeasons with oneOf schemas: List[float], str. Details: " + ", ".join(error_messages))
        else:
            return v
