    ...
    def run(self) -> None:
        ...
        for i, state in enumerate(self.states[1:]):
            ...
            if planted_crop:
                ...
                state.ET_c = (
                    planted_crop.get_crop_coefficient(state.timestamp)
                    * state.ET_0
                )
                ...
                state.ky = 1  # TO DO - calculate ky with the prop of the crop

    def calc_yield(self) -> List[Tuple[date, float]]:
        """
        Calculate the yield
        """
        ...
        for crop in crops:
            ...
            avg_ky = sum(ky_values) / len(ky_values)
            actual_yield = crop.crop.yield_max * avg_ky
            ls_actual_yields.append(actual_yield)
        return list(zip(harvest_dates, ls_actual_yields))
