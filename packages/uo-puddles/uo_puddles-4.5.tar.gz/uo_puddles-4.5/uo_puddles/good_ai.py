...
def setup_simplified_code_exercise(statement_indent_pairs, distractors=[]):
    # Initialize the info text and interactive elements
    info_text = '''Fill in code for all the statements.<br>
    Select indentation for each statement.<br>
    When finished, copy code into Python cell and run it.<br>
    If you do not get the correct result, return here and fix the bug(s) - easier said than done in some cases!<br>
    Click 'Info' button to toggle visibility.<br>
    '''
    info_display = HTMLWidget(value="")  # Initially hidden
    ...
    def update_display(change):
        ...
        # Generate the code display based on current selections
        code_display_html = "<strong>Current Code Configuration:</strong><br><hr>"
        for i, (order_dropdown, indent_dropdown) in enumerate(zip(order_dropdowns, indentation_dropdowns)):
            if order_dropdown.value != 'Select a statement':
                # Apply blue color only to the selected code
                ...
                if indent_dropdown.value == 'Select indentation level':
                    # Add missing indentation comment without changing the color of the code
                    code_line += " <span style='color:gray;'># Missing indentation</span>"
            else:
                # Handle the case where a statement is not selected
