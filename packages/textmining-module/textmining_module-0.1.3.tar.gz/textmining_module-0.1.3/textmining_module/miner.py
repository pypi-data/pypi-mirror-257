...
class KEM:
    ...
    def fit(self, train_ratio=0.7, n_fold=5, n_round=10, optim_metric='f1_macro', n_jobs=-1, skl_verbose=0, verbose=True):
        """
        ...
        self.__optimizeRF(train_ratio, n_fold, n_round, optim_metric, n_jobs, skl_verbose, verbose)
        if not self.__s:
          return self.model
    ...
    def __fetchTargetAssociation(self):
        ...
        if not self.__s:
            ...
            rules = association_rules(frequent_itemsets, metric='confidence', support_only=False, min_threshold=0.5)
    ...
    def __optimizeRF(self, train_ratio=0.7, n_fold=5, n_round=10, optim_metric='f1_macro', n_jobs=-1, skl_verbose=0, verbose=True):
        """
        ...
        if not self.__s:
          ...
          pipeline = Pipeline([("smoten", SMOTEN(k_neighbors=3, sampling_strategy='not majority')),
