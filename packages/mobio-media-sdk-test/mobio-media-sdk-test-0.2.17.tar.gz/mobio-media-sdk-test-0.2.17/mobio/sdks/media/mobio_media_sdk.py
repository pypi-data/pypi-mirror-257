...
from .config import Cache, ConsumerTopic, MobioEnvironment, StoreCacheType
from .constant import (
    CONSTANT_MAP_FORMAT_FILE,
    DirSaveFile,
    Display,
    FieldConstantResponse,
    ParamMessageSaveInfoFile,
    ParamMessageUploadMediaSdk,
)
...
class MobioMediaSDK(object):
    ...
    def config(
        ...
        if not redis_uri:
            redis_uri = MobioEnvironment.REDIS_BASE_URI
        if not redis_type:
            redis_type = MobioEnvironment.REDIS_BASE_TYPE
        ...
        MobioMediaSDK.lru_cache = LruCache(
            ...
            redis_cluster_uri=redis_uri,
            redis_type=self.redis_uri,
        )
        ...
        if kafka_bootstrap_servers:
            ...
            self.kafka_bootstrap_servers = MobioEnvironment.KAFKA_BROKER
        if not self.kafka_bootstrap_servers:
            raise Exception(
                "[ERROR] conf_kafka_bootstrap_servers not exist by key {}".format(
    ...
    def valid_input_upload(file_path, file_data, file_byte, merchant_id, expire=None):
        ...
        if not file_path and not file_data and not file_byte:
            raise Exception(
                "[ERROR] Need to pass 1 of 3 parameters file_path or file_data or file_byte"
            )
        ...
        if expire:
            try:
                ...
                raise Exception(
                    "[ERROR] format expire: {} error: {}".format(expire, ex)
                )
    ...
    def upload_without_kafka(
        ...
        mimetype_str = MobioMediaSDK.get_mimetype(
            file_path=file_path, file_data=file_data, file_byte=file_byte
        )
        MobioMediaSDK.validate_desired_format(
            desired_format=desired_format, mimetype_str=mimetype_str
        )

        if not filename:
            filename = MobioMediaSDK.get_file_name(
                file_path=file_path, file_data=file_data
            )
        host_by_merchant_id = self._get_host_by_merchant_id(merchant_id=merchant_id)
        logger.debug(
            "upload_without_kafka()::get_host_by_merchant_id:: %s"
        ...
        from .utils import convert_bytes, file_capacity
        ...
        self.send_message_to_topic(
            topic=ConsumerTopic.TOPIC_SAVE_INFO_MEDIA_SDK,
            data=json.dumps(data_send_producer),
        )
        logger.debug(
            "upload_without_kafka()::send message to topic:: %s"
            % (time.time() - time_start)
        )
        return {
    ...
    def upload_with_kafka(
        ...
        host_by_merchant_id = self._get_host_by_merchant_id(merchant_id=merchant_id)
        logger.debug(
            "upload_with_kafka()::get_host_by_merchant_id:: %s"
            % (time.time() - time_start)
        )
        decode_filename = quote(filename)
        url = os.path.join(
        ...
        self.send_message_to_topic(
            topic=ConsumerTopic.TOPIC_UPLOAD_MEDIA_SDK,
            data=json.dumps(data_send_producer),
        )
        logger.debug(
            "upload_with_kafka()::send message to topic:: %s"
            % (time.time() - time_start)
        )
        return {
            FieldConstantResponse.URL: url,
    ...
    def create_public_url_without_file(
        ...
        host_by_merchant_id = self._get_host_by_merchant_id(merchant_id=merchant_id)
        logger.info(
            "create_public_url_without_file :: host_by_merchant_id :: %s "
            % host_by_merchant_id
        )
        decode_filename = quote(filename)
        url = os.path.join(
    ...
    def finish_save_file_by_public_url(
        self, filepath=None, file_data=None, file_byte=None, url=None
    ):
        if not url:
            raise Exception("[ERROR] :: url not None")
        local_path = self.get_path_by_url(url, is_check_path_exists=False)
        logger.info(
            "finish_save_file_by_url :: get_path_by_url % s result :: %s "
            % (url, local_path)
        )
        if filepath:
    ...
    def create_dir_save_file(
        cls, folder, merchant_id, type_media, filename, short_link=False
    ):
    ...
    def validate_desired_format(desired_format, mimetype_str):
        ...
        if (not all_format_desired) or (mimetype_str not in all_format_desired):
            ...
            raise Exception(
                "[ERROR] File định dạng {}, không đúng định dạng mong muốn là {}".format(
                    extension, desired_format
                )
            )

    def get_path_by_url(self, url, is_check_path_exists=True):
        if not url:
    ...
    def get_mimetype(file_data, file_path, file_byte):
        ...
        if file_path and os.path.isfile(file_path):
            ...
            mimetype_str = (
                file_data.mimetype if file_data.mimetype else file_data.content_type
            )
        elif file_byte:
            magic_init = magic.Magic(mime=True)
    ...
    def override_file(
        ...
        logger.debug(
            "override_file()::send message to topic:: %s" % (time.time() - time_start)
        )

    def delete_file(self, merchant_id, urls, type_delete=None):
        if not urls:
