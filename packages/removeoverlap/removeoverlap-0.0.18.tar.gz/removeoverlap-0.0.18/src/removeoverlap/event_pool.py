...
class EventPool:
    ...
    def __init__(self, boxes: dict[str, HRectangle]) -> None:
        ...
        for axis_index in range(self.dimension_count):
            ...
            for box in boxes.values():
                ...
                name = box.edges[axis_index].name
                start_event = Event(box.box_id, start, False,
                                    False, EventType.START,name)
                end_event = Event(box.box_id, end, False, 
                                  False, EventType.END, name)
    ...
    def get_intial_bounding_box(self) -> HRectangle:
        ...
        for axis_events in self.events:
            edges.append(
               Edge(axis_events[0].coordinate,axis_events[-1].coordinate, axis_events[0].name)
            )
