...
#
# This is just a front-end to sphinx-build that can call it multiple times for different language
#
# Will build out to _build/LANG by default
#
...
def main():
    ...
    parser = argparse.ArgumentParser(description='build_docs.py: Build evas docs', prog='build_docs.py')
...
def parallel_call(args, callback):
    num_sphinx_builds = len(languages)
    ...
    for language in languages:
        build_info = {}
        build_info['build_dir'] = os.path.realpath(os.path.join(args.build_dir, language))
    ...
    for ret in errcodes:
        if ret != 0:
            print('\nThe following language failed to build:')
            is_error = True
            break
    if is_error:
        for ret, build_info in zip(errcodes, entries):
            if ret != 0:
                print('language: %s, errcode: %d' % (build_info['language'], ret))
...
def call_build_docs(build_info):
    ...
    for buildername in build_info['builders']:
        ...
        if has_doxyfile(build_info['doxyfile_dir']):
            ret += check_docs(build_info['language'],
    ...
    if 'latex' in build_info['builders']:
        ...
        ret = build_pdf(build_info['language'], latex_dir)

    return ret


def build_pdf(language, latex_dir):
    # Note: because this runs in a multiprocessing Process, everything which happens here should be isolated to a single process
...
GH_LINK_RE = r'https://github.com/ikiwihome/evas-docs/(?:tree|blob|raw)/[^\s]+'

# we allow this one doc, because we always want users to see the latest support policy
GH_LINK_ALLOWED = ['https://github.com/ikiwihome/evas-docs/blob/master/SUPPORT_POLICY.md',
                   'https://github.com/ikiwihome/evas-docs/blob/master/SUPPORT_POLICY_CN.md']
