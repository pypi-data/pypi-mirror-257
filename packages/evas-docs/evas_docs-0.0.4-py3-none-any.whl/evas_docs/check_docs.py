...
def check_docs(language, log_file, known_warnings_file, out_sanitized_log_file):
    """
    Check for Documentation warnings in `log_file`: should only contain (fuzzy) matches to `known_warnings_file`

    It prints all unknown messages with `language` prefix
    ...
    if new_messages:
        print('\n%s: Build failed due to new/different warnings (%s):\n' % (language, log_file))
        for msg in new_messages:
            print('%s: %s' % (language, msg.original_text), end='')
        print('\n%s: (Check files %s and %s for full details.)' % (language, known_warnings_file, log_file))
