...
def makeCOMPUSTATDerivedVariables(params):
    ...
    # uses the .any() method with axis=1 to check which rows have at least one True value (i.e., meet the condition) and
    # then uses the .index attribute to get the index labels of those rows (i.e., permnumbers). Finally, .values method
    # selects the array of values.
    FinFirms = FinFirms.index[FinFirms.any(axis=1)].values
    ...
    start_date = IBQ.index.get_loc(197101.0, method='bfill')
