...
def makeCOMPUSTATVariables(params, data, quarterly_indicator=False):
    ...
    if not params.remote_or_not:
        ...
        def process_and_save_var(col, ret, quarterly_indicator=False):
        ...
        with ThreadPoolExecutor(max_workers=params.num_cpus) as executor:
            ...
            if quarterly_indicator:
                futures = [executor.submit(process_and_save_var, var, ret, quarterly_indicator=True) for var in varNames]
            else:
                futures = [executor.submit(process_and_save_var, var, ret, quarterly_indicator=False) for var in varNames]
