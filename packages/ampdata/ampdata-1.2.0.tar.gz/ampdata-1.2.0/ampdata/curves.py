...
class BaseCurve:
    ...
    def _load_data(self, url, failmsg, urlbase=None):
        ...
        raise util.CurveException(
            "{}: {} ({})".format(failmsg, response.content, response.status_code)
        )
...
class TimeSeriesCurve(BaseCurve):
    def get_data(
        ...
        All time series are returned in a :class:`ampdata.util.TS` object.
...
class TaggedCurve(BaseCurve):
    ...
    def get_data(
        ...
        a list of :class:`ampdata.util.TS` objects.
        ...
        tag: str or list, optional
            ...
            will return a list with a :class:`ampdata.util.TS` object for each tag.
...
class InstanceCurve(BaseCurve):
    def search_instances(
        ...
        as a a list of :class:`ampdata.util.TS` objects.
        It also possible to process
        the curve directly in the API using filter and aggregation functions.
        This can be used with great effect to reduce the amount of data
        retrieved if the full set of details is not needed.
        By default this function returns the :class:`ampdata.util.TS` objects
        ...
        issue_dates: list of time-stamps, optional
            List of timestamps to return :class:`ampdata.util.TS` objects for.
        ...
        with_data: bool, optional
            If with_data is False, the returned  :class:`ampdata.util.TS` object
        ...
        if only_accessible is not None:
            warnings.warn(
                "only_accessible parameter will be removed soon.",
                FutureWarning,
                stacklevel=2,
            )
    ...
    def get_instance(
        ...
        as a :class:`ampdata.util.TS` object. It also possible to process
        the curve directly in the API using filter and aggregation functions.
        This can be used with great effect to reduce the amount of data
        retrieved if the full set of details is not needed.
        ...
        args = [
            util.make_arg("with_data", "{}".format(with_data).lower()),
        ...
        if result is None:
            return result
        return util.TS(
            input_dict=result, issue_date=issue_date, curve_type=util.INSTANCES
        )
    ...
    def get_absolute(
        self, data_date, issue_frequency=None, issue_date_from=None, issue_date_to=None
    ):
...
class TaggedInstanceCurve(BaseCurve):
    ...
    def search_instances(
        ...
        given list of tags, if specified) as a a list of :class:`ampdata.util.TS`
        objects.
    ...
    def get_instance(
        ...
        issue_date and all given tags as a lit of :class:`ampdata.util.TS` objects.
        It also possible to process the curve directly in the API using filter
        and aggregation functions. This can be used with great effect to reduce
        the amount of data retrieved if the full set of details is not needed.
        ...
        res = [
            util.TS(
                input_dict=r, issue_date=issue_date, curve_type=util.TAGGED_INSTANCES
            )
            for r in result
        ]
        if unwrap and len(res) == 1:
    ...
    def get_latest(
        ...
        as :class:`ampdata.util.TS` objects. The tag to get the data for
        ...
        tags: string or list, optional
            ...
            :class:`ampdata.util.TS` object. The function returns a timeseries
