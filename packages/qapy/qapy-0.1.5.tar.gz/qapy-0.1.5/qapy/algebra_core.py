...
class ARef(A):
    ...
    def count(self):
        return 1, 0
    def write_internal_core(self, f):
        f.write(f'{self.id},{self.val}\n')

class AAmount(A):
    def __init__(self, p_id):
        super().__init__(ATypes.AAmountType)
        self.id = p_id
    def count(self):
        return 1, 0
    def write_internal_core(self, f):
...
class CEither(C):
    def __init__(self, p_choice_ownership, p_choice_date, choices):
        super().__init__(CTypes.CEitherType)
        self.choice_ownership = p_choice_ownership
        self.choice_date = p_choice_date
        self.choices = choices
    def count(self):
        na = ne = nc = 0
        for choice in self.choices:
            choice_na, choice_ne, choice_nc = choice.count()
            na += choice_na
            ne += choice_ne
            nc += choice_nc
        return na, ne, nc + 1
    def write_internal_core(self, f):
        f.write(f'{self.choice_ownership},{self.choice_date}')
        if self.choices:
            for choice in self.choices:
                f.write(f',{choice.name}')
        f.write('\n')
        for choice in self.choices:
            choice.write_internal(f)
...
class CIfThenElse(C):
    ...
    def write_internal_core(self, f):
        f.write(f'{self.event.name},{self.then.name},{self.otherwise.name},{self.spread_lhs},{self.spread_rhs}\n')
        self.event.write_internal(f)
        self.then.write_internal(f)
        self.otherwise.write_internal(f)
...
class CEnd(C):
    def __init__(self):
        super().__init__(CTypes.CEndType)
    def count(self):
        return 0, 0, 1
    def write_internal_core(self, f):
        f.write('0\n')
...
class CCashFlow(C):
    def __init__(self, p_file):
        super().__init__(CTypes.CCashFlowType)
        self.file = p_file
    def count(self):
        return 0, 0, 1 
    def write_internal_core(self, f):
