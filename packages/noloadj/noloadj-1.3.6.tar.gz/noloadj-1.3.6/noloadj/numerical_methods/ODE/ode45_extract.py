...
from noloadj.numerical_methods.ODE.ode45 import next_der_step_simulation,\
    next_step_simulation,compute_new_point,interpolation
    ...
    def body_fn(state):
        ...
        if type=='threshold':
            ...
            x_now,t_now,y_now,h_now,i_now,c_now=cond(condition,lambda state:
                ...
                f,tol,T,*P),lambda state:state,(x_now,t_now,y_now,h_now,i_now,
                                                c_now))
            dx_now,dy_now,_,_=cond(condition,lambda state:(x_prev,t_prev,dx_prev,
                x_now,t_now,h_prev,f, nPdP,i_now,Mat,dMat,*P_and_dP),lambda
                state:state,(dx_now,dy_now,Mat,dMat))
