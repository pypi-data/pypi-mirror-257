...
def create_pr(
    ...
    for repo_file_path, patch_msgs in create_comments(applied_patches, path):
        ...
        file_part = f"File changed: [{repo_file_path}]({file_link})<br><br>"
        ...
        for patch_msg in patch_msgs:
            ...
            expandable = f"<details><summary>[{patch_msg.title}]({chunk_link})</summary>{patch_msg.msg}</details>"
            chunk_comment_parts.append(expandable)

        msg = f"* {file_part}" + "".join(chunk_comment_parts)
