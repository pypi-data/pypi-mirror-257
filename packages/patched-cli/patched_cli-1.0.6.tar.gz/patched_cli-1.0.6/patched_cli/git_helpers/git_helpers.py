...
def create_pr(
    ...
    for repo_file_path, patch_msgs in create_comments(applied_patches, path):
        ...
        file_link = f"File changed: [{repo_file_path}]({pr.url()}/files#diff-{diff_anchor})<br><br>"

        chunk_comment_parts = []
        for patch_msg in patch_msgs:
            chunk_link = f"[{patch_msg.title}]({pr.url()}/files#diff-{diff_anchor}L{patch_msg.start_line}-L{patch_msg.end_line})"
            expandable = f"<details><summary>{chunk_link}</summary>{patch_msg.msg}</details>"
            chunk_comment_parts.append(expandable)

        msg = f"* {file_link}" + "".join(chunk_comment_parts)
...
def create_comments(
        applied_patches: List[Patch], path: str
) -> Generator[tuple[str, list[PatchMessage]], None, None]:
    for applied_patch in applied_patches:
