...
class TestSamples(unittest.TestCase):
    def test_samples_all(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            if not filecmp.cmpfiles(test_files, result_dir, ['encode.json', 'decode.json']):
                self.fail()
    ...
    def test_direct_command(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            output = subprocess.check_output(f"insight-plugin samples -d {temp_dir} -v", shell=True).decode('utf-8')
