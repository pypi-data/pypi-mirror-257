...
class ConvertPluginToHelp(BaseLoggingFeature):
    ...
    def build_markdown(self, spec: dict, markdown_obj: MdUtils):
        ...
        # Links
        HelpUtil.make_header(markdown_obj, 1, "Links")
        # Is there anything to add here except references? It would seem we could just do a level 1 header References
    ...
    def add_list(
        ...
        if section is not None and isinstance(section, list):
            for item in section:
                markdown_obj.new_line(ConvertPluginToHelp.bullet_string(str(item)))
        return markdown_obj
