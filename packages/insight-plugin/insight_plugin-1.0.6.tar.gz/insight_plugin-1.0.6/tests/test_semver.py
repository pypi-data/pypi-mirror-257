...
class TestSemver(TestCase):
    ...
    def test_error_handling_for_short_changelog_desc(self):
        version_feature = VersionController.new_from_cli(
            target_dir=f"{TEST_RESOURCES}/semver_tests/base64", version_num="1.1.8", changelog_desc="test"
        )
        with self.assertRaises(Exception) as context:
            version_feature.semver()
        self.assertTrue(
            "Changelog description is too short." in str(context.exception)
        )

    def test_error_handling_for_long_changelog_desc(self):
        version_feature = VersionController.new_from_cli(
            target_dir=f"{TEST_RESOURCES}/semver_tests/base64", version_num="1.1.8", changelog_desc="long string "*50
        )
        with self.assertRaises(Exception) as context:
            version_feature.semver()
        self.assertTrue(
            "Changelog description is too long." in str(context.exception)
        )

    def test_valid_changelog_desc(self):
        target_version = "1.1.8"
        version_feature = VersionController.new_from_cli(
            target_dir=f"{TEST_RESOURCES}/semver_tests/base64", version_num="1.1.8", changelog_desc="This should describe any changes made"
        )
        response = version_feature.semver()
        self.assertEqual(None, response)
        spec = PluginSpecUtil.get_spec_file(self.target_dir)
...
class SemverUtil:
    ...
    def copy_files_cur_dir_target_dir(cur_path, temp_dir):
        os.makedirs(f"{temp_dir}/bin", exist_ok=True)  # special case for bin as it needs the directory created first
        files = ['/bin/icon_base64', '/.CHECKSUM', '/help.md', '/plugin.spec.yaml', '/setup.py']
        for file in files:
