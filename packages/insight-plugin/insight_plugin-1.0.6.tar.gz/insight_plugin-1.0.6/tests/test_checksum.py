...
class TestChecksum(unittest.TestCase):
    ...
    def test_no_checksum_from_file(self):
        with self.assertRaises(InsightException) as context:
            ChecksumUtil._create_sum_from_file(os.path.join(plugin_dir_base_64, "file.txt"))
        self.assertEqual(context.exception.troubleshooting, "Verify that the target is real and is a file.")
