...
class IFYs:
  ...
  def roleify(role_id: int | str):
    ...
    if not is_snowflake(role_id, allow_string=True):
      if isinstance(role_id, int | str):
        err = ValueError('role_id is not a valid snowflake, use tcr.discord.is_snowflake() to validate if needed.')
      else:
        err = TypeError(f'Expected str or int, got {type(role_id)} instead.')
