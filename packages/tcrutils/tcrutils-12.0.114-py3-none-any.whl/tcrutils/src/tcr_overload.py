...
This code was written by [James Murphy](https://github.com/mCodingLLC/) from [MCoding](https://www.youtube.com/@mCoding)

[Original Source Code](https://github.com/mCodingLLC/VideosSampleCode/blob/master/videos/077_metaclasses_in_python/overloading.py#L264)

I still included it in my library because of its usefulness, I do not earn any money on this library and I hope it is not a problem.

I am not yet as experienced with python to write such code so I decided to get the code from the mcoding video. This library is mainly for private use by me so I guessed it'd be fine...
"""
...
class OverloadMeta(type):
  ...
  def __new__(mcls, name, bases, namespace, **kwargs):  # noqa: N804
    overload_namespace = {
      key: _Overload(val) if isinstance(val, OverloadList) else val
