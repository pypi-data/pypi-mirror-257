...
ACTION_CHOICES = (("ADD", "Add"), ("OBSERVE", "Observe"), ("DELETE", "Delete"))
...
class EPCISOutputCriteria(models.Model):
    ...
    sender_identifier = models.CharField(
        ...
        help_text=_(
            "Typically an SGLN but an identifier that is in the SBDH "
    ...
    biz_step = models.CharField(
        ...
        help_text=_(
            "The business step URN.  Can be a CBV value or any custom"
    ...
    biz_location = models.CharField(
        ...
        null=True,
        blank=True,
        help_text=_("The business location URN.  Typically representing " "a site"),
    ...
    source_id = models.CharField(
        ...
        help_text=_(
            "A URI that identifies the source specified in the " "Source Type field."
        ),
        null=True,
        blank=True,
    )
    ...
    destination_id = models.CharField(
        ...
        help_text=_(
            "A URI that identifies the Destination specified in the "
    ...
    end_point = models.ForeignKey(
        ...
        help_text=_(
            "A prtocol-specific endpoint defining where any output"
            " data will be sent."
        ),
    )
    ...
    def clean(self):
        ...
        if self.source_type or self.source_id:
            if not (self.source_type and self.source_id):
                raise ValidationError(
                    _(
                        "If either the Source Type or ID "
        ...
        if self.destination_type or self.destination_id:
            if not (self.destination_type and self.destination_id):
                raise ValidationError(
                    _(
                        "If either the Destination Type or ID "
                        "are specified then both must be."
                    )
                )
...
class EndPoint(models.Model):
    ...
    urn = models.CharField(
        ...
        verbose_name=_("URL"),
        help_text=_(
            "A uniform resource locator that includes " "protocol information."
        ),
        null=False,
    )
...
class AuthenticationInfo(models.Model):
    ...
    type = models.CharField(
        ...
        verbose_name=_("Type"),
        help_text=_(
            "The type of authentication to use.  For example, Basic,"
