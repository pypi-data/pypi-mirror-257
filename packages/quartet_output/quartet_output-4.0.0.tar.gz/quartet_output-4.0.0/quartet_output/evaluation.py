...
class HeaderEvaluation:
    def evaluate_header(
        self,
        header: sbdh.StandardBusinessDocumentHeader,
            ...
            for partner in header.partners:
                if partner.partner_type == sbdh.PartnerType.SENDER.value:
                    found_sender = self._check_string(
                        partner.partner_id.value, epc_output_criteria.sender_identifier
                    )
                    if found_sender:
                        break
            for partner in header.partners:
    ...
    def _check_eval_config(self, criteria: EPCISOutputCriteria):
        ...
        """
        return (
            criteria.sender_identifier is not None
            or criteria.receiver_identifier is not None
        )


class EventEvaluation:
    def evaluate_event(
        self, event: events.EPCISBusinessEvent, epc_output_criteria: EPCISOutputCriteria
    ):
        """
        ...
        if can_evaluate:
            ...
            event_type = self.check_event_type(event, epc_output_criteria.event_type)
            action = self.check_action(event, epc_output_criteria.action)
            biz_location = self.check_biz_location(
                event, epc_output_criteria.biz_location
            )
            disposition = self._check_string(
                event.disposition, epc_output_criteria.disposition
            )
            biz_step = self._check_string(event.biz_step, epc_output_criteria.biz_step)
            read_point = self._check_string(
                event.read_point, epc_output_criteria.read_point
            )
            source = self.check_sources(event, epc_output_criteria)
            destination = self.check_destinations(event, epc_output_criteria)
            return (
                event_type
                and action
                and biz_location
        ...
        if epc_output_criteria.source_type and len(event.source_list) > 0:
            for source in event.source_list:
                ret = self._check_string(
                    source.type, epc_output_criteria.source_type
                ) and self._check_string(source.source, epc_output_criteria.source_id)
                if ret:
                    break
        else:
            ret = True
    ...
    def check_destinations(
        ...
        """
        ret = False
        if epc_output_criteria.destination_type and len(event.destination_list) > 0:
            for destination in event.destination_list:
                ret = self._check_string(
                    destination.type, epc_output_criteria.destination_type
                ) and self._check_string(
                    destination.destination, epc_output_criteria.destination_id
                )
                if ret:
                    break
        else:
    ...
    def _check_eval_config(self, criteria: EPCISOutputCriteria):
        ...
        """
        return (
            criteria.biz_location is not None
            or criteria.action is not None
            or criteria.event_type is not None
            or criteria.biz_step is not None
            or criteria.disposition is not None
            or criteria.source_id is not None
            or criteria.source_type is not None
            or criteria.destination_id is not None
            or criteria.destination_type is not None
            or criteria.read_point is not None
        )
