...
class SftpTransportMixin:
    ...
    def sftp_put(
        ...
        logger.debug(
            "Posting data with urn %s and file_name %s and " "extension %s.",
        ...
        try:
            ...
            sftp_client.connect(
                parsed_urn.hostname, parsed_urn.port, **auth, timeout=60
            )
    ...
    def sftp_get_auth(self, output_criteria):
        ...
        if auth_info:
            ...
            if auth_info.private_key:
                ...
                return {"username": auth_info.username, "password": auth_info.password}
        return None
