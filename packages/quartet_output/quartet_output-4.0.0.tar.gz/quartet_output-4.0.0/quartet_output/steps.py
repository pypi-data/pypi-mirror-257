...
class OutputParsingStep(EPCISParsingStep):
    ...
    *EPCIS Output Criteria* model and any inbound EPCIS events to determine
    ...
    def get_output_criteria(self):
        self.info("Retrieving the Step's EPCIS Output Criteria " "parameter value...")
        output_criteria = self.get_parameter(
            _("EPCIS Output Criteria"), raise_exception=True
        )
        self.info(_("EPCIS Output Critieria is set to %s" % output_criteria))
        try:
            return EPCISOutputCriteria.objects.get(name=output_criteria)
    ...
    def execute(self, data, rule_context: rules.RuleContext):
        ...
        parser.parse()
        self.info(
            _("Parsing complete.  %s matching events were found.")
            % str(len(parser.filtered_events))
        )
        rule_context.context[
            ContextKeys.FILTERED_EVENTS_KEY.value
        ] = parser.filtered_events
    ...
    def instantiate_parser(self, data, parser_type, skip_parsing):
        ...
        try:
            if isinstance(data, File):
                parser = parser_type(
                    data, self.epc_output_criteria, skip_parsing=skip_parsing
                )
            elif isinstance(data, str):
                parser = parser_type(
                    io.BytesIO(str.encode(data)),
                    self.epc_output_criteria,
    ...
    def get_parser_type(self, skip_parsing):
        ...
        """
        return SimpleOutputParser if self.loose_enforcement else BusinessOutputParser
...
class FilteredEventStepMixin:
    ...
    def get_parent_epc(self, epcis_event: events.EPCISEvent, default: str = None):
        """
    ...
    def execute(self, data, rule_context: rules.RuleContext):
        ...
        for epcis_event in epcis_events:
            if isinstance(epcis_event, events.AggregationEvent) or isinstance(
                ...
                self.warning(
                    _(
                        "Skipping a transformation or aggregation event. "
        ...
        rule_context.context[ContextKeys.AGGREGATION_EVENTS_KEY.value] = agg_events
...
class AddCommissioningDataStep(rules.Step, FilteredEventStepMixin):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        self.rule_context = rule_context
        self.info("%s filtered events have been found. Processing", len(epcis_events))
        all_events = set({})
        for epcis_event in epcis_events:
            all_events = all_events.union(self.process_event(epcis_event, rule_context))

        rule_context.context[ContextKeys.OBJECT_EVENTS_KEY.value] = list(all_events)
        self.info("Processing complete.")

    def process_event(
        self, epcis_event: events.EPCISBusinessEvent, rule_context: RuleContext
    ):
        """
        ...
        tops = self.db_proxy.get_top_entries(epcs, select_for_update=False)
        ...
        # the defined tops
        all_children = self.db_proxy.get_entries_by_tops(tops, select_for_update=False)
        # now get any entries from any of the other epcs that are parent-level
        # if any
        parents = self.db_proxy.get_parent_entries(epcs, select_for_update=False)
        ...
        all_events = self.db_proxy.get_events_by_entry_list(
            all_children | parents | tops, event_type=EventTypeChoicesEnum.OBJECT.value
        )
        ...
        self.info("Adding %s Object events to the rule context.", len(all_events))
        return all_events
    ...
    def handle_parent_entries(self, parent_entries: EntryList):
        ...
        all_children = self.db_proxy.get_entries_by_parents(
            parent_entries, select_for_update=False
        )
        for child in all_children:
            if child.is_parent:
                all_children = all_children | self.handle_parent_entries([child])
        return all_children
...
class ForwardDataOutputStep(rules.Step, FilteredEventStepMixin):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        if filtered_events:
            self.info("Found some filtered events...forwarding the " "rule data.")
            rule_context.context[ContextKeys.OUTBOUND_EPCIS_MESSAGE_KEY.value] = data
        return data
...
class EPCPyYesOutputStep(rules.Step, FilteredEventStepMixin):
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        only_filtered = self.get_boolean_parameter("Only Render Filtered Events", False)
    ...
    def declared_parameters(self):
        return {
            "Append Filtered Events": _(
                "Whether or not to append any events "
                "found in the rule context under the "
                "FILTERED_EVENTS_KEY to the new "
                "message."
            ),
            "Prepend Filtered Events": _(
                "Whether or not to add the filtered "
            ...
            "JSON": _(
                "If set to True then the output message for the EPCPyYEs"
                "events will be JSON."
            ),
        }
...
class CreateOutputTaskStep(rules.Step):
    ...
    def __init__(self, db_task: models.Task, **kwargs):
        ...
        try:
            ...
            self.run_immediately = self.get_or_create_parameter(
                "Run Immediately",
                "False",
                "Whether or not to execute output " "tasks immediately.",
            ).lower() in ("true",)
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        if not forward_data:
            ...
            self.info(
                _(
                    "Checking the rule context for any data under the "
        ...
        if data:
            self.info(
                _(
                    "Data or a Forward Data parameter "
            ...
            output_rule_name = self.get_parameter("Output Rule", raise_exception=True)
            self.info(
                _(
                    "Looking for the EPCISOutputCriteria placed on the "
            ...
            task_param = models.TaskParameter(
                ...
                description=_(
                    "The name of the EPCIS Output Criteria to "
            ...
            if forward_data:
                self.info(
                    _(
                        "The Forward Data parameter was specified. The "
            ...
            self.info(
                "Created a new output task %s with rule %s", task.name, output_rule_name
            )

    def declared_parameters(self):
        return {
            "Output Rule": _(
                "The name of the rule that will process the "
                "EPCIS output data created by this step."
            ),
            "Forward Data": _(
                "Boolean.  Whether or not to ignore data in the "
...
class TransportStep(
    rules.Step, HttpTransportMixin, SftpTransportMixin, MailMixin, SocketTransportMixin
):
    """
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        try:
            self.info(
                _("Looking for the task parameter with the EPCIS " "Output Name.")
            )
            param = models.TaskParameter.objects.get(
                task__name=rule_context.task_name, name="EPCIS Output Criteria"
            )
            # now see if we can get the output critieria based on the param
            # value
            self.info(
                _(
                    "Found the output param, now looking up the "
                    "EPCIS Output Criteria instance with name %s."
                ),
                param.value,
            )
            output_criteria = EPCISOutputCriteria.objects.get(name=param.value)
            self.info(_("Found output criteria with name %s."), output_criteria)
            # check the url/urn to see if we support the protocol
            protocol = self._supports_protocol(output_criteria.end_point)
            self.info(
                "Protocol supported.  Sending message to %s."
            ...
            raise capture_errors.ExpectedTaskParameterError(
                _(
                    "The task parameter with name EPCIS Output Criteria "
    ...
    def _send_message(
        ...
        if protocol.lower() in ["http", "https"]:
            ...
            self.sftp_put(
                data, rule_context, output_criteria, content_type, file_extension
            )
            ...
            self.socket_send(data, rule_context, output_criteria, self.info)
    ...
    def _supports_protocol(self, endpoint: EndPoint):
        ...
        None.
        """
        parse_result = urlparse(endpoint.urn)
        if parse_result.scheme.lower() in ["http", "https", "sftp", "mailto", "socket"]:
            return parse_result.scheme
        else:
            raise errors.ProtocolNotSupportedError(
                _(
...
class DelayStep(rules.Step):
    ...
    def execute(self, data, rule_context: RuleContext):
        self.info(
            "Checking for the Timeout Interval step parameter. "
        ...
        self.info("Sleeping the thread for %s seconds..." % timeout_interval)
    ...
    def declared_parameters(self):
        return {
            "Timeout Interval": "The amount of time in seconds to pause the " "rule."
        }
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        if len(filtered_events) >= 0:
            ...
            if self.get_boolean_parameter("JSON", False):
                ...
                data = epcis_document.render()
            self.info(
                "Warning: this step is overwriting the Outbound "
...
class AppendCommissioningStep(rules.Step, FilteredEventStepMixin, FilterEPCsMixin):
    """
    ...
    def execute(self, data, rule_context: RuleContext):
        ...
        for filtered_event in rule_context.context.get(
            ...
            if isinstance(filtered_event, template_events.AggregationEvent):
                ...
                raise self.TransformationEventNotSupported(
                    "Auto commissioning for transformation events is not " "supported."
                )
            else:
        ...
        rule_context.context[ContextKeys.OBJECT_EVENTS_KEY.value] = obj_events

    def get_object_events(self, epcs):
        db_proxy = EPCISDBProxy()
        obj_events = db_proxy.get_object_events_by_epcs(epcs, select_for_update=False)
        return obj_events
