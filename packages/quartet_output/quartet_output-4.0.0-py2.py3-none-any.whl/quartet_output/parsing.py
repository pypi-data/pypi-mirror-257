...
class SimpleOutputParser(QuartetParser):
    ...
    def evaluate(self, epcis_event):
        if self.event_evaluation.evaluate_event(
            epcis_event, self.epcis_output_criteria
        ):
...
class BusinessOutputParser(BusinessEPCISParser):
    ...
    def evaluate_header(self, header: sbdh.StandardBusinessDocumentHeader):
        if self.header_evaluation.evaluate_header(header, self.epcis_output_criteria):
...
class JSONParser(BusinessOutputParser):
    def parse(self):
        ...
        if len(events) == 0:
            raise self.NoEventsError(
                "There were no events in the inbound" " JSON file."
            )
        for event in events:
            if "objectEvent" in event:
                ...
                raise self.InvalidEventError(
                    "The JSON parser encountered an"
