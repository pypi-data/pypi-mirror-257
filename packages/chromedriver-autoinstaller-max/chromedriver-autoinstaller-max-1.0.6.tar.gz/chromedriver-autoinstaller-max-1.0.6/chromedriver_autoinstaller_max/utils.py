...
__author__ = "Max Yao <weng.32002@gmail.com>"
...
def get_chromedriver_url(chromedriver_version, download_options, no_ssl=False):
    ...
    if chromedriver_version >= "115" or chromedriver_version is None:  # new CfT ChromeDriver versions have their URLs published, so we already have a list of options
        ...
        url = base_url + chromedriver_version + "/chromedriver_" + platform + architecture + ".zip"
    return url
...
def get_last_known_good_versions(no_ssl=False):
    ...
    last_known_good_versions = None
    try:
        last_known_good_versions = json.loads(requests.get(version_url).text)
    except Exception as exc:
        print(exc)
        pass
    if not last_known_good_versions is None:
        if 'channels' in last_known_good_versions:
            if 'Stable' in last_known_good_versions['channels']:
                if 'downloads' in last_known_good_versions['channels']['Stable']:
                    if 'chromedriver' in last_known_good_versions['channels']['Stable']['downloads']:
                        download_options = last_known_good_versions['channels']['Stable']['downloads']['chromedriver']
    else:
        print("not able to get last know good versions")
...
def get_matched_chromedriver_version(chrome_version, no_ssl=False):
    ...
    if chrome_version >= "115":
        ...
        #version_url = "googlechromelabs.github.io/chrome-for-testing/known-good-versions-with-downloads.json"
        version_url = "googlechromelabs.github.io/chrome-for-testing/latest-versions-per-milestone-with-downloads.json"
        version_url = f"http://{version_url}" if no_ssl else f"https://{version_url}"
        latest_version_per_milestone_list = None
        try:
            latest_version_per_milestone_list = json.loads(requests.get(version_url).text)
        except Exception as exc:
            print(exc)
            pass
        # drill through object to make sure each key exists, to determin in driver download is available
        if not latest_version_per_milestone_list is None:
            ...
            print("get latest version per milestone list fail...")
    ...
    # check old versions of chrome using the old system
        ...
        root = None
        try:
            root = elemTree.fromstring(requests.get(version_url).text)
        except Exception as exc:
            pass
        if not root is None:
            for k in root.iter("{http://doc.s3.amazonaws.com/2006-03-01}Key"):
                if k.text.find(get_major_version(chrome_version) + ".") == 0:
                    browser_major_version = k.text.split("/")[0]
        else:
            print("not able to get tree element")
...
def get_chromedriver_dir(path, major_version, make_version_dir: bool = True):
    chromedriver_dir = ""
    if path:
...
def download_chromedriver(path: Optional[AnyStr] = None, no_ssl: bool = False, make_version_dir: bool = True, detect_installed_version: bool = True):
    """
    ...
    if chrome_version is None:
        ...
        chromedriver_dir = get_chromedriver_dir(path, major_version, make_version_dir)
    
    chromedriver_filename = get_chromedriver_filename()
    chromedriver_filepath = os.path.join(chromedriver_dir, chromedriver_filename)
    if not os.path.isfile(chromedriver_filepath) or not check_version(chromedriver_filepath, chromedriver_version):
        ...
        retry_idx_max = 5
        response_ok = False
        for retry_idx in range(retry_idx_max):
            #print("download webdriver try No:", retry_idx+1)
            if retry_idx>0:
                url = url.replace("https://","http://")
            print("download webdriver from url:", url)
            runtime_error_message = ""
            try:
                response = requests.get(url, verify=False)
                if not response is None:
                    if response.status_code != 200:
                        runtime_error_message = "URL Status Not OK: {url}"
                    else:
                        response_ok = True
            except Exception as exc:
                response = None
                runtime_error_message = "Failed to download chromedriver archive: {url}"
