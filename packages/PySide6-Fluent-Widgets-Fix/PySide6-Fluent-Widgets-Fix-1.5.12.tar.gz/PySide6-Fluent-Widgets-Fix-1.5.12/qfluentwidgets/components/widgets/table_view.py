...
from ...common.enum import QtEnum
...
class TableItemDelegate(QStyledItemDelegate):
    ...
    def initStyleOption(self, option: QStyleOptionViewItem, index: QModelIndex):
        ...
        # font
        option.font = index.data(QtEnum.Qt.FontRole) or getFont(13)

        # text color
        textColor = QtEnum.Qt.white if isDarkTheme() else QtEnum.Qt.black
        textBrush = index.data(QtEnum.Qt.ForegroundRole)   # type: QBrush
        if textBrush is not None:
            textColor = textBrush.color()

        option.palette.setColor(QtEnum.QPalette.Text, textColor)
        option.palette.setColor(QtEnum.QPalette.HighlightedText, textColor)

    def paint(self, painter, option, index):
        painter.save()
        painter.setPen(QtEnum.Qt.NoPen)
        painter.setRenderHint(QtEnum.QPainter.Antialiasing)
        ...
        if index.data(QtEnum.Qt.CheckStateRole) is not None:
