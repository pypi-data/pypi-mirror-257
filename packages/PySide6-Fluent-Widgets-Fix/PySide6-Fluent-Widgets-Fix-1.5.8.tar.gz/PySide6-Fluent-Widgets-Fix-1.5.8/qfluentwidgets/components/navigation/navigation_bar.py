...
class NavigationBarPushButton(NavigationPushButton):
    ...
    def paintEvent(self, e):
        ...
        painter.setRenderHints(QtEnum.QPainter.Antialiasing |
                               QtEnum.QPainter.TextAntialiasing | QtEnum.QPainter.SmoothPixmapTransform)
        painter.setPen(QtEnum.Qt.NoPen)

        # draw background
        if self.isSelected:
            painter.setBrush(QColor(255, 255, 255, 42) if isDarkTheme() else QtEnum.Qt.white)
        ...
        if self.isSelected:
            ...
            painter.setPen(QtEnum.Qt.white if isDarkTheme() else Qt.black)

        painter.setFont(self.font())
        rect = QRect(0, 32, self.width(), 26)
        painter.drawText(rect, QtEnum.Qt.AlignCenter, self.text())
        painter.end()
