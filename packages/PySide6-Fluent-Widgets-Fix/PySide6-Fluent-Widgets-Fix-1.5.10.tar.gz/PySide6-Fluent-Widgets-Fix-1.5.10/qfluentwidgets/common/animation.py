...
class AnimationBase(QObject):
    ...
    def eventFilter(self, obj, e: QEvent):
        if obj is self.parent():
            if e.type() == QtEnum.QEvent.MouseButtonPress:
                self._onPress(e)
            elif e.type() == QtEnum.QEvent.MouseButtonRelease:
                self._onRelease(e)
            elif e.type() == QtEnum.QEvent.Enter:
                self._onHover(e)
            elif e.type() == QtEnum.QEvent.Leave:
                self._onLeave(e)
...
class BackgroundAnimationWidget:
    ...
    def eventFilter(self, obj, e):
        if obj is self:
            if e.type() == QtEnum.QEvent.Type.EnabledChange:
                if self.isEnabled():
...
class DropShadowAnimation(QPropertyAnimation):
    ...
    def eventFilter(self, obj, e):
        if obj is self.parent() and self.parent().isEnabled():
            if e.type() in [QtEnum.QEvent.Type.Enter]:
                self.isHover = True
                ...
                self.start()
            elif e.type() in [QtEnum.QEvent.Type.Leave, QtEnum.QEvent.Type.MouseButtonPress]:
                self.isHover = False
