...
def update_secrets_baseline(payload, repo, branch, branch_name):
    ...
    if not handle_operation(lambda: audit_secrets(repo_path)):
        return
...
def update_secrets_and_repo(repo_path, repo, repositories_path, branch, branch_name):
    secrets_baseline_path = delete_secrets_baseline(repositories_path, repo)
    update_secrets_scan(repo_path)

    # Make sure the .secrets.baseline file exists
    if not os.path.isfile(secrets_baseline_path):
        print(f"Failed to create .secrets.baseline for {repo.name}")
