...
def get_words_txt(language: str) -> Generator[str, None, None]:
    ...
    """
    words = files("pywordle.words").joinpath(f"words.{language}.txt")

    if words.is_file():
        for word in words.read_text().split("\n"):
            yield word
    else:
        print(f"File words.{language}.txt not found.")
...
def saveplay(gamefilename: str, game: Game) -> None:
    ...
    """
    print("Please enter the word played ('Enter' to quit): ")
    word = ""
    try:
        word = input()
    except KeyboardInterrupt:
