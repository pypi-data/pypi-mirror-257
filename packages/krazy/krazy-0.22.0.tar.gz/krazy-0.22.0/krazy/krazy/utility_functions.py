...
def df_retainCols(dfdata, collist): #
    '''
    deletes columns in df which are not there in list of columns
    @params
    dfdata  -   required    :   dataframe
    collist -   required    :   list of columns to be retained
    '''
...
def df_delCcols(dfdata, collist):
    '''
    deletes columns in df which are there in list of columns
    @params
    dfdata  -   required    :   dataframe
    collist -   required    :   list of columns to be deleted
    '''
...
def df_renameCols(dfdata, coldict):
    '''
    renames columns based on dictionery of old vs new column names
    '''
...
def list_sort(listToSort, direction): #sorts list (not dataframe)
    ...
    @params
    listToSort  -   required    :   list to be sorted
    direction   -   optional    :   anything non blank to sort in reverse order
    '''
    if direction == '':
        listToSort.sort()
    else:
...
def df_sort(dfdata,colList):
    '''
    Sorts dataframe on column index (if provided) else on column value
    '''
    if not colList:
        dfdata.sort_index(ascending=True,inplace=True) #sort on index column
    else:
        dfdata.sort_values(by=colList,ascending=True,inplace=True) #sort on column value
...
def df_multi_excel_import(folderpath, shtname, srch, excl):
    ...
    @params
    folderpath  -   required    :   path to folder from where files are to be imported
    shtname     -   optional    :   name of sheet in excel file from where data is to be imported
    srch        -   required    :   string to be included in file name search
    excl        -   optional    :   string to be excluded in file name search
    '''
...
def df_multi_csv_importer(fpath, srch, excl, types):
    '''
    @params:
    fpath  -   directory path  :   from where files are to be imported
    srch   -   mandatory       :   string to be included in file name search
    excl   -   optional        :   string to be excluded in file name search
    types  -   optional        :   dictionery of column names and their data types
    '''
