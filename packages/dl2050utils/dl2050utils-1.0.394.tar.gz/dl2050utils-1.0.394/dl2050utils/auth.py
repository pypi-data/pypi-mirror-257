"""
    ...
    TODO: Response for Auth errors like WRONG_PASSWD or SUSPENDED_FAILS is 200 with STATUS OK, should be 401,403.
          Need to find a way to return an error message with log_and_raise_rest
...
class Auth():
    ...
    async def check_auth(self, request):
        ...
        user = await self.db.select_one('users', {'id':uid})
        if user is None:
            log_and_raise_service(self.LOG, label='AUTH', label2='check_auth', msg='User not found', status_code=401)
        if user['email']!=payload['email']:
            log_and_raise_service(self.LOG, label='AUTH', label2='check_auth', msg='User email mistatch', status_code=401)
        if user['ustatus']!=ACTIVE:
            log_and_raise_service(self.LOG, label='AUTH', label2='check_auth', msg='User not active', status_code=403)
