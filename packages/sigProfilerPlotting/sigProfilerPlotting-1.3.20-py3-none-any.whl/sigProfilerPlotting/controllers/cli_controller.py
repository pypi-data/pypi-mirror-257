...
def parse_arguments_sbs(args: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate SBS plots.")
    common_plotting_arguments(parser)
    return parser.parse_args(args)


def parse_arguments_id(args: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate ID plots.")
    common_plotting_arguments(parser)
    return parser.parse_args(args)


def parse_arguments_dbs(args: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate DBS plots.")
    common_plotting_arguments(parser)
    return parser.parse_args(args)


def parse_arguments_sv(args: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate SV plots.")
    common_plotting_arguments(
        parser
    )  # SV and CNV have fewer parameters, so we adjust accordingly
    parser.add_argument(
...
def parse_arguments_cnv(args: List[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate CNV plots.")
    common_plotting_arguments(parser)
    parser.add_argument(
        "--aggregate", action="store_true", help="Aggregate data for the plot."
    )
...
# Define the functions to dispatch based on the parsed arguments
# These functions will call the appropriate plotting function with the arguments
