...
class TiingoHistoricFeed(HistoricFeed):
    """Support the following historical market data:
     ...
     - EOD historical US stock prices
    ...
    def retrieve(self, *symbols, start_date="2010-01-01", end_date: str | None=None, closing_time="21:00:00+00:00"):
        """Retrieve stock EOD historic data for the provided symbols"""
    ...
    def retrieve_intraday_iex(self, *symbols, start_date="2023-01-01", end_date: str | None=None, frequency="5min"):
        end_date = date.today().strftime("%Y-%m-%d") if end_date is None else end_date
        columns = "date,open,high,low,close,volume"
        query = f"startDate={start_date}&endDate={end_date}&format=csv&resampleFreq={frequency}&token={self.key}&columns={columns}"
        for symbol in symbols:
    ...
    def retrieve_intraday_crypto(self, *symbols, start_date="2023-01-01", end_date: str | None=None, frequency="5min"):
        end_date = date.today().strftime("%Y-%m-%d") if end_date is None else end_date
        symbols_str = ",".join(symbols)
        query = f"symbols={symbols_str}&startDate={start_date}&endDate={end_date}&resampleFreq={frequency}&token={self.key}"
    ...
    def retrieve_intraday_fx(self, *symbols, start_date="2023-01-01", end_date: str | None=None, frequency="5min"):
        end_date = date.today().strftime("%Y-%m-%d") if end_date is None else end_date
        symbols_str = ",".join(symbols)
