...
class MultiStrategy(Strategy):
    """Combine one or more strategied. The MultiStrategy provides additional control on how to handle conflicting ratings for the same symbols:
    ...
    def __init__(self, *strategies: Strategy, policy: Literal["avg", "last", "first"] = "last", weights: list[float] | None = None):
    ...
    def give_ratings(self, event: Event) -> dict[str, float]:
        ...
        match self.policy:
            case "last":
                for ratings in all_ratings: result.update(ratings) 
            case "first":
                for ratings in reversed(all_ratings): result.update(ratings) 
            case "avg":
