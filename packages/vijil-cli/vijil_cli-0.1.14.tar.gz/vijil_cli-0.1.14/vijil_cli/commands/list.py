...
from vijil_cli.utils import format_datetime
from vijil_cli.vijilapi.api_handler import get_model_token_request, list_job_request
        ...
        if len(result) > 0:
            ...
            for job in result:
                table_data.append([
                    ...
                    job.get('model_type', ''),
                    job.get('model_name', ''),
                    "\n".join(job.get('probe_group', '').split(",")),
                    job.get('status', ''),
                    job.get('job_result', ''),
            ...
            headers = ["ID", "Model Hub", "Model Name", "Probe Group", "Status", "Output", "Start Time", "End Time"]
            maxcolwidths = [None, 10, 25, None, None, 30, None, None]
