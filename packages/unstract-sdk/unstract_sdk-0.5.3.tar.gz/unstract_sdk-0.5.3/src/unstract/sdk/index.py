...
default_db_name = "unstract_vector_db"
...
class ToolIndex:
    ...
    def index_file(
        ...
        elif "image/" in input_file_type_mime:
            # ADDED: 9th Feb 2024
            self.tool.stream_log(
                "Image file as input. Will extracting using OCR."
            )
            x2txt = ToolLLMWhisperer(tool=self.tool)
            full_text.append(
                {
        ...
        vector_db_li = vdb_helper.get_vector_db(
            adapter_instance_id=vector_db,
            embedding_type=embedding_suffix,
            collection_name_prefix=default_db_name,
        )
        if vector_db_li is None:
            self.tool.stream_log(
                f"Error loading {vector_db}", level=LogLevel.ERROR
