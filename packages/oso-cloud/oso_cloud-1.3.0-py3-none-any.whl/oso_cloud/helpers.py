...
def to_api_variable_value(instance: types.VariableValue) -> api.VariableValue:
    ...
    if isinstance(instance, str):
        ...
        return api.VariableValue("String", str(instance))
    ...
    return api.VariableValue(instance["type"], str(instance["id"]))
...
def to_api_value(instance: types.Value) -> api.ConcreteValue:
    ...
    if isinstance(instance, str):
        ...
        return api.ConcreteValue("String", str(instance))
    if "id" not in instance or instance["id"] is None:
    ...
    return api.ConcreteValue(instance["type"], str(instance["id"]))
...
def from_api_concrete_value(value: api.ConcreteValue) -> types.OutputValue:
...
def to_api_facts(params: Optional[List[types.Fact]]) -> List[api.ConcreteFact]:
    if not params:
        return []
...
def from_api_facts(facts: Optional[List[api.ConcreteFact]]) -> List[types.OutputFact]:
    if not facts:
        return []
