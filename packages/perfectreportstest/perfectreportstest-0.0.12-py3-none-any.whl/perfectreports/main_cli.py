...
def customReporter(client_logo, tags_to_remove, automation_owners, reportTag, loadCsv, analytics_html, analytics, internal):
    truncated = True
    page = 1
    i = 0
    daysOlder = 0
    ...
    if startDate != "":
        ...
        for name in trend_df['name'].unique():
            trends = (trend_df[trend_df["name"] == name]['Status'].value_counts(
        ...
        # TODO: what to do with these data, add hyperlinks, and then display it?
        if (len(latest) > 0):
            print(latest[['Custom Failure Reasons', 'message']])
        # latest[latest['Custom Failure Reasons'] == 'Application Issue'] # -> Raise POTENTIAL BUG
...
def main():
    ...
    try:
        ...
        customReporter(client_logo, tags_to_remove, automation_owners,
                       reportTag, loadCsv, analytics_html, analytics, internal)
