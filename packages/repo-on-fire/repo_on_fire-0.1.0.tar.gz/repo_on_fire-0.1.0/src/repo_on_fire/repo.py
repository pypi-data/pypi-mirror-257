...
class Repo:
    ...
    def get_workspace_cache_entry(self, url: str, manifest: Optional[str] = None) -> Path:
        ...
        """
        cache_id = self._calc_hash_id(url, manifest=manifest)
    ...
    def call(self, args: List[str], cwd: Optional[Path] = None):
        """Call the repo tool.
        ...
        if cwd is not None:
            cwd_str = str(cwd)
        return call([sys.executable, str(repo_path), *args], cwd=cwd_str)

    def create_or_update_cache_entry(self, url: str, manifest: Optional[str] = None):
        ...
        """
        self._ensure_repo()
        mirror_path = self.get_workspace_cache_entry(url, manifest=manifest)
        with lock_path(mirror_path):
            ...
            if not stamp_file.exists():
                ...
                if manifest is not None:
                    args += ["-m", manifest]
                args += ["--mirror"]
                exit_code = self.call(args, cwd=mirror_path)
                if exit_code != 0:
    ...
    def init_from_cache_entry(
        ...
        if (
            ...
            command += ["--dissociate"]
        print(command)
        exit_code = self.call(command, cwd=workspace_path)
        if exit_code != 0:
            raise RepoOnFireException(
    ...
    @staticmethod
    def _calc_hash_id(url: str, manifest: Optional[str]) -> Tuple[str, str]:
        hash_str = f"{url}-{manifest}"
