...
def is_supported_agg_funcs(funcs):
    ...
    def is_supported(f):
        return isinstance(f, str) and f not in funcs_backends_not_supported
...
class FireDucksGroupBy:
    ...
    def aggregate(self, func=None, *args, **kwargs):
        ...
        if not reason:
            if not is_supported_agg_funcs(aggArgs.funcs):
                reason = f"agg function is not supported {aggArgs.funcs}"
        ...
        if reason:
            # fireducks: GT #807
            if func is DataFrame.sum:
                func = pandas.DataFrame.sum
            return self._fallback_call(
                "agg",
                func,
                *utils._unwrap(args),
        ...
        return self._aggregate(func, self._by, aggArgs)
...
class SeriesGroupBy(FireDucksGroupBy):
    ...
    def _aggregate(self, func, by, aggArgs):
        by = make_groupkey_ops(by)
    ...
    def _head_or_tail(self, is_head, n=5):
        ...
        selection = make_selection_ops(self._selection)
        value = ir.groupby_select_head_or_tail(
            self._obj._value, by, selection, n, is_head
        )
        return Series._create(value)
...
class DataFrameGroupBy(FireDucksGroupBy):
    ...
    def _head_or_tail(self, is_head, n=5):
        ...
        by = make_groupkey_ops(self._by)
        if self._selection is None:
            value = ir.groupby_head_or_tail(self._obj._value, by, n, is_head)
        else:
            selection = make_selection_ops(self._selection)
            value = ir.groupby_select_head_or_tail(
                self._obj._value, by, selection, n, is_head
