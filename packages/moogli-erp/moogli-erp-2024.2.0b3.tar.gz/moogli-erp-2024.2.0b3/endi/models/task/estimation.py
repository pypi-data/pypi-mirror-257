...
from endi.compute.task.common import EstimationCompute
...
class Estimation(Task):
    ...
    def set_signed_status(self, status, request, **kw):
        ...
        return self.signed_state_manager.process(status, self, request, **kw)
    ...
    def gen_deposit_invoice(self, request, user):
        """
        ...
        :rtype: `class:Invoice`
        """
        return self._invoicing_service.gen_deposit_invoice(request, self, user)

    def gen_invoice(self, request, payment_line, user):
        """
        ...
        if payment_line == self.payment_lines[-1]:
            ...
            return self._invoicing_service.gen_sold_invoice(request, self, user)
        else:
            return self._invoicing_service.gen_intermediate_invoice(
                request, self, payment_line, user
            )
    ...
    def update_payment_lines(self, request, payment_times=None):
        self._endi_service.update_payment_lines(
