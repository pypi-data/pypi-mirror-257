...
from endi.interfaces import ITaskPdfRenderingService
...
from endi.models.task import Task, TaskLine, Estimation, Invoice
...
from endi.forms.tasks.invoice import SetProductsSchema
...
from endi.views.files.views import BaseZipFileView
from endi.views.indicators.routes import INDICATOR_NODE_COLLECTION_API_ROUTE
from endi.views.project.files import ProjectFilesView
...
from endi.views.third_party.customer.routes import CUSTOMER_ITEM_ROUTE
from endi.views.price_study.utils import get_price_study_api_urls
from endi.views.progress_invoicing.utils import get_progress_invoicing_plan_url
...
class TaskSetDraftView(BaseView):
    ...
    def __call__(self):
        self.context.set_status("draft", self.request)
...
class TaskGeneralView(BaseView, BaseTaskHtmlTreeMixin, TaskJsAppViewMixin):
    ...
    def __call__(self):
        ...
        if self.request.has_permission("edit.%s" % get_task_view_type(self.context)):
            return HTTPFound(get_task_url(self.request))
