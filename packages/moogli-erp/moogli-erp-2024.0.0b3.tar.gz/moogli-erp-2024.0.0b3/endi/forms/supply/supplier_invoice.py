...
def same_supplier_supplier_order_validator(value):
    ...
    if query.count() > 1:
        return (
            "Toutes les commandes sélectionnées doivent avoir le même " + "fournisseur."
        )
    else:
        return True


def supplier_consistency_validator(value: dict, supplier_invoice: SupplierInvoice):
    """
    ...
    if len(supplier_orders) > 0 and supplier_id is not None:
        query = (
            DBSESSION()
            .query(SupplierOrder.supplier_id)
            .filter_by(
