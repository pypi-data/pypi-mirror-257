"""
    Handle static libraries inside de MooGLi with the help of fanstatic
"""
...
from js.select2 import select2, select2_full, select2_css, select2_fr
...
def get_resource(
    ...
    """
    Return a resource object included in de MooGLi
    """
    library = lib_endi
    return Resource(
        library,
...
def get_opa_group():
    ...
    """
    return Group(
        [main_css, opa_css, opa_vendor_js, base_setup_js, _utils, _svgxuse, select2_css]
    )
...
def get_opa_group_vue():
    return Group([main_css, opa_css, vendor_vue_js, _svgxuse, select2_css])
...
competence_js = get_module_resource(
    "competence", tmpl=True, extra_depends=[select2, select2_fr]
)
...
# Permet d'overrider ou compléter les css de la sortie pdf pour l'affichage
# html
task_preview_css = get_resource("css/task_preview.css")
...
customer_js = get_resource(
    "js/build/customer.js",
    minified="js/build/customer.min.js",
