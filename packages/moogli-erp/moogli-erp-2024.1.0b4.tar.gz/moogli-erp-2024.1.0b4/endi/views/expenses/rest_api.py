...
from endi.events.document_events import StatusChangedEvent
...
from endi.views.status.rest_api import (
...
class RestExpenseSheetView(BaseRestView):
    ...
    def _get_justified_toggle(self):
        ...
        actions = self.context.justified_state_manager.get_allowed_actions(self.request)
    ...
    def _get_type_options(self):
        ...
        options = {
            "expense_types": active_or_used.filter_by(type="expense").all(),
            "expensetel_types": active_or_used.filter_by(type="expensetel").all(),
...
class RestExpenseJustifiedStatusView(StatusView):
    ...
    def check_allowed(self, status):
        self.request.context.check_justified_status_allowed(status, self.request)

    def redirect(self):
        return Apiv1Resp(self.request, {"justified": self.context.justified})

    def log_sheet_status(self, sheet, status, params):
        logger.debug("In pre_status_process : %s %s" % (status, type(status)))
        sheet.statuses.append(
            StatusLogEntry(
    ...
    def notify_sheet_status(self, sheet, status):
        if status is True:
            self.request.registry.notify(
                StatusChangedEvent(self.request, sheet, "justified")
            )

    def status_process(self, status, params):
        return self.context.set_justified_status(status, self.request, **params)


class RestExpenseLineJustifiedStatusView(RestExpenseJustifiedStatusView):
    def notify(self, status, params):
        # Do not log anything for lines
        pass
    ...
    def post_status_process(self, status, params):
        ...
        if lines_justified_status != sheet.justified:
            if lines_justified_status is None:
                ...
                sheet.justified = lines_justified_status
                self.notify_sheet_status(sheet, status)
                self.log_sheet_status(sheet, status, params)
            self.dbsession.merge(sheet)


class RestExpenseSheetJustifiedStatusView(RestExpenseJustifiedStatusView):
    def notify(self, status, params):
        self.notify_sheet_status(self.context, status)

    def pre_status_process(self, status, params):
        self.log_sheet_status(self.context, status, params)
        return super().pre_status_process(status, params)
