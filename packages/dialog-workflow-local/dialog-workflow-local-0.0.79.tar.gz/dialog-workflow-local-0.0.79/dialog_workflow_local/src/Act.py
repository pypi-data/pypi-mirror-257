...
class action(object):
    ...
    def act(self, dialog_workflow_record: DialogWorkflowRecord, got_response: bool):
        ...
        elif action==action_enum.PRESENT_FORM:
    ...
    def present_and_choose_script(self):
        ...
        cursor = connection.cursor(dictionary=True, buffered=True)
        cursor.execute(
            """SELECT d.start_state_id, dml.title FROM dialog_workflow_script_view AS d JOIN dialog_workflow_script_ml_view AS dml on dml.dialog_workflow_script_id=d.dialog_workflow_script_id WHERE dml.lang_code = %s""", [self.language])
    ...
    def present_form_action(self):
        ...
        for question in questions:
            ...
            form=CompoundMessage(message_template_id=message_template_id)
            forms.append(form)
        return True,forms
