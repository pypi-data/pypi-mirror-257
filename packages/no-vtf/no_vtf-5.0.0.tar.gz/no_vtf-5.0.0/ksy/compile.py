...
class ClassUnnester(ast.NodeTransformer):
    ...
    def visit(self, node: ast.AST) -> Optional[ast.AST]:
        if self._state is not ClassUnnester.State.NONE:
            ...
            assert node_optional is None or isinstance(node_optional, ast.AST)
    ...
    def _pre_visit_Name_UPDATING_REFERENCES(self, node: ast.Name) -> ast.Name:  # noqa: N802
        ...
        if longest_match:
            assert not self._collapse_attributes_depth
            self._collapse_attributes_depth = len(longest_match) - 1
            assert len(self._attribute_stack) >= self._collapse_attributes_depth
    ...
    def _check_post_conditions(self) -> None:
        assert self._state is ClassUnnester.State.NONE
        assert not self._class_tree
        assert not self._unnested_classes
        assert not self._class_stack
        assert not self._attribute_stack
        assert not self._collapse_attributes_depth
