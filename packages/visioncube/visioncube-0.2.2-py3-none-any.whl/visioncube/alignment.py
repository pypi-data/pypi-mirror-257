...
class TemplateMatching(AbstractAlignment):
    ...
    def __call__(self, image: np.ndarray):
        ...
        res = cv.matchTemplate(image_gray, self.template, cv.TM_CCOEFF_NORMED)
...
class RotateTemplateMatching(AbstractAlignment):
    ...
    def __init__(self, train_image, size=224, num_angles=20, n=20):
    ...
    def __call__(self, query_image):
        ...
        confidence, (x1, y1), i = self._match(image_gray, i - self.n - 1, i + self.n, 1)  # (x1, y1)相对于resize之后图像的坐标
...
class FeatureBasedAlignment(AbstractAlignment):
    ...
    def __call__(self, image: np.ndarray) -> AlignmentOutput:
        ...
        good_match_len = min(int(len(matches) * 0.8), 10000)
    ...
    def __init__(self, config: LocalizationConfig):
        ...
        if self.config.allow_rotate:
            ...
            if self.config.allow_scale:
                ...
                self.target_list: List[AbstractAlignment] = [
                    RotateTemplateMatching(target, size=self.config.template_size)
                    for target in self.target_list
                ]
        else:
            self.target_list: List[AbstractAlignment] = [
                TemplateMatching(target)
                for target in self.target_list
            ]
...
def main():
    ...
    roi = [360, 512, 2144, 3320]
    input_dir = '/home/xi/Desktop/fusion/query'
    target_path = '/home/xi/Desktop/fusion/target.bmp'
    output_dir = '/home/xi/Desktop/fusion/output'
    ...
    localize = Localization(config)

    os.makedirs(output_dir, exist_ok=True)
    for filename in tqdm(os.listdir(input_dir)):
        input_path = os.path.join(input_dir, filename)
        image = cube.read_image(input_path)
        output = localize(image)[0]  # only one roi
        output_path = os.path.join(output_dir, filename)
        cube.write_image(output.crop, output_path)
    return 0
