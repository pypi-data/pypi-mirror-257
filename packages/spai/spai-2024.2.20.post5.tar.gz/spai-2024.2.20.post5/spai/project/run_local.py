    ...
    if config.storage:
        ...
        for storage in config.storage:
            if storage.type == "s3" and not storage.credentials:
                ...
                #     raise typer.Abort()
                storage.credentials = create_or_retrieve_s3_bucket(config.project)
        # set env variables
        prefix = "SPAI_STORAGE"
        names = []
        for storage in config.storage:
