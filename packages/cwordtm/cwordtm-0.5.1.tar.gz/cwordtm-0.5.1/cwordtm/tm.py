...
#
# Copyright (c) 2022-2024 CWordTM Project 
...
def load_text(textfile):
    ...
    docs = pd.read_csv(textfile, sep=' ', header=None)
    return list(docs[0])
...
def process_text(doc):
    ...
    removing all digits, stemming, removing punctuations and stopwords.
...
class LDA:
    ...
    def preprocess(self):
        """Process the original English documents (wordtm.tm.LDA.docs)
        by invoking wordtm.tm.process_text, and build a dictionary and
    ...
    def preprocess_chi(self):
        """Process the original Chinese documents (wordtm.tm.LDA.docs) 
        by invoking wordtm.tm.process_text, and build a dictionary
...
def lda_process(doc_file, cat=0, chi=False, group=True, eval=False):
...
class NMF:
    ...
    def preprocess(self):
        """Process the original English documents (wordtm.tm.NMF.docs)
        by invoking wordtm.tm.process_text, and build a dictionary
    ...
    def preprocess_chi(self):
        """Process the original Chinese documents (wordtm.tm.NMF.docs) 
        by invoking wordtm.tm.process_text, and build a dictionary
        and a corpus from the preprocessed documents for the NMF model.
        """
...
def nmf_process(doc_file, cat=0, chi=False, group=True, eval=False):
...
class BTM:
    ...
    def preprocess(self):
        """Process the original English documents (wordtm.tm.BTM.docs)
        by invoking wordtm.tm.process_text, and build a dictionary and
    ...
    def preprocess_chi(self):
        """Process the original Chinese documents (wordtm.tm.BTM.docs) 
        by invoking wordtm.tm.process_text, and build a dictionary
...
def btm_process(doc_file, cat=0, chi=False, group=True, eval=False):
