...
class QuantileLayer(tf.keras.layers.Layer):
    ...
    def call(self, inputs):
        ...
        quantile_values = tf.map_fn(lambda x: tfp.stats.percentile(inputs_flat[..., x], quantiles * 100, axis=1, interpolation='linear'),
                                    elems=tf.range(channels), dtype=inputs.dtype)

        # quantile_values shape: [channels, num_quantiles, batch_size]

        # output shape is [batch_size, num_quantiles, channels]
        quantile_values = tf.transpose(quantile_values, [2, 1, 0])
        return quantile_values

    def compute_output_shape(self, input_shape):
        out_shape = (input_shape[0], self.num_quantiles, input_shape[-1])
        return out_shape
