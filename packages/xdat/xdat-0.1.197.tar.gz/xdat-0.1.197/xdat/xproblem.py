...
def cv_split_prep(df, n_splits=12, stratify_on=None, stratify_on_q=None, group_on=None, ts_split_on=None, random_state=None):
    ...
    if group_on is None:
        ...
        assert n_splits > 1
        if stratify_on:
            kfold = model_selection.StratifiedKFold(n_splits=n_splits, random_state=random_state)
        ...
        if stratify_on:
            kfold = model_selection.StratifiedGroupKFold(n_splits=n_splits, random_state=random_state)
