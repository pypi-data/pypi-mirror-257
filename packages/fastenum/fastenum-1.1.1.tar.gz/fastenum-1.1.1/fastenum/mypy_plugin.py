...
def parse_mypy_version(ver: str) -> tuple[int, ...]:
	'''
	Parse version of MyPY, based on https://github.com/python/mypy/issues/6617#issuecomment-1600788021
	'''
	return tuple(map(int, ver.partition('+')[0].split('.')))


MYPY_VERSION = parse_mypy_version(mypy.version.__version__)
...
def _define_method(
	...
	'''
	function_type: Optional[mypy.types.Instance]
	...
	arg_types: List[mypy.types.Type] = []
	arg_names: List[str] = []
	# Kinds are kind of arguments (position, key word,..) see :code:`mypy.nodes.ARG_POS` for example.
	arg_kinds: List[mypy.types.ArgKind] = []

	for arg in arguments:
		assert arg.type_annotation, 'All arguments must be fully typed.'
		arg_types.append(arg.type_annotation)
		arg_names.append(arg.variable.fullname)
	...
	signature = mypy.types.CallableType(arg_types, arg_kinds, arg_names, return_type, function_type)
	...
	func = mypy.nodes.FuncDef(name, arguments, mypy.nodes.Block([mypy.nodes.PassStmt()]))
	...
	cls_info.names[name] = mypy.nodes.SymbolTableNode(mypy.nodes.MDEF, func, plugin_generated = True)
...
def transform_enum_class_def(context: mypy.plugin.ClassDefContext) -> None:
	...
	But if we inspect :code:`context.cls` which is of type :code:`mypy.nodes.ClassDef` (an AST node)
	we get something like:

	  ::
	    ClassDef:3(
	    Color
	    BaseType(fastenum.fastenum.Enum)
	    AssignmentStmt:4(
	      NameExpr(RED [m])
	      IntExpr(1)
	      builtins.int)
	...
	And by inspecting :code:`context.cls.info` which is of type :code:`mypy.nodes.TypeInfo` - still an AST node
	but one which defines the type of our class (where previous defined class definition) you'll get:

	  ::
	    TypeInfo(
	      Name(main.Color)
	      Bases(fastenum.fastenum.Enum)
	      Mro(main.Color, fastenum.fastenum.Enum, builtins.object)
	      Names(
	        BLUE (builtins.int)
	        GREEN (builtins.int)
	        RED (builtins.int))
	      MetaclassType(fastenum.fastenum.EnumMeta))
	...
	info.bases = [base for base in info.bases if base.type.fullname != 'builtins.int']

	# First clear all `mypy.nodes.AssignmentStmt` in class.
	...
	context.cls.defs.body = [
		...
		if not isinstance(node, mypy.nodes.AssignmentStmt)
	]
	...
	self_type = mypy.types.Instance(info, [])
	...
	_define_method(
		...
		[
			mypy.nodes.Argument(
				mypy.nodes.Var('self', metaclass_type),
				metaclass_type,
				None,
				mypy.nodes.ARG_POS,
			),
	...
	_define_method(
		...
		'__getitem__',
		[
			mypy.nodes.Argument(mypy.nodes.Var('cls', metaclass_type), metaclass_type, None, mypy.nodes.ARG_POS),
			mypy.nodes.Argument(mypy.nodes.Var('key', str_type), str_type, None, mypy.nodes.ARG_POS),
		],
		self_type,
	)
	...
	# and filter out only type :code:`mypy.nodes.Var` which are class attributes.
	for name, named_node in info.names.items():
		# We want to modify only class an instance level variables
		if isinstance(named_node.node, mypy.nodes.Var):
			node: mypy.nodes.Var = cast(mypy.nodes.Var, info.names[name].node)

			# We replace original type (which will be int, str, ...)
			# with an ``Instance`` of our class itself (not with the base class - Enum).
			node.type = mypy.types.Instance(info, [])
			...
			info.names[name] = mypy.nodes.SymbolTableNode(mypy.nodes.MDEF, node, plugin_generated = True)
...
def transform_enum_type(context: mypy.plugin.AnalyzeTypeContext) -> mypy.types.Type:
	'''
	...
	# Define meta class in fake module :code:`fastenum.fastenum`
	# This is roughly equivalent to:
	#
	# 	class EnumMeta(builtins.type): pass
	#
	# in module :code:`fastenum.fastenum`.
	...
	meta_cls = mypy.nodes.ClassDef('EnumMeta', mypy.nodes.Block([mypy.nodes.PassStmt()]), [], [])
	meta_cls.fullname = 'fastenum.fastenum.EnumMeta'
	meta_info = mypy.nodes.TypeInfo(mypy.nodes.SymbolTable(), meta_cls, 'fastenum.fastenum')
	...
	# Define Enum class which is using EnumMeta as its metaclass in the fake :code:`fastenum.fastenum` module
	...
	enum_cls = mypy.nodes.ClassDef(
		'Enum',
		mypy.nodes.Block([mypy.nodes.PassStmt()]),
		[],
		[],
		mypy.nodes.NameExpr('EnumMeta'),
	)
	enum_cls.fullname = 'fastenum.fastenum.Enum'
	enum_info = mypy.nodes.TypeInfo(mypy.nodes.SymbolTable(), enum_cls, 'fastenum.fastenum')
	...
	enum_info.metaclass_type = mypy.types.Instance(meta_info, [])
	enum_info.declared_metaclass = mypy.types.Instance(meta_info, [])
	...
	value_attribute = mypy.nodes.Var('value', mypy.types.AnyType(mypy.types.TypeOfAny.explicit))
	...
	enum_info.names['value'] = mypy.nodes.SymbolTableNode(
		mypy.nodes.MDEF,
		value_attribute,
	...
	name_attribute = mypy.nodes.Var('name', str_type)
	name_attribute.is_initialized_in_class = False
	name_attribute.info = enum_info
	enum_info.names['name'] = mypy.nodes.SymbolTableNode(
		mypy.nodes.MDEF,
		name_attribute,
		plugin_generated = True,
	)
	...
	meta_enum_instance = mypy.types.Instance(meta_info, [])

	# MyPY changed the behavior of TypeVars in 1.4.0
	# - Release notes: https://github.com/python/mypy/blob/master/CHANGELOG.md#mypy-14
	# - Internal API changelog: https://github.com/python/mypy/issues/6617#issuecomment-1600788021
	if MYPY_VERSION >= (1, 4):
		self_tvar_type = mypy.types.TypeVarType(
			'_EnumMetaType',
		...
		self_tvar_expr = mypy.nodes.TypeVarExpr(
			'_EnumMetaType',
			f'{meta_info.fullname}._EnumMetaType',
			[],
			meta_enum_instance,
		...
		self_tvar_type = mypy.types.TypeVarType(  # type: ignore[call-arg]
			'_EnumMetaType',
			f'{meta_info.fullname}._EnumMetaType',
			-1,
			[],
		...
		self_tvar_expr = mypy.nodes.TypeVarExpr(  # type: ignore[call-arg]
			'_EnumMetaType',
			f'{meta_info.fullname}._EnumMetaType',
			[],
			meta_enum_instance,
	...
	meta_info.names['_EnumMetaType'] = mypy.nodes.SymbolTableNode(mypy.nodes.MDEF, self_tvar_expr)
	...
	_define_method(
		...
		[
			mypy.nodes.Argument(
				mypy.nodes.Var('cls', meta_enum_instance),
				meta_enum_instance,
				None,
				mypy.nodes.ARG_POS,
			),
	...
	enum_instance = mypy.types.Instance(enum_info, [])
	any_type = mypy.types.AnyType(mypy.types.TypeOfAny.explicit)
	_define_method(
		context,
		enum_info,
		context.type.name,
		'__init__',
		[
			mypy.nodes.Argument(mypy.nodes.Var('self', enum_instance), enum_instance, None, mypy.nodes.ARG_POS),
			mypy.nodes.Argument(mypy.nodes.Var('value', any_type), any_type, None, mypy.nodes.ARG_POS),
		],
		mypy.types.NoneTyp(),
	)
	...
	_define_method(
		...
		[
			mypy.nodes.Argument(
				mypy.nodes.Var('self', meta_enum_instance),
				meta_enum_instance,
				None,
				mypy.nodes.ARG_POS,
			),
	...
	def def_bool_method(name: str) -> None:
		_define_method(
			...
			[
				mypy.nodes.Argument(mypy.nodes.Var('self', enum_instance), enum_instance, None, mypy.nodes.ARG_POS),
				mypy.nodes.Argument(mypy.nodes.Var('other', enum_instance), enum_instance, None, mypy.nodes.ARG_POS),
			],
			bool_type,
		)
	...
	return mypy.types.Instance(enum_info, [])
