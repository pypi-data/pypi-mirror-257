...
from easy_vc_dev.utils.mel_processing import spectrogram_torch
    ...
    def get_audio(self, filename):
        ...
        if os.path.exists(spec_filename):
            try:
                ...
                # print(spec_filename, traceback.format_exc())
                print("load spec error. recreate spec.")
                spec = spectrogram_torch(
                    audio_norm,
                    self.filter_length,
                    self.sampling_rate,
...
class DistributedBucketSampler(torch.utils.data.distributed.DistributedSampler):
    ...
    def _create_buckets(self):
        ...
        for i in range(len(buckets) - 1, -1, -1):  # 中身のないバケットを削除している。
            if len(buckets[i]) == 0:
                buckets.pop(i)
                self.boundaries.pop(i + 1)

        num_samples_per_bucket = []
        for i in range(len(buckets)):  # 各バケットのバッチの数を計算している。
