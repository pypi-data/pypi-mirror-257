...
class Address(MTObject):
    ...
    @address.setter
    def address(self, value: str | IPAddress):
    ...
    @network.setter
    def network(self, value: str | IPAddress):
        if isinstance(value, IPAddress) or value is None:
    ...
    def add(self, address: str, interface: str, network: str = None,
        ...
        :return: 'Address'
        """
        command = f'/ip address add address="{str(IPAddress(address))}" interface="{interface}"'
        command += f' network="{str(IPAddress(network))}"' if network else ''
