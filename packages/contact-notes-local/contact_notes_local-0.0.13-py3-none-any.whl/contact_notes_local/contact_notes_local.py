...
class ContactNotesLocal(GenericMapping):
    ...
    def insert_contact_notes_text_block(self) -> dict:
        logger.start()
        # check if the contact already has notes
        note = self.contact_dict.get('notes', None)
        where_clause = f"contact_id = {self.contact_id} AND note = '{note}'"
    ...
    def insert_contact_note_text_block_table(self, contact_note_id: int) -> list:
        '''
        ...
        for i, text_block in enumerate(text_blocks_list):
            if i == 0:
                ...
                elif not text_block.get('is_action_item') and text_block.get('text'):
                    # text block is text
                    data_json = {
    ...
    def __process_text_blocks_list(self, text_blocks_list: list) -> None:
    ...
    def __convert_date_format(self, date_str: str, date_format: str) -> str:
    ...
    def __extract_start_timestamp(self, text_block: dict) -> str:
