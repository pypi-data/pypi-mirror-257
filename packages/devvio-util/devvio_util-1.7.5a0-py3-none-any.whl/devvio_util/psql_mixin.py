...
class PsqlMixin:
    ...
    def update_record(
        ...
        try:
            ...
            self._logger.debug(
                "PSQL error in UpdateRecord, attempt rollback: {}".format(str(i))
            )
            self.dbquery("rollback")
            return None
        except Exception as e:
    ...
    def delete_record(self, relation: str, constraints: Dict[str, Any]):
        ...
        try:
            ...
            self._logger.debug(
                "PSQL error in DeleteRecord, attempt rollback: {}".format(str(i))
            )
            self.dbquery("rollback")
            return None
        except Exception as e:
    ...
    def set_record(
        ...
        try:
            ...
            try:
                ...
                self._logger.debug(
                    "PSQL error in select portion of SetRecord, attempt create: {}".format(
                        str(i)
                    )
                )
            if num_records < 1:
            ...
            self._logger.debug(
                "PSQL error in SetRecord, attempt rollback: {}".format(str(i))
            )
            self.dbquery("rollback")
            return None
        except Exception as e:
