...
class course(object):
	...
	def assign_grades(self, frame:mystring.frame, student_pid_column:str, proceed=True,debug=False, course_number=None, assignment_number=None):
		self.set_course(course_number)
		self.set_assignment(assignment_number)

		comment_colum = None
		for kol in frame.kolz:
			if "Comments ({0})".format(self.__assignment_number) in kol:
				comment_colum = kol
				break
		
		grade_colum = None
		for kol in frame.kolz:
			if "({0})".format(self.__assignment_number) in kol and "Comments" not in kol:
				grade_colum = kol
				break
		...
		for row in tqdm(frame.roll):
			if row[student_pid_column] and row[student_pid_column].strip() != '':
				student_id = student_list[row[student_pid_column]]
				student_latest_submission = self.assignment.get_submission(student_id)

				if debug:
					print({
						"OldComment":student_latest_submission.body,
						"OldGrade":student_latest_submission.grade,
						"NewComment":row[comment_colum],
						"NewGrade":row[grade_colum]
					})
				
				if proceed:
					student_latest_submission.edit(
						grade=row[grade_colum],
						comment={
							"text_comment":row[comment_colum]
						}
					)
