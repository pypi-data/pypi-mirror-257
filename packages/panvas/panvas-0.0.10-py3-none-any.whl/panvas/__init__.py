...
class course(object):
	...
	def assign_grades(self, frame:mystring.frame, student_pid_column:str, student_id_column:str=None, proceed=True,debug=False, course_number=None, assignment_number=None, ignore_comment_lambda=lambda x:None if x is None or x.strip() in [':',':;',':;;',':;;;',':;;;;'] else x.strip().replace(":;",":")):
		self.set_course(course_number)
		self.set_assignment(assignment_number)

		url = "https://canvas.vt.edu/courses/{0}/gradebook/speed_grader?assignment_id={1}".format(self.__course_number, self.__assignment_number)
		...
		for row in tqdm(frame.roll):
			...
			temp_url = url + "&student_id={0}".format(student_id)
