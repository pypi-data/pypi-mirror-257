...
REQUIRED = [
    ...
    'scikit-learn',
    'pytest',
    'pre-commit<=3.3.2',  # TODO: check if bug for 3.3.3 still exists
] + (['multiprocess'] if sys.platform == 'darwin' else [])


EXTRAS = {
    'keras': ['tensorflow==2.12.1'] if sys.platform == 'linux' else ['tensorflow-macos==2.12.1'],
    'lightning': ['torch==2.1.0', 'lightning==2.1.0'],
}
...
setup(
    ...
    url=URL,
    packages=find_packages(exclude=["tests", "*.tests", "*.tests.*", "tests.*", "temp_*"]),
    entry_points={
        'console_scripts': [
