...
class EarlyStopperForTuning(ABC):
    ...
    def __init__(
        ...
        report_event : Optional[Event]
            ...
            ``report_queue``. (Default value = None)
        report_queue : Optional[Queue]
            `Multiprocessing` ``queue`` to send the results dictionary to the parent process. (Default value = None)
        report_queue_read_event : Optional[Event]
            ...
            ``report_queue`` and reported to `Tune`. (Default value = None)
    ...
    def at_epoch_end(self, epoch: int, logs: Optional[dict] = None, **kwargs: dict) -> None:
        ...
        If ``self.in_tune_session`` is ``True``, the dictionary containing the current results (i.e. the dict returned by
        ``self.build_results_dict``) is added to the ``self.report_queue`` and the ``self.report_event`` is triggered.
        The parent process is expected to take the results from the queue and report them to `Tune`. This cannot be done
        directly as `Tune` seems to associate each trial with the corresponding process-id and thus would not recognize
        the report. After triggering the ``self.report_event``, this function waits for up to 600 seconds for the
        ``self.report_queue_read_event`` to be triggered, which is used to signify that the results have been successfully
        reported to `Tune`. After 120 seconds of waiting, the status of the parent process is checked in a one-second
        interval. If the parent process is not alive anymore, this process self-terminates. After 600 seconds of waiting,
        the self-termination is also triggered even if the parent process is still alive. Once the
        ``self.report_queue_read_event`` is triggered, it is cleared and the function returns.
    ...
    def _do_early_stopping(self, epoch, logs=None, **kwargs: dict) -> bool:
        ...
        **kwargs : dict
        ...
        if is_improvement and not is_overfit:
            if self.verbose > 0:
                print("Epoch {}: Model improved from {:.5f} to {:.5f}".format(epoch, self.best, current))
            self.best = current
        ...
        if not is_improvement and not is_overfit:
            ...
            if self.verbose > 0:
                print("Epoch {}: Model did not improve from {:.5f}".format(epoch, self.best))
        if is_overfit and self.verbose > 0:
            print("Epoch {}: Model is overfitted, reason: {}".format(epoch, " & ".join(overfitting_reasons)))
...
def perform_crossvalidation(
    ...
    if hasattr(run_config, "get_experiment_inputs"):
        inputs_split, targets_split, weights_split, normalized_weights_split = run_config.get_experiment_inputs(
            input_handler, inputs_for_crossvalidation=True, disable_printing=verbose == 0
        )
    else:
        (
            inputs_split,
        ...
        ) = OPTIMA.core.inputs.get_experiment_inputs(
            run_config, input_handler, inputs_for_crossvalidation=True, disable_printing=verbose == 0
        )
