...
from ray.air._internal import session as internal_session
...
def train_model(model_config, run_config, input_handler, inputs_train, inputs_val, targets_train, targets_val,
    ...
    def _train_model(checkpoint_com, report_com, termination_event, start_time,
        ...
        if run_config.model_type == "Keras":
            ...
            if hasattr(run_config, "DataModule"):
                pl_data_module = run_config.DataModule(
                ...
                pl_data_module = OPTIMA.lightning.inputs.DefaultDataModule(
                    inputs_train=inputs_train,
                    targets_train=targets_train,
                    weights_train=normalized_weights_train,
                    inputs_val=inputs_val,
        ...
        if checkpoint is not None:
            ...
            if hasattr(run_config, 'build_model') and run_config.model_type == "Keras":
                model = run_config.build_model(model_config, input_handler=input_handler, inputs_train=inputs_train, output_shape=output_shape)#, seed=model_config["seed"])
                ...
                model.prepare(inputs_train)
            else:
                if run_config.model_type == "Keras":
                    model = OPTIMA.builtin.model.build_model(model_config, input_handler=input_handler, inputs_train=inputs_train, output_shape=output_shape)#, seed=model_config["seed"])
                elif run_config.model_type == "Lightning":
                    raise NotImplementedError("Lightning models requires a LitModel-class to be present in the run-config.")

        # delete the input features as they are no longer needed
        del inputs_val
        del inputs_train

        # in any case, the model needs to be compiled (this is also necessary if only the regularizers have been updated!) (only for Keras)
        if run_config.model_type == "Keras":
    ...
    if in_tune_session and run_in_subprocess:
        end_event = internal_session._get_session()._status_reporter._end_event
    ...
    while (not termination_event.is_set()) and p.is_alive():
        if in_tune_session:
            if checkpoint_event.is_set():
                checkpoint_queue.put(session.get_checkpoint())
...
def do_optimization(args, run_config, defaults_config_str):
    ...
    if hasattr(run_config, 'build_search_space'):
        search_space = run_config.build_search_space(hyperparameter_defaults)
    ...
    if hasattr(run_config, 'InputHandler'):
        input_handler = run_config.InputHandler()
    else:
        input_handler = OPTIMA.builtin.inputs.InputHandler(run_config)
    if hasattr(run_config, 'get_experiment_inputs'):
        inputs_split, targets_split, weights_split, normalized_weights_split = run_config.get_experiment_inputs(input_handler, output_dir=output_dir)
    else:
        inputs_split, targets_split, weights_split, normalized_weights_split = OPTIMA.core.inputs.get_experiment_inputs(run_config,
                                                                                                                        input_handler,
    ...
    if run_config.perform_variable_opt:
        ...
        if hasattr(run_config, 'get_experiment_inputs'):
            inputs_split, targets_split, weights_split, normalized_weights_split = run_config.get_experiment_inputs(
                input_handler, output_dir=output_dir)
        else:
...
def main():
    ...
    if not args.is_worker and not args.cluster == "local":
        ...
        do_optimization(args, run_config, defaults_config_str)
