...
def evaluate_experiment(
    ...
    if not os.path.isfile(os.path.join(results_dir, "evaluation.pickle")):
        ...
        if hasattr(run_config, "evaluate"):
            use_builtin_evaluate = False
            evaluate_func = run_config.evaluate
        else:
            use_builtin_evaluate = True
        ...
        for model_dir in dirs_to_evaluate:
            ...
            for model_info in crossval_model_info[model_dir]:
                ...
                # do the evaluation; for the builtin function, the run_config needs to be provided, but not for the
                # overwritten version
                if use_builtin_evaluate:
                    futures.append(
                        evaluate_remote(
                            run_config,
