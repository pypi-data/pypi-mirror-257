...
class SignatureData:
    ...
    snapshot : bool = True  
    energy_data_frames : list[EnergyFrame]  = None
...
class MarlinData(object):
    ...
    def download_signature_snapshots(self, load_args : {} = {}) -> ({}):
        ...
        for signature in request_json_data_signature:
            ...
            #     if signature['ss_id'] not in load_args['ss_ids']:
                    
            #         continue
            # if 'location' in load_args:
            #     if signature['data_receiver_location_name'] not in load_args['location']:
            #         continue
        ...
        # limit_sig = 5
        limit_sig_cnt = 0
        print (self.snapshot_ids)
        for snapshot_id in self.snapshot_ids:
            print (f'limit:{limit_sig_cnt}, {snapshot_id}')
...
class MarlinDerivedData(object):
    ...
    def build_derived_data(self, simulation_data : SimulationData = None, sample_delta_t : float = 0.5):
        """
        ...
        simulation_data.energy_data_frames = energy_frames
...
if __name__ == "__main__":
    ...
    marlin_data = MarlinData(load_args={'limit' : 1})
    ...
    marlin_data.download_simulation_snapshots(load_args = {'location' : ['clyde_surfer_1']})
