from asyncio import QueueEmpty, get_event_loop
...
from team.sulap import queues

from .class_mix import user
...
@user.pytgc_dec()
async def _(c, u: Update):
    global turun_dewek
    if isinstance(u, StreamAudioEnded):
        queues.task_done(u.chat_id)
        if u.chat_id in daftar_join:
            turun_dewek = False
        elif queues.is_empty(u.chat_id):
            try:
                await c.leave_group_call(u.chat_id)
                turun_dewek = True
