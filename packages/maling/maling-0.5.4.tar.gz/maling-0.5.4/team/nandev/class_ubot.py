...
class Userbot(Client):
    ...
    async def digbanned(self, query):
        chats = []
        chat_types = {
    ...
    def get_text(self, m):
        if m.reply_to_message:
            if len(m.command) < 2:
                text = m.reply_to_m.text or m.reply_to_m.caption
            else:
                text = (
                    (m.reply_to_m.text or m.reply_to_m.caption)
                    + "\n\n"
    ...
    def get_arg(self, m):
        if m.reply_to_message and len(m.command) < 2:
            msg = m.reply_to_m.text or m.reply_to_m.caption
            if not msg:
                return ""
...
"""
    ...
    async def forward_private(self, m):
        ...
        if m.chat.id == 777000:
            return
        async for x in self.search_messages(m.chat.id, from_user=m.from_user.id, limit=1):
            await x.forward(db)
