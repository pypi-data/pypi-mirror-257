...
class SentCode(Object):
    ...
    Parameters:
        type (:obj:`~team.enums.SentCodeType`):
            Type of the current sent code.

        phone_code_hash (``str``):
            Confirmation code identifier useful for the next authorization steps (either
            :meth:`~team.Client.sign_in` or :meth:`~team.Client.sign_up`).

        next_type (:obj:`~team.enums.NextCodeType`, *optional*):
            Type of the next code to be sent with :meth:`~team.Client.resend_code`.

        timeout (``int``, *optional*):
            Delay in seconds before calling :meth:`~team.Client.resend_code`.
    """
