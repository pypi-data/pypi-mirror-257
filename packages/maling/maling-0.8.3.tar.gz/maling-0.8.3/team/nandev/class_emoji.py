...
class Emojii:
    def initialize(self):
        self.ping_var = (
            udB.get_var(self.me.id, "emo_ping") or "5269563867305879894"
        )
        self.emo_ping = (
        ...
        self.pong_var = (
            udB.get_var(self.me.id, "emo_pong") or "6183961455436498818"
        )
        self.emo_pong = (
        ...
        self.proses_var = (
            udB.get_var(self.me.id, "emo_proses") or "5974326532670230199"
        )
        self.emo_proses = (
            self.proses_var
        ...
        self.sukses_var = (
            udB.get_var(self.me.id, "emo_sukses") or "5021905410089550576"
        )
        self.emo_sukses = (
            self.sukses_var
        ...
        self.gagal_var = (
            udB.get_var(self.me.id, "emo_gagal") or "5019523782004441717"
        )
        self.emo_gagal = (
        ...
        self.profil_var = (
            udB.get_var(self.me.id, "emo_profil") or "5373012449597335010"
        )
        self.emo_profil = (
            self.profil_var
        ...
        self.alive_var = (
            udB.get_var(self.me.id, "emo_alive") or "4934091419288601395"
        )
        self.emo_alive = (
            self.alive_var if isinstance(self.alive_var, int) else str(self.alive_var)
        )
        self.warn_var = (
            udB.get_var(self.me.id, "emo_warn") or "6172475875368373616"
        )
        self.emo_warn = (
            self.warn_var if isinstance(self.warn_var, int) else str(self.warn_var)
        )
        self.block_var = (
            udB.get_var(self.me.id, "emo_block") or "5240241223632954241"
        )
        self.emo_block = (
...
async def load_emo(c):
    ...
    a = udB.get_var(c.me.id, "emo_ping")
    b = udB.get_var(c.me.id, "emo_pong")
    c = udB.get_var(c.me.id, "emo_proses")
    d = udB.get_var(c.me.id, "emo_gagal")
    e = udB.get_var(c.me.id, "emo_sukses")
    f = udB.get_var(c.me.id, "emo_profil")
    g = udB.get_var(c.me.id, "emo_alive")
    h = udB.get_var(c.me.id, "emo_warn")
    i = udB.get_var(c.me.id, "emo_block")
    ...
    if uprem == True:
        if (a, b, c, d, e, f, g, h, i) is not None:
            ...
            await asyncio.sleep(0.5)
            udB.set_var(c.me.id, "emo_warn", warn_id)
            await asyncio.sleep(0.5)
            udB.set_var(c.me.id, "emo_block", block_id)
    elif uprem == False:
        if (a, b, c, d, e, f, g, h, i) is not None:
            return
        else:
            ...
            await asyncio.sleep(0.5)
            udB.set_var(c.me.id, "emo_warn", ewarn)
            await asyncio.sleep(0.5)
            udB.set_var(c.me.id, "emo_block", eblock)
