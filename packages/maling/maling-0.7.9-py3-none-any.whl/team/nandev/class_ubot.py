...
class Userbot(Client):
    ...
    async def get_user_dialog(self, query):
        chats = []
        chat_types = {
    ...
    def Emojii(self):
        ...
        ping_var = udB.get_var(self.me.id, "emo_ping") or "5269563867305879894"
        emo_ping = str(ping_var)

        pong_var = udB.get_var(self.me.id, "emo_pong") or "6183961455436498818"
        emo_pong = str(pong_var)

        proses_var = udB.get_var(self.me.id, "emo_proses") or "5974326532670230199"
        emo_proses = str(proses_var)

        sukses_var = udB.get_var(self.me.id, "emo_sukses") or "5021905410089550576"
        emo_sukses = str(sukses_var)

        gagal_var = udB.get_var(self.me.id, "emo_gagal") or "5019523782004441717"
        emo_gagal = str(gagal_var)

        profil_var = udB.get_var(self.me.id, "emo_profil") or "5373012449597335010"
        emo_profil = str(profil_var)

        alive_var = udB.get_var(self.me.id, "emo_alive") or "4934091419288601395"
        emo_alive = str(alive_var)

        warn_var = udB.get_var(self.me.id, "emo_warn") or "6172475875368373616"
        emo_warn = str(warn_var)

        block_var = udB.get_var(self.me.id, "emo_block") or "5240241223632954241"
    ...
    def emojii(self):
        return emojii = self.Emojii()
        
    def ping(self):
        return ping = self.emojii["ping"]()
    def pong(self):
        return pong = self.emojii["pong"]()
    def proses(self):
        return proses = self.emojii["proses"]()
    def sukses(self):
        return sukses = self.emojii["sukses"]()
    def gagal(self):
        return gagal = self.emojii["gagal"]()
    def profil(self):
        return profil = self.emojii["profil"]()
    def alive(self):
        return alive = self.emojii["alive"]()
    def warn(self):
        return warn = self.emojii["warn"]()
    def block(self):
        return block = self.emojii["block"]()
    ...
    async def aexec(self, code, m):
        exec(
            "async def __aexec(m): "
            + "".join(f"\n {l_}" for l_ in code.split("\n"))
        )
        return await locals()["__aexec"](m)
    ...
    async def forward_group(self, m):
        ...
        if not db:
            return
        user = f"[{m.from_user.first_name} {m.from_user.last_name or ''}](tg://user?id={m.from_user.id})"
        lenk = m.link
        teks = f"""
📨 <b>TAGS MESSAGE</b>
• <b>Logs:</b> <code>{self.me.first_name}</code>
• <b>Group:</b> <code>{m.chat.title}</code>
• <b>Dari :</b> <code>{user}</code>
• <b>Pesan:</b> <code>{m.text}</code>

• <b>Tautan Grup:</b> [Disini]({lenk})
"""
        try:
            await self.send_message(
              db,
    ...
    async def forward_private(self, m):
        db =  udB.get_logger(self.me.id)
        if not db:
          return
        if m.chat.id != 777000:
            ...
            try:
                async for x in self.search_messages(m.chat.id, limit=1):
                    await x.forward(db)
                LOG_CHATS_.COUNT += 1
    ...
    async def restart2(self):
        await self.bash("run.sh")
        
    def pytgc_dec(self):
        def decorator(func):
            try:
                if func.__name__ != "stream_end":
                    self.call_py.on_kicked()(func)
                    self.call_py.on_closed_voice_chat()(func)
                    self.call_py.on_left()(func)
                else:
                    self.call_py.on_stream_end()(func)
            except:
                pass
            return func
...
class Bot(Client):
    ...
    async def start(self):
        await super().start()
        LOGGER.info(f"Starting Assistant ({self.me.id}|{self.me.first_name}{self.me.last_name})")