...
from team.sulap.nandev.class_ubot import LOGGER
...
class MongoDB:
    ...
    def get_token(self, user_id):
        usernya = self._db.users.find_one({"_id": user_id})
        if usernya:
            return usernya.get("token")
        else:
            return None
    ...
    def rem_token(self, user_id):
        self._db.users.update_one(
            {"_id": user_id}, {"$unset": {"token": ""}}, upsert=True
        )
