...
class ProjectRPC():
    ...
    def init(board, framework, project_dir,example_code,env="testdemo",env_image="denkchan/demo:latest"):
        # print(project_dir)
        assert project_dir
        if not os.path.isdir(project_dir):
            # print(project_dir)
            os.makedirs(project_dir)
        # args = ["init","-e","testdemo","--project-option","platform=kendryte210","--project-option","env_image=denkchan/demo:latest"]
        # args = ["init", "-e","testdemo","--project-option","boards=%s" % board, "--sample-code","--project-option","env_image=denkchan/demo:latest"]
        args = ["-e",env]
        if board:
        ...
        if env_image:
            args.extend(["--project-option", "env_image = %s" % env_image])
        ...
        # )
        # example_code = "https://gitee.com/SuperIDE/examplecode.git"
        # if example_code:
        #     args.extend(["--sample-code", "example_code = %s" % example_code])
        try:
