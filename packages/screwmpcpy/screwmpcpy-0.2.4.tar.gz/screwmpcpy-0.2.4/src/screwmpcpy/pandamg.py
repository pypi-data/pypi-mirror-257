...
class PandaScrewMpMotionGenerator(PandaScrewMotionGenerator):
    ...
    :param rho_i: Influence distance :math:`rho_i`, defaults to 0.9 rad.
    :type rho_i: float, optional.
    :param rho_s: Stopping distance :math:`rho_s`, defaults to 0.05 rad.
    ...
    :param damping_gain: The joint velocity damping gain used for inequality constraints. If negative or zero,
        ...
        ignored in the quadratic program. Defaults to 1.0.
    ...
    def _get_b_ineq(self, q_robot: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:  # noqa: UP006
        ...
        distu = upper - q_robot
        distl = q_robot - lower
        dist = np.where(distu > distl, distu, distl)
    ...
    def _get_q(self, n_dof: int, joint_gain: float, slack_gain: float) -> np.ndarray:
        ...
        q_mat[:n_dof, :n_dof] *= joint_gain
        q_mat[n_dof:, n_dof:] *= slack_gain
        return q_mat
    ...
    def _get_dq(self, dq_desired: np.ndarray, q_robot: np.ndarray) -> np.ndarray:
        ...
        c_vec = self._get_c_vec(np.squeeze(self._panda_rtb.jacobm(q_robot, jacobian)))
