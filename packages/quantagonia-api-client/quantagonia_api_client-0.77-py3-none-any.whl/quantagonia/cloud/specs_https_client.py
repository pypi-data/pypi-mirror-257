...
class SpecsHTTPSClient():
    ...
    def _getFileContentFromUrl(self, e):
        if type(e) is dict and  "error" in e:
            ...
            return requests.get(e["url"]).text
        else:
            return e
    ...
    def _getCurrentSolution(self, jobid: uuid) -> str:
        ...
        array = self._jsonArrayToPythonArray(json.loads(response.text))
        for e in array:
    ...
    def _getCurrentLog(self, jobid: uuid) -> str:
        ...
        if not response.ok:
            error_report = response.json()
            raise RuntimeError(error_report)
        raw_array = self._jsonArrayToPythonArray(json.loads(response.text))
        result_array = []
        for e in raw_array:
            result_array.append(self._getFileContentFromUrl(e))
        return result_array
