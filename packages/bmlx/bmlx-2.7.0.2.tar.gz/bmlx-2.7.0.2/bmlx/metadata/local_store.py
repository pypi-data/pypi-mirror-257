...
class LocalStore(object):
    ...
    def get_experiments(
        ...
        raise NotImplementedError(
            "Local model does not support get experiment")
    ...
    def create_or_update_component_execution(
        ...
        for input_artifact in input_artifacts:
            input_artifact.external_id = (
                f"{input_artifact.type_id}-{input_artifact.name}"
            )
            input_artifact.name = (
            ...
            if existed_artifact:
                input_artifact.id = 0
                input_artifact.external_id = existed_artifact.external_id
        ...
        for output_artifact in output_artifacts:
            ...
            output_artifact.external_id = (
                f"{output_artifact.type_id}-{output_artifact.name}"
            )
            ...
            if same_uri_artifacts:
                ...
                if target.fingerprint == output_artifact.fingerprint:
                    output_artifact.id = 0
                    output_artifact.external_id = target.external_id
            else:
                existed_artifact = self._ml_store.get_artifact_by_type_and_name(
                ...
                if existed_artifact:
                    output_artifact.id = 0
                    output_artifact.external_id = existed_artifact.external_id
