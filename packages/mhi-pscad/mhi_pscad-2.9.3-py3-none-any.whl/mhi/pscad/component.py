    ...
    def import_parameters(self, csvfile: str) -> None:
        ...
        def to_param_name(label: str) -> str:
            ...
            if not name.isidentifier():
                raise ValueError(r"Invalid column label: {label}")
        ...
        with open(csvfile, newline='') as file:
            ...
            headers = next(reader)
            values = next(reader)
            if len(headers) != len(values) or next(reader, None) != None:
...
class UserCmp(Component, ModuleMixin):
    ...
    definition = rmi_property(True, True, name="definition",
    ...
    def blackbox(self, x: int = None, y: int = None,
                 sub_prefix: str = None, instance_data: bool = None
                 ) -> "Component":
        """
        ...
        if args or kwargs:
            raise NotImplementedError("Blackbox arguments require version >= 5.0.2")
