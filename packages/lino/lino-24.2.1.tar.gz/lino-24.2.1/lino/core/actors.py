...
class Actor(actions.Parametrizable, Permittable, metaclass=ActorMetaClass):
    """
    The base class for all actors (:term:`data view`).  Subclassed by
    ...
    def get_simple_parameters(cls):
        if isinstance(cls.model, type) and issubclass(cls.model,
                                                      fields.TableRow):
    ...
    def get_table_as_list(cls, obj, ar):
        ...
        for i, obj in enumerate(sar.data_iterator):
            ...
            par = '<p class="clearfix">{}</p>'.format(par)
            # url = sar.obj2url(obj)
            # if url is not None:
            #     url = html.escape(url)
...
def resolve_action(spec, action=None):
    ...
    if isinstance(spec, type) and issubclass(spec, Actor):
        if action:
            ...
            #~ print 20121210, a
            if a is None:
                raise Exception("resolve_action(%r, %r) found None" %
                                (spec, action))
        else:
