...
g_interval = 6      # how often to poll for CPU usage

def CPU_thread(window:sg.Window):
    # window.write_event_value('-THREAD STARTED-', None)

    while True:
        cpu_percent = psutil.cpu_percent(interval=g_interval)
        procs = psutil.process_iter()
        print(f'updating with {cpu_percent}')
...
def main():
    ...
    layout = [[sg.Text(font=('Helvetica', 20), text_color=sg.YELLOWS[0], key='-CPU PERCENT-')],
              ...
              [sg.Text('Update every '), sg.Spin([x+1 for x in range(10)], g_interval, key='-SPIN-'), sg.T('seconds')]]

    window = sg.Window('Top CPU Processes', layout, no_titlebar=True, keep_on_top=True,location=location, use_default_focus=False, alpha_channel=.8, grab_anywhere=True, right_click_menu=sg.MENU_RIGHT_CLICK_EDITME_VER_EXIT, enable_close_attempted_event=True, finalize=True)
