...
#    print(f"{e}")
    ...
    if browser == "chrome":
        proc = subprocess.Popen([chrome_browser_path, url]+options,stdout=subprocess.DEVNULL,
    stderr=subprocess.DEVNULL,
    close_fds=True)
    elif browser == "edge":
        proc = subprocess.Popen([edge_browser_path, url]+options,stdout=subprocess.DEVNULL,
    stderr=subprocess.DEVNULL,
    close_fds=True)
    time.sleep(2)
def tear_down():
