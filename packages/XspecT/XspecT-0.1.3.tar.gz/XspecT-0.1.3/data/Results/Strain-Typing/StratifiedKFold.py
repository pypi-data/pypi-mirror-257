...
def get_read_data(file, dic, keys):
    ...
    return (
        np.array(vecs_forwards),
        np.array(labels_forwards),
        np.array(vecs_reverse),
        np.array(labels_reverse),
    )
...
def create_plots():
    ...
    ICs_assembly_full = get_scores(
        "Assemblys as reference, all k-mers.csv", ICs_assembly, keys
    )
    ICs_core_full = get_scores(
        "Core-Genome as reference, all k-mers.csv", ICs_core, keys
    )
    # Quick-run: only every 10th k-mer is used
    ICs_assembly_quick = get_scores(
        "Assemblys as reference, quick run.csv", ICs_assembly, keys
    )
    ICs_core_quick = get_scores(
        "Core-Genome as reference, quick run.csv", ICs_core, keys
    )
    ...
    for key in keys:
        ...
        assembly_quick_label = assembly_quick_label + [key] * len(
            ICs_assembly_quick[key]
        )
    ...
    assembly_full, assembly_full_label = np.array(assembly_full), np.array(
        assembly_full_label
    )
    ...
    print(
        "Please Note: Data from Quick-run (only every 10th k-mer is tested) is used for testing only, meaning that the"
    ...
    print(
        "StratifiedKFold (n=10): Assembled Genomes as reference, all k-mers are tested, polynomial SVM-Kernel"
    )
    acc = []
    for train_index, test_index in skf.split(assembly_full, assembly_full_label):
        ...
        y_train, y_test = (
            assembly_full_label[train_index],
            assembly_full_label[test_index],
        )
    ...
    print(
        "StratifiedKFold (n=10): Assembled Genomes as reference, all k-mers are tested, linear SVM-Kernel"
    )
    acc = []
    for train_index, test_index in skf.split(assembly_full, assembly_full_label):
        X_train, X_test = assembly_full[train_index], assembly_full[test_index]
    ...
    print(
        "StratifiedKFold (n=10): Assembled Genomes as reference, only every 10th k-mer is tested, polynomial SVM-Kernel"
    )
    acc = []
    for train_index, test_index in skf.split(assembly_full, assembly_full_label):
        X_train, X_test = assembly_full[train_index], assembly_full[test_index]
        ...
        acc.append(
            get_cm_and_accuracy(
                X_train, y_train, assembly_quick, assembly_quick_label, "poly"
            )
        )
    ...
    print(
        "StratifiedKFold (n=10): Assembled Genomes as reference, only every 10th k-mer is tested, linear SVM-Kernel"
    )
    acc = []
    for train_index, test_index in skf.split(assembly_full, assembly_full_label):
        X_train, X_test = assembly_full[train_index], assembly_full[test_index]
        ...
        acc.append(
            get_cm_and_accuracy(
                X_train, y_train, assembly_quick, assembly_quick_label, "linear"
            )
        )
    print_stats(acc)
    ...
    print(
        "StratifiedKFold (n=10): Core Genome as reference, all k-mers are tested, polynomial SVM-Kernel"
    )
    acc = []
    ...
    print(
        "StratifiedKFold (n=10): Core Genome as reference, all k-mers are tested, linear SVM-Kernel"
    )
    acc = []
    for train_index, test_index in skf.split(core_full, core_full_label):
        X_train, X_test = core_full[train_index], core_full[test_index]
    ...
    print(
        "StratifiedKFold (n=10): Core Genome as reference, only every 10th k-mer is tested, polynomial SVM-Kernel"
    )
    acc = []
    for train_index, test_index in skf.split(core_full, core_full_label):
        X_train, X_test = core_full[train_index], core_full[test_index]
        ...
        acc.append(
            get_cm_and_accuracy(X_train, y_train, core_quick, core_quick_label, "poly")
        )

    print_stats(acc)

    print(
        "StratifiedKFold (n=10): Core Genome as reference, only every 10th k-mer is tested, linear SVM-Kernel"
    )
    acc = []
    for train_index, test_index in skf.split(core_full, core_full_label):
        X_train, X_test = core_full[train_index], core_full[test_index]
        ...
        acc.append(
            get_cm_and_accuracy(
                X_train, y_train, core_quick, core_quick_label, "linear"
            )
        )
    print_stats(acc)
    ...
    forward, forward_label, reverse, reverse_label = get_read_data(
        "Reads as Input, Assembly as Reference.csv", ICs_assembly, keys
    )

    print(
        "StratifiedKFold (n=10): Assembled genomes as reference, unmodified forward reads"
    )
    acc = []
    for train_index, test_index in skf.split(assembly_full, assembly_full_label):
        X_train, X_test = assembly_full[train_index], assembly_full[test_index]
        ...
        acc.append(
            get_cm_and_accuracy(X_train, y_train, forward, forward_label, "poly")
        )

    print_stats(acc)

    print(
        "StratifiedKFold (n=10): Assembled genomes as reference, unmodified reverse reads"
    )
    acc = []
    for train_index, test_index in skf.split(assembly_full, assembly_full_label):
        X_train, X_test = assembly_full[train_index], assembly_full[test_index]
        ...
        acc.append(
            get_cm_and_accuracy(X_train, y_train, reverse, reverse_label, "poly")
        )

    print_stats(acc)

    forward, forward_label, reverse, reverse_label = get_read_data(
        "Reads as Input, Assembly as Reference, 15% of the nucleotides changed.csv",
        ICs_assembly,
        keys,
    )
    print(
        "StratifiedKFold (n=10): Assembled genomes as reference, forward reads with 15% randomly changed nucleotides"
    )
    acc = []
    for train_index, test_index in skf.split(assembly_full, assembly_full_label):
        X_train, X_test = assembly_full[train_index], assembly_full[test_index]
    ...
    print(
        "StratifiedKFold (n=10): Assembled genomes as reference, reverse reads with 15% randomly changed nucleotides"
    )
    acc = []
    for train_index, test_index in skf.split(assembly_full, assembly_full_label):
        X_train, X_test = assembly_full[train_index], assembly_full[test_index]
    ...
    forward, forward_label, reverse, reverse_label = get_read_data(
        "Reads as Input, Assembly as Reference.csv", ICs_core, keys
    )
    ...
    print(
        "StratifiedKFold (n=10): Core Genome as reference, forward reads with 15% randomly changed nucleotides"
    )
    acc = []
    for train_index, test_index in skf.split(core_full, core_full_label):
        X_train, X_test = core_full[train_index], core_full[test_index]
    ...
    print(
        "StratifiedKFold (n=10): Core Genome as reference, reverse reads with 15% randomly changed nucleotides"
    )
    acc = []
    for train_index, test_index in skf.split(core_full, core_full_label):
        X_train, X_test = core_full[train_index], core_full[test_index]
