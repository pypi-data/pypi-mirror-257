...
def kpoint_plot(data, num_atoms):
    ...
    for i, encut in enumerate(unique_encut):
        ...
        gradient = np.gradient(np.abs(energy_per_atom), encut_data[r"\rho_k [A^-3]"])
        ...
        grad_ax.spines["right"].set_color("red")
        grad_ax.yaxis.label.set_color("red")
        grad_ax.tick_params(axis="y", colors="red")

    fig.tight_layout()
    fig.savefig("kpoint_convergence.pdf", dpi=300)


def cutoff_plot(data, num_atoms):
    ...
    for i, rho in enumerate(unique_rho):
        ...
        gradient = np.gradient(np.abs(energy_per_atom), rho_data[r"Cutoff [eV]"])
