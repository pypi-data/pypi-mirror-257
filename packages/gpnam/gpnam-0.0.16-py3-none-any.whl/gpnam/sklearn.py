...
from .utils import process_in_chunks, check_numpy, sigmoid, ToTensor		 
...
class GPNAM(object):
	...
	def __init__(
		...
		if self.problem == 'classification':
			...
			self.model_wrapper = GPNAMRegressor(self.input_dim, self.kernel_width, device=self.device)
		else:
...
class GPNAMClassifier(object):
	...
	def predict(self, X):
		...
		with torch.no_grad():
			logits = process_in_chunks(self.model, X, batch_size=self.batch_size)
