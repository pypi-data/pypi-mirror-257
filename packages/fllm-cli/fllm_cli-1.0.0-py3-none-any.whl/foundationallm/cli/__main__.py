# --------------------------------------------------------------------------------------------
# Copyright (c) FoundataionaLLM. All rights reserved.
...
from foundationallm.cli.core.intercept_survey import prompt_survey_message
...
try:
    ...
    try:
        ...
        if exit_code == 0 and az_cli.config.getboolean('auto-upgrade', 'enable', False) and \
            ...
            from foundationallm.cli.core._session import VERSIONS  # pylint: disable=ungrouped-imports
            from foundationallm.cli.core.util import get_cached_latest_versions, _VERSION_UPDATE_TIME  # pylint: disable=ungrouped-imports
            if VERSIONS[_VERSION_UPDATE_TIME]:
                ...
                if parse(VERSIONS['versions']['core']['local']) < parse(VERSIONS['versions']['core']['pypi']):  # pylint: disable=line-too-long
                    import subprocess
                    import platform
                    from foundationallm.cli.core.azclierror import UnclassifiedUserFault  # pylint: disable=ungrouped-imports
                    logger.warning("New FLLM CLI version available. Running 'fllm upgrade' to update automatically.")
                    update_all = az_cli.config.getboolean('auto-upgrade', 'all', True)
                    prompt = az_cli.config.getboolean('auto-upgrade', 'prompt', True)
                    cmd = ['fllm', 'upgrade', '--all', str(update_all)]
                    ...
                    if prompt:
                        ...
                        try:
                            ...
                            err_msg = "Unable to prompt for auto upgrade as no tty available. " \
                                "Run 'fllm config set auto-upgrade.prompt=no' to allow auto upgrade with no prompt."
