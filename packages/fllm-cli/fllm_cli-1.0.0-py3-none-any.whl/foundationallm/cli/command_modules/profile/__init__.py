...
from foundationallm.cli.core.commands.parameters import get_enum_type

from foundationallm.cli.command_modules.profile._format import transform_account_list
import foundationallm.cli.command_modules.profile._help  # pylint: disable=unused-import
...
class ProfileCommandsLoader(AzCommandsLoader):
    ...
    def load_command_table(self, args):
        ...
        profile_custom = CliCommandType(
            operations_tmpl='foundationallm.cli.command_modules.profile.custom#{}'
        )
    ...
    def load_arguments(self, command):
        from foundationallm.cli.core.api import get_subscription_id_list
        ...
        with self.argument_context('login') as c:
            ...
            c.argument('allow_no_subscriptions', action='store_true', help="Support access tenants without subscriptions. It's uncommon but useful to run tenant level commands, such as 'fllm ad'")
            ...
            c.argument('scopes', options_list=['--scope', '-s'], nargs='+', help='Used in the /authorize request. It can cover only one static resource.')
            c.argument('resource', options_list=['--resource','-r'], help='The resource to get.')
            c.argument('client_id', options_list=['-c'], help='The client id to use.')
