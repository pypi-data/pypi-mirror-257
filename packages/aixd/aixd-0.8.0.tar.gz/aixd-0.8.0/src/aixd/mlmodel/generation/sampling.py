...
from aixd.sampler.sampler_definitions import sampler_bayesian_kde, sampler_conditional_kde
...
class GenSampler:
    ...
    sampling_type : str, optional, default="sampling"
        ...
        and `sampling`. `Sampling` is the default as it is faster, using a KDE to fitted on
        the training data. `Bayesian` is more time consuming, as it uses an objective to
    ...
    def __init__(self, sampling_type: str = "sampling", datamodule: DataModule = None, engine: str = "sobol") -> None:
        self.sampling_type = sampling_type
        self.datamodule = datamodule
        self.engine = engine

        if self.sampling_type == "sampling" and self.engine not in ["sobol", "lhc", "uniform"]:
            self.engine = "sobol"

        if datamodule is None:
            print("* You need to provide a datamodule")
...
* The sampling types are:
    - sampling (default): fitting a KDE and sampling, rejecting those that do not fall in the conditions
...
* Engine:
    ...
    def generate_z(
        ...
        epsilon_sampler : float, optional (default=0.05)
            When using "sampling" method, for specific requests, in reality we create a range
        ...
        over_generation : int, optional (default=10)
            When using "bayesian" or "sampling" method, the number of samples generated is
        ...
        for ind, y_s in enumerate(y_req):
            while True:
                ...
                if len(y_z):
                    ...
                    print("Creating non conditioned z")
                    ...
                    print("Increasing the epsilon for the sampler to {:.3f}".format(epsilon_sampler))
    ...
    def _get_domains_perdim(self, data: np.ndarray, dobj: List[DataObject]) -> List:
        ...
        for ind, att in enumerate(dobj):
            ...
            if att.domain.domain_type == "Interval":
                list_dom.append([np.min(data[:, dim_ind[0] : dim_ind[1]]), np.max(data[:, dim_ind[0] : dim_ind[1]])])
            elif att.domain.domain_type == "Options":
                list_dom.append(list(np.unique(data[:, dim_ind[0] : dim_ind[1]])))
        return list_dom
    ...
    def _update_domain_dummy(self, list_dom: List, dobj: List[DataObject]) -> List[DataObject]:
        for ind, att in enumerate(dobj):
            if att.domain.domain_type == "Interval":
                ...
                dobj[ind].domain.array = list_dom[ind][1]
        return dobj
    ...
    def generate(
        ...
        assert len(y_req) == len(attributes_req), "Number of attributes on which to condition is correct"

        # Organize the requested values. A list of lists, of dim 1 or 2
        ...
        count_it = 1
        max_it = 10
        if self.sampling_type in ["bayesian", "sampling"]:
            # !! We sample in the original domain, as it is more beneficial, mostly for the categorical values
            # !! Does not work for transformations with no inverse
            y_train_unnorm = self.datamodule.output_ml_dblock.unnormalize(self.datamodule.y_train)
            y_train, _ = self.datamodule.output_ml_dblock.inverse_transform(y_train_unnorm)
            if request_orig_range:
                y_req_norm = self._norm_unnorm_y_req(y_req, ids_att, flag_range, request_orig_range)
            else:
                y_req_norm = copy.copy(y_req)
                y_req = self._norm_unnorm_y_req(y_req, ids_att, flag_range, request_orig_range)

            # Definition of objective given the attributes and requested values

            y_samp_unnorm = np.array([])
            ...
            while True:
                ...
                if self.sampling_type == "bayesian":
                    self.engine = "bayesian"
                    objective = self._create_objective(dobj_req, y_req)
                    # Declaring the sampler, with some objective and Bayesian optimization
                    sampler = sampler_bayesian_kde(datablock.dobj_list, self.engine, objective, condition, y_train)
                elif self.sampling_type == "sampling":
                    # TODO perhaps Bayesian with conditioning
                    # We just sampled from the KDE distribution, and only take the samples
                    # that satisfy some condition. This condition is similar to the objective
                    sampler = sampler_conditional_kde(datablock.dobj_list, self.engine, condition, y_train)

                # Sampling from distribution
                y_samp_aux = sampler.generate(n_samples, output_type="numpy", over_generation=over_generation, verbose=False)
                if len(y_samp_aux):
                    y_samp_unnorm = np.vstack([y_samp_unnorm, y_samp_aux]) if len(y_samp_unnorm) else y_samp_aux
                count_it += 1
                if len(y_samp_unnorm) > n_samples * 0.9:
                    break
                elif count_it > max_it:
                    print("Interrupting the generation with only {} generated samples out of  {}".format(len(y_samp_unnorm), n_samples))
                else:
                    epsilon_sampler += epsilon_sampler * 0.2
                    print("Not enough samples generated. Trying again with an epsilon for the sampler of {:.3f}".format(epsilon_sampler))

            y_samp_unnorm = y_samp_unnorm[:n_samples, :]
            y_samp_unnorm = self._set_req_value(y_samp_unnorm, y_req, ids_att, flag_range)

            # TODO still unnormalize, and see how to pass and use the y_req
            y_samp, _ = self.datamodule.output_ml_dblock.transform(y_samp_unnorm)
            y_samp_norm = self.datamodule.output_ml_dblock.normalize(y_samp)

            return to_torch(y_samp_norm, torch.float32), y_req_norm

        else:
            if self.sampling_type == "around":
                raise Exception("* Sampling type 'around' cannot be used with ranges. Use 'bayesian' or 'sampling' instead")
            else:
                raise Exception("* Sampling type '{}' not implemented".format(self.sampling_type))
    ...
    def _norm_unnorm_y_req(self, y_req: List[Union[int, float, bool, str, List]], ids_att: List[int], flag_range: bool, request_orig_range: bool) -> List:
        """
        ...
        norm_unorm_y_func = self.datamodule.normalize_y if request_orig_range else self.datamodule.unnormalize_y

        list_attr = self.datamodule.output_ml_dblock.columns_df_transf
        if not flag_range:
            y_req = np.asarray(y_req)
            y_req_aux = y_req.reshape(1, -1)
            y_n_un = np.zeros((1, len(list_attr)))
            y_n_un[:, ids_att] = y_req_aux
            y_n_un = norm_unorm_y_func(y_n_un)[:, ids_att].transpose()
            y_req = np.asarray(y_n_un).reshape(y_req.shape)
        else:
            y_vec_n_un = []
            for ind, req in enumerate(y_req):
                y_n_un = np.zeros((len(req), len(list_attr)))
                y_n_un[:, ids_att[ind]] = np.asarray(req).reshape(
                    -1,
                )
                y_n_un = norm_unorm_y_func(y_n_un)[:, ids_att[ind]].transpose()
                y_vec_n_un.append(list(np.asarray(y_n_un).reshape(len(req))))
            y_req = y_vec_n_un
        return y_req
    ...
    def _create_condition(self, attributes: List[str], y_req: List[Union[int, float, bool, str, List]], epsilon_sampler: float, list_dom: List[List] = None) -> Arithmetic:
        ...
        for ind, att in enumerate(attributes):
            ...
            if len(y_req_i) == 1 and att.domain.domain_type == "Interval":
            ...
            if att.domain.domain_type == "Interval":
                ...
                vec_conds.append(self._condition_fix_val(att.name, range_vec))
