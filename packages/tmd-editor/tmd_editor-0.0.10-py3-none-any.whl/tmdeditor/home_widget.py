...
from tmdeditor.func_scene import AllSceneFunctions, InvalidFile
from tmdeditor.func_node import AllNodeFunctions
from tmdeditor.func_edge import AllEdgeFunctions, EDGE_TYPE_BEZIER
from tmdeditor.graphical_view import DrawGraphicalView
...
class NodeEditorWidget(QWidget):
    ...
    def createHomeWidget(self):
        """Set up this ``NodeEditorWidget`` with its layout,  :class:`~tmdeditor.node_scene.Scene` and
        :class:`~tmdeditor.node_graphics_view.QDMGraphicsView`"""
    ...
    def hasSelectedItems(self) -> bool:
        """Is there something selected in the :class:`tmdeditor.node_scene.Scene`?
    ...
    def addNodes(self):
        ...
        edge1 = AllEdgeFunctions(self.scene, node1.outputs[0], node2.inputs[0], edge_type=EDGE_TYPE_BEZIER)
        edge2 = AllEdgeFunctions(self.scene, node2.outputs[0], node3.inputs[0], edge_type=EDGE_TYPE_BEZIER)
        edge3 = AllEdgeFunctions(self.scene, node1.outputs[0], node3.inputs[2], edge_type=EDGE_TYPE_BEZIER)
