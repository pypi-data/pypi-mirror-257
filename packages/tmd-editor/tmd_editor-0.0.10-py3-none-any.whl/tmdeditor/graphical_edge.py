...
from tmdeditor.graphical_edge_types import GraphicsEdgePathBezier, GraphicsEdgePathDirect, GraphicsEdgePathSquareLine
...
class DrawGraphicalEdge(QGraphicsPathItem):
    ...
    def __init__(self, edge:'Edge', parent:QWidget=None):
        """
        :param edge: reference to :class:`~tmdeditor.node_edge.Edge`
        ...
        :Instance attributes:
            ...
            - **edge** - reference to :class:`~tmdeditor.node_edge.Edge`
    ...
    def drawingAssets(self):
        ...
        if self.edge.start_socket and self.edge.start_socket.position == 8:
            self._color = QColor("#FF0000")  # Red for input edges
        elif self.edge.start_socket and self.edge.start_socket.position == 9:
            self._color = QColor("#00FF00")  # Green for output edges
    ...
    def createEdgePathCalculator(self):
        """Create instance of :class:`~tmdeditor.node_graphics_edge_path.GraphicsEdgePathBase`"""
        self.pathCalculator = self.determineEdgePathClass()(self)
    ...
    def determineEdgePathClass(self):
        ...
        from tmdeditor.func_edge import EDGE_TYPE_BEZIER, EDGE_TYPE_DIRECT, EDGE_TYPE_SQUARE
        ...
        if self.edge.edge_type == EDGE_TYPE_SQUARE:
            return GraphicsEdgePathSquareLine
        else:
            return GraphicsEdgePathBezier
    ...
    def paint(self, painter, QStyleOptionGraphicsItem, widget=None):
        """Qt's overridden method to paint this Graphics Edge. Path calculated
            in :func:`~tmdeditor.node_graphics_edge.QDMGraphicsEdge.calcPath` method"""
    ...
    def calcPath(self) -> QPainterPath:
        ...
        is an instance of derived :class:`~tmdeditor.node_graphics_edge_path.GraphicsEdgePathBase` class
