...
from tmdeditor.graphical_scene import DrawGraphicalScene
from tmdeditor.func_node import AllNodeFunctions
from tmdeditor.func_edge import AllEdgeFunctions
from tmdeditor.func_scene_history import AllSceneHistoryFunctions
from tmdeditor.func_scene_clipboard import AllSceneClipboardFunctions
...
class AllSceneFunctions(Serializable):
    ...
    def __init__(self):
        ...
        :Instance Attributes:
            ...
            - **history** - Instance of :class:`~tmdeditor.node_scene_history.SceneHistory`
            - **clipboard** - Instance of :class:`~tmdeditor.node_scene_clipboard.SceneClipboard`
    ...
    def addNode(self, node: AllNodeFunctions):
        """Add :class:`~tmdeditor.node_node.Node` to this `Scene`

        :param node: :class:`~tmdeditor.node_node.Node` to be added to this `Scene`
        :type node: :class:`~tmdeditor.func_node.Node`
        """
        self.nodes.append(node)

    def addEdge(self, edge: AllEdgeFunctions):
        """Add :class:`~tmdeditor.node_edge.Edge` to this `Scene`

        :param edge: :class:`~tmdeditor.node_edge.Edge` to be added to this `Scene`
        :return: :class:`~tmdeditor.node_edge.Edge`
        """
        self.edges.append(edge)

    def removeNode(self, node: AllNodeFunctions):
        """Remove :class:`~tmdeditor.node_node.Node` from this `Scene`

        :param node: :class:`~tmdeditor.node_node.Node` to be removed from this `Scene`
        :type node: :class:`~tmdeditor.func_node.Node`
        """
        if node in self.nodes: self.nodes.remove(node)
        else:
            if DEBUG_REMOVE_WARNINGS: print("!W:", "Scene::removeNode", "wanna remove tmdeditor", node,
                                            "from self.nodes but it's not in the list!")

    def removeEdge(self, edge: AllEdgeFunctions):
        """Remove :class:`~tmdeditor.node_edge.Edge` from this `Scene`

        :param edge: :class:`~tmdeditor.node_edge.Edge` to be remove from this `Scene`
        :return: :class:`~tmdeditor.node_edge.Edge`
        """
    ...
    def loadFromFile(self, filename: str):
        ...
        :type filename: ``str``
        :raises: :class:`~tmdeditor.node_scene.InvalidFile` if there was an error decoding JSON file
        """
    ...
    def setNodeClassSelector(self, class_selecting_function: 'functon') -> 'Node class type':
        ...
        If not set, we will always instantiate :class:`~tmdeditor.node_node.Node` for each `Node` in the `Scene`
