...
import re
from ..utils.dataset import SIndDLinearDataset
from ..model.sind import SIndDLinear


def train_sind_dlinear(model, data_train, data_valid, batch_size, epoch, lr, device, tune_lr_every, gamma, decay=0.001, verbose=10):
...
def training_pipeline(root):
    ...
    data_train = SIndDLinearDataset(cov_obs, panel_obs, treatment_obs, panel_100_obs[['Y']], t0)
    data_valid = SIndDLinearDataset(cov_exp, panel_exp, treatment_exp, panel_100_exp[['Y']], t0)
    ...
    kernel_size = 5
    lr = 5e-5 
    model = SIndDLinear(10, surrogate_num * t0,kernel_size=kernel_size).to(device)
    ...
    = train_sind_dlinear(model, data_train, data_valid, batch_size, epoch, tune_lr_every=tune_lr_every, gamma=gamma, lr=lr, device=device)
