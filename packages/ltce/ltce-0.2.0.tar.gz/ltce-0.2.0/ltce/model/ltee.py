    ...
    def forward(self, X, Y, eps=None, its=None):
        ...
        for i in range(min(seq_len, self.max_len)):
            ...
            with torch.no_grad():
                ...
                # cal a, b
                a = torch.ones([nx]).to(X.device) / nx
                b = torch.ones([ny]).to(X.device) / ny
                v = torch.ones([ny]).to(X.device)
                for _ in range(its):
...
class LTEE(nn.Module):
    ...
    def train_model(self, train_dataset, test_dataset, epoch, lr, batch_size, lbd, decay=0.001, opt='Adam', tune_lr_every=20, gamma=0.1, verbose=10, device='cuda' if torch.cuda.is_available() else 'cpu'):
