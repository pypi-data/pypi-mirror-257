...
from ltyp import notstrbytes as NotStrBytes
...
# %% ../nbs/12_iter.ipynb 8
def isiternotstrbytes(x) -> TypeGuard[Iterable[NotStrBytes]]:
...
def isiternotstr(x) -> TypeGuard[Iterable[NotStrBytes]]:
    '''Checks if `x` is an iterable containing elements other than strings or bytes.
...
def iternotstr(x) -> TypeGuard[Iterable[NotStrBytes]]:
    '''Checks if `x` is an iterable containing elements other than strings or bytes.
