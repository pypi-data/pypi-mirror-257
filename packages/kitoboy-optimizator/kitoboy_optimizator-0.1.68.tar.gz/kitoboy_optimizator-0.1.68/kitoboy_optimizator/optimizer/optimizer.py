...
class Optimizer:
    ...
    async def process_results(self):
        ...
        if self.pass_backtest_results_filter(backtest_results):
            report += f"\nBacktest results PASSED! (Profit:{backtest_results.net_profit} Drawdown: {backtest_results.max_drawdown})"
            if self.start_forwardtest_after_optimization:
                ...
                if self.pass_forwardtest_results_filter(forwardtest_results):
                    ...
                    report += f"\nForwardtest results PASSED! (Profit:{forwardtest_results.net_profit} Drawdown: {forwardtest_results.max_drawdown})"
                else:
                    report += f"\nForwardtest results NOT PASSED!(Profit:{forwardtest_results.net_profit} Drawdown: {forwardtest_results.max_drawdown})"
                    return report
            else:
                await self.create_and_save_params_txt(best_params, result_id)
                self.reporter.backtest_results_report(
