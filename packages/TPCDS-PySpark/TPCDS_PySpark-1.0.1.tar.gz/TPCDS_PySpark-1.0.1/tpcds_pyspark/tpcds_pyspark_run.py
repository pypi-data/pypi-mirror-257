...
usage = """
tpcds_pyspark.py - A TPCDS workload generator using Apache Spark, Python and instrumented with sparkMeasure
Use it to:
- Run the TPCDS benchmark with PySpark
- Collect and analyze Spark performance data, including timing and metrics measurements
- Learn about Spark performance and optimization
- Compare different Spark configurations and versions

Contact: Luca.Canali@cern.ch - Jan 2024
...
spark-submit --conf spark.driver.extraClassPath=./spark-measure_2.12-0.23.jar \
...
spark-submit --master yarn --conf spark.log.level=error  --conf spark.executor.cores=8 --conf spark.executor.memory=64g
             --conf spark.driver.memory=16g --conf spark.driver.extraClassPath=tpcds_pyspark/spark-measure_2.12-0.23.jar
             --conf spark.dynamicAllocation.enabled=false --conf spark.executor.instances=4 
             --conf spark.sql.shuffle.partitions=50 
             tpcds_pyspark_run.py -d tpcds_10 -o ./tpcds_10.csv-n 1 -r 1 
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cluster_output_file", action = 'store', required=False, help="Optional, save the collected metrics to a csv file using Spark, use this to save to HDFS or S3", default=None)
    ...
    tpcds = TPCDS(data_path, data_format, num_runs, queries_repeat_times, queries, sleep_time)
    ...
    if create_metastore_tables or create_metastore_tables_and_compute_statistics:
        ...
        if create_metastore_tables_and_compute_statistics:
            ...
            tpcds.compute_table_statistics()
    ...
    if run_using_metastore:
        ...
        print(f"spark.sql.statistics.histogram.enabled = {hist}")
    ...
    # It runs only if args.output_file is specified
    # The file storing the metrics is args.output_file
    # The file storing configuration is args.output_file + ".metadata"
    # The  file storing summary statistics is args.output_file + ".summary"
    if args.output_file:
    ...
    # Note that the file will be saved in a folder, with the name of the file being the folder name
    # Note the Python version on the executors and the driver must be the same
    if args.cluster_output_file:
        tpcds.save_with_spark(results, args.cluster_output_file)
