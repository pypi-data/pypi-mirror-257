...
class Map:
    ...
    def clip_layers(self, layers):
        if self.mask:
            ...
            transformed_layers = [layer.to_crs(self.epsg) for layer in layers]
            [layer.clip(mask) for layer in transformed_layers]
        else:
            [layer.to_crs(self.epsg) for layer in layers]
