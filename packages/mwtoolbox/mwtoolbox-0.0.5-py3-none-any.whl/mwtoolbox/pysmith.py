...
def get_smith(fig, rect = 111, plot_impedance = True, plot_ticks = False, plot_admittance = False, plot_labels = False):
    ...
    num_thetas = 4#*3 #12 gives in 30 deg intervals, 8 in 45 deg, 24 in 15deg
    thetas = np.linspace(0.0, math.pi*(1-1.0/num_thetas),num_thetas)
    angle_ticks = []#(0, r"$0$"),
    for theta in thetas:
        angle_info = []
    ...
    thetas2 = np.linspace(math.pi, 2*math.pi*(1-0.5/num_thetas), num_thetas)
    angle_ticks2 = []#(0, r"$0$"),
