...
def getindiceordered(cc, cumsumarray):
    """ Faster alternative to getindice, but works only with array
    of node coordinates instead of array of cell lengths. This
    array can be generated by cumsum function of numpy. The 
    coordinate of starting node (0) is not included in this array.
...
def waveguide(boxwidth, boxheight, dielthickness, epsilon, freq, pmllayers, xres, yres):
    """
    :type dielthickness: tabandan başlayarak yukarı dogru sirasiyla dielektriklerin kalinliklarinin listesi
    :type epsilon: tabandan başlayarak yukarı dogru sirasiyla dielektriklerin katsayilarinin listesi
...
def plot_field(ekran, dx, dy, v, parts, pmllayers, surface=True, comp="Ex"):
    ...
    if surface:
        ...
        if plot_field.cb:
            ...
            plot_field.cb = figure.colorbar(temp, cax=plot_field.cb.ax)
        else:

            plot_field.cb = figure.colorbar(temp)
    else:
        if comp=="E_t":
            nz.autoscale(magE)
