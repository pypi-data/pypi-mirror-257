...
def get_line_schedule(line: str) -> None:
    '''Collects the bus schedule for a specific bus line and
    saves it in a json file
    '''

    bus_line_dir = os.path.join('bus_data/bus_lines')
    ...
    for route in bus_routes['result'][line]:
        for stop in bus_routes['result'][line][route]:
            ...
            response = helper_functions.fetch_data(params, url)
...
def collect_bus_schedules(apikey: str) -> None:
    ...
    bus_lines_dir = os.path.join('bus_data', 'bus_lines')
