...
def get_lines(data_set: int, theater_coords: tuple) -> Tuple[list, list]:
...
def plot_theaters_map(data_set: int) -> None:
    ...
    try: 
        with open(f'theaters_data/theaters.json', 'r') as file:
    ...
    for theater in theaters_data:
        ...
        lines_theater.extend(get_lines(data_set, (longitude, latitude))[0])
        lines.extend(get_lines(data_set, (longitude, latitude))[1])
    ...
    plt.savefig(f'plots/plots_{data_set}/Theater_plot_{data_set}.png')
