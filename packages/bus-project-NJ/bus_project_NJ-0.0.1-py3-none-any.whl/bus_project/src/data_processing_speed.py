...
def draw_map(data_set: int) -> None:
    ...
    '''
    helper_functions.check_validity_of_function_call(data_set)
    helper_functions.create_directory(data_set)
    ...
    gdf = helper_functions.read_geojson('warszawa-dzielnice.geojson')
    ...
    plt.savefig(
        os.path.join(
            f'plots/plots_{data_set}',
            f'Warsaw_map_{data_set}.png'))


def plot_number_of_infractions(data_set: int) -> None:
...
def plot_percentage_per_district(data_set: int) -> None:
...
def plot_district_percentage(data_set: int) -> None:
    '''Plots the percentage of infractions that happened in each 
...
def plot_street_percentage(data_set: int, number: int = 10) -> None:
    '''Plots the percentage of infractions that happened on the top
    {number} streets in a bar chart
    '''
    helper_functions.check_validity_of_function_call(data_set, True)
    
    helper_functions.create_directory(data_set)

    streets_tuple = prepare_data(data_set, True)[2]
...
def prepare_data(data_set: int, 
                 streets_on: bool = False) -> Tuple[list, dict, Tuple[dict, int]]:
    ...
    file_path = os.path.join('filtered_data', f'filtered_data_{data_set}.json')
    file = open(file_path)
    ...
    district_json = helper_functions.load_geojson('warszawa-dzielnice.geojson')
