...
DATA_SETS_DIR = 'data_sets' # The directory where the data sets are stored
RADIUS = 700 # The radius in meters for the GUGiK API request


def dump_filtered_data(data_set: int, filtered_data: list) -> None:
    with open(os.path.join("filtered_data",
...
def determine_district(point_coords: tuple) -> str:
    ...
    districts = helper_functions.load_geojson('warszawa-dzielnice.geojson')
...
def filter_data_set(
    ...
    subfolder_path = os.path.join(DATA_SETS_DIR, f"data_set_{data_set}")
