...
def create_patches_for_type(images_folder_path, patch_size, overlap, crop_start, cond_folder_path=None):
    ...
    patches_all = torch.stack(patches)
        ...
        for cond_image in cond_folder.iterdir():
            if cond_image.is_file():
                ...
                if "nd2" in cond_image.suffix:
                    ...
                    tensor_img = transforms.ToTensor()(np.array(img, np.float32))
                cond_patches += crop_to_patches(tensor_img, top, left, patch_height, patch_width, overlap)  # maia's function

        cond_patches_all = torch.stack(cond_patches)
