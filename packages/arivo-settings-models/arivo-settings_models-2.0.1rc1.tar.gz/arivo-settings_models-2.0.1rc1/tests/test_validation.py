...
class ValidationTests(TestCase):
    def test_gates_success(self):
        gt = dump_setting({"gate1": {"gate": "gate1", "name": "EF", "type": "entry"},
    ...
    def test_enforcement(self):
        ...
        # no initial values for enabled and last_edited
        setting_str = '{"strictness": 0, "payment_deadline_hours": 1}'
        pre_time = datetime.utcnow()
        res = parse_setting("enforcement/basic_settings", setting_str)
        post_time = datetime.utcnow()
        self.assertEqual(False, res.enabled)
        self.assertTrue(pre_time <= res.last_edited <= post_time)

        # no initial values for enabled and last_edited
        setting_str = '{"strictness": 1, "payment_deadline_hours": 1}'
        res = parse_setting("enforcement/basic_settings", setting_str)
