...
from sqlorm.sql.sql_alchemy import sql_pool
from sqlorm.utils import TYPES_MAP
...
class Resolver(BaseModel):
    ...
    def to_qb(self, cardinality: Cardinality, from_: str | None) -> QueryBuilder:
        self._qb.cardinality = cardinality
        self._qb.set_from(from_=from_)
        return self._qb
    ...
    def parse_mutation_model(
        ...
        dump_d = model.model_dump(mode="json", exclude_unset=True)
                    ...
                    if computed_c.basemodel_type:
                        if model_v is not None:
                            if computed_c.cardinality == Cardinality.ONE:
                                ...
                                v = json.dumps(
                                    [item.model_dump_json() for item in model_v]
                                )
                if prefix:
    ...
    def build_insert_q_and_v(
        ...
        if (
            ...
            return_select_str = (
                f"SELECT {self._returning_str} FROM {conflicting_from_str}"
            )
        else:
