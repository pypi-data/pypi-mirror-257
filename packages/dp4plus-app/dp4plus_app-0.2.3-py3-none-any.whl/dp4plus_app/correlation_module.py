...
def G09_tens_matrix(isomer_list, TMS = False):
    ...
    for j, isom in enumerate(isomer_list):
        ...
        while True: 
                file = files.pop(0)
                conf_tens, energy = get_scf_tensors(file , energies) #1st conf for template
                
                if conf_tens is None: continue
            
                isom_matrix = np.zeros((conf_tens.shape[0],len(files)+1))
        ...
        #print (isom_matrix)
        #print ('-'*30)
        isom_matrix = np.delete(isom_matrix, clear, axis=1)
        #print (isom_matrix)
...
def get_scf_tensors(file, energies):
    ...
    with open (file,'rt') as f:
        lines=f.readlines()
        for line in lines:
            if 'nmr' in line.casefold(): nmr = True
    ...
    if not nmr or not tensors : 
        if not os.path.exists("Fail files"):
            os.makedirs("Fail files")
            
        shutil.move(file,'Fail files')         #mueve el .log en Error
        
        display = '''¡ Warning !
Files have been relocated to the "Fail files" directory within the working folder.
It is strongly advised to rectify any inconsistencies and rerun the calculations. 
Possible errors include: 
    • Unable to locate "Normal Termination"
    • Absence of "nmr" or "NMR" command line
    • Missing "Isotropic"
        '''
