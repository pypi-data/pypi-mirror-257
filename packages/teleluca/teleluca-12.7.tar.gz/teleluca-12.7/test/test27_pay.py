...
import teleluca
from teleluca.namedtuple import (
    LabeledPrice, Invoice, PreCheckoutQuery, ShippingQuery, ShippingOption,
    SuccessfulPayment)
from teleluca.loop import MessageLoop
...
Run it by:
$ python2.7 script.py <bot-token> <payment-provider-token>
"""
...
def on_shipping_query(msg):
    query_id, from_id, invoice_payload = teleluca.glance(msg, flavor='shipping_query')
...
def on_pre_checkout_query(msg):
    query_id, from_id, invoice_payload, currency, total_amount = teleluca.glance(msg, flavor='pre_checkout_query', long=True)
