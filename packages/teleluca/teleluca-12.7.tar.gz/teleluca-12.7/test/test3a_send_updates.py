...
async def send_everything(msg):
    content_type, chat_type, chat_id, msg_date, msg_id = teleluca.glance(msg, long=True)
    ...
    examine(r, teleluca.namedtuple.Message)
    ...
    nt_show_keyboard = teleluca.namedtuple.ReplyKeyboardMarkup(**show_keyboard)
    nt_remove_keyboard = teleluca.namedtuple.ReplyKeyboardRemove(**remove_keyboard)
    nt_force_reply = teleluca.namedtuple.ForceReply(**force_reply)
    ...
    examine(f, teleluca.namedtuple.File)
    ...
    with open('lighthouse.jpg', 'rb') as f1, open('gandhi.png', 'rb') as f2, open('bookshelf.jpg', 'rb') as f3, open('saturn.jpg', 'rb') as f4:
        ms = [
            teleluca.namedtuple.InputMediaPhoto(media=f1),
            teleluca.namedtuple.InputMediaPhoto(media=('media2', f2)),
            teleluca.namedtuple.InputMediaPhoto(media='https://telegram.org/file/811140935/175c/FSf2aidnuaY.21715.gif/31dc2dbb6902dcef78'),
    ...
    mif = teleluca.message_identifier(r)
    ...
    game_keyboard = teleluca.namedtuple.InlineKeyboardMarkup(inline_keyboard=[[
                        teleluca.namedtuple.InlineKeyboardButton(text='Play now', callback_game=True),
                        teleluca.namedtuple.InlineKeyboardButton(text='How to play?', url='https://mygame.com/howto'),
                    ]])
...
async def get_user_profile_photos():
    ...
    examine(r, teleluca.namedtuple.UserProfilePhotos)
...
async def test_webhook_getupdates_exclusive():
    ...
    except teleluca.exception.TelegramError as e:
...
async def see_every_content_types(msg):
    ...
    content_type, chat_type, chat_id = teleluca.glance(msg)
    from_id = msg['from']['id']
    ...
    examine(msg, teleluca.namedtuple.Message)
    try:
...
# teleluca.aio.api.set_proxy('http://192.168.0.104:8888')

bot = teleluca.aio.Bot(TOKEN)
