...
def derive_all(logger, outdir, cores, panmodel, pam, report, gannots, minflux, biolog):
    
    
    ### PART 1: derive strain-specific models
    
    response = derive_strain_specific(logger, outdir, cores, panmodel, pam, report, gannots)
    ...
    if biolog:
        response = strain_biolog_tests(logger, outdir, cores, pam)
        if response == 1: return 1
...
def derive_command(args, logger):
    ...
    response = derive_all(logger, outdir, args.cores, panmodel, pam, report, gannots, args.minflux, args.biolog)
    if response == 1: return 1
