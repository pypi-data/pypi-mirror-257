...
def task_biolog(accession, args):
    ...
    for substrate, row in biolog_mappings.iterrows():
        ...
        for source in eval(row['source']): 
            ...
            exr_after = row['BiGG_exchange']
            if source == 'C': exr_before = 'EX_glc__D_e'
            elif source == 'N': exr_before = 'EX_nh4_e'
            elif source == 'P': exr_before = 'EX_pi_e'
            elif source == 'S': exr_before = 'EX_so4_e'
            else: exr_before = '?'
            ...
            with ss_model:
                ...
                if exr_before_present:  # the exchange reaction could not be present
                    ...
                    if exr_after_present: # the exchange reaction could not be present
                        ...
                        status = res.status
                        growth = value >= 0.001 and status == 'optimal'
    ...
    df_results.to_csv(outdir + 'biolog/' + accession + '.csv')
...
def strain_biolog_tests(logger, outdir, cores, pam):
