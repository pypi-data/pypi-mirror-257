...
from patent_chart.data_structures import PatentTextLine
...
class PatentUniqueID:
    ...
    def __repr__(self):
        if self.kind_code:
            return f'{self.country_code}{self.patent_number}-{self.kind_code}'
        else:
...
def parse_google_patent_from_pdf_path(path: pathlib.Path | str) -> GoogleParsedPatent:
    ...
    google_patent_result = parse_google_patent(google_unique_id)
    if google_patent_result is not None:
        specification, claims, title = google_patent_result
        ...
        raise ParseError(f'Could not parse google patent for unique id {google_unique_id}')
...
def parse_google_patent_from_unique_id(unique_id: str) -> GoogleParsedPatent:
    ...
    google_patent_result = parse_google_patent(unique_id)
    if google_patent_result is not None:
        specification, claims, title = google_patent_result
        return GoogleParsedPatent(
        ...
        raise ParseError(f'Could not parse google patent for unique id {unique_id}')
...
def parse_claims_from_google_parsed_patent(parsed_patent: GoogleParsedPatent) -> list[GoogleClaim]:
    # Trim beginning of claims
    claim_begin = parsed_patent.claims.find('1.')

    trimmed_claims = parsed_patent.claims[claim_begin:]
