...
SKIP_MODELS = json.loads(os.getenv('VALIDATE_MODELS_SKIP', '[]'))
...
def method_tier_from_model(result):
    return None if result is None else (
        method_tier_from_model(result[0]) if isinstance(result, list) and len(result) > 0 else (
            result.get('methodTier') if isinstance(result, dict) else None
        )
    )


def _skip_model(model: str, term_id: str):
    return next((m for m in SKIP_MODELS if m.get('model') == model and m.get('value') == term_id), None) is not None
...
def run_model(model: str, term_id: str, data: dict):
    ...
    """
    return _import_model(model)(term_id, data) if not _skip_model(model, term_id) else None
