...
class PromptLength(Metric):
    ...
    def compute(self, df: pd.Series, force=False):
        return super().compute(df, force) or ItemResult(len(df[config.prompt_column]), None)
...
class ResponseLength(Metric):
    ...
    def compute(self, df: pd.Series, force=False):
        return super().compute(df, force) or ItemResult(len(df[config.response_column]), None)
