...
class Manifiesto (EcuInfo):
	...
	def getMainFields (self):
		...
		try:
			...
			vehiculo                             = self.getVehiculoRemolqueInfo ("vehiculo")
			...
			remolque                             = self.getVehiculoRemolqueInfo ("remolque")
			...
			# Type of vehicle is known after "remolque" is know
			self.ecudoc ["23_Tipo_Vehiculo"]     = remolque ["tipo"]
			...
			self.ecudoc ["81_Descripcion"]      = Utils.getValue (self.fields, "29_Mercancia_Descripcion")
	...
	def getTransportistaInfo (self):
		...
		try:	
			...
			transportista ["MCI"]               = self.getNumeroManifiesto ()
	...
	#------------------------------------------------------------------
	def getVehiculoRemolqueInfo (self, vehicleType):
		keysVehiculo = {"marca": "04_Camion_Marca", "anho": "05_Camion_AnoFabricacion", "placaPais": "06_Camion_PlacaPais", 
		                 "chasis": "07_Camion_Chasis", "certificado": "08_Certificado_Habilitacion"}
		keyRemolque = {"marca": "09_Remolque_Marca", "anho": "10_Remolque_AnoFabricacion", "placaPais": "11_Remolque_PlacaPais", 
		                 "chasis": "12_Remolque_Otro", "certificado": "08_Certificado_Habilitacion"}

		vehiculo      = {"marca": None, "anho": None, "pais": None, "placa": None,
					     "chasis": None, "certificado": None, "tipo": None}

		keys = keysVehiculo if (vehicleType == "vehiculo") else keyRemolque
	
		try:
			vehiculo ["marca"]       = Utils.getValue (self.fields, keys ["marca"])
			vehiculo ["anho"]        = Utils.getValue (self.fields, keys ["anho"])
			placaPais                = Extractor.getPlacaPais (Utils.getValue (self.fields, keys ["placaPais"]))
			vehiculo ["pais"]        = placaPais ["pais"]
			vehiculo ["placa"]       = placaPais ["placa"]
			vehiculo ["chasis"]      = Utils.getValue (self.fields, keys ["chasis"])

			vehiculo ["certificado"] = self.getCheckCertificadoString (keys ["certificado"], vehicleType)

			if vehiculo ["marca"] in ["N/A", ""] or "XX".lower () in vehiculo ["marca"].lower():
	...
	def getCheckCertificadoString (self, key, vehicleType):
		try:
			textCertificado  = Utils.getValue (self.fields, key)
			print (f">>> Validando certificado de '{vehicleType}' en texto '{textCertificado}'")
	...
	#-- Examples: "CH-CO-0776-23 // CR-CO-0780-23"
	def formatCertificadoString (self, textCertificado, vehicleType):
		try:
			text = textCertificado
			if (text in [None, ""]):
				return None
			...
			# Split into certificate elements
			...
			Utils.printx (f"Excepción formateando certificado desde '{textCertificado}'")
			certificadoString = ""
	...
	def getCargaInfo (self):
		...
		try:
			...
			info ["descripcion"]    = Utils.getValue (self.fields, "25e_Carga_TipoDescripcion")
		except:
