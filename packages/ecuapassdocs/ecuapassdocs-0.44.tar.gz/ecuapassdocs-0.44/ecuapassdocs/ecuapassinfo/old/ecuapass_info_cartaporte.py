...
class CartaporteInfo (EcuInfo):
	...
	def getMainFields (self):
		try:
			...
			self.ecudoc ["02_NumeroCPIC"]        = self.getNumeroCartaporte ()
	...
	def getGastosInfo (self):
		...
		def getValueTabla (firstKey, secondKey):
			try:
				...
				#Utils.printx (f"Sin valor en '{firstKey}'-'{secondKey}' ({self.fields['jsonFile']})")
		...
		try:
			...
			# REMITENTE: 
			gastos ["fleteRemi"]       = getValueTabla ("ValorFlete", "MontoRemitente")
			gastos ["monedaRemi"]      = USD if gastos ["fleteRemi"] else None
			gastos ["otrosGastosRemi"] = getValueTabla ("OtrosGastos", "MontoRemitente")
			gastos ["otrosMonedaRemi"] = USD if gastos ["otrosGastosRemi"] else None
			gastos ["totalGastosRemi"] = getValueTabla ("Total", "MontoRemitente")
	...
	#-------------------------------------------------------------------
	#-- Get subject info: nombre, dir, pais, ciudad, id, idNro
	def getSubjectInfoByza (self, key):
