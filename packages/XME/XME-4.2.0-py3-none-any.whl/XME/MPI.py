...
try:
	...
	try:
		...
		import pickle
from multiprocessing import cpu_count,Pipe,Manager,Lock
...
class MPI:
	...
	def run(self,*args,**kwargs):
		...
		for i in range(1,self.pnum):
			...
			locks[tpid]=Lock()
			locks[tpid].acquire(block=False)
			pc,cc=Pipe()
			Main_Connector[tpid]=pc
			Task_Connector.append(cc)
		xme=XME.XME(self.__task__,self.mainfun,**self.xmeargs)
		kwargs.update({"XMEMPI":(self,status,Main_Connector,locks)})
		return xme.gfun(status,locks,xme.Array(tpids),xme.Array(Task_Connector),garg=args,gargs=kwargs)[0][0]
	...
	def acquire(self, XMEMPI, funid, args=(), kwargs={}, to=ANY_PROCESS,block=False):
		_,status,conns,locks=XMEMPI
		...
		if to==ALL_PROCESSES: return [self.acquire(status, connets, funid, args, kwargs,tpid) for tpid in status.keys()]
			...
			while True:
				for tpid,sta in status.items():
					if sta==STATUS_IDLE and locks[tpid].acquire(block=False):
			...
			while True:
				if status[to]==STATUS_IDLE and locks[to].acquire(block=False):
	...
	def close(self, XMEMPI, to=ALL_PROCESSES):
		#in function close
		#locks are blocked, because the processes are useless in the future 
		_,status,conns,locks=XMEMPI
		if to==ALL_PROCESSES:
			closed=[]
			while True:
	...
	def __task__(self, status, locks, tpid, connect, print=print):
		while True:
			try:
				locks[tpid].release()
