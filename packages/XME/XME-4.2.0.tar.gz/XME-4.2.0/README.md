...
**Update in 4.2: Use lock objects to increase data operation security and optimize the use of MPI modules**
...
def main(XMEMPI):
  ...
  bufferpos01=mpi.acquire(XMEMPI, "distance", args=(points[0],points[1]),block=True)
  bufferpos02=mpi.acquire(XMEMPI, "distance", args=(points[0],points[2]),to=list(status.keys())[0])
  bufferpos12=mpi.acquire(XMEMPI, "distance", args=(points[1],points[2]))
...
def main(nums,subnums=0,XMEMPI=None,print=print):
  ...
  if XMEMPI:
    ...
    def autoupdate():
      while run:
        newlens=len(buffpos)
        for i in buffpos[result.lens:newlens]: result.value+=mpi.get(i)
        result.lens=newlens
        time.sleep(0.1)
    threading.Thread(target=autoupdate).start()
    for i in range(0,nums,subnums):
      buffpos.append(mpi.acquire(XMEMPI, "scatter", args=(subnums,print)))
    result.value+=scatter(nums%subnums,print)
    while result.lens<len(buffpos): time.sleep(0.1)
    mpi.close(XMEMPI)
    run=False
