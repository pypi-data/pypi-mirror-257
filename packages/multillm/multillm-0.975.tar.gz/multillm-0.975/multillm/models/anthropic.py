...
"""
The ANTHROPIC class extends the BaseModel class and overrides the get_response() method, providing an implementation.
...
class ANTHROPIC(BaseLLM):
    ...
    def get_content(self, response):
        ...
        try:
            if self.is_code(str(response)):
                ...
                #print('ANTHROPIC is not code')
                print("{0} response: {1}" .format(self.__class__.__name__,str(response)))
                return str(response), False
        except Exception as e:
            #print("error is_code() {0}" .format(str(e)))
            return('ANTHROPIC response failed {}'.format(e))
    ...
    def get_response1(self, prompt: Prompt, taskid=None, convid = None):
        ...
        try:
            ...
            print("ANTHROPIC Response: {0}" .format(response.text))
            data = response.json()
            
        except Exception as e:
            print('error calling ANTHROPIC: {0}' .format(str(e)))
    ...
    def get_response(self, prompt: Prompt, taskid=None, convid = None):
        ...
        if not response:
            ...
            content, is_code = self.get_content(response.model_dump_json(indent=2))
            if content and taskid:
                self.publish_to_redis(content, taskid)
            return(content), is_code
