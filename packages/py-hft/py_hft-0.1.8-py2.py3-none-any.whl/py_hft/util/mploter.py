...
class Plotor:
  ...
  def plot_one(ax, data, label, mode='plot'):
    if mode == 'twin' or mode == 'twinx':
      ...
      this_ax=ax.twinx(); next(this_ax._get_lines.prop_cycler);# Handle TWIN
    elif mode=='twiny': #mode = 'plot';this_ax=ax.twiny(); next(this_ax._get_lines.prop_cycler);# Handle TWIN
      if '@' in label: mode = label.split('@')[-1]; label = '@'.join(label.split('@')[:-1])
      else: mode = 'plot';
      this_ax=ax.twiny(); next(this_ax._get_lines.prop_cycler);# Handle TWIN
    elif mode=='twinxy':# mode = 'plot';this_ax=ax.twinx().twiny(); next(this_ax._get_lines.prop_cycler);# Handle TWIN
      if '@' in label: mode = label.split('@')[-1]; label = '@'.join(label.split('@')[:-1])
      else: mode = 'plot';
      this_ax=ax.twinx().twiny(); next(this_ax._get_lines.prop_cycler);# Handle TWIN
    else: this_ax = ax
  ...
  def MultiPlot3(m, fig_name='default', show=False, prefix='', save_dir=os.environ['HOME']+'/MultiPlot', png_width=3, zoom=1.0, with_number = False, sc=1, full=False):
    ...
    RedPrint('Saving png to ', png_file, color='blue')
