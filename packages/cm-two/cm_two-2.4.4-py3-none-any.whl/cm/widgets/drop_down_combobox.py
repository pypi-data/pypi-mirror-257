...
class AutocompleteCombobox(Combobox):
    ...
    def set_completion_list(self, completion_list):
        print("SETTING COMP LIST", completion_list)
    ...
    def react(self, event):
        print("\nReaction", event.char)
        print("\tUser type:", self.user_data)
        user_data_index = len(self.user_data)
        #if not event.char:
        #    # Если букву не написали
        #    return
        if not user_data_index:
            return
        print("\tUser data index", user_data_index)
        suggestions = self.get_suggestions()
        self['values'] = suggestions
        print("\tSuggestions:", suggestions)
        if suggestions:
            print("\tSet suggestions")
