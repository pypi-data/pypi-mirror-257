        ...
        try:
            ...
            for signal in self.example_message.signals:
                ...
                if signal.is_multiplexer:
                    ...
                    sig_value[signal.name] = 0 if si.raw_initial is None else si.raw_initial
                    continue  # 忽略多路复用信号本身
            ...
            print('dbc解析错误', e)
            sig_data1 = self.example_message.encode(sig_value, scaling=True, strict=False)
            a = binascii.b2a_hex(sig_data1)
            # 16进制
            self.six_data = re.sub(r"(?<=\w)(?=(?:\w\w)+$)", " ", a.decode('utf8')).split()
            return
...
if __name__ == '__main__':
    data1 = {'HCU_EVLmpSts': '1'}
    a = get_signal_data1(
        'D:\广汽项目\C01D-006-014DB01  CAN DataBase for SC-CANFD_V4.9-博世用.dbc').db
    ob = DbcInit('HCU_PT5', data1, db_obj=a)
    print(ob.dbc_parser())
