...
def dict_mean(dict_list, ignore = ['AIC', 'layout', 'fixed_fit', 'rdm_fit', 'rdm_cor_fit', 'zi_fit', 'simple', 'pvalues']):
    mean_dict = {}
    if ignore is None:
    ...
    def does_it_appear(self, new):
        for d in self._population:
            if self.mixed_list_chescker(d['layout'], new):
                print('same sln appears in population: try again')
                return True
...
class SimulatedAnnealing(object):
    ...
    def run(self, initial_slns=None, mod_init = None):
        ...
        if self._obj_fun.use_random_seed():
            self._obj_fun.set_random_seed()
        self._temp_max = self.Calculate_Temp(initial_slns_struct, acceptance_prob=self._INITAL_ACCEPT_RATE, multi = self.pf.get_objective_is_multi())
        ...
        while (iteration <= self._MAX_ITERATIONS) and (
            ...
            for i in range(self._STEPS_PER_TEMP):
                ...
                # Acceptance Criteria
                if random.uniform(0, 1) <= self.Energy_Acceptance_Criteria(nbr_energy, self._current_energy, current_temperature, self.pf.get_objective_is_multi()):
                ...
                try:
                    logger(iteration, nbr_energy, self.best_energy, name = self.get_instance_name(), local_best1 = self._current_energy)
                except Exception as e:
        ...
        return {'elapsed_time': elapsed_time, 'Iteration': iteration}
...
class Mutlithreaded_Meta(DifferentialEvolution, SimulatedAnnealing, HarmonySearch):
    ...
    def run_sa(self, initial_slns=None, mod_init = None):
        ...
        if self._obj_fun.use_random_seed():
            self._obj_fun.set_random_seed()
        self.temp_max = self.Calculate_Temp_mp(initial_slns_struct, acceptance_prob=self._INITAL_ACCEPT_RATE, multi = self.pf.get_objective_is_multi())
        ...
        while (iteration <= self._MAX_ITERATIONS) and (
            ...
            for i in range(self._STEPS_PER_TEMP):
                ...
                nbr_struct = list()
                for j in range(self._pop_size):
                    nbr_struct1 = self._get_neighbour(current_energy[j]['layout'], round(self._crossover_perc*len(current_energy[j]['layout'])))
                ...
                for j, nbr_energy in enumerate(results_sa):
                    ...
                    # Acceptance Criteria
                    if random.uniform(0, 1) <= self.Energy_Acceptance_Criteria(nbr_energy, current_energy[j], current_temperature, self.pf.get_objective_is_multi()):
                        #print('Accepted New with fitness of', nbr_energy)
                        didchange = self.pf.did_it_change()
                        if didchange:
                            pareto_logger(self.pf.Pareto_F, iteration, self._obj_fun.complexity_level, self._obj_fun.instance_number)
