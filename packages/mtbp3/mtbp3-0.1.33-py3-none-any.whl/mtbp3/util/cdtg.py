...
class catPlotter:
    ...
    def lineplot(self):
        ...
        if self.grid_col:
            df = self.df
            df.loc[:,'GroupedMean'] = df.groupby([self.grid_col, self.group_col])[self.y_col].transform('mean').copy()
            ...
            if self.x_label_rotate:
                for ax in g.axes.flat:
                    _ = ax.set_xticklabels(ax.get_xticklabels(), rotation=self.x_label_rotate, ha='right')
            plt.suptitle(self.title, weight='bold')
            plt.tight_layout()
            ...
            df.loc[:,'GroupedMean'] = df.groupby([self.group_col])[self.y_col].transform('mean').copy()
