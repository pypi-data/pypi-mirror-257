...
class catPlotter:
    ...
    def lineplot(self):
        ...
        if self.grid_col:
            ...
            g.map(sns.lineplot, self.x_col, self.y_col, self.group_col, sort=True, hue='GroupedMean', palette='Blues', linewidth=0.9)
            ...
            if self.point_position=='density':
                if self.y_scale_base > 0:
                    sns.violinplot(df, x=self.x_col, y=self.y_col, log_scale=self.y_scale_base, fill=False, order=self.x_order, order=self.x_order)
                else:
                    sns.violinplot(df, x=self.x_col, y=self.y_col, log_scale=False, fill=False, order=self.x_order, order=self.x_order)
