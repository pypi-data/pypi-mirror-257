...
def count_gates(qobj, basis, qubits):
    ...
    """
    warn(
        "The function `count_gates` will be deprecated. "
...
def gates_per_clifford(
    ...
    if isinstance(transpiled_circuits_list[0], QasmQobj):
        warn(
            "`QasmQobj` input will be deprecated. Use transpiled `QuantumCircuit` instead. "
    ...
    total_ncliffs = len(transpiled_circuits_list) * np.sum(
        np.array(clifford_lengths) + 1
    )
...
def coherence_limit(nQ=2, T1_list=None, T2_list=None, gatelen=0.1):
    ...
    if nQ == 1:
        ...
        coherence_limit_err = 0.5 * (
            1.0
            - 2.0 / 3.0 * np.exp(-gatelen / T2[0])
            - 1.0 / 3.0 * np.exp(-gatelen / T1[0])
        )
...
def twoQ_clifford_error(
    ngates: Dict[int, Dict[str, float]], gate_qubit: List[int], gate_err: List[float]
):
    """
    ...
    """
    warn(
        "The function `twoQ_clifford_error` will be deprecated. "
        "Use `calculate_2q_epc` instead.",
        category=DeprecationWarning,
    )
    ...
    if len(gpc_per_qubits) != 2:
        raise QiskitError(
            "Number of qubits contained in the `ngates` dictionary is not 2. ",
    ...
    for gate_ind, ngate in enumerate(ngates_lists):
        if gate_qubit[gate_ind] == -1:
            ...
            alpha1Q[gate_qubit[gate_ind]] *= (1 - 2 * gate_err[gate_ind]) ** ngate
...
def calculate_1q_epg(
    gate_per_cliff: Dict[int, Dict[str, float]], epc_1q: float, qubit: int
) -> Dict[str, float]:
    r"""
    ...
    if "u3" not in gpc_per_qubit or "u2" not in gpc_per_qubit:
        raise QiskitError(
            "Invalid basis set is given. Use `u1`, `u2`, `u3` for basis gates."
        )
    ...
    return {
        "u1": 0,
        "u2": epc_1q / (n_u2 + 2 * n_u3),
        "u3": 2 * epc_1q / (n_u2 + 2 * n_u3),
    }


def calculate_2q_epg(
    gate_per_cliff: Dict[int, Dict[str, float]],
    epc_2q: float,
    qubit_pair: List[int],
    list_epgs_1q: Optional[List[Dict[str, float]]] = None,
    two_qubit_name: Optional[str] = "cx",
) -> float:
    r"""
    ...
    for ind, (qubit, epg_1q) in enumerate(zip(qubit_pair, list_epgs_1q)):
        if qubit not in gate_per_cliff:
            raise QiskitError(
                "Qubit %d is not included in the `gate_per_cliff`" % qubit
            )
        gpc_per_qubit = gate_per_cliff[qubit]
    ...
    raise QiskitError(
        "Two qubit gate %s is not included in the `gate_per_cliff`. "
        "Set correct `two_qubit_name` or use 2Q RB gate count." % two_qubit_name
    )


def calculate_1q_epc(
    gate_per_cliff: Dict[int, Dict[str, float]], epg_1q: Dict[str, float], qubit: int
) -> float:
    r"""
...
def calculate_2q_epc(
    ...
    for ind, (qubit, epg_1q) in enumerate(zip(qubit_pair, list_epgs_1q)):
        ...
        for gate_name, epg in epg_1q.items():
            ...
            alpha_1q[ind] *= (1 - 2 * epg) ** n_gate
    alpha_c_2q = (
        1 / 5 * (alpha_1q[0] + alpha_1q[1] + 3 * alpha_1q[0] * alpha_1q[1]) * alpha_2q
    )
