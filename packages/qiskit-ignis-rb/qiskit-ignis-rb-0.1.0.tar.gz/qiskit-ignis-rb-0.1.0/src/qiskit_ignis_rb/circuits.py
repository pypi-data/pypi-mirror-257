...
def handle_length_multiplier(length_multiplier, len_pattern, is_purity=False):
    """
    ...
    if hasattr(length_multiplier, "__len__"):
        ...
        if len(length_multiplier) != len_pattern:
            raise ValueError("Length multiplier must be the same length as the pattern")
...
def handle_interleaved_elem(interleaved_elem, rb_group, keep_original_interleaved_elem):
    ...
    for elem in interleaved_elem:
        ...
        if not (isinstance(group_elem, Clifford) and group_gates_type == 0) and not (
            isinstance(group_elem, CNOTDihedral) and group_gates_type == 1
        ):
            raise ValueError("Invalid interleaved element type.")

        if (
            not isinstance(group_elem, QuantumCircuit)
            and not isinstance(group_elem, Clifford)
...
def randomized_benchmarking_seq(
    nseeds: int = 1,
    ...
    interleaved_elem: Optional[
        Union[
            List[QuantumCircuit], List[Instruction], List[Clifford], List[CNOTDihedral]
        ]
    ] = None,
    ...
    if interleaved_gates is not None:
        warn(
            "`interleaved_gates` kwarg is deprecated. "
            "Please use the `interleaved_elem` kwarg that supersedes it.",
    ...
    qlist_flat, n_q_max, max_dim = check_pattern(
        rb_pattern, is_purity, interleaved_elem
    )
    length_multiplier = handle_length_multiplier(
        length_multiplier, len(rb_pattern), is_purity
    )
    ...
    interleaved_elem = handle_interleaved_elem(
        interleaved_elem, rb_group, keep_original_interleaved_elem
    )
        ...
        for elmnts_index in range(length_vector[-1]):
            for rb_pattern_index, rb_q_num in enumerate(pattern_sizes):
                ...
                for _ in range(length_multiplier[rb_pattern_index]):
                    ...
                    general_circ.compose(
                        replace_q_indices(
                            rb_group.to_circuit(new_elmnt),
                            rb_pattern[rb_pattern_index],
                            qr,
                        ),
                        inplace=True,
                    )
                    ...
                    # interleaved rb sequences
                    if interleaved_elem is not None:
                        Elmnts_interleaved[rb_pattern_index] = rb_group.compose(
                            Elmnts_interleaved[rb_pattern_index], new_elmnt
                        )
                        interleaved_circ.compose(
                            replace_q_indices(
                                rb_group.to_circuit(new_elmnt),
                                rb_pattern[rb_pattern_index],
                                qr,
            ...
            if (elmnts_index + 1) == length_vector[length_index]:
                ...
                for rb_pattern_index, rb_q_num in enumerate(pattern_sizes):
                    ...
                    circ.compose(
                        replace_q_indices(
                            inv_circuit, rb_pattern[rb_pattern_index], qr
                        ),
                        inplace=True,
                    )
                ...
                if group_gates_type == 1:
                    ...
                    cnotdihedral_interleaved_circ.compose(
                        circ_interleaved, inplace=True
                    )
                    for _, qb in enumerate(qlist_flat):
                        cnotdihedral_circ.barrier(qr[qb])
                        cnotdihedral_circ.h(qr[qb])
                ...
                if group_gates_type == 1:
                    ...
                    cnotdihedral_circ.name = rb_circ_type + "_X_length_%d_seed_%d" % (
                        length_index,
                        seed + seed_offset,
                    )
                    cnotdihedral_interleaved_circ.name = (
                        rb_circ_type
                        + "_interleaved_X_length_%d_seed_%d"
                        % (length_index, seed + seed_offset)
                    )
