...
class RBFitter(RBFitterBase):
    ...
    def __init__(self, backend_result, cliff_lengths, rb_pattern=None):
        """
        Args:
    ...
    def calc_data(self):
        ...
        self._circ_name_type = (
            self._result_list[0].results[0].header.name.split("_length")[0]
        )
        ...
        for seed in self._nseeds:
            for circ, _ in enumerate(self._cliff_lengths[0]):
                circ_name = self._circ_name_type + "_length_%d_seed_%d" % (circ, seed)
                count_list = []
        ...
        for patt_ind in range(len(self._rb_pattern)):
            ...
            for seedidx, seed in enumerate(self._nseeds):
                ...
                for k, _ in enumerate(self._cliff_lengths[patt_ind]):
                    circ_name = self._circ_name_type + "_length_%d_seed_%d" % (k, seed)
                    counts_subspace = marginal_counts(
                        circ_counts[circ_name], np.arange(startind, endind)
                    )
                    self._raw_data[-1][seedidx].append(
                        counts_subspace.get(string_of_0s, 0) / circ_shots[circ_name]
                    )
            startind = endind
    ...
    def fit_data(self):
        ...
        for patt_ind, _ in enumerate(self._rb_pattern):
            ...
            dcliff = self._cliff_lengths[patt_ind][1] - self._cliff_lengths[patt_ind][0]
            ...
            if y0 > fit_guess[2]:
                fit_guess[0] = (y0 - fit_guess[2]) / fit_guess[
                    1
                ] ** self._cliff_lengths[patt_ind][0]

            self.fit_data_pattern(patt_ind, tuple(fit_guess))

    def plot_rb_data(self, pattern_index=0, ax=None, add_label=True, show_plt=True):
        """Plot randomized benchmarking data of a single pattern.
        ...
        ax.errorbar(
            xdata,
            self._ydata[pattern_index]["mean"],
...
class InterleavedRBFitter(RBFitterBase):
    ...
    def __init__(
        self, original_result, interleaved_result, cliff_lengths, rb_pattern=None
    ):
        """
        Args:
        ...
        self._rbfit_original = RBFitter(original_result, cliff_lengths, rb_pattern)
    ...
    def add_data(self, new_original_result, new_interleaved_result, rerun_fit=True):
        """
        Add a new result.
    ...
    def fit_data(self):
        ...
        for patt_ind, (_, qubits) in enumerate(
            zip(self._cliff_lengths, self._rb_pattern)
        ):
            # calculate nrb=d=2^n:
            nrb = 2 ** len(qubits)
            ...
            systematic_err_1 = (
                (nrb - 1) * (abs(alpha - alpha_c / alpha) + (1 - alpha)) / nrb
            )
            systematic_err_2 = (
    ...
    def plot_rb_data(self, pattern_index=0, ax=None, add_label=True, show_plt=True):
        ...
        if not HAS_MATPLOTLIB:
            raise ImportError(
                'The function plot_interleaved_rb_data \
        ...
        for one_seed_data in self.raw_data[0][pattern_index]:
            ax.plot(xdata, one_seed_data, color="blue", linestyle="none", marker="x")
        for one_seed_data in self.raw_data[1][pattern_index]:
            ax.plot(xdata, one_seed_data, color="red", linestyle="none", marker="+")
        ...
        ax.plot(
            xdata,
            std_fit_function(xdata, *self.fit[0][pattern_index]["params"]),
            color="blue",
            linestyle="-",
            linewidth=2,
            label="Standard RB",
        ...
        ax.plot(
            xdata,
            int_fit_function(xdata, *self.fit[1][pattern_index]["params"]),
            color="red",
            linestyle="-",
            linewidth=2,
...
class PurityRBFitter(RBFitterBase):
    ...
    def __init__(self, purity_result, npurity, cliff_lengths, rb_pattern=None):
        """
        Args:
        ...
        # rb purity fitter
        self._rbfit_purity = RBFitter(purity_result, cliff_lengths, rb_pattern)
    ...
    def calc_data(self):
        ...
        for _, seed in enumerate(self.rbfit_pur.seeds):
            ...
            for pur in range(self._npurity):
                ...
                self._circ_name_type = (
                    self.rbfit_pur.results[result_count]
                    .results[0]
                ...
                for circ, _ in enumerate(self._cliff_lengths[0]):
                    ...
                    circ_name = (
                        "rb_purity_" + str(pur) + "_length_%d_seed_%d" % (circ, seed)
                    )
            ...
            for seedidx, seed in enumerate(self.rbfit_pur.seeds):
                ...
                for k, _ in enumerate(self._cliff_lengths[0]):
                    ...
                    for pur in range(self._npurity):
                        ...
                        circ_name = (
                            "rb_purity_" + str(pur) + "_length_%d_seed_%d" % (k, seed)
                        )
                        ...
                        for indcorr in range(2**self._nq):
                            zcorr = average_data(
                                counts_subspace, self._zdict_ops[indcorr]
                            )
    ...
    def fit_data(self):
        ...
        for patt_ind, (_, _) in enumerate(zip(self._cliff_lengths, self._rb_pattern)):
            # Calculate alpha (=p):
            ...
            self.rbfit_pur.fit[patt_ind]["params_err"][1] = alpha_pur_err

            # calculate purity error per clifford (pepc)
            nrb = 2**self._nq
            pepc = (nrb - 1) / nrb * (1 - alpha_pur)
            self.rbfit_pur.fit[patt_ind]["pepc"] = pepc

            pepc_err = (nrb - 1) / nrb * alpha_pur_err / alpha_pur
            self.rbfit_pur.fit[patt_ind]["pepc_err"] = pepc_err
    ...
    def plot_rb_data(self, pattern_index=0, ax=None, add_label=True, show_plt=True):
        ...
        ax.errorbar(
            xdata,
            self.rbfit_pur.ydata[pattern_index]["mean"],
        ...
        ax.plot(
            xdata,
            fit_function(xdata, *self.rbfit_pur.fit[pattern_index]["params"]),
            color="blue",
            linestyle="-",
            linewidth=2,
        )
...
class CNOTDihedralRBFitter(RBFitterBase):
    ...
    def __init__(
        self,
        cnotdihedral_Z_result,
        cnotdihedral_X_result,
        elmnts_lengths,
        rb_pattern=None,
    ):
        """
        ...
        self._rbfit_Z = RBFitter(cnotdihedral_Z_result, elmnts_lengths, rb_pattern)
        self._rbfit_X = RBFitter(cnotdihedral_X_result, elmnts_lengths, rb_pattern)

        self.rbfit_Z.add_data(cnotdihedral_Z_result)
        self.rbfit_X.add_data(cnotdihedral_X_result)

        if not (cnotdihedral_Z_result is None and cnotdihedral_X_result is None):
            self.fit_data()
    ...
    def add_data(
        self, new_cnotdihedral_Z_result, new_cnotdihedral_X_result, rerun_fit=True
    ):
        """
        Add a new result.
    ...
    def plot_rb_data(self, pattern_index=0, ax=None, add_label=True, show_plt=True):
        ...
        ax.plot(
            xdata,
            rbfit_Z_function(xdata, *self.fit[0][pattern_index]["params"]),
            color="blue",
            linestyle="-",
            linewidth=2,
        ...
        ax.plot(
            xdata,
            rbfit_X_function(xdata, *self.fit[1][pattern_index]["params"]),
            color="red",
            linestyle="-",
            linewidth=2,
            label="Measure state |+...+>",
