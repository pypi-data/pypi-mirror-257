            ...
            for output in step['outputs']:
                if output in outputs_attr:
                    ...
                    if isinstance(att, str):
                        ...
                        for _ in att:
                            run['results'].append(StepResult(id=output, output_id=output, default=_, type='script'))
                    else:
                        if not continue_on_error:
                            status.set_job_error('FAILED', f'Invalid output type {output}, str and list are not supported')
                            run['status'] = status.get_status()
                            return run
