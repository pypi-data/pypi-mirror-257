...
class LocationByIdentifierView(views.APIView):
    ...
    def get(self, request, format=None, identifier=None):
        try:
            location = (
                models.Location.objects.select_related(
                    "location_type",
                )
                .prefetch_related("locationfield_set", "locationidentifier_set")
                .get(SGLN=identifier)
            )
            ...
            response = Response(
                "The location with identifier %s could not " "be found." % identifier,
...
class EntryGeoHistoryView(views.APIView):
    ...
    def get(self, request, format=None, epc=None, epc_pk=None):
        ...
        biz_locations = (
            EntryEvent.objects.order_by("event__event_time")
            .select_related("event")
            .filter(**kwargs)
            .only("event__id", "event__event_time", "event__biz_location")
        )
        ...
        for location in biz_locations:
            ...
            lat, long = models.Location.objects.values_list(
                "latitude", "longitude"
            ).get(SGLN=location.event.biz_location)
            gl.latitude = lat
            gl.longitude = long
...
class GetCompanyPrefixLength(views.APIView):
    ...
    def get(self, request, barcode=None):
        ...
        try:
            ...
            return Response(
                "Invalid barcode.  Must be an 18 or 14 digit " "SSCC 18 or GTIN 14.",
                status=status.HTTP_400_BAD_REQUEST,
            )
        except db.CompanyConfigurationError:
            return Response(
                "No company or trade item matches the value" "barcode provided.",
