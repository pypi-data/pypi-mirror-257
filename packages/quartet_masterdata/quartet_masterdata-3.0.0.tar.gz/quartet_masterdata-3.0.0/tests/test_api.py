...
from quartet_masterdata.management.commands.create_masterdata_groups import Command
...
class APITests(APITestCase):
    def setUp(self):
        ...
        location_field = factories.LocationFieldFactory.create(location=location)
    ...
    def setup_user(self):
        ...
        user = User.objects.create_user(
            username="testuser", password="unittest", email="testuser@seriallab.local"
        )
    ...
    def test_get_gln_by_sgln(self):
        dbproxy = DBProxy()
        self.assertEqual(
            "2345234523454", dbproxy.get_GLN_by_SGLN("urn:epc:id:sgln:23452.3452345.0")
        )

    def test_get_sgln_by_gln(self):
        dbproxy = DBProxy()
        self.assertEqual(
            "urn:epc:id:sgln:23452.3452345.0", dbproxy.get_SGLN_by_GLN("2345234523454")
        )
    ...
    def test_get_cpl(self):
        ...
        factories.TradeItemFactory.create(
            GTIN14="22345678901234", company=company2, NDC="2345-67-8901"
        )
        url = reverse("get-company-prefix-length", kwargs={"barcode": "22345678901234"})
        result = self.client.get(url, format="json")
        self.assertEqual(result.status_code, 200)
    ...
    def _parse_test_data(self):
        ...
        parser = QuartetParser(os.path.join(curpath, "data/epcis.xml"))
    ...
    def test_get_company_prefix_length(self):
        ...
        self.assertEqual(db.get_company_prefix_length("023412378901200010"), 12)
        self.assertEqual(db.get_company_prefix_length("12341244123411"), 6)
        factories.TradeItemFactory.create(company=company2)
        self.assertEqual(db.get_company_prefix_length("12341234123411"), 6)
