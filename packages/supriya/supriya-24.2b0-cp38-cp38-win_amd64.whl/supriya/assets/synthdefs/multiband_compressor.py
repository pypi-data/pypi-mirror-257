...
def _make_synthdef(channel_count=2):
    with SynthDefBuilder(
        ...
        band_1 = LPF.ar(frequency=builder["frequency_1"], source=source)
        band_4 = HPF.ar(frequency=builder["frequency_3"], source=source)
        center = source - band_1 - band_4
        band_2 = LPF.ar(frequency=builder["frequency_2"], source=center)
        band_3 = HPF.ar(frequency=builder["frequency_2"], source=center)
        band_1 = CompanderD.ar(
        ...
        source = Limiter.ar(source=source, duration=builder["limiter_lookahead"])
