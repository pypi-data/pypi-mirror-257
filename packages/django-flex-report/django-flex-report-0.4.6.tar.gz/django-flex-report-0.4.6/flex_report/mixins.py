import ast
import mimetypes
from operator import call, is_not
...
class TablePageMixin(PaginationMixin, TemplateObjectMixin):
    ...
    def apply_user_path(self):
        ...
        path_func = getattr(
            self.model, app_settings.MODEL_USER_PATH_FUNC_NAME, lambda request: {}
        )
        accessed_paths = {
            path_name: (path, call(path_func, request=self.request).get(path_name))
            for path_name, path in paths
        }
        
        assert (
            len((accessed_path := list(filter(is_not(None)), accessed_paths.values())))
            == 1
        ), "Only one path can be accessed at a time."
        accessed_path = accessed_path[0]
        if (accessed_path:=dict(accessed_path)).values()[0] == "__all__":
            return
        self.report_qs = self.report_qs.filter(**dict(accessed_path))
    ...
    def setup(self, request, *args, **kwargs):
        ...
        self.apply_filters()
        filters_valid = (
            lambda: self.template_filters.is_valid()
            and self.quicksearch.is_valid()
            and self.filters.is_valid()
        )
        ...
        if filters_valid():
            ...
            self.report_qs = self.report_qs.distinct().order_by(*self.model._meta.ordering or ["pk"])
        ...
        if self.filters.is_valid() and self.quicksearch.is_valid():
            ...
            self.used_filters = self.get_used_filters(
                {
                    get_column_verbose_name(self.model, k): self.used_filter_format(
                        k, v
                    )
