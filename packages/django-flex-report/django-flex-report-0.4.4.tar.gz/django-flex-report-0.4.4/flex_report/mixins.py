        ...
        for path_name, path in paths.items():
            with contextlib.suppress(FieldError):
                ...
                if method_name := getattr(
                    model, app_settings.MODEL_USER_PATH_FUNC_NAME, False
                ):
                ...
                if not val or not (path_val := val.get(path_name)):
                    continue

                self.report_qs = self.report_qs.filter(**{path: path_val}).order_by(
                    *model._meta.ordering or ["pk"]
                )
