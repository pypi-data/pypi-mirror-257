...
class TablePageMixin(PaginationMixin, TemplateObjectMixin):
    ...
    def apply_user_path(self):
        ...
        assert (
            len((accessed_path := list(filter(partial(is_not, None)), accessed_paths.values())))
            == 1
        ), "Only one path can be accessed at a time."
        accessed_path = accessed_path[0]
        if "__all__" in (accessed_path:=dict(accessed_path)):
            return
        self.report_qs = self.report_qs.filter(**accessed_path)
