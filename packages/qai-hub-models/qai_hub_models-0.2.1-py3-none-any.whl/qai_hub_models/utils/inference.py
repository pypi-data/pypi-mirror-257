...
from qai_hub_models.utils.qnn_helpers import is_qnn_hub_model, qnn_to_torch_outputs
...
def compile_zoo_model_to_hub(
    ...
    return HubModel(hub_model, input_names, device, transpose_io=transpose_io)
...
class HubModel:
    ...
    def __init__(
        ...
        self.transpose_io = transpose_io
    ...
    def __call__(
        ...
        *input_tensors: torch.Tensor
        ...
        if self.transpose_io:
            assert isinstance(inputs, dict)
            inputs = {
                k: [v.transpose(0, 2, 3, 1) for v in vv] for k, vv in inputs.items()
            }
        ...
        if self.transpose_io:
            ...
            output_dataset = {
                k: [v.transpose(0, 3, 1, 2) for v in vv]
                for k, vv in output_dataset.items()
            }
        output_torch = [
        ...
        if is_qnn_hub_model(self.model):
            output_torch = qnn_to_torch_outputs(output_torch)  # type: ignore
