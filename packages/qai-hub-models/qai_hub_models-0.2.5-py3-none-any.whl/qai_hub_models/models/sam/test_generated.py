...
@pytest.mark.skip(
    reason="Compilation fails https://dev.aihub.qualcomm.com/jobs/j7gjqejx5 (encoder) and https://dev.aihub.qualcomm.com/jobs/jlpeykj15 (decoder)"
)
@pytest.mark.compile
@skip_clone_repo_check
...
def test_profile_qnn():
    ...
    for component_name, result in results.items():
        ...
        if os.environ.get("TEST_HUB_ASYNC", 0):
            with open(os.environ["PROFILE_JOBS_FILE"], "a") as f:
                f.write(f"sam_QNN_{component_name}: {profile_job.job_id}\n")
        else:
            result = profile_job.wait()
            assert result.success
