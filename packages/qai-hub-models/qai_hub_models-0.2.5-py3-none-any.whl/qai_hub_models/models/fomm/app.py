...
def load_video_frames(video_filepath: str, sample_rate: int = 1) -> List[np.ndarray]:
    """
...
class FOMMApp:
    ...
    def __init__(self, model: FOMM):
        self.kp_detector = model.detector_model
    ...
    def copy_motion_to_video(
        ...
        with torch.no_grad():
            ...
            for frame_idx in range(driving_tensors.shape[2]):
                ...
                predictions.append(
                    np.transpose(out.data.cpu().numpy(), [0, 2, 3, 1])[0]
                )
