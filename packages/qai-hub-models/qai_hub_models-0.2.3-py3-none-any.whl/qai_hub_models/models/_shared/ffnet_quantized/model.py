...
class FFNetQuantizable(AIMETQuantizableMixin, FFNet):
    ...
    def get_hub_compile_options(self, target_runtime: TargetRuntime) -> str:
        compile_options = super().get_hub_compile_options(target_runtime)
        return (
            compile_options
            + " --force_channel_last_input image --force_channel_last_output output_0 --quantize_full_type int8 --quantize_io"
        )
