...
from . import backend_version_helper, logging_messages, messages
...
from .exceptions import CometException, CometRestApiException, NotFound, QueryException
    ...
    def _raise_if_old_backend(self, method_name: str, minimal_version: str) -> None:
        ...
        if version.compare(minimal_version) < 0:
            raise CometException(
                "Backend is too old. `{}` requires at least `{}` version of backend".format(
                    method_name,
                    minimal_version,
                )
            )
...
class API(object):
    ...
    * API.get_experiments(WORKSPACE, PROJECT_NAME, PATTERN): get all APIExperiments in WORKSPACE/PROJECT/PATTERN
    ...
    def get_latest_registry_model_version_details(
        ...
        """
        LOGGER.warning(
            "This method has been deprecated, we recommend using the api.get_model(...) method to get the Model Object and then using model.find_versions(...)"
        )
    ...
    def get_model_registry_version_assets(
        ...
        """
        LOGGER.warning(
            "This method has been deprecated, we recommend using the api.get_model(...) method to get the Model Object and then using model.get_assets(...)"
        )
    ...
    def download_registry_model(
        ...
        """
        LOGGER.warning(
            "This method has been deprecated, we recommend using the api.get_model(...) method to get the Model Object and then using model.download(...)"
        )
    ...
    def update_registry_model_version(
        ...
        """
        LOGGER.warning(
            "This method has been deprecated, we recommend using the api.get_model(...) method to get the Model Object and then using model.set_status(...) or model.add_tag(...)"
        )
    ...
    def add_registry_model_version_stage(
        ...
        """
        LOGGER.warning(
            "This method has been deprecated, we recommend using the api.get_model(...) method to get the Model Object and then using model.set_status(version='<version>', status='<status>')"
        )
    ...
    def delete_registry_model_version_stage(
        ...
        """
        LOGGER.warning(
            "This method has been deprecated, we recommend using the api.get_model(...) method to get the Model Object and then using model.set_status(version='<version>', status=None)"
        )
    ...
    def get_panel_options(self):
        ...
        # type: () -> Dict[str, Any]
        config = get_config()
        # Not a regular config item:
        return config.override.get("COMET_PANEL_OPTIONS", {})
    ...
    def get_panel_experiments(self):
        ...
        config = get_config()
        # Not a regular config item:
        experiment_keys = config.override.get("COMET_EXPERIMENT_KEYS", None)
