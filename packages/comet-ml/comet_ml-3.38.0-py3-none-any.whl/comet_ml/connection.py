...
def get_points_3d_upload_limits_url(server_address) -> str:
    return server_address + "asset/3d-limits"
...
def create_asset_url(server_address):
...
def _process_upload_with_retries(upload_func, max_retries, **kwargs) -> Response:
    retry_attempt = 0
    response = None
...
def upload_thumbnail(
    ...
    try:
        ...
        LOGGER.error("Thumbnail could not be uploaded: %r", e, exc_info=True)
        Reporting.report(
            event_name=FILE_UPLOADED_FAILED,
            experiment_key=options.experiment_id,
            project_id=options.project_id,
