...
class CrossPyArray(numpy.lib.mixins.NDArrayOperatorsMixin):
    ...
    def __setitem__(self, index: IndexType, value):
        ...
        if isinstance(xindex, int):
            ...
            with context(block, stream=dict(non_blocking=True)) as ctx:
                ctx.copy(block, (*index[:self.axis], int(int_ - dev_offset), *index[self.axis + 1:]), value, None)
            return
    ...
    def to(self, placement):
        ...
        # TODO trivial cases
        with get_device(placement) as ctx:
            _py = ctx.module
            buf = _py.empty(self._shape)
            self.tobuffer(buf)
            # TODO concatenate
            return Initializer(buf).finish()
    ...
    def __cuda_array_interface__(self):
        ...
        Version 3: https://numba.readthedocs.io/en/stable/cuda/cuda_array_interface.html
        Version 2: https://numba.pydata.org/numba-doc/dev/cuda/cuda_array_interface.html
        ...
        assert len(self.device_array) == 1, "Not on a single device"
        canonical = next(iter(self.device_array.values()))
        # TODO check empty
