...
from .const import SCOPE_FACILITY_SHAHKAR_GET
...
class ClientCredentialToken(Token):
    ...
    def revoke(self):
        if not (token:=FinnotechToken.objects.filter(token=self.token, is_active=True)).exists():
            return
        
        token.first().revoke()
        
    def revoke_refresh(self):
        if not (token:=FinnotechRefresh.objects.filter(token=self.refresh_token, is_active=True)).exists():
            return
        
        token.first().revoke()

    @classmethod
    def refresh(cls, http_client, *, token:Token=None):
        ...
        fetched_token = cls(
            **http_client._execute(
            ...
            refreshToken=cls.refresh_token,
        )
    ...
    @classmethod
    def fetch(cls, http_client, *, user=None):
        """
        ...
        if user:
            FinnotechToken.objects.bulk_create(
                FinnotechToken(
                    user=user,
                    scope=scope,
                    refresh_token=FinnotechRefresh.objects.create(
                        user=user,
                        scope=scope,
                        token=fetched_token.refresh_token,
                    ),
                    token=fetched_token.token,
                )
                for scope in fetched_token.scopes
            )
...
class SmsAuthorization(Token):
    ...
    # noinspection PyProtectedMember
    def refresh(self, http_client, *, token=None):
        url = "/dev/v2/oauth2/token"
