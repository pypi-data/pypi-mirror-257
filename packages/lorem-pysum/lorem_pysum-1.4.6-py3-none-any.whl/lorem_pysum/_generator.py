...
from pydantic import (
    ...
    PastDate,
    PastDatetime,
    PaymentCardNumber as OriginalPaymentCardNumber,
    SecretBytes,
    SecretStr,
    StringConstraints,
)
from pydantic.color import Color as OriginalColor
        ...
        for field_name, field_info in model_type.model_fields.items():
            ...
            if (
                (
                    ...
                    and field_info.default is not BaseModel.json
                )
        ...
        *,
        ...
        if _is_union(origin):
            type_choices = [
                ...
                if not issubclass(arg_type, NoneType)  # type: ignore
        ...
        # Network Types
        if field_info.metadata and hasattr(field_info.metadata[0], "allowed_schemes"):
            schemes = field_info.metadata[0].allowed_schemes
        ...
        if type_ == FileUrl:
            return "file:///lorem/ipsum"
        if type_ in [FilePath, DirectoryPath, NewPath]:
            return "/lorem/ipsum"

        # Extra Types
        if type_ in [OriginalColor, Color]:
            return "#000"
        if type_ in [PaymentCardBrand, OriginalPaymentCardBrand]:
            return "Visa"
        if type_ in [PaymentCardNumber, OriginalPaymentCardNumber]:
...
def _get_number_value(metadata: Metadata) -> AnyNumber:
