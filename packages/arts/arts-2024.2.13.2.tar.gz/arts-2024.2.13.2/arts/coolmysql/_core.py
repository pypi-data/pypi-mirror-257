...
class ORM():
    ...
    def __del__(self):
        try:
            self._connpool[0].close()
        except:
            pass
        try:
            self._aconnpool[0].close()
        except:
            pass
    ...
    def get_conn(self, db_name: str=''):
        class core():
            ...
            def __enter__(mSelf) -> List[connect|DictCursor]:
                ...
                mSelf.conn, mSelf.cursor = conn, cursor
                return conn, cursor
...
class Sheet_ORM():
    ...
    def __init__(self, parent, sheet_name: str, _condition: dict=None):
        self.parent = parent
    ...
    def __getitem__(self, key):
        if isinstance(key, int|slice): return self._deepcopy('slice', key)
    ...
    def len(self):
        rdata, cursor = self.execute(f"select count(1) as tatal from {self.sheet_name}{self._parse_where()}", commit=False)
        return rdata[0]['tatal']

    async def alen(self):
        rdata, cursor = await self.aexecute(f"select count(1) as tatal from {self.sheet_name}{self._parse_where()}", commit=False)
        return rdata[0]['tatal']
