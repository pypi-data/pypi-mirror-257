...
uniset = vbool(True, 'uniset', _system=True)  # 全集
empset = vbool(False, 'empset', _system=True)  # 空集
undefined = vbool(False, 'undefined', _system=True)  # 逻辑空值，用于与业务空值None相互区分
SysEmpty = vbool(False, 'SysEmpty', _system=True)
    # 供开发者调用的表示空的标识, 如:
        # 作为默认参数以识别调用者是否传参
        # 在函数内部作为某种状态标识
