...
class TestPlatform:
    ...
    def test_activate_server_stored_config(self, server: str = good_server_4):
        """
    ...
    def test_get_server_status(self, server: str = good_server_4):
        try:
            p_client = Platform(server, self.good_platform1_url, self.good_user_1)
            response = p_client.get_server_status(server)
            print(f"\n\n\tStatus for server {server} is {json.dumps(response, indent=4)}")
            assert True
    ...
    def test_activate_servers_on_platform(self):
        try:
            ...
            server_list = ["fluffy_kv", "fluffy_view", "active-metadata-store",
                           "engine-host","integration-daemon","view-server"]
    ...
    def test_platform_services(self):
        ...
        self.test_activate_server_stored_config()
