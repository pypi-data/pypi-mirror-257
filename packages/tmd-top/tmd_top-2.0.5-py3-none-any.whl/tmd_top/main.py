...
Press the tab key to switch the window, hold down the shift key, and the mouse can optionally copy.
Cpu, men unit is %
Io unit is KB
...
version: v2.0.5
geoip update time: 2024-02-16
    """
    ENABLE_COMMAND_PALETTE = False
    BINDINGS = [
        ("q","quit","(quit)"),
        ('v','input_command','(InputPID)'),
        ('t','slow_sleep_time','(LowSpeedRefresh)'),
        ('y','sleep_time','(RestoreRefresh)'),
        ('c','sort_connect','(ConnectionNumberSort)'),
        ('i','sort_ip','(IpNumberSort)'),
        ('u','sort_up','(UploadSort)'),
        ('d','sort_down','(DownloadSort)'),   
        ('z','sort_cpu','(CpuSort)'), 
        ('x','sort_men','(MemorySort)'),
        ('n','sort_io_r','(IoReadSort)'),
        ('m','sort_io_w','(IoWriteSort)')
        ]
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static('The pid of the current query: None',classes="box",id="instruction_display")
        yield Input(placeholder="Please enter the PID number and press enter",id="input_command")
        yield Footer()

    def on_mount(self) -> None:
        self.title = "TMD-TOP"
        self.query_one('#network').border_title = "NetworkCard"
        self.query_one('#network').border_subtitle = "Total 0"
        self.query_one('#network').loading = True
        self.query_one('#listen').border_title = "MonitorTraffic"
        self.query_one('#listen').border_subtitle = "Total 0"
        self.query_one('#listen').loading = True
        self.query_one('#outside').border_title = "RequestTraffic"
        self.query_one('#outside').border_subtitle = "Total 0"
        self.query_one('#outside').loading = True
        self.query_one('#details').border_title = "Detailed"
        self.query_one('#details').border_subtitle = "Total 0"
        self.query_one('#details').loading = True
        self.query_one('#Introduction').border_title = "Information"
        self.query_one('#instruction_display').border_title = 'Prompt'
        self.query_one('#input_command').border_title = 'QueryPID'
        current_locale = locale.getdefaultlocale()
        self.current = current_locale[0]
        log = self.query_one(Log)
        log.write_line(self.IntroductionText)
        log.write_line('language: ' + str(self.current) + '\n')
        self.update_tables()
        network_table = self.query_one('#network')
        network_table.cursor_type = next(cycle(["row"]))
        network_table.add_columns(*("name", "up", "down"))
        ...
        listen_table.add_columns(*("pid", "name", "ListenAddr","ListenPort","IpNum","ConnectNum","up","down","cpu","men","io_r","io_w"))
        ...
        details_table.add_columns(*("CustomerAddr","CustomerPort","up","down",'Area'))
        ...
        outside_table.add_columns(*("pid", "name","IpNum","ConnectNum","up","down","cpu","men","io_r","io_w"))
    ...
    def watch_network(self) -> None:
        ...
        network_dom.border_subtitle = "Total " + str(row_number)
    ...
    def watch_listen(self) -> None:
        ...
        listen_dom.border_subtitle = "Total " + str(row_number)
    ...
    def watch_outside(self) -> None:
        ...
        outside_dom.border_subtitle = "Total " + str(row_number)
    ...
    def watch_detailed(self) -> None:
        ...
        details_dom.border_subtitle = "Total " + str(row_number)
    ...
    def on_data_table_row_selected(self,event):
        if event.control.name == 'listen':
            ...
            log.write_line('\nThe command executed when querying PID: \n' + str(cmd))
            instruction_display = self.query_one("#instruction_display")
            instruction_display.update('The pid of the current query: ' + value[0])
        ...
        if event.control.name == "details":
            log = self.query_one(Log)
            value =  event.control.get_row(event.row_key)
            log.write_line('\nOption IP (hold down shift to control mouse replication): \n' + str(value[0]))
            #使用api请求显示详细的ip信息
    ...
    def on_key(self,event: events.Key):
        ...
        if event.key == 'enter':
            ...
            instruction_display.update('The pid of the current query: ' + input_value.value)
    ...
    async def update_tables(self) -> None:
        ...
        while True:
            ...
            netstat_command = self.localExecuteCommand('sudo netstat -atpn')
    ...
    def selectTotalListen(self,conn):
        ...
        /*获取内部流量*/
            ...
            from
                ...
                LEFT JOIN pidstat on listen.pid == pidstat.pid
            WHERE 
    ...
    def selectTotalOut(self,conn):
        select_out_sql = '''
            ...
            LEFT JOIN pidstat on pidstat.pid == connected.pid
            WHERE 
...
def main():
    ...
    if not command_exists('pidstat'):
        print("The pidstat command does not exist, please make sure the command is installed on the system")
        sys.exit(0)
    if not command_exists('netstat'):
        print("The netstat command does not exist, please make sure the command is installed on the system")
        sys.exit(0)
    #运行tui界面
    app = GridLayout()
    app.run()   
