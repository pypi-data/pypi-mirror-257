...
geoip update time: 2024-02-16
    ...
    def on_mount(self) -> None:
        ...
        listen_table.add_columns(*("pid", "名称", "监听地址","监听端口","ip数","连接数","上传","下载","cpu","men","io_r","io_w"))
        ...
        details_table.add_columns(*("客户机地址","客户机端口","上传","下载",'地区'))
        ...
        outside_table.add_columns(*("pid", "名称","ip数","连接数","上传","下载","cpu","men","io_r","io_w"))
    ...
    def watch_listen(self) -> None:
        ...
        for i in self.listen:
            data = list(i)
            listen_dom.add_row(*data,key=str(i[2])+"_"+ str(i[0]))
        if self.listen_value != None:
            try:
                index_data = listen_dom.get_row_index(row_key=str(self.listen_value[2]+"_"+self.listen_value[0]))
            except Exception:
                index_data = 0
    ...
    def watch_outside(self) -> None:
        ...
        for i in self.outside:
            data = list(i)
            outside_dom.add_row(*data,key=str(i[2]) + '_' + str(i[0]))
        if self.outside_value != None:
            try:
                index_data = outside_dom.get_row_index(row_key=str(self.outside_value[2]+"_"+self.outside_value[0]))
            except Exception:
                index_data = 0
    ...
    def get_ip_info(self,ip_address):
        ...
        with Reader(database_path) as geoip_reader:
            try:
                ...
                country_name = response.country.names.get('zh-CN') or 'null'
                city_name = response.city.names.get('zh-CN') or 'null'
                subdivision_name = ''
                if len(response.subdivisions) > 0:
                    subdivision_name = response.subdivisions[0].names.get('zh-CN') or 'null'
...
def main():
    ...
    if not command_exists('pidstat'):
        print("pidstat命令不存在,请确保系统已安装命令;")
        sys.exit(0)
    if not command_exists('netstat'):
        print("netstat命令不存在,请确保系统已安装命令;")
        sys.exit(0)
    #运行tui界面
    app = GridLayout()
    app.run()   
