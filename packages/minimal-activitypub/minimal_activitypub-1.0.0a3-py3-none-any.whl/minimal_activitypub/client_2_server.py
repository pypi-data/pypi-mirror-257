...
UTC: Final[timezone] = timezone(offset=timedelta(hours=0), name="UTC")
...
class ActivityPub:
    ...
    def __init__(
        ...
        self.supported_mime_types: Optional[List[str]] = None
        self.max_status_len = 500
        self.ratelimit_limit = 300
        self.ratelimit_remaining = 300
        self.ratelimit_reset = datetime.now(tz=UTC)
        logger.debug(
    ...
    async def determine_instance_type(self: ActivityPubClass) -> None:  # noqa: C901
        ...
        try:
            response = await self.session.get(url=f"{instance}/api/v1/instance")
            await ActivityPub.__check_exception(response=response)
    ...
    async def delete_status(
        ...
        try:
            response = await self.session.delete(url=url, headers=headers)
            self.__update_ratelimit(response.headers)
            await ActivityPub.__check_exception(response=response)
            self.__parse_next_prev(links=response.headers.get("Link"))
    ...
    async def undo_reblog(self: ActivityPubClass, status: Union[str, Dict[Any, Any]]) -> Status:
        """Remove a reblog.
    ...
    async def create_app(
        ...
        try:
            response = await session.post(url=f"{instance_url}/api/v1/apps", data=form_data)
    ...
    async def __pre_call_checks(self: ActivityPubClass) -> None:
        ...
        if self.ratelimit_remaining == 0 and self.ratelimit_reset > datetime.now(tz=UTC):
    ...
    def __update_ratelimit(self: ActivityPubClass, headers: Any) -> None:
        ...
        if self.instance_type in (INSTANCE_TYPE_TAKAHE, INSTANCE_TYPE_PLEROMA):
            ...
            temp_ratelimit_reset = (datetime.now(tz=UTC) + timedelta(minutes=5)).isoformat()
    ...
    @staticmethod
    async def __check_exception(response: Response) -> None:  # Todo: Translate to httpx
    ...
    @staticmethod
    async def __determine_error_message(response: Response) -> str:  # Todo: Translate to httpx
