...
class TestRange(DomainCase):
    ...
    def test_range_iterable(self, range_):
        low, up = range_
        self.assertEqual(range_._lower.bound, low)
        self.assertEqual(range_._upper.bound, up)
...
class IntervalTest(DomainCase):
    @given(
        st.lists(float_ranges(kinds=kinds_with_samples), max_size=5)
        | st.lists(date_ranges(kinds=kinds_with_samples), max_size=5)
    )
    ...
    @given(st.lists(float_ranges(), max_size=5) | st.lists(date_ranges(), max_size=5))
    @settings(suppress_health_check=[HealthCheck.too_slow])
    ...
    @given(
        st.lists(float_ranges(), max_size=5),
        st.lists(float_ranges(), max_size=5),
    )
    @settings(suppress_health_check=[HealthCheck.too_slow])
    ...
    @given(st.lists(float_ranges(), min_size=2, max_size=5), st.data())
    @settings(suppress_health_check=[HealthCheck.too_slow])
    ...
    @given(
        st.lists(date_ranges(), min_size=2, max_size=5),
        st.lists(date_ranges(), max_size=5),
    )
    @settings(suppress_health_check=[HealthCheck.too_slow])
    ...
    @given(
        st.lists(float_ranges(), min_size=2, max_size=5),
        st.lists(float_ranges(), max_size=5),
    )
    @settings(suppress_health_check=[HealthCheck.too_slow])
    ...
    @given(st.lists(date_ranges(), min_size=2, max_size=5), st.data())
    @settings(suppress_health_check=[HealthCheck.too_slow])
    ...
    @given(st.lists(date_ranges(), max_size=5))
    @settings(suppress_health_check=[HealthCheck.too_slow])
